...
| **Constraints** | Runtime business-rule validations that prevent execution when required conditions are not met. | Requiring a customer ID before allowing an order lookup or payment action. |
| **Guardrails** | Safety and quality checks applied to agent input and output content. | Blocking abusive language or redacting sensitive information from response
