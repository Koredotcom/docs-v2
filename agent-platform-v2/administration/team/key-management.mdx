...
Go to **Settings** > **Team** > **Key Management**. The page is organized into the following tabs:
...
Use the preview section to inspect the resolved provider for a selected project or environment.

The preview displays the inheritance chain across:

| Level | Status |
| --- | --- |
| Platform Default | Active or overridden |
| Tenant Default | Active or overridden |
| Tenant Environment | Active or overridden |
| Project Default | Active or overridden |
| Project Environment | Active or overridden |
...
> Important: Do not destroy a key version in your cloud provider until all data has been re-encrypted with the new version. Destroying active key versions may result in permanent data loss.
...
> Note: Audit log retention follows the workspace data retention policy. Professional and Enterprise plans retain logs for at least 90 days.