...
This is a complete agent definition. Every agent reasons by default -- it uses the LLM to decide how to respond to each message. You can optionally add a `FLOW` block to give an agent structured steps (covered in the [next tutorial](/agent-platform-v2/tutorials/build-a-scripted-flow)), but without one the agent handles conversations entirely through LLM reasoning.
...
When you use tools inside a flow section (covered in the [next tutorial](/agent-platform-v2/tutorials/build-a-scripted-flow)), you handle results explicitly with `ON_SUCCESS` and `ON_FAIL`:
...
- [Add structured steps to an agent](/agent-platform-v2/tutorials/build-a-scripted-flow) — Create step-by-step guided conversations with FLOW, GATHER, and branching.
- [Multi-agent and knowledge](/agent-platform-v2/tutorials/multi-agent-and-knowledge) — Connect multiple agents through a supervisor and add a knowledge base.
