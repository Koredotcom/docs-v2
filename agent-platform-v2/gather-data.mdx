...
Use GATHER to collect structured data from users through natural conversation.

`GATHER` collects structured data from users during a conversation. Instead of rigid form-filling, it lets the agent extract field values from natural language — a user saying "I want to fly to Paris on March 10 for two people" can fill three fields at once. Fields can be validated, corrected, and inferred from context without restarting the flow.
...
- You must have an agent created in your project. See [Create Agents](/agent-platform-v2/create-agents).
...
![Gather Fields in Studio](/agent-platform-v2/images/gather-data.png)
...
<Tip>Use `hybrid` when some fields have a predictable format (such as order IDs or phone numbers) but others are open-ended.</Tip>
...
| `LLM` | Asks the LLM to evaluate whether the value is valid. |
...
`SUB_INTENTS` define scoped intents valid only within a step. `CLEAR` removes the specified field values so the agent re-collects them.
...
<Note>`MAX_ATTEMPTS` counts entries to the step, not individual field retries. `max_retries` on a field counts field-level retries separately.</Note>
