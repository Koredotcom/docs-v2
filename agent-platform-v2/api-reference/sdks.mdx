...
The Agent Platform 2.0 provides two SDKs: the **Web SDK** for embedding agent chat and voice interactions in web applications, and the **ABL SDK** for programmatic access to parse, validate, and compile ABL agent definitions. For authentication and error handling conventions, see [API overview](/agent-platform-v2/api-reference/index).
...
Combine the ABL SDK with the [Project export/import API](/agent-platform-v2/api-reference/knowledge-analytics-export#project-exportimport-api) to programmatically manipulate exported agent files:
...
- [Channels](/agent-platform-v2/api-reference/channels) -- Configure SDK channels and widget settings
- [Conversation API](/agent-platform-v2/api-reference/conversation-api) -- REST API for server-side chat integration
