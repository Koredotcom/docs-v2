...
The Agent Platform 2.0 delivers agent conversations through 20+ channels. Each channel has an adapter that normalizes inbound messages, delivers outbound responses, and handles platform-specific features like interactive elements, threading, and media. For authentication and error handling conventions, see [API overview](/agent-platform-v2/api-reference/api-overview).

## Channel architecture

### Channel matrix

{/* AG: Try a wider layout for this page to fit the table. */}
...
Each webhook-capable channel connection generates a unique webhook URL in the format `https://api.ablplatform.com/api/v1/channels/{channelType}/webhook/{identifier}`.

For Meta channels (WhatsApp, Messenger, Instagram), a shared webhook URL (in the format `https://api.ablplatform.com/api/v1/channels/{channelType}/webhook`) with body-based routing is also supported.
...
Access voice analytics at **Project > Analytics > Voice** or via the [Analytics API](/agent-platform-v2/api-reference/knowledge-analytics-export#voice-analytics).
...
- [API overview](/agent-platform-v2/api-reference/api-overview) -- Authentication and general conventions
- [Conversation API](/agent-platform-v2/api-reference/conversation-api) -- Text-based agent interactions
- [SDKs](/agent-platform-v2/api-reference/sdks) -- Embed voice and chat widgets
