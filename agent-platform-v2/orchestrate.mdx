...
| ---------- | ---------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| [Supervisor](/agent-platform-v2/orchestrate#1-supervisor) | Entry point for Multi-Agent systems; intent-based routing                                | User always talks to an agent, who routes the request to a specialized agent as required. |
| [Handoff](/agent-platform-v2/orchestrate#2-handoff)    | Route the conversation to a specialist agent.                                            | The user is transferred to a new agent                                                    |
| [Delegation](/agent-platform-v2/orchestrate#3-delegate) | The agent needs a sub-task done; Sends the sub-task to an agent and get results back     | Transparent -- user does not see the delegation.                                          |
| [Fan-out](/agent-platform-v2/orchestrate#4-fan-out)    | The agent needs multiple things done in parallel. Run multiple agents and merge results. | Transparent -- user sees a single combined result.                                        |
| [Escalation](/agent-platform-v2/escalate) | Transfer to a human agent with full context.                                             | User interacts with a human agent.                                                        |

**Sample Multi-Agent Orchestration**

![Multi-Agent Orchestration](/agent-platform-v2/images/multi-agent-orchestration.png)
...
- Navigate to the **Agents** page to view all agents in the project. ![Orchestration](/agent-platform-v2/images/agent-pattern.png)
- Use the **Canvas view** to visualize relationships between agents. ![Orchestration](/agent-platform-v2/images/canvas-view.png)
...
| LIMITATIONS     | No       | Specifies what the supervisor cannot do to prevent incorrect handling.                                                                   |
...
| CONTEXT   | No       | Data passed to the target agent (pass list, summary, history, memory_grants) |
| RETURN    | No       | Whether control returns to the supervisor after completion (default: false)  |
...
| TIMEOUT    | No       | Maximum time to wait for the sub-agent (e.g., 10s)                                                                                                                      |
...
For more details on memory and access scopes, refer [this guide on memory management across agents](/agent-platform-v2/memory-and-state).
