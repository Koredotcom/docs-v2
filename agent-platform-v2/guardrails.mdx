...
| Setting | Description |
| --- | --- |
| Fail Mode | Controls whether execution continues or is blocked if guardrail evaluation fails. **Fail-open** allows execution to continue if guardrail evaluation fails or times out. **Fail-closed** blocks execution when guardrail evaluation can't be completed successfully. Use fail-closed behavior for high-security or compliance-sensitive applications. |
...
Output guardrails evaluate generated responses before they're returned to the user. Use output guardrails to prevent unsafe responses, redact sensitive information, apply moderation checks, and inspect streaming output during generation.

Use `kind: output` to evaluate generated responses before they're returned to the user.
...
Only one policy can be active per project at a time.
