...
```mermaid
graph LR
    A["1. Foundation<br/>Account & core setup"] --> B["2. Configuration<br/>Agents, tools, & knowledge"]
    B --> C["3. QA & Test<br/>Validate & test safely"]
    C --> D["4. Deployment<br/>Go live & integrate"]
```
...
flowchart TD
    A{Which tool type?}
    A -->|No-code visual builder\nfor step-by-step processes| B[Workflow Tool]
    A -->|Custom logic using\nJavaScript or Python| C[Code Tool]
    A -->|Universal connector to external\ntools via Model Context Protocol| D[MCP Tool]
```
