...
| [Build with Arch using CLI](/agent-platform/cli)        | Command-line client for the platform. You can create machine-readable validation, synchronization, evaluation, release, and deployment automations and CI workflows. You can create or update agents from ABL files, compile and test them, configure enterprise connectors and synchronize their content, and export, verify, or import project packages. The CLI can also invoke Arch AI from the terminal and expose Platform capabilities through an MCP stdio server. |
