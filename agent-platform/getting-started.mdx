...
{/* AG: Update for v1.5.0

| [Build with Arch using CLI](/agent-platform/cli) | Command-line client for the platform that is distributed as an npm package. Interactive commands operate on platform projects. Build CI workflows - supports machine-readable validation, synchronization, evaluation, release, and deployment automation. Command-based workflows can be used for repeatable automation. You can create or update agents from ABL files, compile and test them, configure enterprise connectors and synchronize their content, and export, verify, or import project packages. The CLI can also invoke Arch AI from the terminal and expose Platform capabilities through an MCP stdio server. |
...
|------------------------|--------------------------------------------------|
Agent Platform Account | Sign up for free using your [region-specific URL](#platform-access-and-allowlist-of-ips). |
...
The Agent Platform is available in multiple regions. The Platform sends requests to a fixed set of egress IP addresses. If your systems use IP restrictions, add the region-specific egress IPs below to your allowlist to permit Platform traffic.
