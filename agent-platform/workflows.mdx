...
| **Monitor** | Displays workflow execution metrics, including the total number of runs, workflows currently in progress, response time, and failure rate. |
...
You can export an individual workflow and import it into another project as an editable draft, making it easier to reuse or migrate workflows between projects.

### Export a workflow

1. Open the workflow you want to export.
2. Click **Export**.
3. The workflow is downloaded as a `.workflow.json` file.

The workflow export includes the workflow definition and references to dependent components, such as agents, tools, and auth profiles. These dependent components are not exported with the workflow.

<Note>Workflow trigger definitions are not included in workflow exports. You must configure triggers manually after importing the workflow.</Note>

### Import a workflow

1. From the Workflows page, click **Import**.
2. Click **Choose file** and select the workflow export file.
3. In the Import Workflow dialog, enter a name for the workflow, and click **Preview**.
4. Review the workflow details and dependencies. Missing dependencies are identified in the preview.
5. If dependencies are missing, resolve them or select **Import with unresolved dependencies unselected** to continue without them.
6. Click **Import**.

The imported workflow is created as an editable draft. Unresolved dependencies must be manually relinked before you run or publish the workflow.
...
Use the **Monitor** tab to inspect workflow runs and troubleshoot execution.

The Monitor tab provides:

- **Run history** to review workflow executions and their outcomes.
- **Health metrics** to assess workflow performance and failures, including response time and failure rate.
- **Run details** to investigate an individual execution.

Select a run to view its details.

### Run details

The run details view provides a consolidated view of the execution, including its status, outcome, execution flow, and associated data.

| View | What you can inspect |
|---|---|
| **Summary** | Run metadata, workflow version, trigger, environment, status, and outcome. |
| **Flow log** | The execution flow and status of individual workflow steps, including their duration. |
| **Data** | Payloads, input, and output associated with the run. |

Use the **Summary** view to understand the overall result of a run, the **Flow log** to trace its execution, and the **Data** view to inspect the data processed by the workflow.
...
Use the **Settings** tab to manage workflow metadata, variable namespaces, and workflow deletion.

| Setting | Description |
|---|---|
| **Name** | The name of the workflow. |
| **Slug** | The unique identifier for the workflow. |
| **Description** | A description of the workflow. |
| **Variable namespaces** | Select the variable namespaces that the workflow can resolve in environment and configuration expressions. If no namespace is selected, the default namespace is used. |
| **Delete a workflow** | Permanently delete the workflow, including its versions and triggers. This action cannot be undone. |
