...
* Confirmed scopes and permissions assigned to the credentials for the endpoints that you intend to use.

## Authentication

A platform key is a long-lived bearer credential scoped to a tenant, a project, and a set of scopes. Its format is `abl_<48 hex characters>`, for example `abl_3f7a1c9e42b8dxxxxa1e8c7b4d29f0a3e5c8b1d7f2a4exxxx`. The headers are in the format `X-API-Key: <platform key>`.
...
<Caution>Copy the generated key. It isn't displayed again.</Caution>
...
<Accordion title="Execution scope">
...
<Accordion title="Knowledge base scope">
...
<Accordion title="Analytics scope">
...
<Accordion title="Administration scope">
...
| Error code            | HTTP code | Potential cause                     | Fix                                            |
|-----------------------|-----------|---------------------------------|------------------------------------------------|
...
| `UNAUTHORIZED`        | 401       | Not authenticated               | Verify key is valid and not expired            |
| `FORBIDDEN`           | 403       | Insufficient permissions        | Check credential scope                         |
| `NOT_FOUND`           | 404       | Resource missing or unavailable | Verify resource ID and access rights           |
| `RATE_LIMIT_EXCEEDED` | 429       | Request rate limit hit          | Retry after `Retry-After` duration (seconds)   |
