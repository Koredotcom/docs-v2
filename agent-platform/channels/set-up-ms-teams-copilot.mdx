...
The setup runs across three surfaces in this order: the [Azure Portal](https://portal.azure.com/), the [Microsoft Teams Developer Portal](https://dev.teams.microsoft.com/), and Artemis.

<Steps>
  <Step title="Create an Azure Bot">
    1. Sign in to the [Azure Portal](https://portal.azure.com/) and search the Marketplace for **Azure Bot**.
...
<Tabs>
  <Tab title="Overview">
    ...
    When the toggle is on, Runtime verifies identity through the Bot Connector member endpoint. The **Stronger identity verification setup** panel describes what the toggle needs:

    1. Use the Azure Bot registration values for `app_id`, `client_secret`, and `tenant_id` on this Teams channel connection or its selected auth profile.
    2. Keep the bot installed and reachable in Teams, with the Azure Bot **Messaging endpoint** pointing at this connection's Runtime webhook URL.
    3. Runtime calls the Bot Connector member endpoint `/v3/conversations/{conversationId}/members/{userId}` to resolve the sender's Microsoft profile email or UPN before allowing the turn.
    4. Microsoft Graph consent, OBO settings, and `https://graph.microsoft.com/.default` scopes aren't used by this policy toggle.
    5. For more detail, review Microsoft guidance for Teams member lookup.
  </Tab>
