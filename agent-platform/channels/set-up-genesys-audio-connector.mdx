...
<Tabs>
  ...
  <Tab title="Activity">
    <Note>Tab content not yet confirmed against the product UI — verify with a screenshot before publishing.</Note>
  </Tab>
</Tabs>
...
**Audio** — Do not log raw audio or sensitive transcript content. Use scrubbed trace identifiers and aggregate voice phase metrics for operational debugging.
...
**Fallback** — Define what Architect does when the AudioHook cannot connect, media stalls, STT or TTS fails, or the agent exceeds its response budget.
...
* **AudioHook cannot connect** — confirm the complete connection-specific `wss` callback, an Active connection, a reachable TLS endpoint, and unchanged shared account URLs.
* **Authentication fails** — use the matching project API key and HTTP Message Signature Secret and confirm an ingress proxy does not rewrite signed signaling.
* **Call connects but is silent** — verify PCMU framing, media direction, sample rate, the Genesys action settings, and readiness of the configured STT/TTS providers (Pipeline mode) or S2S provider (Realtime mode).
* **Audio is delayed or choppy** — inspect jitter, packet loss, buffering, backpressure, TTS or S2S first-audio latency, and gateway-to-Runtime network latency.
* **Architect does not resume** — map disconnect and completion variables and confirm the AudioHook session closes with the expected reason.
* **Realtime provider fields don't match what's expected** — confirm the correct S2S Provider is selected under Realtime LLM Voice Settings; each provider (Grok, Gemini, OpenAI) has its own distinct field set.
