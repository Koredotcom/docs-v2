...
<Tab title="Realtime mode">
    ...
    <Note>
      **Azure OpenAI Realtime, Grok Realtime, Google Gemini Live, and OpenAI Realtime** each require a **Realtime model connection** - a project-level model pinned to this channel. Amazon Nova Sonic does not use this field; it authenticates via Voice Service credentials instead.

      To make a model available here:

      <Steps>
        <Step title="Add the model at the workspace level">
          Configure the model under [Voice Services](/agent-platform/administration/ai-configuration#voice-services), then add it to the [model catalog](/agent-platform/administration/ai-configuration#model-catalog) as an LLM provider.
        </Step>
        <Step title="Add the model at the project level">
          Go to **Settings** -> **Model** and add the model you configured at the workspace level.
        </Step>
      </Steps>

      Once added, the model becomes selectable in the **Realtime model connection** dropdown below.
    </Note>

    | Field | Description |
    | --- | --- |
    | **Realtime model connection** | The project-pinned realtime model this provider uses. Shows `No project model selected` until a workspace admin adds one following the steps above; once selected, shows the model name and provider (for example `GPT Realtime 2 · openai`). |

    <AccordionGroup>
      <Accordion title="Amazon Nova Sonic (S2S)">
        | Field | Description |
        | --- | --- |
        | **Voice** | The synthesized voice, for example `Tiffany (default)`. |
        | **Model** | The Amazon Bedrock model ARN, for example `arn:aws:bedrock:us-east-1::foundation-model/amazon.nova-2-sonic-v1:0`. |
        | **Maximum tokens** | The maximum response length, in tokens. Default: `1024`. |
        | **Temperature** | Response creativity. Default: `0.70`. |
        | **Top P** | Nucleus sampling threshold. Default: `0.90`. |
        | **Endpointing sensitivity** | How quickly the model treats the caller as finished speaking - `Low`, `Medium`, or `High`. `Medium` is the general-conversation default; `High` responds sooner but may interrupt callers; `Low` allows more pause before responding. |
      </Accordion>

      <Accordion title="Azure OpenAI Realtime (S2S)">
        | Field | Description |
        | --- | --- |
        | **Voice** | The synthesized voice, for example `Marin (recommended)`. |
        | **Temperature** | Response creativity, from `0.6` (Focused) to `1.2` (Creative). Default: `0.8`. |

        Under **Turn Detection** - tunes how Azure OpenAI detects speech starts and decides when the caller has finished speaking:
        ...
        | **Silence Duration (ms)** | Quiet time before Azure OpenAI treats the caller turn as complete. Default: `700`. |
