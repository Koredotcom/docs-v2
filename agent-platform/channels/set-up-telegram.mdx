...
Do this work in Telegram before you open Artemis. BotFather issues the username and token, and a few of its settings change how your bot behaves in groups.

<Steps>
  <Step title="Create the bot with BotFather">
    Open Telegram, start a private chat with the verified **@BotFather** account, and send `/newbot`. BotFather asks for a display name, then a username, and returns the bot username and token together in its confirmation message.

    Check the verification badge before you start. Accounts impersonating BotFather exist, and a fake one harvests tokens.
  </Step>

  <Step title="Keep the exact username">
    Telegram usernames run 5 to 32 characters, allow letters, numbers, and underscores, and conventionally end in `bot`. Copy the username itself rather than the `t.me` link that BotFather shows alongside it.

    Artemis strips a leading `@` and stores the normalized value, so either form works. Nothing else does: a display name, a numeric bot user ID, a chat ID, a token fragment, or a full URL all fail to route.
  </Step>

  <Step title="Set up discovery, if your users need it">
    Use `/setdescription`, `/setabouttext`, `/setuserpic`, and `/setcommands` to control what users see before they start a chat and which commands Telegram suggests as they type.

    These settings shape discovery only. They don't affect message delivery, and they don't replace the webhook Artemis registers.
  </Step>

  <Step title="Decide how the bot behaves in groups">
    ...
    This holds whatever you set in BotFather. Turning privacy mode off widens what Telegram sends, but Artemis still applies the mention and reply rule, so the bot stays quiet in busy groups by design.
  </Step>
...
<Warning>The bot token authorizes every Bot API call for that bot, and it travels in the request URL. Treat it like a password: keep it out of tickets, screenshots, logs, browser code, and client-side configuration. If it leaks, revoke it in BotFather and update the credential in Artemis.</Warning>
...
<Steps>
  ...
  <Step title="Test the start command and any deep link">
    Send `/start`, then open a deep link carrying a start payload if your agent uses one. Artemis treats the command as a welcome action and passes the payload through as its value, so your agent can branch on where the user came from.
  </Step>
...
| Media | Files up to 20 MB download. Artemis rejects anything larger before upload. |
