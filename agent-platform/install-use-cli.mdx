...
<Important>Continue with release publication, deployment, and evidence only after the validation, synchronization preview, and evaluation succeeds.</Important>
...
You can run the CLI as an MCP stdio server. To start the server, use the command `artemis-platform-cli mcp`. A sample config to for your MCP client to launch the CLI is below.
