...
It stores the conversation context, maintains state across interactions, and remains active until it is closed.

Closing a session finalizes the conversation and is essential for platform operations. When a session closes, the platform finalizes billing, computes conversation metrics such as containment, resolution, and abandonment, and, on messaging channels, ensures the user's next message starts a new conversation rather than resuming an existing one. 

{/* AG: items to be documented when available or in the next round of updates

- How to search sessions. How to filter and/or sort to zero-in on a few specific sessions.
- To filter using phone number, see feature request ABLP-4425.
...
| Caller phone number | Filter by the phone number captured on the session. Enter a number to find every session for a specific caller. |
...
|-------------|---------|-----------------|
| `completed` | The conversation reached its natural end | `completed` |
| `abandoned` | The user walked away | `abandoned` |
| `agent_hangup` | The agent side dropped the conversation | `abandoned` |
| `transferred` | Handed to a human or another system | `escalated` |
| `failed` | Ended because of a system failure | `abandoned` |
| `timeout` | Closed automatically after inactivity | `ended` |
| `unengaged` | Opened but the user never really engaged | `ended` |
...
COMPLETE:
  ...
  WHEN: intent contains "bye"
    RESPOND: "Thanks for contacting us. Goodbye!"
```
<Note>
...
Note that When `TERMINAL: false` is specified in a `COMPLETE` block, the current interaction ends, but the session remains active. The response is sent to the user, and their next message starts a new interaction within the same session. [Learn More.](/agent-platform/abl/reference/multi-agent-and-supervisor#complete)

{/*  Internal APIs as of now - hence not mentioning as of now. 

### Closing a session using API 

Use an API to close a session

- An administrator closes stale or inactive sessions.
- Maintenance or cleanup jobs close abandoned or orphaned sessions.
- A CRM or ticketing system marks the case as resolved outside the conversation.

When closing a session through the API, specify an appropriate disposition to indicate why the session ended. The disposition is used for billing, reporting, and analytics.
*/}

### Automatic session closure

The platform automatically closes sessions when they exceed configured inactivity or configured session lifetime limits, or when channel-specific events indicate that the conversation has ended.

The platform can automatically close a session when it exceeds either of the following limits:
...
For SDK channels, sessions closed because of inactivity are recorded with `terminal_source: sdk_idle_timeout` and the `timeout` disposition. Sessions explicitly ended by the application or the user are recorded with `terminal_source: sdk_end_session` and the `completed` disposition. 

Use these fields in the [Query builder in Analytics explorer](/agent-platform/analytics-insights/analytics-explorer#query) to build queries that :
...
When a conversation is transferred to a human agent or another external system, the session disposition is set to 'transferred` and status is set to 'escalated`.

The session lifecycle after the external system completes the conversation on their end depends on the routing configuration.
...
| End reason sent | Disposition | Status |
|-----------------|-------------|--------|
| `transferred` | `transferred` | `escalated` |
| `completed` / `agent_completed` / `conversation_complete` | `completed` | `completed` |
| `abandoned` / `user_left` / `user_exit` | `abandoned` | `abandoned` |
| `agent_hangup` | `agent_hangup` | `abandoned` |
| `failed` / `error` | `failed` | `abandoned` |
| Missing or unrecognized value | `completed` | `completed` |
