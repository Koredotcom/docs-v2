...
| Discover sites | Discovers Confluence Cloud sites accessible using the selected OAuth profile. |
...
Configure permission-aware synchronization. When enabled, Confluence permissions are synchronized and enforced during search so that users only receive results they are authorized to access.
