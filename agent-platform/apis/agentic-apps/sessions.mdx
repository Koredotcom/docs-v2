...
```bash
curl --location 'https://staging-agent-platform.kore.ai/api/v2/apps/aa-6b8e45ca-40a5-4667-a151-8225d0ffe592/environments/env/sessions' \
--header 'Content-Type: application/json' \
--header 'x-api-key: kg-6454c2ba-7a94-47b7-a7ef-4ddd24b70ab0.8bff1187-83a4-4ec7-b4cf-cdbcf2449c93' \
--data '{
  "sessionIdentity": [
    {
      "type": "userReference",
...
|---------------------|--------|-------------------------------------------------------------------------------------------------------------------------------|
| env_name            | string | Environment name. Case-insensitive. Defaults to **draft** if not provided.                                                     |
| start_time          | string | Start of the date range for session retrieval. Must be in ISO 8601 format (e.g., `2026-04-01T00:00:00.000Z`). Defaults to 24 hours before `end_time`. |
...
```
curl -X POST 'https://your-domain.com/api/public/apps/aa-a9532f0f-ef54-46d0-ae1c-1ac0e4b86c2d/sessions/list' \
--header 'Content-Type: application/json' \
...
|:------ |:------ |
| Method | GET |
| Endpoint | `/apps/:appId/environments/<EnvName>/sessions?sessionId={sessionId}` <p> `/apps/:appId/environments/<EnvName>/sessions?sessionReference={sessionReference}` </p> |
| Content-Type | application/json |
| Authorization Header | `x-api-key: <API-KEY>` |
...
```bash
curl --location 'https://your-domain.com/apps/:appId/environments/<EnvName>/sessions?sessionId=s-123'
