...
The API response includes the tool ID, dock status ID and the tool undeployment status. After receiving the response, use the `dockStatusId` to call the [Get Dock Status API](../apis-list/get-dock-status) and verify the status of the task.
...
To use the API, you will need an API key. [Learn more](../../apis/index#how-to-create-the-api-key).
