...
When an export request is initiated, the API triggers the export process and returns a `dockStatusId`, which can be used with the [Get Dock Status API](/agent-platform/apis/apis-list/get-dock-status) to track the export progress. The API response also includes an `exportJobId` for tracking the specific export job.
...
1. Follow the steps mentioned [here](/agent-platform/tools/index#how-to-export-a-tool) to export a tool.
