...
All Agentic App API requests are made to the following base URL:
...
All requests must include an **API Key** in the **`Authorization`** header. You can generate the API key from the application. Refer to [this](../ai-agents/agentic-apps/deployment/access-deployed-version.md#generate-the-api-key-for-app-authentication) to learn more about generating an API Key.
...
1. [Login](../getting-started/sign-up-sign-in.md) to the Platform and [access](../administration/overview#access-settings-console) the Settings Console.
2. On the left menu, click **Security & Control** > **API Scopes**.
3. Create an API app and include all the required scopes, and the API keys as mentioned [here](../administration/security-and-control#create-an-api-app).
...
| Use Cases   | APIs     |
|--------|----------|
|  Create, retrieve, or end a session with an agent to maintain conversation context.     |     [Sessions API](./agentic-apps/sessions)     |
|  Send user input to an agent within a session and receive the agent's response.      |  [Runs API](./agentic-apps/execute) |
|  Upload or delete attachments to a conversation. You can reference these attachments as context during an agent run.   |   [File Attachment APIs](./agentic-apps/upload)       |
|    Create a new Agentic App.    |  [Create App API](./agentic-apps/create-app)   |

### File Management API

| Use Cases   | API     |
|--------|----------|
| Upload a small or large public file in the [supported formats](./apis-list/upload-file-api). | [File Upload API](./apis-list/upload-file-api) |

### View Process Status API

| Use Cases   | API     |
|--------|----------|
| Check the status of an ongoing or completed job related to tools or models. | [Get Dock Status API](./apis-list/get-dock-status) |

### Manage Tools APIs

| Use Cases   | APIs    |
|--------|----------|
| Import a new tool.     |  [Import a New Tool API](./apis-list/import-a-new-tool) |
| Import new configurations, datasets, or updates into a tool.      |   [Import to an Existing Tool API](./apis-list/import-to-an-existing-tool) |
| Export a tool's configuration and associated data, including its flow, for backup, sharing, or reuse.      | [Export a Tool API](./apis-list/export-a-tool) |
| Deploy a specific tool into an environment. | [Deploy a Tool API](./apis-list/deploy-a-tool) |
| Undeploy a specific tool from an environment. | [Undeploy a Tool API](./apis-list/undeploy-a-tool) |

### Manage Models APIs

| Use Cases  | APIs     |
|--------|----------|
|  Import a model in chunks.     |    [Import a Model API](./apis-list/import-a-model)      |
|  Export a trained AI model.     |   [Export a Model API](./apis-list/export-a-model)       |
| Deploy a model into the environment in the Ready to Deploy state and configure its parameters. You must perform the initial deployment manually in the Platform account. Consecutive deployments must happen via the public API.      | [Deploy a Model API](./apis-list/deploy-a-model)         |
| Undeploy a model from the environment.      |   [Undeploy a Model API](./apis-list/undeploy-a-model) |
| Manage external model connections.      |   [External Model Connection APIs](./apis-list/connections-api) |
