...
import { FAQSearch } from "/snippets/FAQSearch.jsx"
...
### How do I access the Agent Platform?

To sign up, use your email address or use Google or GitHub login via single sign-on (SSO). A free tier is available so you can [start building agents](/agent-platform/getting-started) immediately.

To reset a forgotton password, click **Forgot password** on the login page and enter the email address associated with your account. Follow the password reset link delivered in your inbox. If you signed up using Google or GitHub SSO, you don't have a platform password. Instead, sign in using your identity provider.

### How do I invite team members to my workspace?

Open **Settings** > **Team** > **Members** and click **Invite member**. Enter their email address and select a [role](/agent-platform/glossary#role).

Invited members receive an email with a link to join your workspace.

{/* AG: below is part of a single QnA about managing workspace */}

### How do I switch between workspaces?

Click the workspace name in the upper-left corner of Studio. A popup list contains the names of all the workspaces that you can access. Select a workspace to switch to it. Each workspace has its own projects, members, and billing.
...
{/* AG: below is part of a single QnA about managing projects */}
...
{/* AG: Remove instructions from here and link to [create agents](/agent-platform/create-agents) . */}
...
Go to **Settings > AI Configuration > LLM Providers** and click **Add provider**. Select the provider (for example. OpenAI, Anthropic, Azure OpenAI, Google Gemini), paste your API key, and click **Validate and save**. The platform will verify that the key works before saving it.

{/* AG: Remove exact names. Just link to the [list of supported models](/agent-platform/supported-models) */}

### Which LLM models are supported?

Agent Platform supports models from:
...
{/* AG: Move the exact steps to a dedicated trobleshooting article later. Give link to it from here. */}
...
4. If your organization uses a network proxy or firewall, ensure outbound HTTPS traffic to the provider (for example. `api.openai.com`, `api.anthropic.com`) is allowed.

{/* AG: Move the below to the trobleshooting article later. */}
...
2. **Missing keywords.** Every block requires its opening keyword (for example. `AGENT CustomerSupport`, not just `CustomerSupport`).
...
Use `#` for line comments. ABL does not support block comments. Use multiple `#` lines instead. 
...
3. **Validate the collected value.** Use constraints to ensure the input meets requirements (for example. `CONSTRAINT email "Please provide a valid email address"`).
...
The Agent Platform provides multiple memory scopes:
...
Add specific instructions about response style in the agent's `INSTRUCTIONS` block (for example. "Keep responses under 3 paragraphs unless the user asks for detail"). For more control, use `MAX_RESPONSE_TOKENS` and `TEMPERATURE` settings in the agent configuration.
...
2. **Check the model.** More capable models (for example. Claude Sonnet, GPT-4o) follow complex instructions better than smaller models.
...
### Can I call an external API at conversation startup, without using a regular tool call, to load context?

Yes — using ABL `CALL`, specifically a top-level `ON_START`. This is the supported pattern for fetching context before the agent responds. There isn't a way to call an external API that fully bypasses the tool execution pipeline; `CALL` still runs through the runtime's tool executor under the hood. See [External API Call on Start](/agent-platform/faq/external-api-call-on-start).

### My HTTP tool returns a connection error

{/* AG: The 3 errors mentioned below aren't mentioned anywhere else in the docs. These must be included in a troubleshooting section. */}

If the tool call fails with `ECONNREFUSED`, `ENOTFOUND`, or `ETIMEDOUT`, try the following troubleshooting steps:
...
If the tool returns a 401 unauthorized error, then try the following:
...
2. **For OAuth 2.0:** Verify the OAuth connection is configured and the token has not expired. Go to **Project > Settings > Auth Profiles** to check credential status.
3. **Check the API key scope.** Some APIs require specific scopes or permissions for each endpoint.

### OAuth token refresh fails

1. **Verify the refresh token is present.** If the OAuth provider don't issue a refresh token during the initial authorization, re-authorize with the `offline_access` scope.
...
4. **Check for CORS issues** When connecting from the browser, MCP servers must allow the platform's origin.
...
### How do I store tool credentials securely?

Store sensitive values such as API keys, access tokens, and credentials as **Config Variables** with the type set to Secret. Secrets are encrypted and referenced by name in your tool configuration.

1. Go to **Project** > **Settings** > **Config Variables**.
2. Click **Add Variable** and enter a name (for example. `WEATHER_API_KEY`) , set the type as secret and add the value.
3. Reference the secret in your tool with `{{config.WEATHER_API_KEY}}`.

All credentials are encrypted at rest and never exposed in traces or logs.
...
Agent Platform supports the following channels out of the box:
...
Go to **Project Settings > Channels > API**, click **Create API Channel**, and copy the generated API Key. Use the key to authenticate requests to `https://agents.kore.ai/api/v1/conversations`. Full API reference documentation is available at **Project Settings > Channels > API > View API Docs**.

### What are the different environments?

Agent Platform provides three environments for each project:
...
### Does Agent Platform auto-scale?

Yes. As a SaaS platform, Agent Platform handles scaling automatically. Conversation handling, LLM requests, and knowledge base indexing all scale based on demand. You do not need to provision servers, configure load balancers, or manage infrastructure.

{/* AG: Hiding rate limit info temporarily

### What are the rate limits and how to increase it?

In the platform, feature usage is throttled by rate limit thresholds, that are different for different plans and usecases. To know more, see [the platform rate limits](/agent-platform/rate-limits).

*/}

### What about self-hosted deployment?

Agent Platform offers an enterprise self-hosted option for organizations that require deploying the platform within their own infrastructure. This is available on Enterprise plans and includes dedicated support for installation, configuration, and ongoing operations. For details, see [Billing, quotas, and operations](/agent-platform/administration/workspace-account).
...
The Agent Platform enforces strict multi-tenant isolation at every layer:
...
Go to **Settings > Security** and configure **Allowed service domains** with the domains your tools need to access. When this list is non-empty, any HTTP tool call to a domain not in the list is blocked. You can use wildcard patterns (for example. `*.internal.example.com`).
...
Quick setup: add credentials at **Settings > AI Configuration > LLM Providers**, register models at **Settings > AI Configuration > Models**, assign each model to a tier (Fast, Balanced, Powerful, Voice), and set defaults. See [AI model configuration](/agent-platform/administration/ai-configuration) for complete details.

### How do I control which providers my team can use?

Use tenant LLM policies at **Settings > AI Configuration > Policies**. Configure a provider allowlist, model allowlist, and token budgets per project or per user. Policies apply to all projects in the workspace and can't be overridden at the project level.
...
Go to **Project > Settings > Environment Variables**, add variables with scope (all environments or specific environment), and reference them in ABL with `{{env.VARIABLE_NAME}}`. For sensitive values (API keys, tokens), use **Auth Profiles** or mark the environment variable as secret -- both are encrypted at rest.
...
{/* AG: We should remove the specific info from here and leave a link to a dedicated tool calls article. */}
...
- **`HTTP 401`** -- Check the API key or OAuth token. Verify the credential is active in Auth Profiles.
...
### How do I set the language for a voice agent?

For Pipeline Voice, configure the **Language** field in the voice channel settings under **Speech Recognition**.

For Realtime LLM Voice, a separate language configuration is not required. The S2S model automatically detects the language from the caller’s speech.

### Does Artemis support keypad or DTMF input during voice calls?

Yes. Artemis supports DTMF for pipeline voice agents through the Kore Voice Gateway. It can collect keypress input from callers like PINs, or account numbers, and send DTMF tones into an active call to navigate to external IVRs like banking or booking systems. [Learn More](/agent-platform/dtmf-support).
