...
sidebarTitle: FAQs
description: Answers to common questions about Agent Platform, organized by topic.
---
...
### How do I sign up for Agent Platform?

You can sign up with your email address or use Google or GitHub single sign-on. A free tier is available so you can start building agents immediately. See [Quick Start Guide](/agent-platform/getting-started) for details.
...
Yes — using **ABL `CALL`**, specifically a top-level **`ON_START` `CALL`**. This is the supported pattern for fetching context before the agent responds. There isn't a way to call an external API that fully bypasses the tool execution pipeline; `CALL` still runs through the runtime's tool executor under the hood. See [External API Call on Start](/agent-platform/faq/external-api-call-on-start).
