...
Tool credentials (API keys, tokens) are managed through **Auth Profiles** or **Environment Variables**:

1. **Auth Profiles** (recommended): Go to **Project > Settings > Auth Profiles**, create a profile with the appropriate auth type (API Key, Bearer, OAuth2, etc.), and reference it in your tool with `auth_profile: <profile-name>`.
2. **Environment Variables** (for non-credential config): Go to **Project > Settings > Environment Variables**, add a variable marked as secret, and reference it with `{{env.VARIABLE_NAME}}`.
