---
title: Artemis Web SDK overview
sidebarTitle: Overview
---

The platform provides Web SDK to embed agent chat and voice interactions in web applications.

{/* AG: Hiding API ref link temporarily

For authentication and error handling conventions, see [API overview](/agent-platform/api-reference/index).

*/}

The Web SDK (`@koreai/artemis-web-sdk`) provides a TypeScript/JavaScript library for embedding agent chat and voice interactions in web applications. The SDK supports vanilla JavaScript, React hooks, and a web component for drop-in integration.
...
| `getSessionId()` | `string` or `null` | Get the current session ID                            |
...
[Learn more: SDK end-to-end auth setup guide](/agent-platform/sdk/web-sdk-end-to-end-auth-setup).

---

{/* AG: Requested platform dev for ABL SDK package name - will update after confirmation.
...
<Note>The parser also recognizes `MODE:` as a legacy keyword for backward compatibility. Definitions containing `MODE:` are detected as `agent-based` type but `MODE:` is deprecated and should not be used in new definitions. Use per-step `REASONING: true/false` within a `FLOW:` section instead.</Note>
...
{/* AG: Hiding API ref link temporarily; first list item of related links doesn't have a CTA.
