...
<Note>The `strategy` field is only effective inside FLOW GATHER blocks (see [§3.20.2](#3202-gather-within-flow-steps)). In top-level GATHER, the runtime always uses LLM-based extraction; specifying `strategy` at the top level is accepted but has no effect.</Note>
