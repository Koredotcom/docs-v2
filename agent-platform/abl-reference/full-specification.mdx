...
sidebarTitle: ABL Complete Specifications
---
...
The following indicates the implementation status of the features:
...
- 🗺️ — **Roadmap** — type definitions exist but runtime execution isn't yet implemented.

---

## Design Philosophy

### Step-Based vs Agent-Based
...
> **Dual Format Support**: ABL supports both the traditional uppercase keyword format (`.agent.abl`) and a YAML-based format (`.agent.yaml`). The `.abl` format requires **uppercase keywords** (`AGENT:`, `GOAL:`, `TOOLS:`). The `.yaml` format uses **lowercase keywords** (`agent:`, `goal:`, `tools:`). Keywords aren't interchangeable between formats — using `agent:` in a `.abl` file produces a parser error.
...
- Can reference memory (for example, "References user's past preferences")
...
| -------------- | ------------------ | ------------------------- | ----------------------------------------------------------------------------------------- |
| `auth_profile` | `auth_profile_ref` | `string`                  | Reference to an auth profile by name or config variable (for example,`"{{config.GOOGLE_AUTH}}"`) |
...
- A `GATHER` field with `ENTITY_REF` must not declare `TYPE`, `OPTIONS`, or `VALUES`. The referenced entity owns that contract.
...
#### NLU, INTENTS, MULTI_INTENT, MESSAGES, and TESTS
...
TESTS:
  happy_path:
    ...
    expected: { toolCalled: "lookup_order", responseContains: "order" }
```
...
| `TESTS`        | Named test cases for authoring/evaluation tooling — each test has an input and an expected block (toolCalled, responseContains, signal, stateUpdates). Parsed and surfaced to tooling; the runtime skips the section and never executes it.|
...
<Note>The `strategy` field is only effective inside FLOW GATHER blocks (see [this](#enhanced-gather-within-flow)). In top-level GATHER, the runtime always uses LLM-based extraction; specifying `strategy` at the top level is accepted but has no effect.</Note>
...
Agent Platform provides six base storage types and more than 25 semantic-type mappings for migration from [AI for Service Platform](/ai-for-service/index):
...
**Kore Entity Type Mappings** (for migration from AI for Service):
...
### TEMPLATES (Named Response Templates)
...
### BEHAVIOR_PROFILES (Context-Dependent Behavior)
...
- `CORRECTIONS` - Allow natural corrections like "actually 4 guests not 3"
  ...
  - When `CORRECTIONS: false` (default): The same message is treated as new input for the current step. Use this for:
    ...
    - Strict sequential forms where backtracking is not allowed (for example, regulatory compliance)
...
### Conditional Branching (ON_INPUT)
...
### Execution Pipeline (Supervisor Pre-Classification)
...
| Function          | Signature                                        | Description                                            |
| ----------------- | ------------------------------------------------ | ------------------------------------------------------ |
| `MASK`            | `MASK(s, pattern, char?) → string`               | Mask string (for example, `MASK(acct, "last4")` → `****1234`) |
| `FORMAT_CURRENCY` | `FORMAT_CURRENCY(n, currency, locale?) → string` | Format as currency (for example, `$1,234.56`)                 |
| `FORMAT_DATE`     | `FORMAT_DATE(d, format, tz?) → string`           | Format date (for example, `"MMM DD, YYYY"`)                   |
| `ORDINAL`         | `ORDINAL(n) → string`                            | Ordinal suffix (for example, `1` → `"1st"`)                   |
...
> **Legacy syntax**: `ON_<event>:` (for example, `ON_SESSION_START:`) is deprecated. Use `ON: session:start` instead. Legacy names are auto-normalized: `session_start` → `session:start`, `agent_enter` → `agent:*:after`.
...
### Supervisor for Agent-Based Agents (Unified AgentIR)
