...
| `ON_RETURN`     | `string`                | No       | --      | Instruction for handling the return (for example. step to resume at).                               |
...
2. The `ON_RETURN` instruction guides what happens next (for example. resume a step, re-evaluate conditions).
