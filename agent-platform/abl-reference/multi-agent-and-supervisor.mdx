...
<Note>
  \*`role` is optional, but omitting it on an agent that has a `BEHAVIOR` block produces a
  deprecation warning and falls back to legacy inference. Declaring `role` is the recommended,
...
`routing.flow_policy_ref` (or `transfer_flow_policy_ref`) names a policy declared in a top-level
`TRANSFER_FLOW_POLICIES:` block. A transfer flow policy defines what happens while a caller waits in
queue or when no agent is available — for example, playing a message or triggering a follow-up
workflow. The policy body is free-form (its shape is defined by the human-agent integration, not the
ABL parser).
...
<Warning>
  `COMPLETE`'s `WHEN` is **deterministic** — it must be a boolean expression over session state.
  Plain-English / intent-style text is rejected at compile time (`PLAIN_ENGLISH_COMPLETE_WHEN`). Use
  `NLU`/routing for intent matching, not `COMPLETE WHEN`.
</Warning>
...
- [Lifecycle & hooks](/agent-platform/abl-reference/lifecycle-and-hooks) -- ON_START and hooks that interact with multi-agent flows
