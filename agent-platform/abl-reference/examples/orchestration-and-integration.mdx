...
<Note>ABL handles parallel tool execution natively -- when the LLM determines that multiple tools are independent, the runtime executes them concurrently. For step-based execution, you can use `DELEGATE` blocks with `PARALLEL: true` to explicitly fan out.</Note>
...
ESCALATE:
  ...
  routing:
    ...
    skill_tags: [context_preservation]
```
