...
| ---------- | -------- | ---------- |
| `INVALID_SIP_TRANSFER_DESTINATION` | Error | A trusted-source (`config`/`system`) `sipTransferId` default is empty, contains a `{{ }}` template, is not a valid `sip:`/`sips:` URI, or uses a dynamic source (`expr`, `lookup`, `runtime`, `auth`). The destination must be a static literal — a target derived from conversation data could be influenced by the caller. |
...
Omit the `sipTransferId` default entirely to rely solely on the project-configured Agent Transfer target. For a **dynamic** per-call target, configure it in project Agent Transfer settings rather than the DSL — templated `sipTransferId` defaults are rejected by design.
