...
<Note>
  ...
  signatures. GATHER fields and [`ENTITIES:`](/agent-platform/abl-reference/nlu) entries use a
...
<Note>
  Table names must match `^[a-z_][a-z0-9_]*$` and field names `^[a-zA-Z_][a-zA-Z0-9_.]*$`. A small
  inline map table can also be written in shorthand: `my_table: { key1: value1, key2: value2 }`.
</Note>
...
<Note>
  The DSL keyword for the MIME allowlist is **`allowed_types`** (it populates the field's allowed
  MIME types). `allowed_mime_types` is not recognized by the `.agent.abl` parser.
</Note>
...
- [Expressions & functions](/agent-platform/abl-reference/rich-content-and-expressions#expressions-and-functions) -- type coercion in expressions and built-in type-checking functions
