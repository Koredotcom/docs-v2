...
<Note>
  As shorthand, a `READS:` sub-block declares persistent paths with `ACCESS: read`, and a `WRITES:`
  sub-block declares them with `ACCESS: write` — an alternative to listing each path under
  `persistent:` with an explicit `ACCESS`.
</Note>
...
Array targets can accumulate values instead of being overwritten, using the
block form of `STORE`:
