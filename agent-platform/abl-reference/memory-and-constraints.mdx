...
<Warning>
  Legacy event names such as `search:before`, `ON_START`, or `ON_SEARCH` are no longer supported and
  are rejected during compilation — use the canonical patterns above. Note that `tool:<name>:before`
  is not a valid recall event (only `tool:<name>:after`).
</Warning>
...
- [Expressions & functions](/agent-platform/abl-reference/rich-content-and-expressions#expressions-and-functions) -- condition expression syntax
- [Multi-Agent & Supervisor](/agent-platform/abl-reference/multi-agent-and-supervisor) -- HANDOFF and ESCALATE actions referenced in ON_FAIL
