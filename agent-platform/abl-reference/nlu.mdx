...
The top-level `MULTI_INTENT:` section configures how the agent handles a single user message that
contains **more than one intent** — for example, *"Book me a flight to NYC and also cancel my hotel
reservation."* When enabled, the NLU pipeline detects and triages multiple intents per turn, and the
`strategy` controls how they are dispatched.
...
<Note>
  The user-facing prompts for the `disambiguate` and `primary_queue` flows (the "I noticed more than
  one request…" text) are customizable via the [`MESSAGES:`](/agent-platform/abl-reference/lifecycle-and-hooks#messages) block
  (keys such as `multi_intent_disambiguate_header` and `multi_intent_queued_notice`).
</Note>
