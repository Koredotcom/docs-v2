...
The Artemis Platform CLI lets you create, review, customize, test, and deploy agents, tools, and enterprise content connectors from your terminal or CI pipeline, without opening the browser.

The CLI supports both interactive development and repeatable automation workflows. The sections below follow the agent lifecycle: create, review, customize, test, and deploy.
...
To generate using Arch AI, you must [configure a primary model](/agent-platform/). You can create a session even when the tenant isn't configured for generation.

**Related:** continue an Arch AI conversation in [Iterate with Arch AI](#iterate-with-arch-ai), inspect its output in [Inspect Arch AI-generated state](#inspect-arch-ai-generated-state), and refresh ingested content in [Keep enterprise content current](#keep-enterprise-content-current).
...
**Related:** Arch AI-generated projects are created in [Generate a project with Arch AI](#generate-a-project-with-arch-ai) and refined in [Iterate with Arch AI](#iterate-with-arch-ai).
...
**Related:** apply incoming Git changes in [Apply Git updates](#apply-git-updates), and start the initial content sync in [Connect enterprise content](#connect-enterprise-content).
...
**Related:** push local customizations before pulling in [Track customizations with Git](#track-customizations-with-git).
