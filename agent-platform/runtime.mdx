...
The Runtime enforces per-tenant rate limits on a rolling 1-minute window. For default values, see [the platform rate limits](/agent-platform/drafts/rate-limits).

When a limit is exceeded, the Runtime returns HTTP `429`. See [rate limit headers](/agent-platform/drafts/rate-limits#rate-limit-headers). Contact your account team to adjust limits for your plan.
