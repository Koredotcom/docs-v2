---
title: How to choose between a reasoning agent and a FLOW-based agent
sidebarTitle: Reasoning or FLOW
---
...
An Agent Business Language (ABL) agent can run without a `FLOW:` block or with one.
...
Use this pattern when the customer must not skip required fields or when operations teams need to inspect the exact conversation path.
...
| ------------------------------------------------------ | --------------------------------------------------------------- | --------------------------------------------------------------------------- |
| Using `FLOW` for every conversation                    | Developers want control even when a goal is enough.             | Start with a reasoning agent when the outcome is flexible and low-risk.     |
| Using `WHEN: true` as the main completion rule         | It makes examples succeed before the customer outcome is complete. | Complete on meaningful fields, tool results, or final state.                |
| Adding a reasoning step that never affects the outcome | The flow has a reasoning step but no clear purpose.             | Give the step a narrow goal and explain what it prepares for the next step. |
