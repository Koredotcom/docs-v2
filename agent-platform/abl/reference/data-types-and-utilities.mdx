...
Dates represent calendar dates without a time component. They're stored as ISO 8601 format (`YYYY-MM-DD`).
...
Datetimes include both date and time with timezone information. They're stored in ISO 8601 format.
...
| Source     | Description                                              |
|------------|----------------------------------------------------------|
| `system`   | Set by the runtime (for example, session ID, timestamp). |
...
<Note>`TypeDefinition` is the **structural** type algebra used for `MEMORY:` variables and tool signatures. GATHER fields and [`ENTITIES:`](/agent-platform/abl-reference/nlu) entries use a broader **entity type** set that additionally includes `text`, `free_text`, `integer`, `float`, `currency`, `email`, `phone`, `enum`, `pattern`, and `location`. Those extra type names are not part of `TypeDefinition`.</Note>
...
|-------------------|-------------------------|----------|----------|---------------------------------------------------------------------------------------------------------------|
| `source`          | `string`                | Yes      | `inline` | Data source type: `inline`, `collection`, or `api`. Legacy aliases: `mongodb` → `collection`, `http` → `api`. |
| `values`          | `string[]`              | No       | --       | Static values (required for `source: inline`).                                                                |
| `map`             | `Record<string,string>` | No       | --       | Inline key → value mapping (an alternative to `values`; keys become the valid values).                        |
| `table_name`      | `string`                | No       | --       | Logical table name (required for `source: collection`). Alias: `collection`.                                  |
...
| `headers`         | `Record<string,string>` | No       | --       | Custom HTTP headers for `source: api` (for example, an `Authorization` header).                               |
...
| `fuzzy_match`     | `boolean`               | No       | `false`  | Whether fuzzy (approximate) matching is enabled.                                                              |
| `fuzzy_threshold` | `number`                | No       | `0.85`   | Minimum similarity score (0.0--1.0) for fuzzy matches to be accepted.                                         |

<Note>Table names must match `^[a-z_][a-z0-9_]*$` and field names `^[a-zA-Z_][a-zA-Z0-9_.]*$`. A small inline map table can also be written in shorthand: `my_table: { key1: value1, key2: value2 }`.</Note>
...
A GATHER field references a lookup table through its `semantics.lookup` metadata, which names the
...
Tool parameters can source a value from a lookup table via `SOURCE: lookup(<table>)` in a
`.tools.abl` parameter block (see [Tools](/agent-platform/abl-reference/tools)); this resolves to a
...
| `category`              | `string`   | No       | `document` | Content category: `document`, `image`, `audio`, or `video`.                            |
| `required`              | `boolean`  | No       | `true`     | Whether the attachment is mandatory.                                                   |
| `max_file_size_mb`      | `number`   | No       | --         | Maximum file size in megabytes. Alias: `max_size_mb`.                                  |
| `allowed_types`         | `string[]` | No       | --         | Allowed MIME types. If omitted, all types for the category are accepted.               |
| `ocr_enabled`           | `boolean`  | No       | `false`    | Enable OCR text extraction for `document` and `image` categories.                      |
| `transcription_enabled` | `boolean`  | No       | `false`    | Enable audio transcription for `audio` and `video` categories. Alias: `transcription`. |
| `key_frame_extraction`  | `boolean`  | No       | `false`    | Enable keyframe extraction for `video` category.                                       |

<Note>The DSL keyword for the MIME allowlist is **`allowed_types`** (it populates the field's allowed MIME types). `allowed_mime_types` is not recognized by the `.agent.abl` parser.</Note>
...
The extracted text is available in expressions and template strings:
...
- If the file's MIME type is not in `allowed_types`, the upload is rejected.
- If `allowed_types` is omitted, all standard MIME types for the category are accepted.
...
The `DESTINATIONS:` block declares named **outbound HTTP targets** for delivering data or
notifications to external systems. Each destination has a name (the entry key), a `url`, and
optional method/auth/headers.
...
DESTINATIONS:
  doc_processor:
    ...
    headers:
      X-Source: "abl-agent"
      Authorization: "Bearer {{config.PARTNER_API_KEY}}"
...
| Property  | Type                     | Required | Default | Description                                                        |
| --------- | ------------------------ | -------- | ------- | ------------------------------------------------------------------ |
| `name`    | `string`                 | Yes      | --      | Destination identifier (the entry key).                            |
| `url`     | `string`                 | Yes      | --      | Outbound URL. Validated for SSRF at compile time.                  |
| `method`  | `string`                 | No       | --      | HTTP method (`GET`, `POST`, `PUT`, …), upper-cased by the parser.  |
| `auth`    | `string`                 | No       | --      | Auth strategy reference (resolved from project config — no secrets in ABL). |
| `headers` | `Record<string, string>` | No       | --      | Custom HTTP headers. Supports `{{config.X}}` placeholders.         |

---

**Related articles:**

- [Expressions and functions](/agent-platform/abl-reference/rich-content-and-expressions#expressions-and-functions) -- type coercion in expressions and built-in type-checking functions
- [Memory and Constraints](/agent-platform/abl-reference/memory-and-constraints) -- TYPE declarations in session and persistent variables, attachment data in session
