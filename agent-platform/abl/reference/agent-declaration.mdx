...
| `token_budget`             | `number`  | No       | _none_           | Cumulative LLM token budget for a single execution/session; primarily used to bound debug/test runs |
...
### Voice coordinator policy

For realtime voice agents, `EXECUTION.voice.coordinator_policy` sets developer-controlled delivery preferences — tone, pace, brevity, and accessibility of spoken output. It shapes how the voice coordinator speaks; the semantic content still comes from the agent's goal and prompts.
...
EXECUTION:
  voice:
    coordinator_policy:
      language: en-US
      tone: warm                              # neutral | warm | formal | energetic
      pace: slow                              # slow | normal | fast
      brevity: concise                        # concise | standard
      interruption: acknowledge_then_resume   # resume_latest | acknowledge_then_resume
      greeting_style: brief                   # brief | warm | none
      accessibility:
        confirm_critical_values: true
        spell_sensitive_values: true
```

| Field                              | Type                                                    | Description                                                              |
| ---------------------------------- | ------------------------------------------------------- | ------------------------------------------------------------------------ |
| `language`                         | `string`                                                | BCP-47 language tag for delivery (e.g. `en-US`).                        |
| `tone`                             | `"neutral"` \| `"warm"` \| `"formal"` \| `"energetic"`  | Speaking tone.                                                          |
| `pace`                             | `"slow"` \| `"normal"` \| `"fast"`                      | Speaking pace.                                                          |
| `brevity`                          | `"concise"` \| `"standard"`                             | How terse responses should be.                                         |
| `interruption`                     | `"resume_latest"` \| `"acknowledge_then_resume"`        | How to resume after the caller interrupts.                             |
| `greeting_style`                   | `"brief"` \| `"warm"` \| `"none"`                       | Opening-greeting style.                                                 |
| `accessibility.confirm_critical_values` | `boolean`                                          | Read back critical values (amounts, dates) for confirmation.           |
| `accessibility.spell_sensitive_values`  | `boolean`                                          | Spell out sensitive values (codes, IDs) character by character.        |

<Note>Every key also accepts its camelCase spelling (`greetingStyle`, `confirmCriticalValues`, `spellSensitiveValues`). This delivery policy is separate from the per-output [scoped `barge_in`](/agent-platform/abl/reference/flow#scoped-barge-in) policy.</Note>
