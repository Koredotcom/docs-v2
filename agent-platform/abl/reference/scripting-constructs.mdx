...
For escalation from an error handler, use `THEN: ESCALATE with REASON: "..."`. Don't use an unsupported nested escalation-priority form inside `ON_ERROR`. `THEN` in an `ON_ERROR` handler takes only one of a fixed set of actions (`CONTINUE`, `COMPLETE`, `ESCALATE`, `RETRY_STEP`, `HANDOFF`, `BACKTRACK`) — not a step name. The handler's own key is likewise matched against a fixed canonical set of error types (`default`, `llm_error`, `tool_error`, `routing_error`, `validation_error`, `timeout_error`, `unknown_error`); older names such as `tool_timeout`, `invalid_input`, and `api_error` are not valid types and never match — see [Error types](/agent-platform/abl-reference/lifecycle-and-hooks#error-types).
