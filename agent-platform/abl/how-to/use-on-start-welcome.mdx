---
title: How to use ON_START to greet users before the first message
sidebarTitle: Greet with ON_START
---
...
```yaml expandable=true
AGENT: On_Start_Personalized_Agent
GOAL: "Greet the user with values set during startup"

ON_START:
  SET: startup_source = session.channel
  RESPOND: "Hello. You reached us from {{startup_source}}."
...
This works on every channel that triggers a welcome. On Kore Agent Assist, Agent AI triggers it when you select **Use 'Trigger Welcome message' as the first request to Agentic**. For details, see [Agent AI integration](/agent-platform/integrations/agent-ai#choose-first-request-trigger).
