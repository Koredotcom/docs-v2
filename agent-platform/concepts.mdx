...
Agent Platform is built on four interconnected pieces: **Agents**, **Supervisors**, **Tools**, and **Sessions**. Understanding how they relate gives you a foundation for everything else on the platform.
...
| Building Block | Analogy                   | Responsibility                              |
| ---------- | ------------------------- | ------------------------------------------- |
...
Every agent is defined in ABL&trade; (Agent Blueprint Language), a schema-driven language purpose-built for multi-agent orchestration. You declare _what_ the agent should do; the platform figures out _how_ to execute it. ABL definitions compile into immutable intermediate representations (IRs) — every version is auditable, every deployment is reproducible.
...
{/* AG: Check the casing in the table below for all caps, especially since backticks are used so casing should also be literal. */}
