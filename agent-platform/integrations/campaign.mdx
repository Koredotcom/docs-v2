---
title: CCAI outbound campaign integration with Artemis
sidebarTitle: Campaign
---

<Badge icon="arrow-left" color="gray">[Back to Integration overview](/agent-platform/integrations)</Badge>

This guide explains how to configure the AI for Service (CCAI) outbound voice campaign to place calls through Artemis and trigger a specific deployed agent for each call. It reflects the current Campaigns for Artemis implementation.
...
<Steps>
  <Step title="Configure the Artemis project">
  ...
  <Step title="Configure the CCAI campaign in AI for Service">
    ...
    1. Click **Create New Voice Campaign**, or edit an existing voice campaign. [Learn more](/ai-for-service/contact-center/campaigns#create-a-voice-campaign).
    1. Select the dialing mode **Agentless**, and turn on **Enable Campaigns for Artemis**.
    1. Click **Edit Settings** and enter the Artemis linkage:
...
Campaigns for Artemis supports one dialing mode, **Agentless**. The agent handles the call end-to-end; Start Flow and experienceFlow aren't used in V2.

In Agentless mode, the outbound tag carries the complete userInfo object, so standard and custom contact fields reach the Runtime session. For details, see [Read Campaign Contact Data in an Artemis Runtime Session](/agent-platform/integrations/read-campaign-contact-data).
...
| A BYOC call fails or uses the wrong trunk        | The BYOC connection lacks a distinct carrier, so the fallback is refused.                          | Set the BYOC connection's own carrier; it must not equal the shared default.                                                    |
...
- [Read Campaign Contact Data in an Artemis Runtime Session](/agent-platform/integrations/read-campaign-contact-data)
- [CCAI integration with Artemis](/agent-platform/integrations/ccai)