...
- **No file attachments**: File uploads aren't supported in either mode yet.
- **No automatic reconnect**: If the underlying connection drops, the session doesn't resume automatically and the conversation requires a new transfer.
...
- Your Genesys Cloud region's login and WebSocket URLs (see [Note your region URLs](#4-note-your-region-urls)).
...
### 2. Create an inbound message flow
...
3. Configure the following fields:
    ...
    | Integration Type | Choose based on the deployment created in [Genesys Cloud setup](#3-create-a-deployment). | `messaging` — Web Messaging (recommended). `webchat` — Guest Chat v2 / Third-Party Widget. |
    | Genesys Cloud Login URL | Genesys Cloud region URL. See [region URLs](#4-note-your-region-urls). | Used to derive the Guest Chat API base URL for WebChat mode. |
    ...
    | Genesys WebSocket URL (optional) | Genesys Cloud region URL. See [region URLs](#4-note-your-region-urls). | Required for Web Messaging mode, such as `wss://webmessaging.mypurecloud.com/v1`. |
...
5. Use the Genesys Cloud integration for Agent Transfer routing. Go to **Project Settings** > **Agent Transfer** > **Default Routing** > **Connection** and select the integration.
...
1. Open **Studio** > **Agent Detail** > **Test**.
2. Trigger an agent transfer in the test conversation.
3. Open Genesys Cloud **Agent Desktop** and accept the incoming interaction from the configured queue.
4. Confirm that messages sent from the test conversation appear on the agent side, and that agent replies appear back in the Artemis conversation.
...
| WebSocket connection not found (404) | Wrong WebSocket URL for the region | Verify the region-specific endpoint against the [region URLs](#4-note-your-region-urls) table. |
