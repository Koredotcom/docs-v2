...
| Option                                                        | Description                                                                                              |
|---------------------------------------------------------------|----------------------------------------------------------------------------------------------------------|
| Use 'Trigger Welcome message' as the first request to Agentic | Triggers the agent's welcome as soon as the widget loads, before any customer message. Artemis runs the agent's `ON_START` handler, including its `SET` and `CALL` statements, and returns the welcome rendered with live session values. For details, see [How to use ON_START to greet users](/agent-platform/abl/how-to/use-on-start-welcome). |
| Pass landing summary as first request to Agentic App          | Sends the app-customer conversation's landing summary as the first request to Artemis on agent transfer. |

<Note>If you don't select either option, Artemis generates NBAs only when a customer sends a message.</Note>
...
|-----------------------------|-------------------------------------------------------------------------------------|
| Landing Summary             | Summary of the customer-app conversation before transfer to a human agent.          |
