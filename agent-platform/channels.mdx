...
| **Messaging** | Slack, WhatsApp, Teams, and other messaging apps | You register the external provider's credentials. The platform returns a webhook URL that the provider calls when a user sends a message. |
...
| --- | --- | --- |
| Web SDK | Embed a chat widget in your website with a single script tag | Set up the Web SDK |
...
| Channel | Description | Setup |
| --- | --- | --- |
| Realtime LLM Voice | Live voice conversations using realtime LLM models (OpenAI Realtime, Gemini Live) via Kore.ai Voice Gateway | Set up Realtime LLM Voice |
| Pipeline Voice | Traditional STT -> LLM -> TTS voice pipeline via Kore.ai Voice Gateway, with configurable ASR/TTS models | Set up Pipeline Voice |
...
| Microsoft Teams | Integrate agents into Teams channels and chats | Set up Microsoft Teams |
...
| AIforWork | Kore.ai's Employee Experience (EX) platform -- bidirectional agent messaging integration | Set up AI for Work |
...
* Deploy Agents -- deployments, environments, and the **Deployments** -> **Channels** workspace.