...
- Max batch size: 40 files per upload.
...
* **Advanced (custom JSON)** — add custom fields not covered by the standard list, as JSON, e.g. `{"custom_string_1": "value", "custom_number_1": 42}`.
...
- **Extraction quality** controls how aggressively the crawler cleans and filters page content during extraction. Default is **Balanced**.
