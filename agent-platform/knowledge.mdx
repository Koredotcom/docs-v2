...
Use this option to automatically recrawl the configured website at a specified date and time. Scheduled crawls help keep indexed content up to date without requiring manual execution.

To configure a scheduled crawl:

- Enable the Scheduled Crawl toggle.
- Specify the following settings:

  | Field | Description |
  |-------|-------------|
  | **Date** | Select the date on which the scheduled crawl should begin. |
  | **Time** | Specify the time when the crawl should start. |
  | **Timezone** | Select the timezone in which the scheduled crawl should run. |
  | **Repeat** | Specify how often the crawl should run. Select **Run Once** to execute the crawl only once, or choose a recurring schedule to automatically recrawl the website at the configured interval (if supported). | run. Select Run Once to execute the crawl only once, or choose a recurring schedule to automatically recrawl the website at the configured interval (if supported).

- Click **Save** to save the schedule.

<Note>This configuration is available only after the initial sync is successful. Go to the source, click **Reconfigure Crawl Strategy** and then click on **Configure** to view the scheduler. </Note>
...
|-----------|-------------|
| Aha! | Index ideas, features, and roadmaps from Aha! |
| Airtable | Index spreadsheets and databases from Airtable bases |
| Amazon S3 Connector | Index files from S3 buckets |
| Asana | Index tasks and projects from Asana |
| Axero | Index Axero spaces, articles, blogs, wikis, files, announcements, and comments |
| Azure Blob Storage | Index blobs from Azure Blob Storage containers |
| Bitbucket | Index repository metadata, pull requests, comments, and commit timelines |
| Bigtincan | Index training courses and content from Bigtincan |
| [Box Connector](/agent-platform/connectors/box) | Index files and folders from Box cloud storage |
| ClickUp | Index tasks and spaces from ClickUp |
| Coda | Index docs and tables from Coda |
| [Confluence Cloud Connector](/agent-platform/connectors/confluence) | Index Cloud spaces, pages, blogs, attachments, comments, and tables |
| Confluence Server | Index Server spaces, pages, blogs, attachments, and comments |
| Datadog | Index metrics, dashboards, and monitors from Datadog |
| Discord | Index messages from Discord servers and channels |
| DotCMS | Index content and pages from DotCMS |
| Dropbox | Index files and folders from Dropbox |
| Dynamics 365 | Index entities and records from Microsoft Dynamics 365 |
| Egnyte | Index files and folders from Egnyte cloud storage |
| Figma | Index design files and components from Figma |
| File Upload | |
| Freshdesk | Index tickets and knowledge base from FreshDesk |
| Freshservice | Index solution articles and tickets from Freshservice |
| Front | Index knowledge base articles from Front |
| GitBook | Index documentation from GitBook spaces |
| GitHub | Index repositories, issues, and wikis from GitHub |
| GitLab | Index repositories, issues, and wikis from GitLab |
| Gmail | Index emails and attachments from Gmail |
| Google Chat | Index messages from Google Chat spaces |
| Google Cloud Storage | Index objects from Google Cloud Storage buckets |
| Google Docs | Index documents from Google Docs |
| [Google Drive Connector](/agent-platform/connectors/googledrive) | Index files and folders from Google Drive |
| Guru | Index knowledge cards and collections from Guru |
| Help Scout | Index docs, articles, and conversations from HelpScout |
| Hive | Index projects, tasks, and actions from Hive |
| HubSpot | Index contacts, deals, and knowledge base from HubSpot |
| Intercom | Index conversations and articles from Intercom |
| Invision Community | Index forums, articles, and discussions from Invision Community |
| JFrog Artifactory | Index artifacts and repositories from JFrog Artifactory |
| Jenkins | Index build jobs, pipelines, and logs from Jenkins |
| [Jira Connector](/agent-platform/connectors/jira) | Index Jira Cloud issues, comments, worklogs, and attachments |
| Jira Data Center | Index Jira Data Center or Server issues, comments, worklogs, and attachments |
| [Custom Connector](/agent-platform/connectors/custom-connector) | Index structured data from JSON endpoints and files |
| Linear | Index issues and projects from Linear |
| LumApps | Index content and communities from LumApps intranet |
| Microsoft Teams | Index messages and files from Microsoft Teams channels |
| Microsoft Word Online | Index documents from Microsoft Word Online |
| Miro | Index boards and content from Miro whiteboards |
| Monday.com | Index boards and items from Monday.com |
| Notion | Index pages and databases from Notion workspaces |
| OneDrive | Index files and folders from Microsoft OneDrive |
| Opsgenie | Index alerts, incidents, and on-call schedules from Opsgenie |
| Oracle Knowledge | Index records and knowledge articles from Oracle applications |
| PagerDuty | Index incidents and services from PagerDuty |
| Pipedrive | Index deals, contacts, and activities from Pipedrive |
| Quip | Index documents and spreadsheets from Quip |
| ReadMe | Index API documentation from ReadMe |
| Re\:amaze | Index conversations and knowledge base from Re\:amaze |
| Rich Media | Upload and index images, audio, and video files |
| RSS Feed | Index articles and content from RSS feeds |
| SAP | Index records and documents from SAP systems |
| [Salesforce Connector](/agent-platform/connectors/salesforce) | Index Salesforce Knowledge, cases, CRM records, attachments, comments, and custom objects with native ACL sync |
| [ServiceNow Connector](/agent-platform/connectors/servicenow) | Index knowledge articles, tasks, attachments, comments, and ACL-aware records |
| [SharePoint Connector](/agent-platform/connectors/sharepoint) | Index sites, libraries, and documents from Microsoft SharePoint |
| Shortcut | Index stories, epics, and iterations from Shortcut |
| Shopify | Index products, orders, and content from Shopify stores |
| Slab | Index posts and topics from Slab knowledge base |
| Slack | Index messages and files from Slack channels |
| Slite | Index notes and documents from Slite |
| Sitemap | Discover and index pages from XML sitemaps |
| Stack Overflow for Teams | Index questions and answers from Stack Overflow for Teams |
| Swagger / OpenAPI | Index API specifications from Swagger and OpenAPI definitions |
| Teamwork | Index projects, tasks, and milestones from Teamwork |
| TestRail | Index test cases, runs, and plans from TestRail |
| Trello | Index boards and cards from Trello |
| Vimeo | Index video transcripts and metadata from Vimeo |
| Wolken Service Desk | Index tickets and knowledge articles from Wolken Service Desk |
| Workday | Index employee records and documents from Workday |
| WordPress | Index posts, pages, and content from WordPress sites |
| Wrike | Index projects, tasks, and folders from Wrike |
| xMatters | Index incidents, on-call schedules, and workflows from xMatters |
| YouTrack | Index issues and projects from JetBrains YouTrack |
| YouTube | Index video transcripts and metadata from YouTube |
| [Zendesk Connector](/agent-platform/connectors/zendesk) | Index tickets, Help Center articles, and comments from Zendesk |
| Zeplin | Index design specs and styleguides from Zeplin |
| Zoho CRM | Index leads, contacts, and records from Zoho CRM |
| Zoom | Index meeting transcripts and recordings from Zoom |
| Zulip | Index messages and topics from Zulip chat streams |
