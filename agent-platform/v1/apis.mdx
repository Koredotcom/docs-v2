...
<Badge icon="arrow-left" color="gray">[Back to TOC](/agent-platform/v1/index#references)</Badge>
...
Access to APIs in the Agent Platform is governed at three levels:

1. Account-Level Permissions : Access to API keys and scopes is controlled by account-level roles and permissions.

Users must have appropriate permissions to:

- View or generate API keys
- Access Agentic Apps or Tools
- Configure API scopes

Without required permissions, API keys and scopes are not accessible in Apps and Tools. [Learn More](/agent-platform/v1/administration/security-and-control#api-scopes).

2. Agentic App-Level API Access: Each Agentic App has its own API scope and corresponding API keys. These are required to access Agentic app APIs. 

<Note>API keys generated at this level are scoped only to that specific Agentic App.</Note>

3. Tool-level API Access

Workflow Tools in the library can also be deployed and accessed independently. Use when:

- Tools need to be accessed directly (outside app execution).
- External systems integrate with tools individually.

<Note>Tool API keys are independent of Agentic App keys.</Note>

**API Scope Hierarchy**

| Level        | Scope Applies To | Purpose                         |
|--------------|----------------|----------------------------------------------|
| Account      | Entire platform | Controls visibility and permissions          |
| Agentic App  | Individual app | Used for app execution and management APIs               |
| Tool         | Individual tool | Used for direct tool access (workflow tools)   |

## Agentic App APIs

All Agentic App API requests are made to URL `https://<your-agent-platform-env>.<domain>.com/api/v2/`. All requests must include an API Key in the `Authorization` header. You can generate the API key from the application. 

### Generate an API Key for an Agentic App

1. Navigate to the required Agentic App.
2. From the left menu, go to Deploy → API Scopes.
3. Create a new API scope by selecting the required permissions.
4. Generate an API key and associate it with the created scope.
...
![Session Resolution using Identify fields](/agent-platform/v1/apis/agentic-apps/images/session-resolution.png "Session resolution")
...
* Prevent misuse - Protect against unauthorized or excessive use.
...
1. [Login](/agent-platform/v1/getting-started) to the Platform and [access](/agent-platform/v1/administration/overview) the Settings Console.
2. On the left menu, click **Security & Control** > **API Scopes**.
3. Create an API app and include all the required scopes, and the API keys as mentioned [here](/agent-platform/v1/administration/security-and-control#create-an-api-app).
...
| Use Cases                                                                                                           | APIs                                                             |
|:--------------------------------------------------------------------------------------------------------------------|:-----------------------------------------------------------------|
| Create, retrieve, or end a session with an agent to maintain conversation context.                                  | [Sessions API](/agent-platform/v1/apis/agentic-apps/sessions)       |
| Send user input to an agent within a session and receive the agent's response.                                      | [Runs API](/agent-platform/v1/apis/agentic-apps/execute)            |
| Upload or delete attachments to a conversation. You can reference these attachments as context during an agent run. | [File Attachment APIs](/agent-platform/v1/apis/agentic-apps/upload) |
| Create a new Agentic App.                                                                                           | [Create App API](/agent-platform/v1/apis/agentic-apps/create-app)   |
| Manage API Keys.                                                                                           | [API Key Management](/agent-platform/v1/apis/agentic-apps/api-key-mgmt)   |
| Export and Import App Configurations                                                                                           | [Import/Export APIs](/agent-platform/v1/apis/agentic-apps/import-export)   |

### File Management API

| Use Cases                                                                                                       | API                                                               |
|:----------------------------------------------------------------------------------------------------------------|:------------------------------------------------------------------|
| Upload a small or large public file in the [supported formats](/agent-platform/v1/apis/apis-list/upload-file-api). | [File Upload API](/agent-platform/v1/apis/apis-list/upload-file-api) |

### View Process Status API

| Use Cases                                                                   | API                                                                   |
|:----------------------------------------------------------------------------|:----------------------------------------------------------------------|
| Check the status of an ongoing or completed job related to tools or models. | [Get Dock Status API](/agent-platform/v1/apis/apis-list/get-dock-status) |

### Manage Tools APIs

| Use Cases                                                                                             | APIs                                                                                        |
|:------------------------------------------------------------------------------------------------------|:--------------------------------------------------------------------------------------------|
| Import a new tool.                                                                                    | [Import a New Tool API](/agent-platform/v1/apis/apis-list/import-a-new-tool)                   |
| Import new configurations, datasets, or updates into a tool.                                          | [Import to an Existing Tool API](/agent-platform/v1/apis/apis-list/import-to-an-existing-tool) |
| Export a tool's configuration and associated data, including its flow, for backup, sharing, or reuse. | [Export a Tool API](/agent-platform/v1/apis/apis-list/export-a-tool)                           |
| Deploy a specific tool into an environment.                                                           | [Deploy a Tool API](/agent-platform/v1/apis/apis-list/deploy-a-tool)                           |
| Undeploy a specific tool from an environment.                                                         | [Undeploy a Tool API](/agent-platform/v1/apis/apis-list/undeploy-a-tool)                       |

### Manage Models APIs

| Use Cases                                                                                                                                                                                                                        | APIs                                                                             |
|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|:---------------------------------------------------------------------------------|
| Import a model in chunks.                                                                                                                                                                                                        | [Import a Model API](/agent-platform/v1/apis/apis-list/import-a-model)              |
| Export a trained AI model.                                                                                                                                                                                                       | [Export a Model API](/agent-platform/v1/apis/apis-list/export-a-model)              |
| Deploy a model into the environment in the Ready to Deploy state and configure its parameters. xYou must perform the initial deployment manually in the Platform account. Consecutive deployments must happen via the public API. | [Deploy a Model API](/agent-platform/v1/apis/apis-list/deploy-a-model)              |
| Undeploy a model from the environment.                                                                                                                                                                                           | [Undeploy a Model API](/agent-platform/v1/apis/apis-list/undeploy-a-model)          |
| Manage external model connections.                                                                                                                                                                                               | [External Model Connection APIs](/agent-platform/v1/apis/apis-list/connections-api) |
