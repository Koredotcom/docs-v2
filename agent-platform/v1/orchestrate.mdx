...
- [Single Agent](/agent-platform/v1/orchestration/single-agent): One agent handles all requests. Best for focused, well-defined domains.

- [Supervisor](/agent-platform/v1/orchestration/supervisor): Central orchestrator coordinates multiple specialized agents. Best for complex, parallelizable tasks.

- [Adaptive Network](/agent-platform/v1/orchestration/adaptive-network): Agents dynamically hand off to each other. Best for sequential, multi-domain workflows.
...
flowchart TD
    A([User Request]) --> B[<b>Intent Analysis</b><br />Parse user input, identify required capabilities, determine complexity]
    B --> C[<b>Agent Selection</b><br />Match capabilities to agent descriptions, consider agent availability and scope, select single agent or multiple agents]
    C --> D[Agent A]
    C --> E[Agent B]
    C --> F[Agent C]
    D --> G[<b>Response Aggregation</b><br />Collect agent outputs, resolve conflicts, synthesize unified response]
    E --> G
    F --> G
    G --> H([User Response])
```
...
| Transcription Prompt | Context for ASR model | Helps recognize domain-specific terms and improve accuracy. [Learn More](/agent-platform/v1/howto/configure-transcription).|
| Welcome Message | Enable the agent to play a predefined greeting at the start of a voice session. |This is supported for Gemini real-time models.|

#### Add a Welcome Message to Voice Interactions
Use the Welcome Message feature of voice models to automatically play a greeting when a session starts. The agent can use this message to introduce itself, provide context, and guide users before they speak, creating a more natural voice interaction experience.

To configure it:
- Navigate to **Orchestrator** > **Voice-to-Voice Interactions**. 
- Choose a supported model and click on **Settings**. 
- Enable the **Welcome Message** toggle and provide the message in text format.

<Note>
1. The feature is supported for Gemini real-time (voice-to-voice) models.
2. The welcome message is played once, at the very start of the session, before the first user turn. It does not affect subsequent turn-taking, delegation, or task-status handling.
</Note>
...
The Input Processor is an app-level feature that executes a custom script before orchestration begins. It's used to preprocess user input, enrich context, and populate memory variables that can be used by agents during execution. 

When enabled, this processor is executed before the welcome event is triggered.

<Note>Input Processor is not supported for voice interactions.</Note>
...
- Provide the script that updates the output before delivering to the end user. You can use **JavaScript** or **Python** for this scripting.
  ...
  - Access [memory stores in the script using these methods](/agent-platform/v1/memory).
...
// Extract balance amount(number) from text (for example. "Your balance is 7236")
