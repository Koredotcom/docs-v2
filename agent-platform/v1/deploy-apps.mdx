...
flowchart LR
    A[Develop] --> B[Version] --> C[Deploy]
    C --> D[Draft <br /> Environment]
    C --> E[Staging <br /> Environment]
    C --> F[Production <br /> Environment]
```
---
<Note> Only authorized users can access version control and environment management features. [Learn more](/agent-platform/v1/agents/agentic-apps/settings/sharing-permissions).</Note>
...
<Note> The `containerId` parameter must match an existing HTML element ID in your webpage. Create a container element (for example. `<div id="container-id"></div>`) where the chat widget will appear before initializing the SDK. </Note>
