...
- Email addresses must include a valid domain (for example. `user@example.com`). Leading/trailing spaces are not allowed.
...
To change the role for a single user, see [Reassign a Role](/agent-platform/v1/administration/role-management#delete-a-custom-role).
...
For detailed instructions on creating and managing roles, see [Role Management](/agent-platform/v1/administration/role-management).
...
- **Default fields**: Manage visibility and edit permissions for standard fields (for example. First Name).
- **Controlled fields**: Manage visibility and edit permissions for optional fields (for example. District).
