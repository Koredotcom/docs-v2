...
For role and permission details for API-scoped apps, see [Role Management](/agent-platform/v1/administration/role-management#module-wise-permissions).

---

## Custom Scripts

Admins can import, deploy, and manage custom scripts directly from **Settings** → **Manage Custom Scripts**. Scripts run in isolated containers and can be invoked via the [API node](/agent-platform/v1/tools/nodes#api-node) endpoint or embedded in the Function node of a tool flow.
...
For full API node configuration options, see [API Node](/agent-platform/v1/tools/nodes#api-node).
