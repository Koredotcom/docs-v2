...
2. **The Platform processes uploaded files** in two ways:
    * **Content Extraction** — For [supported file types](/agent-platform/v1/memory#file-attachments), the Platform extracts content and uses it as context.
    * **Metadata Storage** — For [all supported file types](/agent-platform/v1/memory#file-attachments), the Platform captures and stores the following metadata in `sessionMeta.artifacts`:
        ...
        | `isActive` | Whether the document applies to the current conversation. [Learn more](/agent-platform/v1/agents/agentic-apps/settings/app-configurations#attachment-configuration). |
        | `downloadUrl` | Temporary direct access URL, **valid for 30 days** from date of upload |

3. **Metadata is accessible** in agent and supervisor prompts or code tools, and can be passed to third-party services for further processing. [Learn more](/agent-platform/v1/agents/agentic-apps/attachment-support#file-metadata).
