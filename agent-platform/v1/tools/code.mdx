...
In JavaScript, parameters are injected directly as `$ParameterName` variables. Assign them to local variables at the top of your scrip - `myinput = $value; //input variable = value`.
...
To read from or write to the memory store inside your script, refer to [Memory Stores](/agent-platform/v1/memory#in-code-tools-javascript).

**Working with Context Object**

Refer to [this](/agent-platform/v1/tools/context-object) to learn **context object structure**. Context object can be accessed as `context.<variables>`.
...
Access secrets and configuration from environment variables:
