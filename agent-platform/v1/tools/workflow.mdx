...
<Badge icon="arrow-left" color="gray">[Back to TOC](/agent-platform/v1/index#build-agents)</Badge>
...
flowchart LR
    Start --> Validate --> CallAPI["Call API"] --> End
```
...
Workflow tool permissions are managed through the App's roles and permissions. App scoped workflow tools do not require separate tool-level access.

* Users can view workflow tools associated with an app on the **App** → **Tools** page.
* To open, edit, or manage a workflow tool, users must have the corresponding **Workflow Tools** permission enabled for their role. 
* To configure, go to **Administration**  → **User Management** → **Role Management**. Open or create a role. Set **Role type** 
to **App**. Under **Enable/Disable agentic app access**, expand **Workflow Tools**. Provide the required permissions.

For more information, see [Roles and Permissions](/agent-platform/v1/administration/role-management#app-role-permissions).
...
For more information, see [Environment Variables](/agent-platform/v1/agents/agentic-apps/settings/variables).
...
- API keys aren't transferred during import.
- The imported tool can't be shared with other users.
...
| Section | Description |
|--------|-------------|
| [Overview](/agent-platform/v1/tools/workflow#tool-overview)| Displays high-level tool metadata, status, and summary information. |
| [Guardrails](/agent-platform/v1/tools/workflow#guardrails) | Enable safety and compliance scanners (input/output) for the tool. |
| [Environment Variables](/agent-platform/v1/tools/workflow#environment-variables) | Manage tool-specific values referenced across the workflow. |
| [Versions](/agent-platform/v1/tools/workflow#versions) | Create, restore, deploy, undeploy, or delete immutable snapshots of the workflow tool. |
| [API Keys](/agent-platform/v1/tools/workflow#api-keys) | Generate and manage API keys for calling the tool’s endpoint. |
| [Audit Logs](/agent-platform/v1/tools/workflow#audit-logs) | View operation logs related to this tool for governance and traceability. |
| [Tool Monitor](/agent-platform/v1/tools/workflow#tool-monitor) | Monitor performance metrics, execution histories, and runtime diagnostics. |
| [Sharing & Permissions](/agent-platform/v1/tools/workflow#sharing-and-permissions) | Assign or modify user access levels for the tool. |
| [Configurations](/agent-platform/v1/tools/workflow#configurations) | Provides import, export, and delete options for the workflow tool draft. |

To start building workflow logic by using nodes, go to [Designing Workflows](/agent-platform/v1/tools/workflow#designing-workflows).
...
For more information about workflow tool versions and app versions, see [Workflow tools and App versioning](/agent-platform/v1/tools/workflow-tool-and-app-version).
...
|:----------------------------------|:------------------------------------------|:----------------|:--------------------------|
| Mandatory input field missing     | A required input field isn't provided     | Data Validation | 400 Bad Request           |
| Invalid data type for input field | An incorrect data type is provided        | Data Validation | 400 Bad Request           |
...
Account owners can invite users to collaborate on specific tools. Invited users can access the **Models** and **Data** modules for the invited account but can only see the tools they're invited to.
...
| Action | Description |
|--------|-------------|
| [Import a tool as a version](/agent-platform/v1/tools/workflow#import-a-tool-as-a-version) | Replaces the current draft with the imported version after successful validation. |
| [Export a tool](/agent-platform/v1/tools/workflow#export-a-tool) | Exports the current draft workflow. |
| [Delete Tool](/agent-platform/v1/tools/workflow#export-a-tool) | Deletes the workflow tool after confirmation. |
...
flowchart TD
    ...
    ValidateInput -->|Valid| APICall["API Call"]
    ValidateInput -->|Invalid| Error["Error"]
    APICall --> End
    Error --> End
```
...
To save a fixed snapshot of the draft, create a version. A version includes the current flow, guardrails, and environment variables. For more information about version lifecycle, deployment, and endpoints, see [Versions](/agent-platform/v1/tools/workflow#versions). 
...
Workflows are built using different node types, each handling a specific function in the flow. For more information on the different types of nodes, see [Node Types](/agent-platform/v1/tools/nodes).
...
```mermaid actions={false}
flowchart LR
    Start --> WeatherAPI["API Node: Weather API <br /> URL: api.weather.com <br /> Method: GET <br /> Params: location, units"]
    WeatherAPI --> FormatOutput["Function: Format Output <br /> Transform response to user-friendly format"]
    FormatOutput --> End
```
...
Click **Deploy** to generate API endpoints. (see [Deploy a Tool](/agent-platform/v1/tools/workflow#deploy-a-tool))
...
```mermaid actions={false}
flowchart TD
    Condition["Condition <br /> amount > 100"] -->|True| True
...
| "No available nodes" error in menu | All valid nodes are linked | Create a new node or unlink existing ones |
| Flow doesn't execute | Broken or incomplete connections | Check for stray nodes or missing End Nodes |
| Error when connecting to a previous node | Backward looping isn't allowed | Reconnect to a valid forward step in the flow |
...
2. Click the **History/Log** icon at the upper-right corner of the page. The change log panel opens on the right, showing the most recent changes first.
