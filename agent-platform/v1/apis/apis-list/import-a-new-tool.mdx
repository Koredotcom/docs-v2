...
This API imports a new tool by providing the necessary **file IDs**, ensuring all the necessary configurations, including the tool’s flow data, app definitions, and environment variables are correctly imported. After triggering the import, the response returns a `dockStatusId`. Use this ID to call the [Get Dock Status API](/agent-platform/v1/apis/apis-list/get-dock-status) and verify the successful import of the tool.

To import a tool, follow the steps below:

1. **Upload Files**: Use the [File Upload API](/agent-platform/v1/apis/apis-list/upload-file-api) to upload the files for the flow definition, app definition, and environment variables. [Learn more](/agent-platform/v1/tools#import-to-create-a-new-tool). This API returns unique **file IDs** for each file. 
2. **Import the Tool**: Call the [Import Tools API](/agent-platform/v1/apis/apis-list/import-a-new-tool) using the retrieved **file IDs** along with the tool name.
3. **Track import Status**: The API response returns a `dockStatusId` that helps monitor the import progress using the [Get Dock Status API](/agent-platform/v1/apis/apis-list/get-dock-status).
...
To use the API, you will need an API key. [Learn more](/agent-platform/v1/apis#how-to-create-the-api-key).
