...
The API response includes the tool ID, dock status ID and the tool undeployment status. After receiving the response, use the `dockStatusId` to call the [Get Dock Status API](/agent-platform/v1/apis/apis-list/get-dock-status) and verify the status of the task.
...
| status | The undeployment status (`SUCCESS`, "<i>IN_PROGRESS</i>", or `FAILED`). | String |
