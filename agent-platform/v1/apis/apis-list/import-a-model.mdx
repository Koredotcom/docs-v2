...
* [Start Import Session](#start-import-session) - Import Model - Start Session API.
* [Upload Model in Chunks](#upload-model-in-chunks) - Import Model - Chunk Upload API.
* [Complete the Import Process](#complete-the-import-process) - Import Model- Completion API.
...
The **Import Model - Chunk Upload** API uploads a file chunk for model import. The `sessionId` from the [Start Import Session API](#start-import-session) must be used here to upload the file in incrementing chunks identified by a unique `chunkNumber`.
...
The **Import Model- Complete** API completes the model import process after all file chunks have been successfully uploaded. The ` sessionId` from the [Start Import Session API](#start-import-session) must be used for this API.
