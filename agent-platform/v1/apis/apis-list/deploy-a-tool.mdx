...
The API response includes the **tool ID** and the **tool deployment status**. After receiving the response, use the `dockStatusId` to call the [Get Dock Status API](/agent-platform/v1/apis/apis-list/get-dock-status) and verify the tool deployment status.
