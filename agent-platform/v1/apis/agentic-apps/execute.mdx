...
<Badge icon="arrow-left" color="gray">[Back to API List](/agent-platform/v1/apis)</Badge>
...
An array of objects, each with a type and value, used to identify or create sessions and manage user session mappings. The three supported identifier types are:

1. sessionIdentity(highest priority)
2. sessionReference
3. userReference(lowest priority)

[Learn More](/agent-platform/v1/apis#session-resolution-process).
...
| Artifacts | Array of responses from the tools in the order of execution. For example, if tool 1 is executed followed by tool 2, artifacts[0] contains the response from tool 1 and artifacts[1] contains the response from tool 2. The response from a tool is updated in this array only if the `Include tool response in artifacts` flag is enabled in the corresponding tool. [Learn More](/agent-platform/v1/tools/code#step-4-define-output-parameters). |
...
| tags | Custom Tags related to user, session or message. Typically used in AI for Service app for analytics. Tag types: user, session, message.  <br />  `{ "tags": { "message": { "Key": "Value" }, "user": { "Key": "Value" }, "session": { "Key": "Value" } } }` |
...
A runId is returned in response. Use this ID to check the status of the run. Refer to the [Run Status API](#find-run-status) to learn more. 
...
1. **Upload the file**. 
    ...
    Use the [File Upload API](/agent-platform/v1/apis/agentic-apps/upload) to upload the desired files. Uploaded files remain available only for the duration of the session.
...
| Fields | Description |
|:------ |:------ |
| sessionIdentity | This is an array of objects with a `type` and `value` used to identify or create sessions and manage user session mappings. The three supported identifier types are: The objects can be of three types: <ol> <li>userReference (highest priority)</li> <li>sessionReference</li> <li>sessionIdentity(lowest priority)</li> For more details, see [Session Resolution Process](/agent-platform/v1/apis#session-resolution-process) <p> This is useful for verifying the context and permissions on the run. </p></ol> |
