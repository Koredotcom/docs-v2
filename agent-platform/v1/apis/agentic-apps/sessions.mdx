...
| Fields | Description |
|:------ |:------ |
| sessionIdentity | This is an array of objects with a `type` and `value` used to identify or create sessions and manage user session mappings. The three supported identifier types are: The objects can be of three types: <ol> <li>sessionIdentity(highest priority)</li> <li>sessionReference</li> <li>userReference(lowest priority)</li> </ol>  Refer [Session Resolution Process](/agent-platform/v1/apis#session-resolution-process) for a detailed description. |
...
- Invokes [End of Conversation Event](/agent-platform/v1/agents/agentic-apps/settings/events#end-of-conversation-event).
</Note>
