...
For the complete list of supported models, see [Supported Models](/agent-platform/v1/models/supported-models).
...
**Prerequisites**: Create an IAM role in AWS with Bedrock permissions and a trust policy allowing Agent Platform to assume the role. See [Configuring Amazon Bedrock](/agent-platform/v1/models/configuring-aws) for IAM setup.
...
- **Option B: Use Service Principal**
    ...
    Authenticate through an Azure Active Directory Service Principal. Requires a pre-configured Microsoft Foundry connection. Enter a unique Connection name to identify this connection.
...
4. In your resource group, go to **Access control (IAM)** → **Add role assignment**. Assign a role (for example. **Contributor**) and select your registered app.
...
4. Configure model settings under **Model configurations**. Enable the features your model supports:
  ...
  | Modalities | Text-to-Text, Text-to-Image, Image-to-Text, Audio-to-Text |
    ...
    <Note>You must enable Tool calling for the model to work in Agentic Apps.</Note>
...
**1. Enable Model Features**

Enable only the features your model supports. Enabling unsupported features may cause unexpected behavior.
...
| Support tools | Function calling for Agentic Apps and AI nodes |
| Parallel tool calling | Multiple tool calls per request |
| Modalities | Text-to-Text, Text-to-Image, Image-to-Text, Audio-to-Text |
...
| Option | Description |
|----------|-------------|
| **Defaults** | Configure common model parameters such as temperature, max tokens, frequency penalty, presence penalty, and top_p using the standard controls. Available parameters vary based on the selected API reference. |
| **Custom** | Use the custom parameters JSON editor to define model-specific hyperparameters or vendor-specific settings that are not available in the standard controls. The JSON configuration is saved with the model, and only the parameters defined in it are sent with each model request.|
