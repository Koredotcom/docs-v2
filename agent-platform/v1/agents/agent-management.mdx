...
| **Run Once** | The script runs only on the first agent call in a session. Subsequent calls within the same session skip the pre-processor. Use this for one-time setup tasks such as loading user context or initializing session state. |
