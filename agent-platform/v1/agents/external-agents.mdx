...
<Note>
  The Agent Platform supports A2A v1.0 and A2A v0.3, over both HTTP+JSON and JSON-RPC transport bindings.
</Note>
...
| **Request Body** | Payload structure for POST or PUT requests — see [Request Format](/agent-platform/v1/agents/external-agents#request-and-response-formats) |
...
Add delegation rules to define how this agent routes tasks to other agents in the application. See [Adaptive Network Orchestration](/agent-platform/v1/orchestration/adaptive-network) for details.
...
See the full field reference in [Execute API](/agent-platform/v1/apis/agentic-apps/execute).
...
![Adapter architecture](/agent-platform/v1/agents/images/externalagents/adapter.png)
