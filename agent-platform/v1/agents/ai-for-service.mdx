---
noindex: true
title: AI for Service with Agent Platform
...
AI for Service is an enterprise-grade platform for building AI agents that deliver personalized, multilingual conversations across channels.

Integrated with the Platform's multi-agent orchestration, AI for Service enables organizations to rapidly deploy intelligent service solutions — automating complex interactions and continuously optimizing them to drive higher customer satisfaction and lower operational costs.
...
1. Navigate to the **Start Flows** in the AI for Service application.
    ...
    ![alt_text](/agent-platform/v1/agents/images/integration/ai-for-service/flows.png "XO Flows")
...
3. **Configure the Automation Node.**
    ...
    Open the settings for the Automation Node. Under Autonomy Level for Automation AI, select **Full Autonomy.** This setting enables integration with the Agentic Apps in the same workspace as that of AI for Service. The Agentic Apps offer fully autonomous AI Agents that adapt dynamically to the user's interactions.
    ![alt_text](/agent-platform/v1/agents/images/integration/ai-for-service/autonomy.png "Autonomy")
    ...
    3. **Real-time Voice Interactions**: Enable this to support two-way real-time voice streaming via the Voice Gateway. This feature uses multi-modal AI models for intelligent voice interactions. When real-time voice interaction is enabled, Voice Gateway uses the underlying models configured in the Agentic App to add voice capabilities to the application.
    ...
    ![alt_text](/agent-platform/v1/agents/images/integration/ai-for-service/metadata.png "Metadata")
    ...
    Refer to [this](/agent-platform/v1/memory) to learn more about accessing memory stores.
    Refer to the AI for Service node configurations documents.

---

### Handling Agent Transfer Event from Agent Platform

Context sharing from the Platform to AI for Service occurs when specific events get triggered. These event-driven handoffs are managed through the Automation Node in AI for Service, which facilitates the integration between the two platforms.

When an agent transfer event is triggered, the system passes control, along with relevant context data, from the Platform back to AI for Service. This ensures a seamless user experience, allowing AI for Service to continue the interaction based on the defined flow and transferred context.

![alt_text](/agent-platform/v1/agents/images/integration/ai-for-service/agent-transfer-event.png "Agent Transfer")
...
![alt_text](/agent-platform/v1/agents/images/integration/ai-for-service/agent-trasfer-node.png "Agent Transfer Node")
...
* Real-time voice feature is available only when the Voice Gateway is enabled.

* Real-time voice interactions require voice support to be enabled in the associated Agentic App. [Learn more](/agent-platform/v1/agents/agentic-apps/settings/app-configurations).
