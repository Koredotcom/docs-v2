...
Connecting an agent to Microsoft Agent 365 does two things:

* It registers the agent as a verified identity in your Microsoft 365 tenant, making it discoverable and interactable in Microsoft Teams and governable through the [Microsoft Admin Center](https://learn.microsoft.com/en-us/microsoft-365/admin/manage/agent-365-overview?context=%2Fmicrosoft-agent-365%2Fcontext&view=o365-worldwide).
* It automatically integrates the agent with the Microsoft Agent 365 Observability SDK, which exports traces for agent invocation, inference, and tool calls to the Agent 365 cloud service. View these traces in the [Microsoft Defender Portal](https://learn.microsoft.com/en-us/microsoft-agent-365/admin/threat-protection).
...
5. Return to **Agents → Agent Overview** and confirm that the Agent 365 UI is available and preview features are visible.

<Note>After acceptance, an Agent 365 Preview subscription is provisioned with about 25 licenses. These licenses are for agent instances (digital workers), not individual users.</Note>

![Agent 365 Overview page in the Microsoft 365 Admin Center](/agent-platform/v1/agents/agentic-apps/settings/images/microsoft/image-3.png)
...
Replace `{your-application-client-id}` with the Application (client) ID shown on the **Overview** page.
...
<Note>Granting admin consent requires a Global Administrator account. If the developer holds Global Admin, they can grant the permissions directly. Otherwise, the Global Admin must approve these permission grant requests.</Note>
...
4. Confirm all five permissions show green checkmarks under **Status**.

![API permissions list showing the five delegated Microsoft Graph permissions with admin consent granted](/agent-platform/v1/agents/agentic-apps/settings/images/microsoft/image-8.png)

<Warning>Add delegated permissions only. Application-type permissions cause CLI failures. If any permission shows Type "Application", remove it and re-add it as Delegated.</Warning>
...
**Outcome:** CLI tools are installed, the Azure CLI is authenticated, and `a365.config.json` is created with your project settings and the deployment flag set correctly.
...
<Note>`a365 setup all` and `a365 publish` require interactive browser sign-in with a Global Administrator account. If the developer holds Global Admin, they can run Step 3 independently. Otherwise, the Global Admin must run these commands.</Note>
...
![Review Environment Variables page in Kore Agent Builder with the three A365 values entered](/agent-platform/v1/agents/agentic-apps/settings/images/microsoft/image-2.png)
...
![Teams Developer Portal Configuration page with Connection type set to API Based and the backend URL entered](/agent-platform/v1/agents/agentic-apps/settings/images/microsoft/image-4.png)
...
![Create Instance dialog for the agent in Microsoft Teams](/agent-platform/v1/agents/agentic-apps/settings/images/microsoft/image-5.png)

The agent appears in your Teams chat list.

![Agent listed in the Microsoft Teams chat list](/agent-platform/v1/agents/agentic-apps/settings/images/microsoft/image-6.png)
...
![Advanced Hunting query results in the Defender portal showing agent activity events](/agent-platform/v1/agents/agentic-apps/settings/images/microsoft/image-7.png)
