...
- An AI model with tool-calling support added to the Platform. Supported providers: OpenAI, Gemini, Anthropic, Azure OpenAI.
  See [Add an External Model](/agent-platform/v1/models/external-models#add-a-model-via-easy-integration).
---
...
flowchart LR
    A["**1. Create an App** <br /> Set name, icon, description, and AI model"]
    B["**2. Configure Agents** <br /> Define roles, instructions, tools, and knowledge sources"]
    C["**3. Test** <br /> Run diagnostics and simulate scenarios"]
    D["**4. Deploy** <br /> Publish the app for end users"]
...
| **External Agent**       | Enable to allow the application to communicate with external agents through proxy agents within the app. [Learn more](/agent-platform/v1/agents/external-agents).                                                                                                                                  |
...
| Pattern | Description |
|---------|-------------|
| **Single Agent** | A single agent independently handles requests and generates responses. Ideal for apps with one primary capability. [Learn more](/agent-platform/v1/orchestration/single-agent). |
| **Supervisor** | A central controller analyzes requests, delegates tasks to specialized agents, and synthesizes responses. [Learn more](/agent-platform/v1/orchestration/supervisor). |
| **Adaptive Network** | Agents collaborate in a distributed manner, intelligently routing tasks based on capabilities and context. [Learn more](/agent-platform/v1/orchestration/adaptive-network). |
...
| **Voice-to-Voice Interactions** | Enable to allow users to interact with the app via real-time voice conversations. Once enabled, select the AI model for speech processing and voice responses. See [Supported Models](/agent-platform/v1/models/supported-models). Use the gear icon to modify model settings. |
...
[Learn more](/agent-platform/v1/orchestration/single-agent).
...
[Learn more](/agent-platform/v1/orchestration/supervisor).
...
Configure agents with delegation rules to define how tasks are routed across the network. [Learn more](/agent-platform/v1/orchestration/adaptive-network).
...
See [Set Up an Agent](/agent-platform/v1/agents/create-an-agent).

---

### 3. Test

Run [Diagnostics](/agent-platform/v1/deploy/diagnostics) before testing. Diagnostics validates agents, tools, events, and configurations against production-readiness criteria and flags issues before deployment.

Then simulate scenarios to evaluate agent behavior and responses.
See [Simulate and Test the App](/agent-platform/v1/agents/agentic-apps/simulation-and-testing).

---

### 4. Deploy

Publish the app for users. See [App Deployment](/agent-platform/v1/deploy-apps)
