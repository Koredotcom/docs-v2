...
Click the **settings** icon next to the model selector to configure model generation parameters such as temperature and max tokens. For setup instructions, see [External Models](/agent-platform/v1/models/external-models).
...
**Workflow Tool:** Design the flow using the visual builder. See [Workflow Tools](/agent-platform/v1/tools/workflow).

**Code Tool:** Write JavaScript or Python logic directly. See [Code Tools](/agent-platform/v1/tools/code).

**MCP Tool:** Connect to an MCP server and select available tools. See [Configure an MCP Server](/agent-platform/v1/tools/mcp#configure-mcp-tools).
...
5. Provide sample values in the input fields, and select **Execute**.
6. View the results under **Test Results** in JSON format. The Logs tab under Test Results presents the execution logs from the tool.
...
See [Knowledge Integration](/agent-platform/v1/knowledge) for configuration details.
