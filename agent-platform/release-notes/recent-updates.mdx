...
A button inside a `RICH_CONTENT` message can be configured with a URL so that clicking it opens the link directly in a new tab, instead of posting an action back to the agent. It’s supported on the Web SDK/widget, Telegram, and LINE; other channels automatically fall back to a normal button. [Learn more →](https://docs.kore.ai/agent-platform/abl-reference/rich-content-and-expressions)

<p className="rn-subtitle">Git integration for agent projects</p>

Developers can connect a Git repository to a project to sync agent definitions and localization assets with version control, including push/pull from the Studio editor and conflict-resolution tools for simultaneous edits. [Learn more →](https://docs.kore.ai/agent-platform/administration/integrations-settings)
...
Voice agents on the KoreVG/Jambonz pipeline can now set re-prompt messages, retry counts, and timeout windows for silent callers, with locale-aware prompts and a final action (hang up, custom message, or transfer). Learn more →

<p className="rn-subtitle">Custom vocabulary for speech-to-text</p>

Speech recognition can now be tuned to recognize domain-specific terms, like product names and jargon, for more accurate transcription. Learn more →

<p className="rn-subtitle">Filler-agent context and language routing for IVR</p>

Filler ("thinking") audio during voice calls now carries conversation context and follows the caller's active language. Learn more →

<p className="rn-subtitle">No more silent gaps during backend tool calls</p>

Updates to filler and no-input handling now reduce dead air, repeated speech, and lost language context while a voice agent waits for a backend tool call. The updates require no configuration change.


<p className="rn-title">Channels</p>

<p className="rn-subtitle">Kore.ai Agent Assist channel</p>

~~You can now connect an agent to Kore.ai Agent Assist for real-time guidance to human agents during live conversations. [Learn more →](https://docs.kore.ai/agent-platform/channels)~~

<p className="rn-subtitle">Microsoft Teams: user-identity and channel-template support</p>

Microsoft Teams connections can now resolve caller identity and support channel message templates. [Learn more →](https://docs.kore.ai/agent-platform/channels/digital#set-up-microsoft-teams)

<p className="rn-subtitle">HTTP Async (webhook) channel: interruption and streaming support</p>

The HTTP Async channel now supports mid-response interruption and streamed delivery of agent responses. [Learn more →](https://docs.kore.ai/agent-platform/channels)

<p className="rn-subtitle">Multiple message rendering</p>

Agents can now send several distinct messages in one turn, each rendered as a separate bubble instead of one combined block. Learn more →

<p className="rn-title">APIs and SDKs</p>

<p className="rn-subtitle">Web SDK additions for contact-center and custom integrations</p>

For custom and contact-center integrations, the Web SDK introduces `ContactCenterClient` and `AudioCodesSipAdapter`.
It features `SDKSessionScope.activityVisibilityPolicy` to manage how fillers, thought summaries, action progress, system notices, and tool previews are displayed. Display metadata for pre-uploaded attachments can be handled via `SendMessageOptions.attachments`. Additionally, a new `displayText` field for action submissions allows quick replies and buttons to reflect user selections directly in the transcript. For managing agent-media events, WebRTC control messages for accept, reject, and terminate operations are included. The update also provides new timestamp helper utilities, specifically `DEFAULT_MESSAGE_TIMESTAMP_SETTINGS`, `formatMessageTimestamp`, and `resolveMessageTimestampSettings`. Learn more →

<p className="rn-title">Analytics and Insights</p>

<p className="rn-subtitle">Public API for saved dashboard-widget data</p>

External tools, BI connectors, and scheduled exports can now fetch the data behind any saved custom-dashboard widget through a public REST endpoint, without re-implementing its underlying query. It supports both synchronous requests and asynchronous jobs with polling and cursor-based pagination. Learn more →

<p className="rn-subtitle">Alerts and monitoring configuration</p>

You can now configure alert rules and monitoring thresholds directly from the Insights area.
Learn more →

<p className="rn-subtitle">PII entity preview</p>

You can now preview which entities in a conversation will be detected and masked as PII before publishing an agent. Learn more →

<p className="rn-title">Integrations and Single Sign-on</p>

<p className="rn-subtitle">Native SSO for Azure Marketplace / Liftr deployments</p>

~~Customers deploying through the Azure Marketplace (Liftr) listing can now sign in using their existing Azure identity, without a separate Kore.ai credential. Learn more →~~

<p className="rn-subtitle">Workspace sign-up, SSO, and access requests have new controls</p>

Tenantless users are now routed to onboarding instead of being left in an ambiguous state, workspace SSO is separated from org SSO, and sign-up or workspace creation can be gated by feature flag. Review these settings after upgrading, especially if your org uses SSO or gated sign-up.

Invited and password-recovery SSO users are now routed through separate flows, and Azure SSO app selection and Azure BYOK metadata are represented as structured fields instead of opaque JSON — review these if your workspace uses Azure SSO or BYOK.
...
[Learn more →](https://docs.kore.ai/agent-platform/workflows)


<p className="rn-title">LLM and Model Configuration</p>

<p className="rn-subtitle">Authentication profiles with custom headers</p>

~~Authentication profiles support bearer‑token binding for LLM provider calls. Teams can attach custom HTTP headers, including bearer tokens, to every LLM request and manage authentication consistently across workspace and project scopes. This provides a secure and centralized way to control how LLM integrations connect to external services~~.  Learn more →
