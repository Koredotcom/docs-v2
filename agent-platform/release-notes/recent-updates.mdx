...
You can configure a button in a `RICH_CONTENT` message with a URL. Upon clicking, it opens the URL in a new tab, instead of posting an action back to the agent. It's supported on the Web SDK/widget, Telegram, and LINE; other channels automatically fall back to a normal button.
...
Microsoft Teams connections can now resolve caller identity and support channel message templates. 
...
{/* AG: On hold awaiting engineering confirmation
...
<p className="rn-title">Breaking changes and what to check before moving to 1.4.0</p>
...
**Update release automation for the new import/export routes**

Project import and export moved to `/project-io/import`, `/project-io/import/preview`, and `/project-io/export`. Update any custom release CLI, OpenAPI client, or automation that referenced the previous import/export paths.

```
Old → new routes:
    POST /api/projects/{projectId}/import/preview   →  POST /api/projects/{projectId}/project-io/import/preview
    POST /api/projects/{projectId}/import/apply 	→  POST /api/projects/{projectId}/project-io/import
    GET  /api/projects/{projectId}/export       	→  GET  /api/projects/{projectId}/project-io/export
```

If a release flow gets an HTTP 502 from the old `/import/apply` route but succeeds through `/project-io/import`, treat that as an endpoint migration issue first — check for a stale route or gateway allowlist entry before assuming an application/import failure.

Treat `/project-io/*` as the stable route family for this flow — routes named `internal-*` or similar are service-to-service only and aren't part of the customer API spec.
