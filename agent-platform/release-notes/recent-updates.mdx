...
This update includes the following feature enhancement and bug fixes.

<p className="rn-title">Voice</p>

<p className="rn-subtitle">Keep TTS Streaming Alive During Silence (Microsoft/Azure)</p>

Voice channels using Microsoft streaming text-to-speech no longer risk a silent dead-air disconnect after a period of caller/agent silence. The channel's Speech Synthesis settings now include a configurable keep-alive interval (5–25 seconds, 15s default) that proactively refreshes the TTS connection before the provider times it out, so every subsequent turn keeps streaming audio normally. <br />[Learn more →](/agent-platform/enable-voice)
