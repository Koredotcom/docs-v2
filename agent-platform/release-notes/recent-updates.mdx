...
You can now configure a dedicated Public Switched Telephone Network (PSTN) transfer target number for agent-initiated call transfers, alongside the existing SIP transfer configuration. Outbound PSTN transfers now correctly present the provisioned caller ID. [Learn more →](/agent-platform/administration/agent-behavior-settings#voice-gateway)
...
Microsoft Teams is now supported as a verified-identity channel, with identity enrichment, auth-profile-based credential mapping, and a new channel setup experience in Studio. Teams conversations can also now stream the agent's thinking or progress updates in real time, with a new streaming toggle in the Teams channel configuration.  [Learn more →](/agent-platform/channels/digital#configuration)
