...
A button inside a `RICH_CONTENT` message can be configured with a URL so that clicking it opens the link directly in a new tab, instead of posting an action back to the agent. It is supported on the Web SDK/widget, Telegram, and LINE; other channels automatically fall back to a normal button. [Learn more →](https://docs.kore.ai/agent-platform/abl-reference/rich-content-and-expressions)
