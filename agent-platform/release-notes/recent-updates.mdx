...
A button inside a `RICH_CONTENT` message can be configured with a URL so that clicking it opens the link directly in a new tab, instead of posting an action back to the agent. It is supported on the Web SDK/widget, Telegram, and LINE; other channels automatically fall back to a normal button. 
<br />[Learn more →](https://docs.kore.ai/agent-platform/abl-reference/rich-content-and-expressions)

<p className="rn-subtitle">Git integration for agent projects</p>

Developers can connect a Git repository to a project to sync agent definitions and localization assets with version control, including push/pull from the Studio editor and conflict-resolution tools for simultaneous edits. 
<br />[Learn more →](https://docs.kore.ai/agent-platform/administration/integrations-settings)
...
Speech recognition can now be tuned to recognize domain-specific terms, like product names and jargon, for more accurate transcription.
