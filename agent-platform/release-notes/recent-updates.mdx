...
Tools that return sensitive values — API keys, tokens, credentials — can now have those values captured into an encrypted, session-scoped vault that stays invisible to the LLM. Use the new `{{_secret.X}}` syntax to restore a captured secret into a downstream tool's HTTP headers or request body, without exposing the raw value to the model or the conversation transcript.

<p className="rn-title">Voice</p>

<p className="rn-subtitle">Configure a PSTN transfer target number</p>

Projects can now configure a dedicated Public Switched Telephone Network (PSTN) transfer target number for agent-initiated call transfers, alongside the existing SIP transfer configuration. Outbound PSTN transfers also correctly present the provisioned caller ID.
...
<p className="rn-subtitle"> Microsoft Teams: verified identity and thought streaming (Phase I)</p>

Microsoft Teams is now supported as a verified-identity channel, with identity enrichment, auth-profile-based credential mapping, and a new channel setup experience in Studio. Teams conversations can also now stream the agent's thinking/progress updates in real time, with a new streaming toggle in the Teams channel configuration.
