...
**Insights** provides a comprehensive analytics and monitoring layer for your AI agent program. It brings together executive dashboards, specialized evaluation pages, voice channel diagnostics, and a configurable pipeline framework into a single section, giving teams full visibility into agent quality, customer sentiment, operational efficiency, and cost performance. Whether you are tracking a production incident in real time or preparing a quarterly business review, Insights surfaces the data you need without requiring custom integrations or external BI tools.
...
The **Analytics** page is the operational control center for monitoring event volume, LLM performance, token consumption, and cost in near real time. Unlike the Dashboard's executive summary, Analytics offers granular time controls down to 30-minute windows, making it the go-to page for investigating production incidents, tracking the impact of model changes, or auditing LLM spend during peak traffic periods.
...
| **Sessions Explorer**    | Browse and filter individual sessions with full conversation details, trace counts, token usage, and duration. Each session row expands into a detailed view with turn-by-turn replay, token breakdown, and model information (see Session Detail View below). |
...
The **Agent Performance** page lets you monitor and compare the quality of every agent in your project across all evaluation dimensions. It surfaces which agents are performing well, which need attention, and how agent quality is trending over time — making it essential for teams managing multi-agent architectures where different agents handle different conversation types.

**Navigation**: **Project** → **Insights** → **Agent Performance**

**Date range selector**
Use the toggle in the top-right corner to select **7d**, **30d**, or **90d**. A **Compare** button next to the date selector opens a side-by-side agent comparison view.
...
The **Quality Monitor** page provides a centralized health check across all evaluation dimensions, making it the single page to visit when you need to answer: "How is overall quality trending, and are there any dimensions that need immediate investigation?" It aggregates outputs from multiple pipelines into a unified scoring view with trend analysis, dimension-level drill-downs, and issue flagging.
...
The **Customer Insights** page helps you understand what customers are asking about and how they feel about the experience. It combines intent classification, sentiment scoring, frustration detection, and resolution tracking into a single view, making it the primary page for product managers, CX leaders, and conversation designers who need to identify emerging topics, detect dissatisfaction early, and measure whether the agent is resolving the intents it encounters.
...
The **Feedback** page surfaces end-user feedback that chat sessions in your project capture directly. It gives you access to raw ratings and verbatim comments — the unfiltered voice of your users — so you can identify patterns in satisfaction, detect agents or channels that are underperforming, and prioritize improvements based on what users actually say.

**Navigation**: **Project** → **Insights** → **Feedback**

**Date range selector**
Use the toggle to select **Today**, **7d**, or **30d**.
...
The **Voice Analytics** page provides a dedicated dashboard for monitoring call quality, speech recognition accuracy, and end-to-end latency across the voice processing pipeline. For teams running voice-enabled agents, this page is critical for ensuring that audio quality, recognition accuracy, and response latency meet the standards callers expect — and for diagnosing degradations before they affect customer satisfaction at scale.
...
The **Pipelines** page is where Insights shifts from pre-built dashboards to customer-defined analytics. The platform ships with a comprehensive library of built-in pipelines covering the most common evaluation needs — from sentiment analysis to anomaly detection — and provides a visual node-based editor for creating custom pipelines that encode your organization's specific quality criteria. Pipelines are the engine behind Agent Performance, Quality Monitor, and Customer Insights: without active pipelines, those pages have no data to display.
...
All Insights pages respect the selected date range, though the available options vary by page:

| Page                                                           | Available ranges                               |
|----------------------------------------------------------------|------------------------------------------------|
| Dashboard, Agent Performance, Quality Monitor, Customer Insights | 7d, 30d, 90d                                  |
| Analytics                                                      | 30m, 1h, 3h, 6h, 12h, 24h, 2d, 7d, 30d, Custom |
| Voice Analytics                                                | 24h, 7d, 30d                                  |
| Agent Transfer                                                 | Today, 7d, 30d                                 |
| Feedback                                                       | 7d, 30d, 90d                                  |

Use shorter ranges (7d or less) for operational monitoring and incident investigation. Use 30d for monthly reviews and reporting. Use 90d for trend analysis and strategic planning.
