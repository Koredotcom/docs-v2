...
Built-in patterns use platform-managed detection logic for commonly used PII types.

Each pattern can be enabled individually and configured with its own redaction strategies, consumer access settings, and live test configuration.

| Pattern | Description |
|---------|-------------|
| **Email Address** | Detects email addresses in text. |
| **Phone Number** | Detects phone numbers in supported formats. |
| **Social Security Number** | Detects US Social Security Numbers (XXX-XX-XXXX). |
| **Credit Card Number** | Detects payment card numbers (Visa, Mastercard, American Express, and others). |
| **Date of Birth** | Detects common date-of-birth formats. |


### Configure built-in patterns

Select **Configure** next to a built-in pattern to configure its settings.
...
In addition to built-in patterns, you can create custom patterns to identify organization-specific or domain-specific PII.

### Configure custom patterns

Select **Add Pattern** to create a custom pattern and configure the following settings.

| Section | Description |
|---------|-------------|
| **Basics** | Specify the pattern **Name**, **Description**, and enable or disable the pattern. |
| **Regex-based Detection** | Identifies PII using regular expressions.<br /><br />• **Built-in Library** – Uses a platform-managed regular expression from the built-in pattern library.<br />• **Custom Regex** – Uses a user-defined RE2-compatible regular expression.<br /><br />See [Regex-based Detection](#regex-based-detection). |
| **Entity-based Detection** | Identifies PII using entity recognition.<br /><br />• **Inbuilt Entity** – Uses a platform-provided entity detector for supported entity types.<br />• **Model-based** – Uses the selected model and an **Identification Prompt** to identify custom entity types.<br /><br />See [Entity-based Detection](entity-based-detection). |
...
Custom patterns support **Regex-based** and **Entity-based** detection methods.

### Regex-based detection

Regex-based detection identifies PII using regular expressions.

| Option | Description |
|---------|-------------|
| **Built-in Library** | Uses a platform-maintained regular expression from the built-in pattern library. |
| **Custom Regex** | Uses a user-defined RE2-compatible regular expression. You can also configure an optional **Validation Expression** to validate detected matches before they are identified as PII. |

<Note>
Custom regex replacement labels support capture-group backreferences using <code>$1</code> or <code>\1</code>. Use <code>$$</code> to insert a literal dollar sign (<code>$</code>).
...
Entity-based detection identifies PII using entity recognition.

| Option | Description |
|---------|-------------|
| **Inbuilt Entity** | Uses a platform-provided entity detector for supported entity types, such as **EMAIL_ADDRESS**, **PHONE_NUMBER**, and **PERSON**. |
| **Model-based** | Uses the selected model and an **Identification Prompt** to identify custom entity types. |

<Note>
Model-based detection requires an <b>Identification Prompt</b> that describes the entity to identify.
</Note>
...
At runtime, PII Protection detects configured PII patterns and applies the configured redaction strategy and render mode before the data is exposed to each consumer.

The platform:

- Applies the configured render mode for each consumer.
- Uses the **Logs** consumer configuration when rendering PII in session history and trace views.
- Maintains stable tokenized and randomly generated values across subsequent views of the same session.
- Prevents LLM consumers from receiving plaintext PII. If **Original** is configured, the platform automatically replaces the detected value with a stable token before it is sent to the model.