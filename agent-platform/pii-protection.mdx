...
Custom dimensions used in session analytics can be configured to allow raw PII values to be stored.

By default, PII protection rejects detected PII in custom dimensions. You can configure the default behavior for custom dimensions to either:

| Behavior | Description |
|---|---|
| **Protect PII** | Rejects detected PII before it is stored. |
| **Allow raw PII** | Stores custom dimension values as provided, including detected PII. |

You can also define exceptions for individual custom dimensions when their behavior should differ from the default policy.

> **Note:** Allowing raw PII is an opt-in configuration. Use it only when your use case requires the original PII value to be stored.
