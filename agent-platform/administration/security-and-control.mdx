...
Click **+ Add request header** to add a row with a **Field Name** and **Value**. Up to 20 headers can be defined per profile. Headers are sent in the order listed, after the platform's standard headers (Content-Type, Authorization, etc.), which cannot be overridden.
...
mTLS enables the Platform to present a client certificate during the TLS handshake when calling enterprise systems that require it — at the token endpoint, for downstream API requests, or both. Configure the following fields when mTLS is enabled.
...
* **Certificate Alias** — Enter the name (up to 100 characters) to identify this certificate. Does not affect authentication behavior.
