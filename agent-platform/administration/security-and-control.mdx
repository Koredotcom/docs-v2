...
Enable SSO to let users authenticate through an external Identity Provider (IDP) instead of the default sign-in flow. Only account owners and admins can enable or disable SSO.

| Benefit | Description |
|---------|-------------|
| Secure Access | Reduces password fatigue and phishing risk by centralizing credentials |
| Centralized Management | Grant or revoke access across all accounts from one place |
| Improved UX | Eliminates repeated logins within a session |
| Access Control | Enforce security policies consistently across all applications |

### Supported Protocols & Providers

| Protocol | Providers |
|----------|-----------|
| [SAML](#saml) | Okta, OneLogin, Other |
| [WS-Federation](#ws-federation) | Windows Azure, Other |
...
1. Log in and go to **Autonomous Agents**.
2. Click **Settings** in the top navigation.
3. Go to **Security & Control** → **Single Sign On**.
...
A success message confirms SSO is active, with an activation timestamp.

<Note>If SSO is already enabled, use the **Enable SSO** tab to update parameters, disable the current config, or switch to a different protocol/provider.</Note>
...
SAML transfers user identity between the IDP and SP using digitally signed XML documents. The IDP authenticates the user and sends a signed response; the SP validates it using the certificate fingerprint and grants access.
...
Disabling SSO reverts the account to the default sign-in flow and clears the active SSO configuration. Previously entered parameters remain visible from the **Enable SSO** tab.

1. On the **Single Sign-On** page, click **Disable SSO**.
2. Click **Yes** in the confirmation dialog.

### Exclude Users from SSO

Account owners can exempt specific users from mandatory SSO, letting them choose between SSO or the default sign-in (email/password, Google, etc.).

**When to use:**
- SSO provider configuration error preventing login
- Expired SSO configuration profile
- Technical issues or business policy changes at the IDP

**Key rules:**
- The account owner is always excluded by default. It's recommended to exclude at least one additional user.
- Excluded users can switch between accounts without SSO re-authentication.
- Non-excluded users must use SSO when it's enabled.

**Steps:**
1. Go to the **Single Sign-On** page.
2. In the **Manage Users** field, type or select user email addresses. Press **Tab** to add multiple users.
3. Click **Save**.

When an excluded user logs in with SSO enabled, they can click **Continue** to use the configured SSO provider, or **Having trouble logging in with SSO?** to use the default sign-in method.
...
For role and permission details for API-scoped apps, see [Role Management](../user-management/role-management.md#module-wise-permissions-and-access-levels).

---

## Custom Scripts

Admins can import, deploy, and manage custom scripts directly from **Settings** → **Manage Custom Scripts**. Scripts run in isolated containers and can be invoked via the [API node](../../ai-agents/tools/tool-flows/types-of-nodes/api-node.md) endpoint or embedded in the Function node of a tool flow.
...
For full API node configuration options, see [API Node](../../ai-agents/tools/tool-flows/types-of-nodes/api-node.md).

---

## Related

- [Settings Console](../../administration/overview.md) — Full overview of Platform admin features
- [Role Management](../user-management/role-management.md#module-wise-permissions-and-access-levels) — Permissions for API-scoped apps
- [API Node](../../ai-agents/tools/tool-flows/types-of-nodes/api-node.md) — Integrate scripts and external APIs in tool flows
- [Monitoring](../monitoring/overview.md) — Track usage and audit activity across your account
