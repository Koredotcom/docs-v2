...
The **Restricted data** section blocks specific analytics tables and columns from saved widgets. The project ships with a default set of restrictions covering sensitive sources. You can remove a default restriction or add your own, then save the policy.

**Block a table**

1. Choose a table from the **Select a table** dropdown.
2. Click **Block table**.

Blocked tables appear as removable tags. Click the delete icon on a tag to lift the restriction.

The default blocked tables include:
...
1. Choose a scope from the table dropdown. Select **All tables** to block the column everywhere it appears, or select a specific table to block it there only.
2. Choose a column from the **Select a column** dropdown.
3. Click **Block column**.

Blocked columns appear as removable tags, grouped by scope. The default blocked columns include:

| Scope | Columns |
|---|---|
| All tables | `attributes`, `content`, `conversation_json`, `evidence`, `metadata`, `output`, `payload`, `prompt`, `query_text`, `trace_events` |
| `abl_platform.anomaly_detections` | `contributing_factors` |
| `abl_platform.context_evaluations` | `lost_context_items`, `flag_reasons` |
| `abl_platform.conversation_mentions` | `mention_text` |
| `abl_platform.goal_completions` | `goal_detected`, `summary`, `criteria` |
