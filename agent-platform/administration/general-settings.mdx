...
| **Strategy**                      | Defines the execution order for detected intents. Default: `primary_queue`. See multi-intent strategies below.  |
| **Unknown Relationship Strategy** | Fallback strategy when the platform can't determine the relationship between detected intents. Default: `parallel`.|
| **Max Intents**                   | Maximum number of intents the platform processes in a single turn. Default: `3`. Possible values: `1-10`.                                                          |
| **Confidence Threshold**          | Minimum confidence score `(0 - 1)` required to act on a detected intent. Default: `0.6`.                                                       |
| **Queue Max Age (ms)**            | Maximum time in milliseconds a queued intent remains eligible for processing. Default: `600000`. Max value: `3,600,000`.                   |

    **Multi intent strategies**

    | Value | Behavior |
    |-------|----------|
    | `primary_queue` | First intent is handled immediately; others are queued and addressed in sequence after the primary completes. |
    | `sequential` | Intents are addressed one by one in detection order, waiting for each to complete before starting the next. |
    | `parallel` | All intents are dispatched simultaneously and resolved concurrently. |
    | `disambiguate` | When multiple intents are detected, ask the user to clarify which one they want handled first. |
    | `auto` | The runtime selects the best strategy based on the intent relationship and conversation context. |

    **Unknown Relationship strategy**

    When the classifier can't determine how two intents relate to each other, this field is used.

    | Value | Behavior |
    |-------|----------|
    | `sequential` | Address them one at a time in detection order. |
    | `parallel` | Handle them concurrently. |
    | `disambiguate` | Ask the user which intent to prioritize. |
...
Project-wide defaults for voice speech recognition. A channel connection can override these.
 
| Field                             | Description                                                                                                                                                             |
| ---------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Default minimum STT confidence** | Reject transcripts below this confidence (0–1) and treat them as no-input. `0` disables the gate. Provider-relative - calibrate in staging before production. Default: `0.00`. |
