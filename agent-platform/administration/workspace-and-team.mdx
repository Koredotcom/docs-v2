...
The picker gives identifiers without describing them. See the [Permissions reference](/agent-platform/administration/permissions-reference) for what each permission grants, which ones reverse a protection, and which ones to keep apart from one another.
...
Go to **Team** -> **Security & Compliance** to manage authentication and access controls. The page carries three tabs: **MFA**, **SSO**, and **Audit Logs**.
