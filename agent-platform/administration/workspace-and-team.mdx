...
Select **Refresh** to reload the log, and **Export CSV** to download the view you're currently filtering. Use the filter bar to narrow results by search text, date range, or category.

### Filtering

* **Search** matches actor, action, resource, trace, IP, or metadata.
* **From / To** set the date range. Quick presets - **1h**, **24h**, **7d**, **30d** - apply a range in one click, and **Reset** clears every active filter.
* **Categories** appear as toggle buttons; select one or more to narrow the log to those event types.
* **More filters** exposes additional filter fields beyond search, date, and category.
* Select **Apply** to run the filters.
...
Categories group actions by the part of the platform they touch, which is how you narrow a broad search quickly. The platform organizes events into the following categories: Auth & access, Workspace governance, Project, agent & workflow configuration, Deployment lifecycle, Evaluation, Channel configuration, Tools, modules & credentials, Data protection, Operate sessions, Insights, dashboards & reports, KMS, Connector configuration, and Archives, retention & Git.
...
Select any row to open a detail panel for that event. Alongside the columns shown in the table, the panel adds:

| Field         | Description                                                        |
| ------------- | ------------------------------------------------------------------- |
| Event type    | The full event name, matching the Action column (for example, `project_member_role_changed`). |
| Actor type    | Identifies whether the actor was a `user` or a system process.      |
| Resource type | Names the type of resource the target belongs to (for example, `project_member`). |
| Environment   | Names the environment the event occurred in (for example, `dev`).           |
| Metadata      | A JSON object with event-specific detail - for a role change, for example, the target user ID and the previous and new role. |
