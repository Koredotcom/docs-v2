...
![Key Management page](/agent-platform/images/admin_kms.png)
...
|----------------------------|-----------------------------------------------------------------------------|
| **KEK** | Master key stored in your cloud provider's KMS.                            |
| **DEKs** | Versioned keys generated by the platform to encrypt data. Each DEK is wrapped by your KEK before storage. Active DEKs can rotate, while retired DEKs remain available for decryption as long as ciphertext references them. |
| **Encryption at rest**     | When the platform encrypts a secret, it generates a DEK, encrypts the data, wraps the DEK with your KEK, and stores both. Decryption reverses this process. |
...
| **OCI Vault KMS**    | AES keys                | OCI regions         |
