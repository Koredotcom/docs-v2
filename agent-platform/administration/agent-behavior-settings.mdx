...
Recordings appear inside the session they belong to, in a [**Call Recording**](/agent-platform/sessions) panel alongside the rest of that session's detail and diagnostics. There's no single list of every recording in a project - open the session you want from **Sessions** to find its recording. [Learn More](/agent-platform/sessions).
{/* AG: Check later how to mark a recording as immutable. When we get this info, then document it somewhere and link from above. */}

{/* Shruti - moved to Sessions page.
### Playback recordings
