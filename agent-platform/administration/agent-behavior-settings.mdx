...
<Note>This differs from the Studio softphone Record call switch. The softphone switch captures a temporary browser audio file for a single test call that can only be downloaded, is lost when the popover is closed, never appears on the session page, and is unaffected by these settings.</Note>
...
| **Retention days**           | Number of days a completed recording stays available before it's automatically deleted, unless it's marked immutable. Accepts 1-2555 days; an empty field is treated as 1 day. | 90 days  |
