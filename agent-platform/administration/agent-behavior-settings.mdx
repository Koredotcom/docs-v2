...
Once call recording is available at the workspace level, a project admin turns it on for individual projects. Recording applies only to calls that start after you enable this setting - calls already in progress or completed earlier aren't captured retroactively.

<Note>Call recording is a plan-gated capability. An account admin must turn on call recording for the workspace (**Account** → **Workspace Settings**) before a project can turn it on for itself. If your project's **Settings** menu doesn't show a **Recording** tab, ask your account admin to confirm the workspace-level setting.</Note>
...
Select **Save changes** to apply your settings, or **Reset** to discard them.
...
| Status | Meaning |
| ------ | ------- |
| No status shown | The call hasn't finished, or recording wasn't on for the call. The panel reads **No recording is available for this session yet.** Select **Refresh** to check again. |
...
Once a recording reaches **Completed**, the panel shows its duration, storage size, and retention (expiry) date, and authorized users can:

- Play/pause, seek/zoom waveform, rewind/fast‑forward 10s, adjust speed (0.75x–2x), and download audio.
- Select **Open conversation** or **Open traces** to jump to the matching point in the session's transcript or trace view — playback position stays synced with both.
