...
| Field | Description |
|---|---|
| Enabled | Turns no-input handling on or off. **Use project default** inherits the setting configured at the project level. |
| Mode | Controls how no-input is handled. **Use project default** inherits the project-level setting; **Dynamic** additionally exposes: **Order** (order in which no-input messages are played across retries), **Play Style** (how a message is played to the user), **Messages** (click **+ Add message** to configure one or more no-input prompts), and **Generation** settings for AI-generated messages — **Enabled** (turns generated messages on/off, **Inherit** uses a higher-level setting), **Generation Model** (model used to generate the message, **Runtime default model** uses the platform default), **Prompt Library Prompt ID** (ID of the prompt used, runtime uses its active version), and **Max Output Tokens** (max length of the generated message, default 150).  |
...
| On Exhausted | Behavior once Max Retries is reached. **Use project default** inherits the setting configured at the project level. Use **Hangup** to end the conversation and **Custom** to present a custom message to the user.  |
...
| **On Overlap** | Behavior when user and agent speak simultaneously. **Inherit** uses the setting configured at a higher level. |
...
| **No-Input Handling** | Behavior when the user doesn't respond within the expected timeout window. |
    ...
    **No-Input Handling Configuration**
    ...
    | Mode | Controls how no-input retries are handled. **Dynamic** additionally exposes a **Generation** section for AI-generated messages. |
    | Timeout (ms) | How long the agent waits for user input before treating the turn as no-input. Default: 5000 ms. |
    | Max Retries | Number of times the agent re-prompts the user after a no-input timeout before moving to the exhausted behavior. Default: 3. |
    | On Exhausted | Behavior once Max Retries is reached, e.g. **Hangup**. |
    | Relaxed Interval (ms) | Timeout duration used for retries after the first, typically longer than the initial timeout to be more lenient. Default: 15000 ms. |
    | Order | Controls the order in which no-input messages are played across retries, e.g. **Random**. |
    | Play Style | Controls how a no-input message is played to the user. **Use project default** inherits the project-level setting. |
    | Messages | Click **+ Add message** to configure one or more no-input prompts the agent can play to the user. |
    | Generation *(Dynamic mode only)* | Configures AI-generated no-input messages, as an alternative or supplement to authored messages: **Enabled** (turns generated messages on/off, **Inherit** uses a higher-level setting), **Generation Model** (model used to generate the message, **Runtime default model** uses the platform default), **Prompt Library Prompt ID** (ID of the prompt used, runtime uses its active version), and **Max Output Tokens** (max length of the generated message, default 150). |
