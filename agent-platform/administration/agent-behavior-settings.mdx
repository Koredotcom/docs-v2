...
<Note>This is different from the **Record call** switch in the Studio softphone. The softphone switch captures a temporary audio file for a single test call in your browser; you can only download it, and it's lost as soon as you close the popover. It never appears on the session page and isn't affected by the settings on this page.</Note>
...
| **Retention days**           | Number of days a completed recording stays available before it's automatically deleted, unless it's marked immutable. Accepts 1-2555 days; an empty field is treated as 1 day. Your workspace can also set a minimum and maximum, and the project value stays within those limits. | 90 days  |
...
A locked (immutable) recording is never deleted - not by retention cleanup, and not by a delete request. Locking requires the **recording:mark-immutable** permission (project admin role only). This permission must be granted separately - a general admin grant doesn't include it.

<Note>Locking and deleting a recording aren't yet available as buttons in Studio. The session panel offers play, download, and refresh only. To lock a recording for a legal hold, or to delete one for a customer request, use the platform recording API or contact Kore support.</Note>
