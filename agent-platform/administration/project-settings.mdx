...
The Settings section organizes its pages into five groups. Click any item to jump to its detailed documentation.
...
| Field | Description |
|---|---|
| **Extraction Strategy** | Determines the method the platform uses to extract field values from user input. `auto` selects the best-fit strategy based on the message and field configuration. |
...
| **Unknown Relationship Strategy** | Fallback strategy the platform uses when it can't determine the relationship between detected intents. `parallel` processes all detected intents simultaneously in the same turn. |
| **Max Intents** | Maximum number of intents the platform processes in a single user turn. Default: `3`. |
| **Confidence Threshold** | Minimum confidence score (0–1) required for the platform to act on a detected intent. The platform discards intents below this threshold. Default: `0.6`. |
| **Queue Max Age (ms)** | Maximum time in milliseconds a queued intent remains eligible for processing. The platform drops intents older than this value. Default: `600000`. |


**Field Inference**

The **Field Inference** section enables the platform to use an LLM to infer values for fields the user hasn't explicitly provided, reducing the number of clarifying questions the agent must ask.

| Field | Description |
|---|---|
| **Confidence Threshold** | Minimum confidence score (0–1) required for the platform to accept an inferred field value. The platform doesn't apply inferences below this threshold. Default: `0.8`. |
...
| **Model Tier** | Selects the LLM tier the platform uses for inference calls. `fast` uses a lower-latency model optimized for speed. |
...
| Field | Description |
|---|---|
| **Currency Mode** | Sets the conversion mode. `static` uses fixed exchange rates that you configure in the platform rather than live API-based rates. |
...
| Field | Description |
|---|---|
| **Fillers Enabled** | Master toggle for the filler system. When off, the platform emits no filler messages regardless of other settings. |
| **Chat Fillers** | When enabled, filler messages appear as transient `status_update` messages in chat clients during processing. |
| **Voice Fillers** | When enabled, the agent speaks filler messages aloud on voice channels while it processes a response. |
| **Chat Delay (ms)** | Minimum elapsed processing time in milliseconds before the platform emits a static chat filler. Default: `1200`. |
| **Voice Delay (ms)** | Minimum elapsed processing time in milliseconds before the platform emits a static voice filler. Default: `500`. |
...
| **Status Tags** | When enabled, the platform surfaces `<status>` tags that the response model emits inline as filler messages, letting response-model-generated status updates reach the user. |
...
| **Filler Model** | The LLM the platform uses to generate contextual filler messages when **Generated Fillers** is on. Defaults to the project's runtime default model. |
| **Conversation History** | Number of recent user and assistant messages (0–20) the platform includes in the filler generation prompt to produce context-aware fillers. Default: `0` (no conversation context). |
| **Previous Fillers** | Number of previously emitted filler messages (0–20) the platform includes in the filler generation prompt to avoid repetitive phrasing. Default: `0`. |
...
<Note>
**Conversation History** and **Previous Fillers** default to `0`, meaning contextual filler generation stays dormant until you configure it explicitly. Existing projects remain unaffected unless you change these values. When using a custom **Filler Prompt**, include the `{{conversationHistory}}` and `{{previousFillers}}` placeholders to take advantage of the context these settings pass in.
</Note>


**PII Redaction**

The **PII Redaction** section controls how the platform detects and scrubs personally identifiable information from agent conversations. Baseline credential and secret scrubbing stays active at all times, even when you disable PII protection.

| Field | Description |
|---|---|
| **Enable PII Protection** | Activates detection and redaction of PII types — emails, phone numbers, SSNs, credit card numbers, and IP addresses — across conversations. Baseline credential and secret scrubbing stays active even when this toggle is off. |
| **Redact Input** | When enabled, the platform redacts PII it detects in user messages before sending the message to the LLM. Context-aware exemption: the platform excludes fields it's actively gathering from redaction to preserve extraction accuracy. |
| **Redact Output** | When enabled, the platform redacts PII it detects in agent responses before delivering the response to the user. |
...
When no lookup tables exist, the section displays a **No lookup tables configured** placeholder. Click **+ Add Lookup Table** to create the first table.
...
Test the connection before saving. After you enable Git integration, you can push agent definitions to the repository, pull changes into Studio, and use conflict resolution tools to manage simultaneous edits.
...
**Project Settings**
The **Enable Agent Assist for this project** toggle controls whether the Agent Assist facade is available. When enabled, human agents receive AI-powered suggestions, knowledge lookups, and response drafts in real time. When you disable it, the facade returns 404.
