...
Project settings let administrators control who can access the project, which models and tools are available, how the runtime behaves, and what security and compliance policies apply. The **Settings** section covers membership, API keys, model configuration, runtime tuning, data retention, integrations, agent behavior overrides, PII protection, and channel policies.

## Overview

The Settings section organizes its pages into six groups. Click any item to jump to its detailed documentation.
...
- Requires a configured `tool_selection` model in your LLM provider settings. The pipeline silently skips if no model is available.
...
- If the pipeline model fails three consecutive times, the circuit breaker bypasses the pipeline for 60 seconds to avoid adding latency to every turn.
</Warning>
...
The **PII Redaction** section controls how the platform detects and scrubs personally identifiable information from agent conversations. Baseline credential and secret scrubbing remains active even when you disable PII protection.
...
<Note>
**Project Canonical** tables defined here are the recommended source of truth for field validation and fuzzy matching. **Agent-local** `LOOKUP_TABLES` defined in individual agent configurations remain available for backward compatibility, but new projects should use project-level tables.
</Note>
...
**Project Settings**: The **Enable Agent Assist for this project** toggle controls whether Agent Assist is active for this project. When enabled, human agents receive AI-powered suggestions, knowledge lookups, and response drafts in real time. When disabled, the Agent Assist API returns a 404 error.
...
Built-in patterns use pre-configured, optimized detection logic. You can adjust redaction, per-consumer access, and enabled state for each pattern. Disable high-risk patterns only if you have a specific reason to do so.
