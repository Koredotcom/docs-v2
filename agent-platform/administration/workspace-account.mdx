...
The **Workspace Info** section shows the following read-only details about your workspace.
...
<Note>Call recording is an Enterprise feature. If the Call Recording section does not appear in Workspace Settings, your workspace doesn't have it enabled. Contact your Kore representative to get it enabled. You can't enable it in the platform.</Note>

### Danger zone

{/* AG: project deletion now leads to archival for 30 days. Worth checking if workspace is also archived for restoration. */}
...
The Secrets page has three tabs: **Proxy Configs**, **OAuth Tokens**, and **External Vaults**.
...
* External vault references: Pointers to secrets held in your own cloud vault, resolved at execution time.

### Proxy configs

The **Proxy Configs** tab lists HTTP proxy configurations for outbound tool requests, with a running count (for example, *0 configs*) and a refresh icon to reload the list.
...
| Field                                     | Description                                                                                                                       |
| --------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| **Name** *(required)*                     | A descriptive label for this proxy, for example *Corporate Proxy*, *EU Proxy*.                                                    |
| **Proxy URL** *(required)*                | The proxy server address, for example `https://proxy.example.com:8080`.                                                          |
| **Auth type** *(required)*                | The authentication type the proxy expects. Default: **None**.                                                                     |
| **Environment** *(required)*              | The environment this proxy applies to, or **All environments (global)** to apply it everywhere.                                   |
| **URL patterns** *(required)*             | Comma-separated patterns for outbound requests that should route through this proxy, for example `*.example.com, api.partner.com`. |
| **Bypass patterns** *(optional)*          | Comma-separated patterns to exclude from the proxy, for example `localhost, *.internal`.                                          |
| **Priority** *(required)*                 | Determines evaluation order when multiple proxy configs could match the same request. Default: `0`.                              |
| **CA certificate** *(optional)*           | PEM-encoded CA certificate content, if the proxy uses a custom or private CA.                                                      |
| **mTLS client certificate** *(optional)*  | PEM-encoded client certificate, for mutual TLS.                                                                                   |
| **mTLS client private key** *(optional)*  | PEM-encoded private key matching the client certificate.                                                                          |

5. Toggle **Enable immediately** to activate the proxy as soon as it's saved.
6. Optionally, click **Test connection** to check the current proxy URL, authentication, and TLS settings before saving.
7. Click **Add Config**.
...
The **External Vaults** tab lets you register a secrets vault in your own cloud directory. Secret values are read at execution time and are never stored by the platform - only a reference to the vault entry is kept.

<Note>Currently only **Azure Key Vault** is available. **AWS Secrets Manager** and **GCP Secret Manager** appear in the provider list but are not yet selectable.</Note>

**Add an external vault:**

1. Go to **Settings > Account > Secrets**.
2. Click the **External Vaults** tab.
3. Click **Add Vault**.
4. In the **Add Vault** dialog, enter:

| Field                | Description                                                                                                                                  |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Provider**         | The vault provider. Currently **Azure Key Vault**.                                                                                            |
| **Name**             | An identifier for this vault. Lowercase letters, digits, and underscores only. Used in `{{secret....}}` references and can't be changed later — choose it deliberately. |
| **Vault URL**        | Your vault's URL, for example `https://my-vault.vault.azure.net`.                                                                            |
| **Azure Tenant ID**  | Your Azure AD tenant ID.                                                                                                                       |
| **Authentication**   | How the platform authenticates to your vault. See **Authentication options** below.                                                          |
...
| Option                              | Description                                                                                                                                    |
| -------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| **Platform managed (recommended)**  | Kore governs the credential, rotation, and trust settings; these are read-only here.                                                          |
| **Service principal**               | Use a credential you control. Select an existing **Azure AD auth profile** holding the service principal's client ID and secret - or create one first if none exist. |
...
| Credit Balance | Credits allocated to your workspace. Displays `No credits allocated` if none are assigned. |
