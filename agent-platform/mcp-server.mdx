...
sidebarTitle: Platform MCP Server
---
...
- **Node.js 18 or later** — the server runs via `npx` command.
- Access to [Agent Platform account](/agent-platform/getting-started#platform-access-and-allowlist-of-ips). [Authenticate](#authentication) when you connect to it for the first time.
- MCP-compatible client.

## Install and Configure

`@koreai/arch-mcp-tools` is a standard stdio MCP server. The clients launch it using the command `npx -y @koreai/arch-mcp-tools`. Add it under a server named `arch-agent-platform` using your client's config.

You can provide the environment URL in one of the following ways:

- Configuration file. Use `AGENTS_URL` in the server's `env`.
- Pass `serverUrl` to `platform_connect`.
- On the first use, the tool asks for the environment.
