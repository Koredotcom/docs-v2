...
LINE splits this across two consoles, and each owns a different half of the job.
...
Sign in to both with the same account. The Developers Console asks first-time users to register a developer profile with a name and email, and an account that skips that step can't see the channel it just created.
...
Three LINE identifiers look plausible in the Destination ID field and none of them route. The **Channel ID** on the Basic settings tab is numeric. The **Bot basic ID** on the Messaging API tab starts with `@`. **Your user ID**, also on the Basic settings tab, is your own personal LINE account, not the bot's. The correct value comes from an API call, covered below.

## Create the Official Account and enable the Messaging API

Start here even if someone has already created an Official Account, because enabling the Messaging API is what creates the channel you'll configure.

<Steps>
  <Step title="Register a LINE Business ID">
    LINE accounts for business sit behind a Business ID, which you can register with an existing personal LINE account or with an email address. Register one at [account.line.biz](https://account.line.biz/signup?redirectUri=https://entry.line.biz/form/entry/unverified) if your organization doesn't already have one.
  ...
  <Step title="Create the Official Account">
    Complete LINE's [Official Account entry form](https://entry.line.biz/form/entry/unverified) with your business details. Submitting it creates the account.

    Open [LINE Official Account Manager](https://manager.line.biz/) and confirm the new account appears in your list before you continue.
  </Step>

  <Step title="Enable the Messaging API">
    In Official Account Manager, select the account, then open **Settings** -> **Messaging API** and enable the Messaging API. This is what creates the channel that Artemis connects to.
  ...
  <Step title="Turn off LINE's automatic replies">
    ...
    Reach the settings from either console. In Official Account Manager, open **Settings** -> **Response settings**. In the Developers Console, the channel's **Messaging API** tab shows **Greeting messages** and **Auto-reply messages** with an **Edit** link beside each that opens Official Account Manager at the right place.

    Disable both when the agent should own every reply. Keeping a greeting message is reasonable if you author it deliberately, but leaving the defaults on reads to users as a broken bot rather than as a settings problem.
  </Step>
</Steps>

## Collect the credentials

All three values live in the Developers Console, spread across two tabs of the same channel. Sign in at [developers.line.biz/console](https://developers.line.biz/console/), select your provider, then select the Messaging API channel.
...
<Steps>
  ...
  <Step title="Get the Destination ID from Get bot info">
    The console never shows this value, so you call for it. Use LINE's [Get bot info](https://developers.line.biz/en/reference/messaging-api/#get-bot-info) endpoint with the channel access token you just issued, and copy the `userId` from the response.
...
Go to **Deployments** -> **Channels** -> **LINE** -> **+ New** and complete the following.

| Field | What it controls |
| ----- | ---------------- |
| **Display Name** | The label operators see in the connections list. Name it for the Official Account and the environment it serves, because a project can hold several LINE connections. Routing ignores this value. |
| **Destination ID** | The routing key. Enter the `U`-prefixed bot user ID from Get bot info. |
| **Channel Access Token** | The credential for every outbound call Artemis makes to LINE. |
| **Channel Secret** | The key Artemis uses to verify the signature on each inbound event. |
...
| **Provider Verification Strength** | How far Artemis trusts LINE's identity assertion when it links a person to an existing session. **Weak** suits most deployments. Choose **Strong** only when you trust those assertions enough to base continuity and account-linking decisions on them. |
| **Initial Status** | Whether the connection starts handling messages. Set it to **Active**, because LINE's verification resolves only when the connection is already active. |

Select **Create**, then open the connection and copy the **Webhook URL** from the **Overview** tab.

## Configure and verify the webhook in LINE
...
<Steps>
  <Step title="Paste the webhook URL">
    Find **Webhook settings**, select **Edit** under **Webhook URL**, paste the Artemis endpoint exactly as shown, and select **Update**.
  ...
  <Step title="Restrict API access by IP, if you chose a long-lived token">
    ...
    This narrows the blast radius of an indefinite credential considerably, so it's worth doing whenever you pick that token type. It doesn't apply to the other three.
  </Step>
</Steps>

## Test a real LINE conversation

Artemis has no synthetic test for this channel, so a real conversation is the only end-to-end proof.

<Steps>
  <Step title="Add the Official Account as a friend">
    Scan the QR code or use the add-friend link on the **Messaging API** tab.

    Test from an ordinary LINE account rather than an identity that exists only as a console administrator, because the two behave differently.
  </Step>

  <Step title="Send a unique text message">
    Send a short message you can search for later, and confirm the agent replies in the same chat.
  </Step>

  <Step title="Confirm exactly one response">
  ...
  <Step title="Exercise each input type your agent supports">
    Send the message types your Official Account will receive, and confirm the agent's authored behavior for each.

    Receiving a media event isn't the same as handling it. An agent flow that expects text will accept an image event and then have nothing to do with it, so test the authored path rather than only the delivery.
  </Step>

  <Step title="Test groups only if you need them">
    Enable **Allow bot to join group chats** in Official Account Manager when group or multi-person conversations matter, invite the bot, then message or mention it.

    Confirm sessions stay scoped to the LINE source, so a group conversation and a one-to-one conversation with the same person don't merge.
  </Step>
</Steps>
...
Match the symptom to the check. Most failures trace to the Destination ID, a reissued credential, or the **Use webhook** toggle.

| Symptom | What to check |
| ------- | ------------- |
| Verify doesn't report success | Confirm the connection exists and is active, that the Artemis endpoint is reachable over public HTTPS with a certificate from a trusted authority and a complete chain, that the URL carries no extra path segment, and that LINE receives a 200 response. |
| Artemis reports no connection found | Replace the Destination ID with the `U`-prefixed `userId` from Get bot info. The Channel ID, the Bot basic ID, and personal user IDs don't route. |
| Signature verification fails | Copy the Channel Secret from the matching channel's **Basic settings**, check whether another administrator reissued it, and confirm no proxy or middleware rewrites the raw body. |
| Verification succeeded but no messages arrive | Turn on **Use webhook**. A saved and verified URL delivers nothing until you do. |
| Messages arrive but the agent doesn't reply | Confirm the Channel Access Token is current and neither expired nor revoked, the connection is active, and the bound environment has a runnable deployment. A v2.1 token that passed its 30-day ceiling produces exactly this symptom. |
| People receive two replies | Disable greeting messages and auto-reply messages, or author them so they don't overlap with the agent. |
| Direct messages work but groups don't | Enable **Allow bot to join group chats**, reinvite the bot if you changed the setting after inviting it, and confirm the test event comes from the expected group or room. |
| Text works but media doesn't | Confirm Artemis supports the event type, the token can download message content, attachment limits allow the file, and the agent flow handles attachments. |
