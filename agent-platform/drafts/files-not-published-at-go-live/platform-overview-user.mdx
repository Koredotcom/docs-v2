...
order: 1
description: 'Agent Platform is the enterprise control plane for agentic AI — define, orchestrate, and deploy AI agents using ABL (Agent Blueprint Language), where deterministic governance meets autonomous reasoning.'
---
...
Agent Platform serves four primary personas, each with a different entry point:

- **Agent developer** — You define agent behavior in ABL DSL or Studio's visual editor. You write supervisor routing logic, tool integrations, and knowledge base configurations. Start with the [Quickstart](/agent-platform/quickstart), then move to [Your first agent](/agent-platform/tutorials/build-your-first-agent).
- **QA / Tester** — You create evaluation sets with personas, scenarios, and LLM-based judges to validate agent behavior at scale. Start with the [Evaluation guide](/agent-platform/tutorials/safety-testing-publishing).
...
Agent Platform takes a fundamentally different approach to building AI agents. This section covers the core design decisions and how they compare to alternatives.
...
| Capability                 | Agent Platform                      | LangGraph        | CrewAI      | Bedrock Agents     |
| -------------------------- | --------------------------------------- | ---------------- | ----------- | ------------------ |
...
Building an agent on Agent Platform follows a natural progression from design to deployment.
