# HowTo Review: route-by-tool-result-or-memory
...
- Correction to an earlier draft of this review: `WHEN` comparisons are **not** rewritten into an `IS NOT SET OR ...` form by the compiler — that rewriting (`autoGuardConstraint`) only applies to `CONSTRAINTS`/`REQUIRE` conditions, never to `HANDOFF`/`SUPERVISOR` `WHEN`. Equality comparisons like `profile.fraud_risk == "high"` already handle an unset field correctly on their own (an unset field compares not-equal, so the condition simply evaluates `false`) — no special syntax or guard is needed, and the trace shows your literal authored condition text unchanged. No article change is needed for this.
- Note the current `HANDOFF` history default: since every `HANDOFF` in the example omits `HISTORY`/`CONTEXT.history`, each child now receives full conversation history by default (changed from `auto` since this article was drafted).
- Note that copying tool-result fields into flat `SET` variables (`profile_tier = profile.tier`, etc.) is optional — the example references `profile.*` directly in every `WHEN`, so the copies exist only for use in `CONTEXT.pass`. Call this out so a customer doesn't think the copy step is required before a field can be used in a condition.
- Add a troubleshooting note about quoted natural-language fallback conditions: the classifier that decides whether a quoted `WHEN` string is natural language or a structured expression treats `AND`/`OR`/`NOT` case-sensitively (uppercase only) in one check, but an earlier quote-unwrapping step matches `and`/`or` case-insensitively. The article's own fallback text (`"no fraud, open-case, or premium routing condition is matched"`) is written in safe lowercase and resolves correctly to semantic evaluation — but a customer who capitalizes those words in their own fallback text could get a different (and likely broken) classification. This is worth a explicit warning since customers will write their own NL fallback conditions.
- Mark `customer_id` as a project-local assumption (companion-resource rule) — it's used as a tool parameter and passed via every `CONTEXT.pass`, but nothing in the example declares where it comes from (assumed pre-existing session/auth state).
- Rename "Design choices" to "How it works" (matches article template; the prior name was flagged as vague in review).
- Convert "Common mistakes" from a bullet list to the required table format, and add a "Troubleshooting" table (currently missing).

None of these are correctness-breaking — the article's ABL is valid and its core pattern (read-only lookup tool, `AS`-bound result, deterministic routing on the result's fields) is accurate and current. The changes fill in completeness/self-service gaps identified by the Operational Self-Service Gate and persona review.

## Evidence

| Claim                                                                                                  | Current evidence                                                                                                                                                                                                                                                                                    | Impact                                                                                             |
| ------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------- |
| Inline `TOOLS:` block syntax (`description`, `side_effects`, `confirm`, `params`) is current and valid | `packages/core/src/parser/agent-based-parser.ts:3780-3840` shares `parseToolProperties` with `tool-file-parser.ts`; `confirm` is not in `TOOL_IMPLEMENTATION_PROPERTIES` so it's allowed inline; real test fixtures (`agent-based-parser.test.ts:160,194`) use identical bare-word `confirm:` style | No change needed — confirms the example is accurate                                                |
| `autoGuardConstraint` only applies to `CONSTRAINTS`/`REQUIRE`, never to `HANDOFF`/`SUPERVISOR` `WHEN`  | `packages/compiler/src/platform/ir/compiler.ts:2414` (its single caller); `apps/runtime/src/services/pipeline/null-safe-eval.ts` doc comment confirms equality (`==`/`!=`) "handle null correctly" without any guard rewriting                                                                      | No article change; corrects an earlier draft that wrongly claimed HANDOFF conditions get rewritten |
...
| Quoted-NL fallback classifier case-sensitivity asymmetry                                               | `apps/runtime/src/services/execution/routing-primitive-executor.ts` (`EXPRESSION_SHAPED_ROUTING_GUARD`, case-insensitive) vs `packages/compiler/src/platform/constructs/routing-condition-classifier.ts` (`isStructuredRoutingCondition`, case-sensitive AND/OR/NOT)                                | Add troubleshooting note; article's own example is safe as written                                 |
| `customer_id` has no declared source                                                                   | No `MEMORY`/`GATHER` in the example                                                                                                                                                                                                                                                                 | Mark as project-local assumption                                                                   |
| Root-level guard-variable parser fix (ABLP-2996) and delegate guard fix (ABLP-3241)                    | Confirmed not applicable — all WHEN paths are multi-segment; article uses only HANDOFF, no DELEGATE                                                                                                                                                                                                 | No article change needed for these two                                                             |
...
- `agent-platform/drafts/abl-howtos-docs/articles/route-by-tool-result-or-memory.md`
- `agent-platform/drafts/abl-howtos-docs/evidence/route-by-tool-result-or-memory-2026-08-21-evidence.md` (already current)
```
