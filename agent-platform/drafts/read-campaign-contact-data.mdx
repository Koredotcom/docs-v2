...
sidebarTitle: Read Campaign Contact Data
---

This guide explains how an Artemis agent can use customer and campaign fields supplied by an XO campaign during an outbound voice session.
...
Campaign control fields are stored directly under `customerData`; they are not part of `userInfo`.
...
There are no standard nested `customerData.custom` or `customerData.metadata` objects in the current campaign contract. Do not use those paths for contact-list columns.

## Prerequisites

- Campaigns for Agent Platform 2.0 is enabled and configured.
...
<Steps>
 ...
 Create or import a campaign contact list with the required standard and custom columns. For example:
    ...
    **Deterministic Flow Example**
    ...
    The agent should not rely on a contact field unless the selected campaign dialing path sends it. Write prompts so the conversation still works when an optional value is absent.

    Recommended behavior:

    - Use a field only after confirming that it is populated for the campaign list.
    - Do not speak placeholders or internal identifiers to the customer.
    - Ask the customer for a missing value only when it is necessary for the task.
    - Do not guess or fabricate a missing campaign value.
...
The current campaign implementation does not propagate contact data identically in every dialing mode:
...
Before depending on arbitrary custom contact fields in a progressive or power campaign, verify that the actual Runtime session contains them. If they are absent, the campaign producer must propagate `userInfo`; an ABL-only change cannot recover data that was not sent.

## Troubleshooting

| Symptom                                                   | Likely cause                                                         | Resolution                                                   |
| --------------------------------------------------------- | -------------------------------------------------------------------- | ------------------------------------------------------------ |
| `customerData.userInfo` is absent                         | The dialing path did not send the full contact object                | Check the dialing mode and inspect the outbound campaign tag |
...
| A value does not resolve                                  | The ABL path or field capitalization differs from the contact column | Use the exact `customerData.userInfo.<fieldName>` path       |
| `customerData.custom` or `customerData.metadata` is empty | Those are not standard campaign containers                           | Move/read the contact field under `customerData.userInfo`    |
...
| The field exists in Campaigns but not Runtime             | Data was not included in the provider tag                            | Fix or enable propagation in the campaign producer           |
