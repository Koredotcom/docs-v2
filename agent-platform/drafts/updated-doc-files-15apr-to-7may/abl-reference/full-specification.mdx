...
> This page is generated from `docs/reference/ABL_SPEC.md` by `pnpm abl:docs:generate`.
...
- [8. Common Pitfalls](#8-common-pitfalls)
  ...
  - [8.4 ESCALATE PRIORITY Uses Strings; HANDOFF PRIORITY Uses Integers](#84-escalate-priority-uses-strings-handoff-priority-uses-integers)
  - [8.5 ESCALATE Inside ON_ERROR Is Silently Dropped](#85-escalate-inside-on_error-is-silently-dropped)
  - [8.6 VOICE INSTRUCTIONS in Templates Are Not Compiled](#86-voice-instructions-in-templates-are-not-compiled)
...
> **Compatibility note**: Legacy `ON_<event>` aliases still parse, but canonical docs and generated artifacts use `ON: session:start`-style event names.
...
> **PRIORITY values**: ESCALATE PRIORITY uses string values: `low`, `medium`, `high`, `critical` (default: `medium`). Note: HANDOFF PRIORITY uses integers (parsed via `parseInt`). See [§8.4](#84-escalate-priority-uses-strings-handoff-priority-uses-integers) for wrong/correct examples.
...
**Compilation:** Templates compile to `AgentIR.templates: Record<string, string>` for the `DEFAULT` text only. Channel-specific variants are compiled transiently and copied onto the referenced IR node's `rich_content` during template resolution; they are not exposed as a named runtime registry.

> **Implementation Note:** `VOICE INSTRUCTIONS:` is parsed as a template sub-key but the content is currently **not included** in the compiled IR output. Voice instructions defined in templates are silently dropped during compilation. Use behavior profile `VOICE:` configuration or per-step `VOICE:` blocks for voice control until this is fully wired.
...
### 8.4 ESCALATE PRIORITY Uses Strings; HANDOFF PRIORITY Uses Integers

ESCALATE and HANDOFF parse PRIORITY differently. Don't mix the two conventions.
...
**HANDOFF** — integer values (parsed via `parseInt`):

```
HANDOFF:
  - TO: Billing_Agent
    WHEN: billing_issue == true
    PRIORITY: 5                 # ← Integer (lower = higher priority)
```
...
### 8.6 VOICE INSTRUCTIONS in Templates Are Not Compiled

`VOICE INSTRUCTIONS:` is accepted as a template sub-key but the content is **not included** in the compiled IR output. For voice control, use behavior profile `VOICE:` configuration or per-step `VOICE:` blocks.
