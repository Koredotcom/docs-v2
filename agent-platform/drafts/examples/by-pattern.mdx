---
title: ABL Examples by Pattern
sidebarTitle: Examples by Pattern
noindex: true
hidden: true
---

A comprehensive collection of working ABL examples organized by agent pattern. Each example includes the full ABL definition, explanation of key constructs, and guidance on when to use that pattern.

<Note>Copy any example into the ABL Studio visual editor to see the flow graph rendered in real time. Also, when an example routes on `intent.category`, define the matching categories in the supervisor's top-level `INTENTS:` section. `INTENTS:` declares classifier vocabulary; `HANDOFF:` maps categories to agents.</Note>
...
| Construct               | Purpose                                                         |
|-------------------------|-----------------------------------------------------------------|
| `FLOW` section          | Gives the agent a fixed step graph.                             |
| `entry_point` / `steps` | Declares the step sequence and starting point.                  |
| `THEN:`                 | Explicit transition between steps.                              |
...
| `hints.cacheable`    | Whether results can be cached across turns.                             |
| `hints.latency`      | Expected response time (`fast`, `medium`, `slow`).                      |
| `hints.timeout`      | Maximum wait time in milliseconds.                                      |
| `hints.side_effects` | Whether the call modifies external state.                               |
...
| Construct              | Purpose                                                                                                                                                                  |
|------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| No `type:` on the tool | Sandbox tools are declared with a signature but no `type: http` or `type: mcp` -- the runtime resolves them from **Project Tools** (code files uploaded to the project). |
| `memory` API           | Injected automatically. Sandbox code can read session state with `memory.get_content()` and persist results with `memory.set_content()`.                                 |
| Isolated execution     | The sandbox code cannot access the network, filesystem, or other sessions.                                                                                               |
...
| `server_tool: "navigate"` | The specific tool name on the MCP server (defaults to the ABL tool name if omitted).                            |
| Full feature support      | MCP tools support the full range of ABL tool features: parameters, return types, `hints`, `on_result` handlers. |

<Note>See [MCP Tools](/agent-platform/tools/mcp) for details on registering MCP servers with your project.</Note>
...
GOAL: "Identify the customer's need and route to the correct specialist with relevant context"
...
INTENTS:
  human_escalation: "Customer explicitly asks for a human queue, operator, or manual review."
  balance: "Customer wants a balance check."
  ...
  technical: "Customer needs technical support."
  troubleshooting: "Customer needs troubleshooting help."
  farewell: "Customer is ending the conversation."
  unknown: "Customer intent is ambiguous and needs clarification."
...
MEMORY:
  ...
  recall:
    - ON: session:start
      ACTION: prompt_llm
      INSTRUCTION: "Check if user is returning and has recent context"
...
HANDOFF:
  ...
  # P5 -- Fallback
  - TO: Fallback_Handler
    WHEN: intent.category == "unknown" OR intent.unclear == true
    CONTEXT:
...
| `EXPECT_RETURN: true` / `false` | Whether the supervisor regains control after the child completes.                                                                                        |
| `ON_RETURN`                     | What to do when control returns (for example, check if the customer has additional needs).                                                               |
...
| Source attribution   | Always include source attribution so users can verify answers against original documents. |

<Note>See [Knowledge Bases and Knowledge Tools](/agent-platform/knowledge) for knowledge base ingestion, indexing, and search strategy configuration.</Note>
...
| `kind: input` / `output` / `both` | When the check runs.                                                                                                                                           |
...
| `action: redact`                  | Removes the sensitive content and continues.                                                                                                                   |
| `action: block`                   | Stops the message entirely.                                                                                                                                    |
| `action: warn`                    | Logs a warning but allows the message through.                                                                                                                 |
| `action: escalate`                | Routes to human review.                                                                                                                                        |
...
| `ACCESS: read` / `readwrite`       | Controls whether the agent can modify persistent data.                         |
...
INTENTS:
  human_escalation: "Customer requests a human loan officer."
...
ESCALATE:
  ...
  routing:
    ...
    skills: [underwriting, credit_review]
    priority: 1
...
INTENTS:
  human_escalation: "Customer requests a human specialist."
...
MESSAGES:
  ...
  escalation_format: "Connecting you with a specialist. Reason: {{reason}}."
```

This example demonstrates every major construct available to an agent without a `FLOW` section:

| Construct                          | Purpose                                                                                 |
|------------------------------------|-----------------------------------------------------------------------------------------|
| `GOAL` / `PERSONA` / `LIMITATIONS` | Identity and behavioral boundaries for the LLM                                          |
| `INSTRUCTIONS`                     | Step-by-step guidance for the reasoning LLM                                             |
| `TOOLS`                            | HTTP tools with `hints` for caching, latency, and timeouts                              |
| `GATHER`                           | Input collection with validation, inference, and extraction hints                       |
| `MEMORY`                           | Session and persistent variables                                                        |
| `CONSTRAINTS`                      | Labeled rule groups such as `always` and `action_rules`; labels are organizational only |
| `DELEGATE`                         | Synchronous sub-agent calls                                                             |
| `HANDOFF`                          | Context passing and return mapping between agents                                       |
| `ESCALATE`                         | Routing and human completion handlers                                                   |
| `COMPLETE`                         | Multiple completion conditions                                                          |
| `ON_START` / `ON_ERROR`            | Lifecycle hooks for session start and error recovery                                    |
| `EXECUTION`                        | Model, temperature, and fallback configuration                                          |
| `TEMPLATES`                        | Multi-format response templates                                                         |
| `GUARDRAILS`                       | Input and output safety checks                                                          |
| `NLU`                              | Intents, entities, and glossary                                                         |
| `HOOKS`                            | Before/after-turn logic                                                                 |
| `MESSAGES`                         | Reusable message strings                                                                |
...
FLOW:
  ...
  global_digressions:
    # Use ESCALATE for direct human queues.
    - INTENT: "cancel"
    ...
    - INTENT: "speak to agent"
      ...
      THEN: ESCALATE with REASON: "Customer requested human support"
...
- [Multi-agent and supervisor](/agent-platform/abl-reference/multi-agent-and-supervisor) -- full reference for `HANDOFF`, `DELEGATE`, `ESCALATE`, and `COMPLETE`
- [Flow](/agent-platform/abl-reference/flow) -- deterministic step graphs used by the flow-based patterns
- [Gather](/agent-platform/abl-reference/gather) -- input collection, validation, and inference options
- [Guardrails](/agent-platform/abl-reference/guardrails) -- input/output safety checks used in the guardrails pattern
- [Memory and constraints](/agent-platform/abl-reference/memory-and-constraints) -- session, persistent, and project-scoped memory used in the memory and recall pattern
- [NLU](/agent-platform/abl-reference/nlu) -- intents, entities, and glossary declarations
- [Examples by industry](/agent-platform/abl-reference/examples/by-industry) -- these same patterns applied in production banking, travel, healthcare, and retail examples
