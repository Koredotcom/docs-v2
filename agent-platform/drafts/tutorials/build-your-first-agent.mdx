...
flowchart LR
    A["**Part 1** <br /> Create Your First Agent"]
    B["**Part 2** <br /> Add Tools to Your Agent"]
    A --> B
```
...
This is a complete agent definition. Every agent reasons by default -- it uses the LLM to decide how to respond to each message. You can optionally add a `FLOW` block to give an agent structured steps (covered in the [next tutorial](/agent-platform/tutorials/build-a-scripted-flow)), but without one the agent handles conversations entirely through LLM reasoning.
...
When you use tools inside a flow section (covered in the [next tutorial](/agent-platform/tutorials/build-a-scripted-flow)), you handle results explicitly with `ON_SUCCESS` and `ON_FAIL`:
...
- [Add structured steps to an agent](/agent-platform/tutorials/build-a-scripted-flow) — Create step-by-step guided conversations with FLOW, GATHER, and branching.
- [Multi-agent and knowledge](/agent-platform/tutorials/multi-agent-and-knowledge) — Connect multiple agents through a supervisor and add a knowledge base.

<Tip>Check out the **Template Gallery** in Studio for ready-made agent definitions across use cases — customer support, e-commerce, banking, and more.</Tip>
