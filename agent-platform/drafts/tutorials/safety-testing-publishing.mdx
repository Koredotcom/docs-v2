---
title: Safety, testing and publishing
sidebarTitle: Safety, testing, and publishing
noindex: true
hidden: true
---
...
flowchart LR
    A["**Part 1** <br /> Set Up Guardrails"]
    B["**Part 2** <br /> Evaluate Your Agent"]
    C["**Part 3** <br /> Publish Your Agent"]
    A --> B --> C
```
...
Agent Platform is a managed SaaS platform -- you never deploy containers, manage servers, or configure infrastructure. Publishing means making your agent _available_ to real users through one or more **channels** (web widget, Slack, WhatsApp, voice, API, and more).
...
- **Name:** `Website Chat`
- **Allowed origins:** Enter the domains where you embed the widget (for example. `https://yoursite.com`)
- **Welcome message:** The first message users see (for example. "Hi! How can I help you today?")
...
```html
<script
  src="https://<platform-url>/widget.js"
...
- **Version label:** Enter a label (for example. `v1.0.0`)
...
```bash
curl -X POST https://<platform-url>/api/v1/sessions \
...
```bash
curl -X POST https://<platform-url>/api/v1/sessions/{sessionId}/messages \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"content": "Hello!"}'
```
...
{/* AG: Temporarily commenting to avoid broken links. After reinstating the linked files, remove the comment.

---

{{related}}

- [Safety and guardrails](/agent-platform/guides/safety-and-guardrails) — Advanced guardrail configuration and custom check functions.
- [Testing and evaluation](/agent-platform/guides/testing-and-evaluation) — Advanced evaluation strategies and CI/CD integration.
- [Channels](/agent-platform/guides/channels) — Connect to Slack, WhatsApp, voice, and the REST API.
- [Publishing and operations](/agent-platform/guides/publishing-and-operations) — Advanced monitoring, alerting, and version management.
