...
The Agent Platform delivers agent conversations through 20+ channels. Each channel has an adapter that normalizes inbound messages, delivers outbound responses, and handles platform-specific features like interactive elements, threading, and media. For authentication and error handling conventions, see [API overview](/agent-platform/api-reference/index).
...
Each webhook-capable channel connection generates a unique webhook URL in the format `https://agents.kore.ai/api/v1/channels/{channelType}/webhook/{identifier}`.

For Meta channels (WhatsApp, Messenger, Instagram), a shared webhook URL (in the format `https://agents.kore.ai/api/v1/channels/{channelType}/webhook`) with body-based routing is also supported.
...
{/* AG: Update the headings. */}

#### Create a new channel connection

Use `POST /api/projects/:projectId/channel-connections`.

Auth is required.

#### List all channel connections for a project

Use `GET /api/projects/:projectId/channel-connections`.

Auth is required.

#### Update a channel connection

Use `PATCH /api/projects/:projectId/channel-connections/:id`.

Auth is required.

#### Delete a channel connection

Use `DELETE /api/projects/:projectId/channel-connections/:id` or use the Studio UI at **Project** > **Channels**.
...
#### Configured and available voice services

Use `GET /api/v1/voice/capabilities`.
...
| Field        | Type    | Description                                   |
|--------------|---------|-----------------------------------------------|
| `twilio`     | Boolean | True if you've configured Twilio.             |
| `deepgram`   | Boolean | True if you've configured Deepgram STT.       |
| `elevenlabs` | Boolean | True if you've configured ElevenLabs TTS.     |
| `fullVoice`  | Boolean | True if you've configured all voice services. |
...
Access voice analytics at **Project > Analytics > Voice** or via the [Analytics API](/agent-platform/api-reference/knowledge-analytics-export#voice-analytics).
...
- [Conversation API](/agent-platform/api-reference/conversation-api) -- Text-based agent interactions
- [SDKs](/agent-platform/api-reference/sdks) -- Embed voice and chat widgets
