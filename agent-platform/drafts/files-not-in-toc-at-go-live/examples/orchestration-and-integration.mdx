...
HANDOFF:
  ...
  - TO: Intake_Agent
    ...
    RETURN: true
    ON_RETURN: "advance_to_validation"
    MAP:
  ...
  - TO: Validation_Agent
    ...
    RETURN: true
    ON_RETURN: "advance_to_processing"
    MAP:
  ...
  - TO: Processing_Agent
    ...
    RETURN: true
    ON_RETURN: "advance_to_confirmation"
    MAP:
...
MEMORY:
  ...
  persistent:
    ...
    - user.customer_id
  recall:
    - ON_START: "Check if returning customer"

HANDOFF:
  # Priority 0: Human escalation (always checked first)
  - TO: Human_Agent
    WHEN: user.wants_human == true OR user.frustration_detected == true
    CONTEXT:
      pass: [customer_id, conversation_summary]
      summary: "Customer needs human assistance"
    RETURN: false
...
- `RETURN: false` on human escalation ends the automated flow.
- `ON_RETURN: "reclassify_intent"` for the fallback agent re-evaluates the customer's clarified intent.
...
CONSTRAINTS:
  pre_action: # label only; add WHEN / BEFORE for explicit runtime scoping
...
- Constraint labels such as `pre_action` are organizational only; use the rule conditions themselves, `WHEN`, or structural `BEFORE` for runtime gating.
...
- `ON_RETURN: "check_resolution"` evaluates whether to complete or escalate further.
...
CONSTRAINTS:
  pre_action: # label only; use WHEN / BEFORE for explicit runtime scoping
...
HANDOFF:
  ...
  - TO: Booking_Manager
    ...
    CONTEXT:
      ...
      history: full
      grant_memory: [user.preferences, user.loyalty_tier]
    RETURN: true
  ...
  # Transfer to Human with maximum context
  - TO: Human_Agent
...
| `history: last_n: 20` | Forward the last N turns (useful for long conversations)        |
| `grant_memory: [...]` | Give the child agent access to specific persistent memory paths |
| `MAP`                 | Map child agent outputs back to parent session variables        |
...
| Escalation Chain           | L1 -> L2 -> L3 -> Human | Tiered support              | `HANDOFF` with escalation level                |
...
2. In the Agent Platform, navigate to **Project Settings > Credentials**.
...
4. In the Agent Platform, add the token as a Bearer credential named `slack`.
...
MEMORY:
  ...
  recall:
    - ON_START: "Check if returning WhatsApp user"
...
4. In the Agent Platform, add the token as a Bearer credential named `whatsapp`.
...
- See [Examples by Pattern](/agent-platform/examples/by-pattern) for fundamental ABL construct examples.
- See [Examples by Industry](/agent-platform/examples/by-industry) for production implementations using these patterns.
- See the [ABL Language Reference](/agent-platform/abl-reference/language-overview) for complete syntax documentation.
- See the [Tools Reference](/agent-platform/abl-reference/tools) for detailed tool configuration options.
