...
- `ON_RETURN.action` and `ON_RETURN.set` keep the parent supervisor's tier state executable.
- Each tier receives only declared context fields so agents do not repeat previous steps.
...
**Anti-pattern: Context loss.** If you use `EXPECT_RETURN: false` without adequate `pass` and `summary`, the receiving agent starts with no context. Always pass at minimum: customer ID, verified status, and a summary of what has been discussed.
