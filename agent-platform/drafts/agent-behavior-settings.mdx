...
Once call recording is available at the workspace level, a project admin or developer can turn it on for individual projects. Recording applies only to calls that start after you enable this setting - calls already in progress or completed earlier aren't captured retroactively.

<Note>

**Not the same as the softphone **Record call** switch. The Studio softphone has its own **Record call** switch for one-off testing. It captures a temporary audio file in your browser for a single test call, which you can only download - it isn't stored, doesn't appear on the session page, and is lost when you close the popover. Everything else on this page is about server-side recording.

An account admin must turn on call recording for the workspace (**Account** > **Workspace Settings**) before you can turn it on for a project. If your project's **Settings** menu doesn't show the **Recording** option, ask your account admin to enable it.

</Note>

**Navigation**: **Project** > **Settings** > **Recording**

A notice at the top of the page reflects the current state:

| Notice | Meaning |
| --- | --- |
| "Project-level recording settings are active for this project." | This project has its own saved settings. |
| "This project is currently using workspace recording defaults..." | No project-level settings are saved yet. |
| "Workspace-level recording is disabled..." | The workspace switch is off — enable it first. |
...
| **Retention days** | Number of days a completed recording stays available before it's automatically deleted, unless it's marked immutable. Allowed range: 1–2555 days (about 7 years). | 90 days |

### Project Recording Settings

**Enable Recording**

Turn on **Enable call recording** and click **Save changes**. No need to touch **Override account default** unless you have a specific reason.

**Retention Days**

Don't leave the field empty - an empty value is read as **1 day** (minimum). Workspace policies may enforce min/max limits; project values are kept within those bounds. Changes apply only to *new* recordings - existing audio keeps the retention set at creation. Retention cleanup runs hourly; expired audio may remain up to an hour past expiry unless marked immutable.

**Saving Settings**

Click **Save changes** to apply, or **Reset** to discard and restore the last saved values.

### Recording Availability

After a call ends, finalized audio is fetched and stored in the background. Availability typically takes a few minutes, up to **15 minutes**. The page doesn't auto-refresh, so click **Refresh** to check status. Treat this as an issue only if the recording is still unavailable after about 15 minutes.

<Note>

- **Override account default**: leave as-is unless required.
- **Retention days**: empty is read as 1 day; workspace limits may apply.
- **Retention changes**: affect only future recordings, not existing ones.
- **Availability**: audio may take up to 15 minutes; refresh manually.
- **Cleanup**: runs hourly; expired audio may linger briefly unless immutable.

</Note>

### Playback recordings

Recordings appear inside the session they belong to, in a **Call Recording** panel alongside the rest of that session's detail and diagnostics. Recordings are reached one session at a time — there's no single list of every recording in a project. If you don't know which session you want, use the **Sessions** filters to find the voice session first.

**Navigation**: **Project** → **Sessions** → select a session

| Status | Badge color | Meaning | What to do |
| --- | --- | --- | --- |
| **Pending** | Blue | The call ended and the audio is queued for collection. Playback isn't available yet. | Wait, then click **Refresh**. |
| **Fetching** | Accent, pulsing | The audio is being collected right now. | Wait, then click **Refresh**. |
| **Completed** | Green | The audio is stored and ready to play or download. | Play or download it. |
| **Failed** | Red | The audio couldn't be collected or stored. | Read the failure reason on the panel, then contact support. |
| **Deleted** | Gray | The audio is gone; only the entry remains. This is final. | Nothing further to do. |

### Session Recording Panel

**No Recording Yet**

If no recording exists, the panel shows: *"No recording is available for this session yet."* Click **Refresh** to check again.

**Completed Recording**

Once completed, the panel displays the call ID, duration, storage size, and retention (expiry) date.

Authorized users (`recording:read/write`) can:

- Play/pause, rewind/fast-forward 10s, and adjust speed (0.75x–2x). First play has a short load delay.
- View the waveform: caller (green, top) and agent (purple, bottom). Zoom 1x–4x. The filled portion shows playback progress.
- Use the **Synced to** indicator to match transcript and traces with playback. Click **Open conversation** or **Open traces** to jump, or use **Jump to audio** from those tabs.
- Download audio (available only after completion).

Every playback and download is recorded in the audit log.

### Locking and Deleting

**Locking (Immutable)**

Locked recordings can't be deleted, whether by retention cleanup or by a delete request. Locking requires the **mark immutable** permission, which is exclusive to the project admin role — general admin rights don't include it.

**Deleting**

Deleting requires `recording:delete` (project admin or developer). Deletion removes the audio from storage first, then marks the recording *Deleted*. If storage deletion fails, the status remains unchanged and the delete can be retried. Deleting a project, session, or workspace also deletes its recordings.

<Note>

**Lock and delete aren't yet available in Studio.** The panel supports **Play**, **Download**, and **Refresh** only. Use the **Recording API** or contact **Kore Support** for a legal hold or a customer deletion request — plan accordingly before promising self-service deletion or legal hold.

For externally hosted providers (for example, CCAI), deletion removes metadata only — the underlying media remains with the provider.
...
The platform doesn't play a "this call is recorded" prompt or show indicators. If required by law, add consent or notice to the agent's opening prompt.

**Storage Location**

Recordings are stored in the workspace's configured storage. Use customer-managed storage to enforce regional residency. The panel indicates whether storage is Kore-managed or customer-managed.