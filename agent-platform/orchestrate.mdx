...
### Input Processor
The Input Processor is an app-level feature that executes a custom script before orchestration begins. It is used to preprocess user input, enrich context, and populate memory variables that can be used by agents during execution. 
...
- **Enrich Context**: Enrich requests with additional context (e.g., user metadata, defaults)
