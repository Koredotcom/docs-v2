...
6. If one agent fails, its `ON_FAIL` handler runs independently, other agents continue.
...
| Partial failure  | Single ON_FAIL handler                | Each agent has its own ON_ON_FAILndler - one failing doesn't stop the others  |
