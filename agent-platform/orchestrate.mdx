...
flowchart TD
    U[End User] --> S[Supervisor Agent]
    S -- routes to --> B[Billing Agent]
    S -- routes to --> T[Technical Support Agent]
    S -- routes to --> SA[Sales Agent]
    B -- delegates to --> P[Payment Processing Agent]
    T -- fans out to --> D[Diagnostics Agent]
    T -- fans out to --> K[Knowledge Search Agent]
```

## Configure orchestration

You can configure orchestration in two ways: using Studio or using the ABL editor. Both methods are equivalent and interchangeable.
...
### Using the ABL editor

- Open the agent and switch to **ABL view**.
