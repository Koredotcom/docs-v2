...
Multi-Agent orchestration involves distributing tasks across specialized agents, each focused on a specific domain or task, such as billing, authentication, or order tracking.

Multi-Agent Orchestration encompasses the following components:
...
Together, these mechanisms let you build systems where users experience a single, unified assistant — while specialized agents work together behind the scenes.
...
| ---------- | ---------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| [Supervisor](/agent-platform/orchestrate#1-supervisor) | Entry point for Multi-Agent systems; intent-based routing                                | User always talks to an agent, who routes the request to a specialized agent as required. |
| [Handoff](/agent-platform/orchestrate#2-handoff)    | Route the conversation to a specialist agent.                                            | The user is transferred to a new agent                                                    |
| [Delegation](/agent-platform/orchestrate#3-delegate) | The agent needs a sub-task done; Sends the sub-task to an agent and get results back     | Transparent -- user does not see the delegation.                                          |
| [Fan-out](/agent-platform/orchestrate#4-fan-out)    | The agent needs multiple things done in parallel. Run multiple agents and merge results. | Transparent -- user sees a single combined result.                                        |
| [Escalation](/agent-platform/escalate) | Transfer to a human agent with full context.                                             | User interacts with a human agent.                                                        |
...
- Updates apply immediately after successful compilation.

<Note>The UI and ABL are interchangeable. Changes made in ABL reflect in the UI and vice versa.</Note>
...
- Routes them to specific agents based on user intent, session state, channel, or custom conditions.
...
A multi-agent system can have only one Supervisor. It's the entry point for all user interactions.
...
Handoffs can originate from a Supervisor routing to a specialist agent, or directly from one agent to another for peer-to-peer routing. In both cases, the target agent receives the context and continues the conversation from that point.
...
| `ON_FAILURE` | No       | Action if the sub-agent fails or times out. Options: RESPOND "message" (show a message and continue), ESCALATE (trigger escalation), RETRY count (retry the delegation) |
...
| Partial failure  | Single ON_FAILURE handler                | Each agent has its own ON_FAILURE handler - one failing does not stop the others |
...
For more details on memory and access scopes, refer [this guide on memory management across agents](/agent-platform/memory-and-state).
...
| Timeout on slow agents in fan-out                 | Set `TIMEOUT` on each delegate independently. Use `ON_FAILURE` to handle agents that do not respond in time.                                                     |
