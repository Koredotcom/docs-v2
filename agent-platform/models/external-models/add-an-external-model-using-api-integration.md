...
<Note> This configuration requires at least one feature to be enabled. Users can enable any feature, but the model must support it. Otherwise, you may see unexpected behavior. </Note>
...
<Note> This feature must be enabled for the model to be used in Agentic Apps. If tool calling is not enabled, the model cannot be used within Agentic Apps or for executing tool calls in the AI Text-to-Text node in workflow tools.</Note>
