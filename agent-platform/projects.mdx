...
- View projects using the available categories: Active, My Projects, Shared with Me, Project Access, Pinned, and Archived.
...
Because the project is the boundary for access control and deployment, resources defined in one project aren't visible in another. Moving assets between projects requires the [export and import](#exporting-and-importing-projects) flow.

## Before you begin

Before creating or managing projects, confirm the following.

- **Project access.** You must be signed in to Studio with a user account that belongs to the target workspace.
...
Every project has a unique project ID. Studio doesn't display the project ID as a field on any page. The ID appears in the browser address bar whenever you have the project open.

You need the project ID when you do the following:

- Call the Agent Platform public APIs, which take the project ID as the `projectId` path parameter. The API key you use must be bound to the same project.
- Configure the Artemis CLI or the MCP server against a specific project.
- Raise a support request about a specific project.

**To find the project ID:**

1. On the Projects dashboard, select the card of the project you want. Studio opens the project workspace.
2. In the browser address bar, locate the 36-character identifier in the URL. The identifier uses the UUID format `xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx`.

   ```text
   https://<studio-host>/<workspace-path>/<projectId>/overview
   ```

3. Select the identifier and copy it.

<Note>The host name and the surrounding path segments differ between environments, but the project ID is always the UUID segment. The project ID stays the same for the life of the project. If you delete a project and recreate it, the new project receives a different ID.</Note>

<Tip>The [project switcher](#project-switcher) changes the project ID in the URL. After you switch projects, confirm that the project name in the header matches the project you intend to query before you copy the ID.</Tip>

### Project sidebar

The left sidebar provides hierarchical navigation inside a project. It's organized into three top-level sections, with **More** option expanding the grouped sub-pages.
...
You can expand a group to see its child pages. Use the back button to return to the top-level navigation.

To increase horizontal working space on the UI, you can collapse the sidebar to an icon-only mode. Also, it auto-collapses when the agent editor is active.
...
Use the **Export Project** dialog to package an agent project into a portable .zip file. The archive contains the project's agents, tools (including code, http, knowledge, MCP, and workflow tools), and any optional layers you select—such as guardrails, workflows, and channels. You can [import the project archive](#import-project) into another environment or create its backup as a versioned snapshot.

Before you select the **Export** option, review the project contents in the export dialog.
...
- **Export Warnings**: If the platform detects any conditions that affect the export, such as automatic normalization of tool formats, it surfaces them in the Export Warnings. All warnings are informational and don't block the export process. Review warnings before exporting to understand what the platform has automatically adjusted. 
- **Export Layers**: Include or exclude different parts of the project from the exported archive.
- **Export Options**: Include or exclude all published agent, config-variable, and workflow versions.

<Note>
- You must configure the credentials in the target environment after import. The export only includes the structural requirements and metadata (such as which profile types/auth fields are needed).
- Any hardcoded secret values remaining in text/YAML files are replaced with `{{config.X}}` references before archiving.
- The export includes the complete workflow definition, including nodes, edges, versions, triggers, and tool bindings.
</Note>

### Import project

To import an agent project or individual agent files, use the **Import Project** dialog. Navigate to **Project** > **Overview** > **Import**. The import supports merge-based import, stale content removal, environment reference adaptation, and three-way conflict detection.
...
| Field or Option      | Description                                                      | Notes                                        |
|:---------------------|:-----------------------------------------------------------------|:---------------------------------------------|
...
Expect the following folder structure in a `.zip` archive of a project.
...
├── tools/                           # Core - tool definitions, including code tools, http tools, workflow tools, knowledge base tools, mcp tools.
│   └── hotels-api.tools.abl
├── config/                          # project configuration
└── core/                            # Core - mcp server config
├── connections/                     # Connections
├── guardrails/                      # Guardrails
├── workflows/                       # Workflows
└── search/                          # Knowledge base and index configuration
    └── indexes/
    └── knowledge-bases/
```
...
The Import Mode section controls how the incoming project is reconciled with the existing, target project in the platform. The default mode is to merge.
...
When checked, this modifier changes how each import layer (for example. agents, tools, connections) is handled: any content belonging to that layer that is present in the target but absent from the archive is removed. Content that exists in both the archive and the target is updated normally.
...
- Eval references - Normalizes evaluation dataset or metric references so they're valid within the current environment's evaluation framework. If a referenced modelId doesn't exist in the target tenant, the user is prompted to pick a replacement. 
...
1. Navigate to the project where you want to import content and select **Import**.
2. Drag your file into the upload area, or browse and select a file. Supported formats are: .zip, .agent-bundle.json, .ablenc, .abl, .yaml.
3. Choose your **Import Mode** options. Merge is always active. Check **Replace project contents** if you want to remove layer-level content from the target that's absent from the archive.
4. Optionally, check **Adapt references for this environment** to normalize bindings and references for the current tenant.
5. Optionally, check **Use a base export for conflict checks** to enable three-way conflict detection.
6. Select **Import** to start the import.
7. Review any validation errors or conflict reports surfaced by the platform after import completes.

<Note>Imported workflows include the complete workflow definition, including nodes, edges, versions, triggers, and tool bindings. As a result, workflows are functional immediately after import and do not require manual reconfiguration.</Note>
...
<Warning>Deleting a project permanently removes all agents, tools, knowledge bases, workflows, sessions, and deployment configurations in that project. Deleted projects are archived for 30 days.</Warning>
...
If you expect access to a feature and don't have it, contact your workspace administrator.