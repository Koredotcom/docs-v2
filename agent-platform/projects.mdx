...
Click **Export** to download a .zip archive to your browser. The file follows the standard folder structure and you can import it using the Import Project dialog.
...
<Note>
- Imported workflows include the complete workflow definition, including nodes, edges, versions, triggers, and tool bindings. As a result, workflows are functional immediately after import and do not require manual reconfiguration.
- Credentials must be reconfigured in the target environment after import. The ZIP file includes only the required credential metadata, such as profile types and authentication fields.
</Note>
