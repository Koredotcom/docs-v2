...
---
<Badge icon="arrow-left" color="gray">[Back to Connectors](/ai-for-service/quality-ai/connectors)</Badge>

The AWS S3 Connector enables you to pull conversation recordings and chat transcripts from an S3 bucket into Quality AI Express on a configurable schedule. Use this connector to analyze interactions from third-party Contact Center as a Service (CCaaS) solutions.
...
<Note>Mono recordings require two separate audio files — one for the agent and one for the customer. A single mixed mono file isn't supported.</Note>
...
Each recording type requires specific CSV fields. The core fields are the same across all types and only the recording-specific fields differ.
...
### ​Agent and queue validation scenarios

- The system processes the conversation if the onboarding team hasn't onboarded the agent, but has onboarded the queue.
- The system processes the conversation if the onboarding team has onboarded both the agent and the queue, even when the agent isn't mapped to the queue.
- The system rejects the conversation during validation if the onboarding team hasn't onboarded the queue or configured it.
...
- All file sizes are under 50 MB, and file names don't contain any spaces.
...
2. Select **Test** to validate the connector configuration. If a validation fails, correct the issue and select Re-Test.
3. Verify that the system successfully completes the following validation checks:
   - **Authentication**: Validates the connection string and Azure storage account access.
   - **Filepath Availability & Accessibility**: Verifies that the configured Blob path exists and is available.
...
The connector displays the validation progress and any errors directly in the **Test** tab. 
...
* Conversations display in **Quality AI Express** dashboards.
...
| Permission Denied   | Access denied to S3 bucket | Add S3 read permissions to the IAM user or role. Verify the bucket policy and bucket region. |
...
| Problem          | Symptom                  | Resolution                                                                                                                       |
| ---------------- | ------------------------ | -------------------------------------------------------------------------------------------------------------------------------- |
| Timestamp Errors | Invalid timestamp format | Use ISO 8601 format (`YYYY-MM-DDTHH:MM:SSZ`) and include the UTC timezone. Verify that the end time occurs after the start time. |

### Performance Considerations

Processing time varies based on factors such as conversation length, file size, ASR transcription latency (for voice interactions), and LLM processing time. 
