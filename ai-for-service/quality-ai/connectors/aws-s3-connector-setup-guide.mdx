...
- If the agent is not onboarded but the queue is onboarded, the system processes the conversation.
- If both the agent and queue are onboarded, the system processes the conversation even when the agent is not mapped to the queue.
- If the queue is not onboarded or configured, the system rejects the conversation during validation.
