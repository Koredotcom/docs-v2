...
2. Confirm the following checks pass:
   ...
   | **File Path Access**    | S3 bucket available.      |
...
3. If a check fails:
   ...
   | Check               | Resolution                                                                      |
   |---------------------|---------------------------------------------------------------------------------|
   | **Authentication**  | Verify the credentials and IAM permissions and confirm they haven’t expired.    |
   | **File Access**     | Check the bucket name, region, and folder paths, and confirm that the file URLs are available.   |
   | **Format/Metadata** | Confirm that `test.csv` exists and has the correct structure, column headers, and timestamps. |
...
- The processing logs don’t display any errors.
...
| ------------------- | --------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| **Invalid Credentials** | Authentication failed error | Verify the access key and secret key. Check the IAM role ARN format and confirm that the credentials have not expired.                  |
| **Permission Denied**   | Access denied to S3 bucket  | Add S3 read permissions to the IAM user or role. Verify the bucket policy and confirm that the bucket region matches the configuration. |
...
Processing typically takes approximately 3–5 minutes per conversation, depending on conversation length, ASR transcription latency (for voice), and LLM response latency.
