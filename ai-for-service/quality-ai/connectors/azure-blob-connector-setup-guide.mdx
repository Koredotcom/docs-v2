...
| Data Type | Format | Files per Conversation | Notes |
|---|---|---|---|
| **Stereo Voice** | WAV, MP3 | 1 | Single dual-channel recording (agent + customer). |
| **Mono Voice** | WAV, MP3 | 2 | Separate agent and customer recordings. Mixed audio isn't supported. |
| **Voice Transcript** | JSON | 1 | Pre-transcribed voice conversation. |
| **Chat Transcript** | JSON | 1 | Message-level chat transcript. |
...
- All required files are uploaded to Azure Blob Storage.
- Metadata `.csv` files contain required fields and valid values.
- Mono conversations have separate agent and customer files.
- The container and folder paths are accessible.
- A `test.csv` file exists in each ingestion path.
- Blob permissions allow the connector to access the files.
- File and folder names contain no spaces or special characters.
