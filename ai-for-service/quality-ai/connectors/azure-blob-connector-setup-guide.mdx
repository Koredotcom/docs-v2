...
The Azure Blob Storage Connector uses the selected **Auth Type** to authenticate with Azure Blob Storage. 

### Connection String 

Use this method to authenticate the connector using an Azure Storage connection string. 
...
<Note> The authentication fields displayed in the connector depend on the selected **Auth Type**.</Note>

## Supported Data Types and File Requirements

The Azure Blob Storage Connector supports multiple interaction data types for ingestion and analytics.
...
| **Mono Voice**       | WAV, MP3 | 2 files (agent and customer)    | Requires separate agent and customer recordings. Mixed audio files containing both speakers aren't supported.  | Enhanced Analytics      |
...
**Option A: Separate Voice and Chat Paths** (Voice and Chat in Separate Folder Structures)
...
**Option B: Unified Path** (Voice and Chat in a Single Folder Structure)
...
Processing time varies based on factors such as file size, audio duration, transcription latency, and AI analysis complexity.
