...
The Agent-Specific Overview Dashboard provides supervisors with a comprehensive view of individual agent performance across evaluations, audits, sentiment, quality, and resolution metrics. It helps monitor performance trends, identify coaching opportunities, and support corrective actions when needed.

The dashboard automatically updates based on selected filters and displays performance data for the chosen agent. It includes interaction trends, Auto QA and manual evaluation scores, scorecard performance, L3-level sentiment and resolution insights, evaluation review status, and dispute activity (when enabled).

Supervisors can filter by date range, language, queue, channel (voice or chat), and contact direction to refine insights. With Cross Queue Data Access, they can view all queues; otherwise, access is limited to assigned queues.
...
| **Evaluation Reviw and Dispute Tracking** | Monitor evaluation results, including acknowledgement status and dispute activity for audited conversations. |
...
<Note> The **Overview** tab focuses on aggregated performance trends. Use the **Evaluation** tab to investigate individual interactions.</Note>

---

### Agent Performance Metrics

Provides a summary of agent activity and quality performance of the selected date range. Each widget shows the current value and % change (↑ improvement, ↓ decline). 
...
Each metric displays the current value and trend change compared to the previous reporting period.
...
| Section | Description |
|--------|-------------|
| **Strongest Attributes** | Displays the top five highest-scoring attributes for the selected scorecard.  |
| **Opportunity Areas** | Displays the bottom five attributes highlighting areas that need coaching attention. |

Selecting an attribute displays the underlying evaluation metrics contributing to the score.
...
Resolution Insights helps supervisors evaluate how well agents resolve customer issues by highlighting resolution trends across L3 topics and identifying coaching opportunities and resolution gaps.
...
When you select a scorecard, the average score for that agent within the chosen global date range is displayed. If the number of conversations is calculated after assignment, the scorecard doesn't display any score for that agent. If any conversations exist, then the scorecard is shown in the scorecard trends dropdown.
