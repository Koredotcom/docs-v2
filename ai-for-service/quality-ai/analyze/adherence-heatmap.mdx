...
By default, the heatmap displays only eligible evaluated interactions. The system excludes interactions marked Below Threshold or Duration Unavailable unless a reviewer manually evaluates them. Adherence metrics use only valid and comparable data. The same rule applies to AI Agent (Automation AI) conversations. A reviewer must manually audit an AI Agent interaction through the Manually Evaluated Flagged Interaction path described under [Handling Flagged Interactions](#handling-flagged-interactions) before the system includes it in the heatmap.
