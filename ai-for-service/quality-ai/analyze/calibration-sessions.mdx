...
QA Analysts may score the same customer conversation differently because evaluation criteria can be interpreted in different ways. Without a structured way to compare these evaluations, teams have limited visibility into scoring inconsistencies, analyst alignment, and adherence to team standards.
...
The workflow provides teams with a consistent way to measure analyst alignment, establish reference outcomes, and improve how evaluation criteria are applied over time.
...
| **Debrief and compare results**    | Compare analyst scores side by side for each evaluation metric after all assigned evaluations are submitted.                                                              |
...
| Status     | Description                                                               |
| ---------- | ------------------------------------------------------------------------- |
| **Draft**  | The manager has started configuring the session but has not activated it. |
...
1. Select a **Conversation Type** to define which conversations are available to add.
   - **Audited**: Displays conversations that are audited.
...
Managers can open the conversation preview or remove conversations before continuing. At least one conversation is required to proceed.
...
QA Analysts access their assigned Calibration Sessions from the **Calibration** tab in their Audit Allocations area. The system also sends an email notification when a session is assigned to an analyst.

The **Calibration** tab appears when at least one session is assigned to the analyst. Each session displays the session name, assigned-by name, scoring progress (for example, 2 of 4 conversations scored), and session status (**Not Started**, **In Progress**, or **Submitted**).
...
* Review the calibration report after the session is closed.
...
* Comments are required when **Analyst Justification** is set to **Mandatory**.
...
5. Add notes when additional scoring context is required.
...
Debrief View opens from a conversation in the session, expanding a metric-by-metric comparison grid for that conversation — the same grid structure the closed Calibration Session Report reveals inline when you select the view icon on a Conversation-Level Summary row, described under [Calibration Session Report](#calibration-session-report).
...
| **Metric**                  | Displays the evaluation criterion being compared.                                                                                                                    |
...
| **AutoQA Verdict**          | Displays the AutoQA result for the metric, when available. Displays as **Adhered**, **Not Adhered**, **N/A**, or **Not Found** when AutoQA did not produce a result. |
...
1. Open Debrief View after all required analyst evaluations are submitted.
...
7. Continue until all required disagreements are resolved.
...
| **Consensus Alignment Summary** | Displays per-analyst results for **Parameters Evaluated**, **Consensus Alignment**, **Inter-Auditor Agreement**, and **Accuracy**. An **AutoQA** benchmark row appears below the analyst rows and is evaluated against human consensus but excluded from the consensus calculation. |
...
Select [Export CSV](/ai-for-service/assets/agentai-interaction-calibration.csv) to download the calibration results as a CSV file.
...
Every calculation treats **N/A** as a full third verdict alongside **Adhered** and **Not Adhered**. No analyst response is excluded from any metric's denominator.
...
A metric reaches consensus when one verdict is chosen by strictly more than half the analysts:
...
The Metric Agreement Rate is computed only when a consensus verdict exists.

### Session Agreement Rate

The **Session Agreement Rate** is the average agreement rate across all metrics that reached consensus. Metrics labeled **No Consensus** are excluded.
...
The **Session N/A Rate** measures the proportion of all metric responses that were marked **N/A**.
...
The agreement rate shown on each conversation row in the **Conversation-Level Summary** is computed using the same method as the **Session Agreement Rate**, but scoped to a single conversation.
...
* **Consensus calculation:** Consensus is determined using only human analyst verdicts. The AutoQA verdict is excluded from **N** and from the **A**, **B**, and **C** counts.
* **AutoQA Consensus Alignment:** Calculated using the **Auditor Agreement Rate** formula, with AutoQA treated as an additional participant whose verdict is compared against the human-only consensus.
...
The **Calibration Session Report** is available to all session participants after the session closes, including analysts who do not have the **Audit Allocations** permission.
