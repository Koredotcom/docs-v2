...
Audit Allocations supports allocation lifecycle management, with available tabs based on your role and permissions.

| Permission Level                           | Tabs Available                         | Description                                                                                                                                                                       |
| ------------------------------------------ | -------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **With allocation creation permission**    | **My Allocations**, **Assigned to Me** | Track and manage your allocations, edit assignments, and monitor completion. This also evaluate interactions assigned to you across inbound, outbound, callbacks, follow-ups, and campaigns |
...
Select **New Audit Allocation** to start the three-step creation process: Settings, Allocation, and Assignment. For details, see [Creating Audit Allocations](/ai-for-service/quality-ai/analyze/creating-audit-allocations).
...
| **Emotions** | Detected customer interaction emotions (for example, frustrated, satisfied, neutral). |
...
Use Filters to narrow assigned interactions by selecting one or more values; the table updates to show matching records.
...
<Note> Manual evaluation metrics are excluded from Custom audit allocation filters, so they don’t appear in the Metric Name quick filter during allocation configuration. </Note>
...
2. Review the complete interaction with all available context:
   ...
   - Interaction metadata (duration, channel, queue, and direction)
