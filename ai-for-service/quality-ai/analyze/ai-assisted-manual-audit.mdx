...
- If the QA User has marked the same agent and the mark is still pending, the Audit Screen displays a confirmation message indicating that the agent already exists in the Coaching Candidates queue.
...
| **Not Adhered** | The response doesn't meet the compliance requirement. Display timestamps only for violation scenarios where a specific message or event causes the issue, and do not display them for omission-based failures. |
...
QA users (auditors or supervisors) can self-assign unclaimed interactions for auditing. Interactions that are audited, completed, or assigned to another user are not eligible.
...
The system displays the **Submit** button only for interactions assigned to you through Audit Allocations.
...
| **Supervisor and reviewer changes** | Tracks who made each change and modified content. |
