...
The system updates insights automatically.  
...
<Note>The Review screen is available only when **Agent Accept & Dispute** is enabled in **Quality AI** > **Settings** > **Quality AI General Settings** and at least one evaluation form has disputes configured. If the setting is off or no forms have disputes enabled, the Review screen is not accessible. </Note>
...
If disputes are disabled for the evaluation form associated with a conversation, agents can't comment, accept, or dispute any metric for that evaluation. The metric-level Accept and Dispute buttons are not displayed. The evaluation displays as **Audited** with no Review Status.
