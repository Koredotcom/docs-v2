...
<Note> These above fields are mandatory for Quality AI evaluation of third-party desktop conversations.</Note>
...
<Note> For third-party desktop conversations recorded through Agent AI, the webhook API must provide the configured `queueId`, `queueName`, and `agentEmail`. Queue mapping and evaluation depend on these values. </Note>
