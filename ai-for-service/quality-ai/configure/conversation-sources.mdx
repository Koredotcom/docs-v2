...
Quality AI also evaluates conversations handled by AI-assisted human agents. Agent AI ingests conversations recorded through supported external Contact Center as a Service (CCaaS) or Customer Relationship Management (CRM) agent desktop applications when they include the required conversation metadata.
...
Quality AI supports message-level offset timestamps without validating the conversation start or end time. This format applies only to chat conversation ingestion and doesn't affect voice conversation ingestion.

- **Format:** Integer or decimal number
- **Examples:** `0` (start), `45` (45 seconds in), `120.5` (2 min 0.5 sec in)

This format doesn't validate start and end dates. It uses timestamps only for reporting and filtering. This change applies only to chat ingestion.
...
Disabling Quality AI Express hides the following CCAI metrics:
