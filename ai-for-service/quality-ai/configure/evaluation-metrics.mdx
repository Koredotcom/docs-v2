...
### By Transfer Etiquette

Evaluates how well agents communicate with customers before initiating internal transfers during voice interactions, ensuring consistent transfer etiquette and maintaining service quality throughout the handoff.

**Key Features:**

- **Customer Notification**: Verifies that agents notify customers before initiating a transfer.
- **Reason Explanation**: Checks whether agents explain why the transfer is necessary.
- **Destination Information**: Evaluates how agents communicate the transfer destination to customers.
- **Transfer Detection**: Automatically detects transfer events and evaluates multiple transfers within a conversation.
- **GenAI Detection**: Uses contextual LLM-based analysis to evaluate transfer etiquette.
- **Deterministic Detection**: Uses embedding-based semantic matching with predefined utterances.
- **Configurable Detection Window**: Evaluates the last utterance or agent utterances within a configurable time window before the transfer.

**Use Cases:**

Transfer etiquette compliance, customer experience improvement, agent coaching, internal handoff quality, and consistent communication during warm and cold transfers.

For full configuration details, see [By Transfer Etiquette](/ai-for-service/quality-ai/configure/evaluation-criteria/metrics-measurement-types/by-transfer-etiquette).
