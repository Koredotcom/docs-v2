...
![GenAI-based Features](/ai-for-service/quality-ai/configure/evaluation-criteria/metrics-measurement-types/images/by-questions-v1-v2.png)
...
| Capability | By Question V1 | By Question V2 |
|------------|----------------|----------------|
| Rule model | Trigger and answer are configured as two separate rules. | A single unified rule describes the condition and the expected agent behavior together. |
| Evaluation flow | Trigger and answer are evaluated independently; the application combines the results afterward. | The LLM evaluates in two steps — first whether the condition applies, then whether the agent complied. |
| Not Applicable | Inferred by the application from the trigger result; no AI-generated explanation. | A first-class output with its own AI-generated justification. |
| Multi-condition logic | Each condition needs its own separately configured metric. | A single metric can express multi-condition logic. |
| Justifications | Two disconnected reasoning fragments. | Separate, coherent justifications for the condition and the adherence verdict. |
| Evaluation Window | Not supported. | Turn-based window: entire, first X, or last X turns. |
...
Evaluates responses only when specific triggers or conditions occur; used for conditional rules that apply in certain situations.
