...
Both versions use a parent metric with multiple sub-metrics, each with its own question, weight, and logic. A single evaluation call processes all sub-metrics and returns results with justifications. 
...
- **By AI Agent V2 (Artemis)**: Uses Agent Platform V2 (Artemis) with project-based authentication and advanced API configuration.
