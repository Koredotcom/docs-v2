...
- **Direction-aware Assignment**: Assign separate scorecards for Inbound and Outbound contacts to evaluate each direction using relevant criteria.
- **Multiple Evaluations**: Apply multiple scorecards to each interaction to assess agents.
...
- **Not Applicable Scoring Treatment**: Choose whether to exclude metrics marked Not Applicable from scorecard scoring or award them full weightage, independently of the evaluation form's setting. 
...
2. By default, the system displays the **Settings** tab.
...
7. Select a **Not Applicable Scoring Treatment**:

   - **Exclude from Score** (default): Removes a Not Applicable metric's weightage from both the numerator and denominator when the system aggregates metric results into Agent Attribute and Scorecard scores.
   - **Award Full Weightage**: Adds a Not Applicable metric's weightage to both the numerator and denominator, so the metric contributes full marks to the Agent Attribute and Scorecard scores while its displayed outcome stays Not Applicable.
   
This setting uses a separate configuration from the **Not Applicable Scoring Treatment** configured on the evaluation form. It doesn't inherit the evaluation form's setting and applies only to future evaluations. When the two settings use different values, the scorecard score can differ from the corresponding evaluation form score.

7. Turn on to **Set the minimum duration required to complete evaluations**.
...
10. Select **Next**.
   ...
   <Note>At least one scorecard you must set as the default. The first scorecard you create becomes the default automatically. Before removing the default setting from a scorecard, assign another scorecard as the default.</Note>
...
1. Select **Create** to create the Agent Scorecard with the selected agent attributes and assigned agents and agent groups.

    ![New Scorecard](/ai-for-service/quality-ai/configure/agent-scorecards/images/newly-created-scorecard.png)

   <Note>Toggle the Scorecard **Status** to **Active** to begin agent-level scoring.</Note>
...
The scorecard-level Not Applicable Scoring Treatment determines how Not Applicable metric outcomes contribute to Agent Attribute and overall Scorecard scores. The system applies this setting across the Agent Leaderboard, Agent Dashboard, and agent-specific dashboards. It doesn't inherit the **Not Applicable Scoring Treatment** configured on the evaluation form.

| **Treatment** | **Effect on Agent Attribute and Scorecard Scores** |
| --- | --- |
| **Exclude from Score** (default) | Excludes the Not Applicable metric's weight from the score calculation by removing it from both the numerator and denominator. |
| **Award Full Weightage** | Includes the Not Applicable metric's weight as full credit by adding it to both the numerator and denominator. The metric remains marked as **Not Applicable**. |

A change to this setting doesn't retroactively rescore conversations evaluated. It applies only to metric contributions from conversations scored after the change.

<Note> Not Applicable** scoring is configured independently for evaluation forms and scorecards. Different settings can result in different scores for the same interaction.</Note>
...
3. Update the **Settings**, including **Not Applicable Scoring Treatment**, **Agent Attributes**, or **Assignments**, as needed.
...
To delete an agent scorecard:

1. Select the vertical ellipsis (⋮) for the scorecard you want to delete.
2. Remove any unassigned attributes. If an attribute belongs to another scorecard, remove that scorecard assignment first.
3. Select **Delete** to permanently remove the scorecard.

<Note>Deleting an agent scorecard permanently removes all data associated with the scorecard. Deleted scorecards can't be recovered.</Note>

#### Delete the Default Scorecard

You can't delete the default scorecard directly.

Before deleting the current default scorecard:

1. Assign another scorecard as the default.
2. Select the vertical ellipsis (**⋮**) for the current default scorecard.
3. Select **Delete**.

If you attempt to delete the default scorecard without assigning another scorecard as the default, the system displays a warning dialog.

#### Delete an Agent Scorecard

1. Select the vertical ellipse (⋮) on any existing scorecard.

1. Delete attributes only if they're unassigned; otherwise, remove associated scorecards first.
...
1. Add **Total Negative Weightage** cumulative score percentage.
...
1. Enable to mark the metric **Fatal Error** (if it's a critical failure condition).
1. Select **Create**.
...
| Scenario                       | Behavior                                                                                  |
| ------------------------------ | ----------------------------------------------------------------------------------------- |
| **Deleting any attribute**         | All data linked to the attribute is permanently deleted and can't be recovered           |
...
| **Eligible for deletion**          | Only unassigned agent attributes can be deleted.                                           |
   ...
   <Note>You can't delete an attribute that's assigned to a scorecard. Remove it from all scorecards first. Deleting an attribute removes all associated data.</Note>
