...
- **Not Applicable Scoring Treatment**: Choose whether metrics that resolve to Not Applicable are excluded from scorecard scoring or awarded full weightage, independent of the evaluation form's setting. 
...
7. Select a **Not Applicable Scoring Treatment**:
   ...
   - **Award Full Weightage**: Adds a Not Applicable metric's weightage to both the numerator and denominator, so the metric contributes full marks to the Agent Attribute and Scorecard scores while its displayed outcome stays Not Applicable.
      ...
      This setting is independent of the Not Applicable Scoring Treatment configured on the evaluation form — it doesn't inherit that setting, and it applies only to future evaluations. Because the two settings are configured separately, a scorecard score can differ from the corresponding evaluation form score when they're set differently.
...
The scorecard-level **Not Applicable Scoring Treatment** determines how **Not Applicable** metric outcomes contribute to **Agent Attribute** and overall **Scorecard** scores. It is configured independently of the evaluation form's **Not Applicable Scoring Treatment** and is used consistently across the **Agent Leaderboard**, **Agent Dashboard**, and agent-specific dashboards.
...
A change to this setting doesn't retroactively rescore conversations already evaluated. It applies only to metric contributions from conversations scored after the change.
...
2. Remove any attributes that are no longer assigned. If an attribute is associated with another scorecard, remove the associated scorecard assignment first.
3. Select **Delete** to permanently remove the scorecard.

> **Note:** Deleting an agent scorecard permanently removes all data associated with the scorecard. Deleted scorecards cannot be recovered.
