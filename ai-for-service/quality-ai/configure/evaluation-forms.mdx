...
Each queue supports one evaluation form per channel and contact direction (for example, Voice–Inbound vs. Voice–Outbound), enabling separate configurations for inbound and outbound interactions. QA Managers can also designate metrics for manual scoring (excluding them from scorecards) and set minimum duration thresholds to filter short or incomplete interactions from AutoQA, ensuring direction-aware and accurate evaluations. 
...
5. Assign metric **Weightage** value based on the selected scoring type: use percentage values for percentage-based forms and points for points-based forms.
