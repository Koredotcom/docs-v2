...
2. Select **Export Conversation Data**, **Export Transcripts**, or **Export Plain Text Transcript**.

    ![Export Options](/ai-for-service/analytics/overview/images/conversation-export-options.png)

    Or select **Download Chat Transcript** or **Download Latency Report**. These download as .csv files.

    ![Download Options](/ai-for-service/analytics/overview/images/actions-tab-options.png)
...
3. Select the **Filter By** criteria.
...
Connection lost with Agent `<Agent Name>`: Timestamp and error type: SIP drop, WebRTC ICE failure, Agent heartbeat timeout, or Unknown.
...
| `Agents` | All agents who participated in the call, including any agent added after a network interruption re-route. |
...
| `Snooze Duration` | The duration for which the conversation was snoozed. |
...
| `media_generation_failed`      | Media generation failed due to a technical issue.                                                                                                                                |
| `media_conversion_failed`      | Unable to fetch the recording. We'll automatically retry within 24 hours, or you can click 'Retry' to try again now. If the issue persists, please try again after 15 minutes.  |
| `partial_download_failed`      | Unable to fetch the recording. We'll automatically retry within 24 hours, or you can click 'Retry' to try again now. If the issue persists, please try again after 15 minutes.  |
| `partial_upload_failed`        | Unable to fetch the recording. We'll automatically retry within 24 hours, or you can click 'Retry' to try again now. If the issue persists, please try again after 15 minutes.  |
| `failed`                         | Media generation failed due to a technical issue.                                                                                                                                |
| `media_unavailable_short_duration` | Media unavailable. This call was too short to generate a recording.                                                                                                            |
| `recording_disabled`             | Media unavailable. Recording is disabled for this interaction.                                                                                                                   |
...
![Screen Recording Download](/ai-for-service/analytics/overview/images/screen-recording-download.png)
...
* Analyze call quality per agent  
    ...
    ![Agents Tab](/ai-for-service/analytics/overview/images/agent-diagnostics.png)
...
### Network Interruption Re-routing

When a call is re-routed, the Agents tab lists the original agent and the new agent as separate rows with individual timestamps and call quality metrics.

| Agent          | Call Exit Reason       |
| :------------- | :--------------------- |
| Original agent | Network Interruption   |
| New agent      | Standard exit reason   |
