...
| Filter | Description |
|--------|-------------|
| **Date** | Select a specific period to display conversations from that time. Format: **MM-DD-YYYY**. Options: **24 Hours** (default) - data from the preceding 24 hours; **7 Days** - data from the past seven days, starting from the selected or current day; **Custom** - choose a start and end date (up to 90 days). You can fetch data for specific dates and timezones by selecting the date (beyond 7 days or custom) dropdown, then **Apply** and select the timezone from the dropdown. By default, the system displays UTC +5:30.|
