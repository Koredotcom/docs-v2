...
**Supported Browsers:** Use the latest version of Google Chrome, Microsoft Edge, Apple Safari, or Mozilla Firefox. Microsoft Internet Explorer mode in Edge isn't supported.
...
AI for Service offers trials for Automation AI and Contact Center AI. For Agent AI and Search AI, select **Contact Us** to express interest.
...
You can also sign up using Google or Microsoft 365. The Platform uses SAML-based authentication for sign-up and sign-in.
...
If you're organization uses SSO for authentication, the enterprise identity provider manages the policies. Consult with your IT administrator for password requirements and reset procedures.

For users not using SSO and authenticating directly with the Platform, the following policies apply:
...
* You can't reuse any of your last five passwords.
* After five consecutive failed login attempts, your account is temporarily locked for 30 minutes or until an administrator unlocks the account.
* You can enable Two-Factor Authentication (2FA) for enhanced security.
* When you request a password reset from the sign-in page, you receive a link to create a new password. The link expires after 15 minutes. You can request a new password reset link if it expires.
* To proactively change your password, go to your Profile page, select **Change Password**, and follow the on-screen instructions.

For issues related to password reset or account access, contact [Support](https://support.kore.ai/).
...
- In this documentation, we use `Workspace` and `account` interchangeably.
...
1. Sign in to the Platform Admin Account.
...
To let invited users build and manage apps:

1. Sign in to Platform Admin Account.
2. Select **User Management** > **Users**.
3. Select the target users, choose **Platform** from the **Actions** dropdown, and click **Apply**.
...
When you open an app, the top menu provides access to:
...
1. On the app listing page, select **New App**.
