...
- Each PDF page is converted to an image, preserving visual structure and layout.
...
> **Note:** Requires image-based embedding model selection in Vector Configuration. Answer generation not supported by XO GPT with this strategy. This strategy is supported for a limited set of languages. [Learn More](/ai-for-service/searchai/tips-and-faqs#multilingual-support). 
...
To reorder, drag and drop strategies up or down as needed.
...
> **Note:** Only POST APIs and Sync APIs are currently supported. You can map one or more API response fields to the Search AI schema fields corresponding to the content.
...
- **Enable/Disable:** Use ellipsis menu or status toggle. Disabling a stage does not delete or remove it permanently. When a stage is disabled, the output of the previous stage in the sequence is directly sent as input to the next stage.
...
| **Input**             | Full source documents/pages                                                   | Individual chunks                                             |
...
**Ordering:** Data processes through stages sequentially. The sequence directly affects the cumulative output. Use drag-and-drop to reorder.
...
> Note: By default, new applications use the BGE-M3 model for vector generation.
...
| Field Combination  | The specific set of fields (e.g., Chunk Title, Chunk Text, Record Title) selected to generate a vector                     |
...
The **Kore.ai XO GPT model** is configured for vector generation by default. To verify or change the embedding model:
...
> **Note:** The model and prompt selected on the Gen AI page are applied here by default. Changes made here are also reflected in the **Vector Generation - Text** feature on the Gen AI page.
...
> **Note:** The order of selected fields influences the generated embedding. Semantically rich fields improve embedding quality and search accuracy.
...
> **Note:** Changes to the prompt here are automatically reflected in the **Vector Generation - Image** feature on the Gen AI page.
