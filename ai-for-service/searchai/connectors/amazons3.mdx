...
The Amazon S3 connector supports real-time content updates via AWS S3 Event Notifications through SNS. Once configured, file changes in S3 buckets are reflected in responses almost immediately.
...
- Example, URL format: `https://platform/api/1.1/searchai/webhook/{stream-id}/{connector-id}?secret={secret}`
