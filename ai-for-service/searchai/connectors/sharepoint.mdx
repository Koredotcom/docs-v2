...
At the start of each sync (full or incremental), the connector fetches all sensitivity labels defined in your tenant using the [Microsoft Graph](https://learn.microsoft.com/en-us/graph/api/tenantdatasecurityandgovernance-list-sensitivitylabels?view=graph-rest-1.0&tabs=http) and builds an in-memory label map for the duration of the sync. For each document ingested, the connector calls the [Extract Sensitivity Labels](https://learn.microsoft.com/en-us/graph/api/driveitem-extractsensitivitylabels?view=graph-rest-1.0&tabs=http) to retrieve the label assigned to that document and resolves the human-readable display name from the cached map.
