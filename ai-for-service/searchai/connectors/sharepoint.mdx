...
1. Log in to the [Azure Portal](https://portal.azure.com/#home) and go to **Azure Active Directory**.
   ...
   ![Azure Home](/ai-for-service/searchai/connectors/images/sharepoint/azure-home.png "Admin Center")

2. Go to **App Registrations** under **Applications** and click **New Registration**.

   ![App Registration](/ai-for-service/searchai/connectors/images/sharepoint/new-app-registration.png "Application Registration")
...
3. Enter the application name. Set the account type to **Accounts in any organization directory** (multi-tenant). Set the Redirect URL for your region and click **Register**.
   ...
   ![Account Types](/ai-for-service/searchai/connectors/images/sharepoint/account-types.png "Account Types")

4. Save the **Client ID** and **Tenant ID** from the **Overview** section.

   ![Client Credentials](/ai-for-service/searchai/connectors/images/sharepoint/client-credentials.png "Client Credentials")
...
5. Go to **Certificates and Secrets** and [generate a client secret](https://learn.microsoft.com/en-us/azure/active-directory/develop/quickstart-register-app#add-a-client-secret).
   ...
   ![Client Credentials](/ai-for-service/searchai/connectors/images/sharepoint/client-secret.png "Client Credentials")

6. Enter a description, set the expiration to 24 months, and click **Add**.

   ![Client Credentials](/ai-for-service/searchai/connectors/images/sharepoint/generate-secret.png "Client Credentials")

7. Save the client secret immediately — it cannot be viewed again after leaving the page.

   ![Client Credentials](/ai-for-service/searchai/connectors/images/sharepoint/save-secret.png "Client Credentials")

8. Go to **API Permissions** and click **Add a permission**.

   ![API Permissions](/ai-for-service/searchai/connectors/images/sharepoint/api-permissions.png "API Permissions")
...
9. Add the following permissions under **Microsoft Graph**:
   ...
   **Delegated Permissions** (for Authorization Code Grant Type):
   ...
   ![Request Permissions](/ai-for-service/searchai/connectors/images/sharepoint/request-permissions.png "Request Permissions")

   **Application Permissions** (for Client Credentials Grant Type):
   - `Sites.Selected`
   - `GroupMember.Read.All` — required only if RACL is enabled
   - `Organization.Read.All` — required only if RACL is enabled
   - `SensitivityLabels.Read.All` — required to extract Microsoft Purview sensitivity label metadata during sync

10. Click **Grant Admin Consent** to apply the permissions.

    ![Grant Permissions](/ai-for-service/searchai/connectors/images/sharepoint/grant-permissions.png "Grant Permissions")
...
![Select Sites](/ai-for-service/searchai/connectors/images/sharepoint/sitelist.png "Select Sites")
...
![Select Sites](/ai-for-service/searchai/connectors/images/sharepoint/site-filter.png "Select Sites")
...
The SharePoint connector supports indexing sensitivity label metadata from documents classified using [Microsoft Purview Information Protection](https://learn.microsoft.com/en-us/purview/information-protection). During each sync, sensitivity labels (such as Confidential, Internal Only, or Public) are extracted from SharePoint documents and stored as searchable metadata fields, enabling users to filter search results by document classification.

**How It Works**

At the start of each sync (full or incremental), the connector fetches all sensitivity labels defined in your tenant using the [Microsoft Graph List Sensitivity Labels API](https://learn.microsoft.com/en-us/graph/api/tenantdatasecurityandgovernance-list-sensitivitylabels?view=graph-rest-1.0&tabs=http) and builds an in-memory label map for the duration of the sync. For each document ingested, the connector calls the [Extract Sensitivity Labels API](https://learn.microsoft.com/en-us/graph/api/driveitem-extractsensitivitylabels?view=graph-rest-1.0&tabs=http) to retrieve the label assigned to that document and resolves the human-readable display name from the cached map.

The following metadata fields are stored on each indexed document:

| Field                | Description                                                                 |
|-----------------------|-----------------------------------------------------------------------------|
| `sensitivityLabelId`    | Unique identifier of the sensitivity label                                  |
| `sensitivityLabelName`  | Human-readable display name (for example: "Confidential," "Internal Only") |
| `tenantId`              | Tenant identifier, used to scope labels correctly in multi-tenant environments |

Documents with no sensitivity label assigned are stored with the sensitivity label field unset and appear under an Unlabeled option in the filter.

<Note>If the tenant-level label fetch fails at the start of a sync, the sync is aborted. Document-level label resolution is not possible without the label map.</Note>

**Prerequisites**

To enable sensitivity label extraction, the Azure AD app registration must include the following application permission under Microsoft Graph:
- `SensitivityLabels.Read.All`

Grant admin consent for this permission before running a sync. 

**Filtering by Sensitivity Label**

Once sensitivity label metadata is indexed, users can filter search results by label using the Sensitivity Label filter in the advanced filters panel.
- Operator: Equals
- Selection: Multi-select - select one or more labels to restrict results to documents carrying any of the selected labels.
...
![Office 365 Membership](/ai-for-service/searchai/connectors/images/sharepoint/racl/config-tab.png "Configuration")

Click **Members** to view the list of members and their roles.

![Office 365 Members](/ai-for-service/searchai/connectors/images/sharepoint/racl/members.png "Members")

Go to **Settings > Site Permissions** to view SharePoint Security Groups.

![Site Permissions](/ai-for-service/searchai/connectors/images/sharepoint/racl/site-permissions.png "Site Permissions")
...
![File Permissions](/ai-for-service/searchai/connectors/images/sharepoint/racl/file-permissions.png "File Permissions")

![Domain Access](/ai-for-service/searchai/connectors/images/sharepoint/racl/domain-access.png "Domain Level Access")
