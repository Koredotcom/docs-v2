...
A single Jira connector supports multiple authentication profiles, allowing it to sync content from multiple Jira accounts. Each profile uses its own credentials and domain, and content sync works across all configured profiles.
...
<Note>All timestamps are stored in UTC. The `doc_source_type` field distinguishes content types. Issue comments are concatenated into a single field; individual comment metadata is not indexed.</Note>
...
![Manage Content](/ai-for-service/searchai/connectors/images/jira/manage-content.png "Manage Content")

Select **Ingest filtered content** and click **Edit Configuration** to apply filters.

![Filters](/ai-for-service/searchai/connectors/images/jira/date-filter.png "Filters")
...
![Advanced Filters](/ai-for-service/searchai/connectors/images/jira/advanced-filter.png "Advanced Filters")
