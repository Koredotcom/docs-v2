...
1. Go to the **Authorization** tab and enter the Search AI callback URL for your region, then click **Save changes**.
...
2. Go to the **Permissions** tab and add the following APIs:`User Identity API`, `Confluence API`, and `Jira API`.

    ![App Permissions](/ai-for-service/searchai/connectors/images/confluencecloud/app-permissions.png "App Permissions")

3. **User Identity API** - Click **config**, then **Edit Scopes**, grant the following permissions, and click **Save**:
...
4. **Confluence API** - Go to **Granular Scopes**, select **Edit Scopes**, add the following permissions, and click **Save**:
...
5. **Jira API** - Go to **Classic Scopes**, select **Edit Scopes**, add `read:jira-user`, and click **Save**.

    ![API Scope](/ai-for-service/searchai/connectors/images/confluencecloud/jiraapi-scope.png "API Scope")

6. Go to **Settings** and locate the **Client ID** and **Secret** at the bottom of the page. Save these for use in the next step.

    ![Settings](/ai-for-service/searchai/connectors/images/confluencecloud/settings.png "Settings")

7. Go to the [Confluence home page](https://www.atlassian.com/software/confluence), click your **Profile**, and navigate to the **Products** tab. The host URL is displayed at the bottom of the page.
...
3. Open **Automation**.
4. Click **Create rule**.
5. Select **Incoming webhook** as a trigger (Page published, Page updated, Page deleted).
6. Add a **Send web request** action with ingestion endpoint URL and headers.
...
<Note>This rule sends outbound events from Confluence to Search AI each time the trigger condition is met. Site Admin access is required when creating a global rule that applies across all spaces.</Note>

### Adding the Webhook Client Secret to the Headers

When configuring the webhook URL in Search AI (or any connected application), you must include the Webhook Client Secret in the request headers. Without this header, Confluence won't authenticate incoming webhook calls.

**Add the following HTTP header to your webhook configuration:**

* Header name: (for example, Authorization or as specified in your setup)
* Header value: (your webhook secret)

The webhook secret doesn't expire. If you suspect it has been compromised, click Regenerate in the Atlassian Automation rule builder to immediately invalidate the old token and generate a new one. Update the new secret in all connected applications afterward.
...
2. To selectively ingest content, select **Filtered content** and click **Configure**. Define rules using a parameter, operator, and value.
...
2. When webhooks are enabled, content changes in Confluence Cloud are reflected in search results in near real‑time (typically within seconds).
