...
| Custom filters    | Additional query-style filters can be configured to further narrow ingestion scope based on Wolken-specific fields. Click on 'Add' and add the field name. The specified field must exist in the source system and the filter value must be valid for that field. Standard comparison operators are supported for custom fields. | `query: Case Status`|
