...
To sync Salesforce Knowledge Articles, the following configurations must be in place before running a sync. Without these, Knowledge Articles will not be accessible through the API and the sync may fail or return no articles.
...
<Note>Both settings are required. If either is missing, Knowledge Article API calls will return no results or fail silently during sync.</Note>
