...
3. Qualified chunks from all retrievals are combined based on their scores .
4. From each retrieval, the configured number of **Chunks Sent to LLM** are selected. The combined set is then constrained by the configured token budgets before being sent to the LLM..
5. A single answer is then generated using the selected chunks.
...
<Note> 
- To execute only selected retrievals for a request via the Advanced Search API, use the optional `parallelCalls` field. [Learn More](/ai-for-service/apis/searchai/advance-search).
...
| Relevant Results Preview | Show relevant search result previews below each answer. When disabled, results are hidden from the answer view and can still be accessed from **All Results**. | - | Enabled |
