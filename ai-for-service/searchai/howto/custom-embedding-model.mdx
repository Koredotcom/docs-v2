...
1. Click **+New Model** and select **Custom Integration**
1. In the **Configurations** tab, provide:

  | Field            | Description                                             |
  |------------------|---------------------------------------------------------|
  | Integration Name | Unique identifier for this integration                  |
  | Model Name       | Your model name (for example, `text-embedding-ada-002`) |
...
For improved relevance, fine-tune embedding models with your domain-specific data using the [Fine-Tune Embedding Utility](/ai-for-service/searchai/advance-config-and-dev-tools#model-optimization) from the Search AI Toolkit.
