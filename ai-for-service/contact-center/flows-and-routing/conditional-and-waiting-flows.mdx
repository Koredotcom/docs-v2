...
<Note>The Click-to-Call flow appears in Conditional Flows only after it's enabled in [Theme & Design](/ai-for-service/channels/add-web-mobile-client#ai-agent-theme-design).</Note>
...
Callback Number Confirmation is an optional feature that lets callers to verify or update the phone number they want to receive the callback on before being placed into the callback queue. When enabled, the system launches the Callback Number Confirmation dialog flow after the caller chooses the callback option from the waiting experience. 
...
(Optional) Turn on the Callback Number Confirmation toggle to prompt callers to confirm or update their callback number before they enter the callback queue. 

   ![Callback Option](/ai-for-service/contact-center/flows-and-routing/images/callback-options.png)
