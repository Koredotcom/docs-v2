...
4. Shortcut Command (optional)-A shortcut for this specific response. When configured, agents can invoke it directly using `/groupshortcut/templateshortcut`.
    ...
    <Note>Shortcut names must be unique across the system and can only contain letters and numbers. Special characters are not supported. If the shortcut name is available, the response is saved and a success notification is displayed.</Note>
...
2. Use the keyboard arrow keys to navigate the filtered results and press **Enter**, or click the response with the mouse. The selected response is inserted into the chat editor.

<Note> Responses are not inserted automatically. You must explicitly select a response from the filtered results.</Note>
