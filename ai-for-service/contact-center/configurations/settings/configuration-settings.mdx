...
![Co-Browse](/ai-for-service/contact-center/configurations/settings/images/co-browse-system-settings.png)
...
<Note>All changes are recorded in [Audit logs](/ai-for-service/administration/audit-logs) for traceability.</Note>
...
**Steps to configure Obscure Customer Info in Analytics - Interactions:**
...
2. Enable the options you need:
    ...
    | **Mask IP address in transcripts** | Masks IP addresses in conversation transcripts across all channels. When turned on, IP addresses display as masked values. |

<Note>All settings can be turned on or off independently. Mask IP address in transcripts is turned on by default. All other settings are turned off by default. If no settings are turned on, the platform displays customer information as Email ID or Phone Number.</Note>
...
- Turn on the option to hold customer replies for the last handling agent before rerouting.
...
2. Turn on the Arrival and Closure summaries as required.
...
**Arrival and Closure Summary Prompts**

Arrival Summary and Closure Summary use independent prompt sections. Administrators can customize each prompt separately. Changes to one prompt don't affect the other.

**Context Variables in Prompts**

Prompts support runtime context variables. Before sending the prompt to the LLM, the platform resolves all referenced variables and replaces the placeholders with their current values.

**Supported variable types**

| Variable Type            | Examples                                                                      |
| :----------------------- | :---------------------------------------------------------------------------- |
| System context variables | Session ID, Conversation ID, User ID, Channel.                                |
| Custom context variables | IVR-collected data, flow variables, session attributes, custom context objects. |
...
3. Select the locations to activate translation.

   | Location                        | What gets translated                                                 |
   | :------------------------------- | :-------------------------------------------------------------------- |
   | Dashboard → Interactions         | Content displayed in the Interactions tab.                          |
   | Arrival Summary                  | The AI-generated summary shown when a conversation arrives.           |
   | Internal Chat                    | Messages exchanged in internal chat between agents or supervisors.       |
   | Monitor Tab                      | Content displayed in the supervisor Monitor tab.                          |
   | Disposition Summary              | The wrap-up disposition summary.                                      |
   | Agent AI Widget                  | Content displayed in the Agent AI widget.                                 |
   | Live Agent Console               | Chat messages, Assist, Search, and My Bot tab content in the console. |
   | Automation Transcripts           | Transcripts generated during automated (bot) conversations.           |
   | Agent AI Conversation Logs       | Logged conversation content in Agent AI Logs.                         |

    ![Custom Engine](/ai-for-service/contact-center/configurations/settings/images/translation-configuration-options.png)

    ![Configuration Window](/ai-for-service/contact-center/configurations/settings/images/custom-configuration.png)

    <Note>
   Each location is an independent selection. Enabling one location doesn't automatically enable translation for content in another. For example, selecting **Live Agent Console** translates chat, Assist, Search, and My Bot tab content in the console, but it doesn't enable translation for the **Agent AI Widget** or **Internal Chat**—you must select those locations separately if you need their content translated too.
   </Note>

4. (Optional) Limit Translation Languages to User's Assigned Languages: When enabled, the translation dropdown displays only the languages assigned to the logged-in user, rather than all available languages.

5. (Optional) Conditionally Enable Translation for Unsupported Languages: When enabled, the translation option is displayed or hidden based on whether the user's language is within the agent's configured languages. 

6. Select **Save**.

For more information on translation settings, see [Multilingual Message Localization and Translation](/ai-for-service/contact-center/configurations/settings/multilingual-message-localization-and-translation).
...
2. Turn on the toggle to activate the feature. The system generates intelligent disposition suggestions based on actual conversation content.
