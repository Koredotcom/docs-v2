...
2. Turn on the translation toggle, then select a provider:
    ...
    ![Custom Engine](/ai-for-service/contact-center/configurations/settings/images/translation-configuration.png)
...
* If the user's language is not within the agent's configured languages, the translation option is enabled.
* If the user's language cannot be determined, the translation option is enabled by default.

<Note>When conditional translation is enabled, translation is disabled for both the user and the agent. Messages are not translated, even if the agent writes in a different language. Conditional translation is based on the user's language set at the time of agent transfer. It doesn't respond to language changes or multilingual conversations at runtime. This behavior applies only in the agent console live interaction pane.</Note>
