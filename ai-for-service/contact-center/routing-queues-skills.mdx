...
Priorities can change while a conversation waits in queue. To implement flexible prioritization, use the [Set Queue node](/ai-for-service/flows/node-types/set-queue#set-queue-node) and [Script node](/ai-for-service/automation/dialogs/node-types/script-node#script-node) in the Flow Editor.
