...
- Checks skill eligibility only-proficiency ranking isn't applied.
...
Priorities can change while a conversation waits in queue. To implement flexible prioritization, use the [Set Queue node](/ai-for-service/flows/node-types/set-queue) and [Script node](/ai-for-service/automation/dialogs/node-types/script-node) in the Flow Editor.
...
<Note>If the customer ends the chat before transfer completes, the transfer is dropped and the interaction isn't assigned to any queue or agent. This applies to chat conversations using WebSDK v1.0.</Note>
...
<Note> This is applicable for existing users.</Note>
...
| **Timed Slot Release** | Slot remains occupied until the agent completes disposition or the timer (10 seconds-90 minutes) expires |
...
<Note>Agents added through an agent group aren't displayed in the agents list. Both individual agents and agent groups are listed separately.</Note>
...
<Note>Deleting a queue removes all corresponding routing rules. A queue can't be deleted if an active campaign is associated with it — deactivate all campaigns first.</Note>

---

### Agent Subscription Status

The Edit Queue view includes visibility into each assigned agent's runtime subscription status. This lets supervisors identify agents who have opted out of the queue at runtime and restore their subscription if needed.

For each agent assigned to the queue, the following information is displayed:

| Indicator | Meaning |
|-----------|---------|
| Active (default) | The agent is subscribed to the queue and eligible for routing. |
| Off-Queue | The agent has deselected the queue at runtime. The agent remains assigned in configuration but isn't eligible for routing. |

<Note>Off-Queue agents remain part of the queue's configuration. The Off-Queue indicator reflects the agent's deselection and doesn't change the assignment.</Note>

**Restoring an Agent's Queue Subscription**

Supervisors can restore a deselected agent's queue subscription from the queue view.

Steps:

1. Select the queue and open the Assignments tab.
2. Locate the agent shown as Off-Queue.
3. Select Restore to reinstate the agent's routing eligibility for this queue.

  ![Override Off Queue](/ai-for-service/contact-center/routing/images/override-off-queue.png)

<Note>Restoring a subscription from the queue view overrides the agent's deselection. The agent becomes immediately eligible for routing from this queue.</Note>
...
- Can't access contact center functionality or appear in agent status lists.
...
- Don't appear in active analytics dashboards.

A pop-up message appears when selecting queues, with a disabled tag displayed next to the user.

<Note>Agent permissions can't be disabled if the agent is the only member of the queue or is engaged in an active conversation.</Note>
...
<Note>Deleted skills can't be restored.</Note>
...
A Skill Group organizes similar skills. For example, Sales is a skill group containing skills like upselling, cross-selling, and presales.
...
<Note>Deleted skill groups can't be restored. You must delete all associated skills before deleting the group.</Note>
...
Skills can be configured to expire automatically. When a skill expires, it's removed from the agent and the agent is notified. This is useful for time-limited certifications or role-specific training.
