...
![Status Timer](/ai-for-service/contact-center/console/images/status-timer.png)

---

### Dark Mode

Dark mode switches the Agent Console to a dark color scheme across all screens, backgrounds, buttons, icons, and interactive components. The console saves the preference and applies it across sessions.

To turn on dark mode:

1. Select the profile icon at the upper-right corner of the Agent Console.
2. Turn on the **Dark Mode** toggle.

   ![Dark Mode](/ai-for-service/contact-center/console/images/profile-dark-mode.png)

<Tip>Dark colors configured for Skills or Disposition codes - for example, black - may not be visible in dark mode. Use lighter shades to keep codes readable in both light and dark modes.</Tip>
...
Each conversation in the conversation tray displays one of five statuses. The table below describes the meaning of each. 

| Status                  | Meaning                                                                          |
| :---------------------- | :------------------------------------------------------------------------------- |
| Overdue                 | SLA threshold is breached; immediate action is required.                   |
| Ongoing                 | The agent is actively engaged in a live conversation.                            |
| Snoozed                 | Conversation is intentionally deferred; a follow-up is scheduled.          |
| Idle                    | Conversation is open but awaiting a response.                                    |
| After Call Work (ACW)   | The agent is completing post-call tasks, such as notes and dispositions.         |

The Conversation Tray displays the following details to help agents distinguish and manage conversations: 

* Status icons — a unique icon per conversation state.
* Tinted backgrounds — a soft color applied to all non-ongoing conversations.
* Notifications — a counter for unread customer or system activity. Counts greater than 9 display as 9+.
* Dynamic elapsed timers — shorthand time display with hover-expanded detail (Overdue status only).

   ![Conversation Tray Details](/ai-for-service/contact-center/console/images/conversation-statuses.png)
...
Each conversation in the Conversation Tray and Live Interaction Pane displays a timer that updates in real time. The administrator configures the timer metric.

| Metric                     | What It Measures                                                                           |
| :------------------------- | :----------------------------------------------------------------------------------------- |
| Total Chat Handling Time   | Total duration from when the interaction entered the contact center.                       |
| Agent Handling Time        | Time the Agent has spent handling the conversation since accepting it.                       |
| Agent Last Message Time    | Elapsed time since the agent sent a message in this conversation.                           |
| Interaction Handling Time  | Cumulative agent handling time across all agents, excluding queue wait periods.            |
...
2. On the Dialer tab, select the country, enter the phone number, and select the transcription language.
   ...
   ![Outbound Call Dialer](/ai-for-service/contact-center/console/images/outbound-call-transcription-language.png)
...
<Note>When Auto Answer mode is enabled, the Transfer and End buttons don't appear until you send your first message.</Note>

### Understanding Queue Position

Each incoming chat interaction displays a Queue Position number that indicates where it stands in line relative to available agent capacity. This number helps you understand the expected wait before a conversation is assigned.

What Queue Position Means

Queue position 1 doesn't mean a conversation is already being handled. It means that conversation is next in line — either waiting for an agent to accept it, or first in the queue to be assigned when a slot opens.

| Queue Position | What it Means |
|:---|:---|
| 1 | The conversation is immediately next — either held with an agent pending acceptance, or first in queue for the next available slot. |
| 2, 3, ... | The conversation is waiting behind others in First In First Out (FIFO) order. |

How Queue Positions Are Calculated

When agents are available, the system fills vacant slots with the first conversations in queue. Those conversations move to an Agent Queue and show position 1 — they are assigned to an agent and awaiting acceptance.

Conversations that can't be immediately assigned wait in their respective queues (such as Default or Gold). The first conversation in each queue also shows position 1, because it is next in line the moment a slot opens. Conversations behind it are assigned positions 2, 3, and so on, in FIFO order.

Example:

Consider 9 chats across queues with 3 available agents (total capacity: 5 slots):

| Conversations | Queue | Position | Reason |
|:---|:---|:---|:---|
| 5 chats | Agent Queue | 1 | Immediately assigned to fill 5 vacant agent slots; awaiting acceptance. |
| 1st chat in Default Queue | Default | 1 | First in line; will be assigned when the next slot opens. |
| 1st chat in Gold Queue | Gold | 1 | First in line; will be assigned when the next slot opens. |
| Remaining chats | Default / Gold | 2, 3 | Waiting behind the first chat in their queue, in FIFO order. |

<Note> Multiple conversations can show position 1 at the same time — each across a different queue or agent slot. Position 1 is relative to each queue's or agent's own capacity, not a single global count.</Note>
...
<Note>Both components must meet the minimum version requirement at the same time. Upgrading only one application doesn't enable the feature.</Note>

Steps to start video call recording:

1. Select **Start Video Call Recording** to record a video call. 

   ![Start Video Call recording](/ai-for-service/contact-center/console/images/start-video-call-recording.png)
   ...
   ![Accept/Reject Video Call recording](/ai-for-service/contact-center/console/images/accept-reject-recording.png)
...
2. (Optional) To pause recording, turn off the Recording On toggle. Turn it on to resume recording. Use this when collecting PII or PCI data.
   ...
   ![Pause Video Call Recording](/ai-for-service/contact-center/console/images/recording-on-toggle.png)
...
<Note>This feature is in Beta. Contact Support to enable it. Safari isn't recommended for video calls.</Note>
...
* Install the [Windows desktop recording application](https://bitbucket.org/koreteam1/kore-desktop-installer/src/main/Kore%20Desktop%20Recorder/Kore%20Desktop%20Recorder%20Setup%201.0.0.exe).
* Ensure the application runs before accessing the Console.

When the agent logs in, the system checks the desktop application through a localhost WebSocket and displays a message with a download option if the application isn't installed.

![Download Application](/ai-for-service/contact-center/console/images/download-application.png)

**Recording Behavior**:

* Recording starts automatically when the agent accepts a voice call and continues throughout the interaction, including when the call is on hold and during After Call Work (ACW).
* Recording stops after the agent submits the ACW disposition.
* Agents can manually pause and resume recording by selecting the Screen Recording icon, for example, while collecting sensitive information. The system excludes any paused duration from the recording.
* Recording captures the entire desktop across all monitors.
* Recording includes both the agent's and the caller's audio.
* There's no maximum duration limit for agent screen recordings.
* Recordings are stored in WebM format.

   ![Screen Recording Option](/ai-for-service/contact-center/console/images/screen-recording-option.png)

### Agent AI Widget During Video Calls

When a chat conversation escalates to a video call within the same interaction session, the Agent AI widget remains active and continues to assist with the video call. 

**What Stays Active**

Agent AI retains the full session context established during the chat phase, including:

* Prior chat transcript
* Customer metadata and case history
...
When a network interruption occurs during a voicemail callback (outbound call), the re-routing behavior follows the same logic as for outbound call interruptions. 
...
<Note>The consulting agent must be Available with zero load. A consult can't be started during an ongoing conference, and a conference can't be started during an ongoing consult.</Note>
...
<Note>Agents with zero Chat/Email slots aren't listed in agent transfers.</Note>
...
### Re-routing Voice Calls due to Network Interruption

When an agent's network is interrupted, the platform plays a configured message and re-routes the call into the queue at the highest priority for a new agent to handle.
Re-routing applies to inbound calls, manual outbound calls, campaign outbound calls, and voicemail callbacks. 

<Note>A conversation can be re-routed a maximum of two times. The maximum queue wait time configuration applies to re-routed calls.</Note>

Session and routing behavior:

* The new agent's connection works like a call transfer.
* The session closes when the customer disconnects or the call ends naturally.
* If the new agent declines, the call re-enters the queue at the highest priority.
* If the customer selects Voicemail, Callback, or Consult Call while waiting, that takes precedence and the new agent doesn't receive the re-routing notification.

When a new agent is added, a notification displays below the queue; in the original agent experience, when the interruption flow triggers, the active call is removed from their screen and the Console returns to its default state.

CSAT attribution: CSAT is attributed to the new agent, consistent with agent transfer scenarios.
...
1. During an active chat, select **Transfer**.
   ...
   ![Transfer](/ai-for-service/contact-center/console/images/transfer-chat-option.png)
...
<Note>The customer chat remains active throughout the consult. The customer isn't placed on hold and isn't notified of the internal session.</Note>
...
<Note>The target agent receives a notification when they're added to the customer conversation. Internal consult messages aren't exposed to the customer.</Note>
...
<Note>Response template groups and templates appear in the order in which they are configured. To change the display order, see [Reorder Response Template Groups](/ai-for-service/contact-center/configurations/operational-configurations#reorder-response-template-groups)</Note>
...
Each note displays the date and time it was last modified. Creating, editing, or deleting a conversation note generates a [Change Log](/ai-for-service/app-settings/change-logs) entry.

| Action                          | Timestamp Shown                        |
| :------------------------------ | :------------------------------------- |
| Note created, not yet edited    | Creation date and time.                |
| Note edited                     | Date and time of the most recent edit. |

When an agent edits a note, the displayed timestamp updates immediately to reflect when the change was made.

Example:

| State          | Display                   |
| :------------- | :------------------------ |
| After creating | 15 Jul 2026 \| 10:30 AM   |
| After editing  | 16 Jul 2026 \| 02:15 PM   |

### Notes Translation

When translation is active, the system translates conversation notes into the selected language.

![Translated Notes](/ai-for-service/contact-center/console/images/translated-note.png)
...
Workbin lets you park an email conversation temporarily, freeing your slot while waiting for a customer reply. Administrators must enable this feature.

**To park a conversation:**

Select **Workbin** in the email interaction header. The conversation leaves your active tray and your slot is released immediately.

![Move to Workbin](/ai-for-service/contact-center/console/images/move-to-workbin.png)
...
- If you are unavailable, it waits for the configured duration, then routes to the queue.
  ...
  ![Workbin Conversation](/ai-for-service/contact-center/console/images/workbin-conversation.png)

- If no wait behavior is configured, the system applies standard queue routing immediately.

**Supervisor options while a conversation is in Workbin:**

- Assign the conversation directly to an agent.
- Transfer the conversation to a queue (doesn't wait for you).

### Personal Workbin View

The Personal Workbin View provides agents with a dedicated space to track and manage conversations that are moved to the Workbin.

The Personal Workbin View icon is visible in the Console only after an administrator enables the feature. [Learn more](/ai-for-service/contact-center/configurations/settings/configuration-settings#configure-email-work-bin).

**Accessing the Personal Workbin**

Select the **Personal Workbin** icon in the upper right corner.

![Workbin Icon](/ai-for-service/contact-center/console/images/console-workbin.png)

The view lists all open conversations that are moved to the Workbin and aren't reassigned by a supervisor.

![Personal Workbin Emails](/ai-for-service/contact-center/console/images/workbin-duration.png)

<Note>If a supervisor reassigns a conversation to another agent, it's removed from your Personal Workbin.</Note>

**What Appears in the Personal Workbin**

The view displays conversations that meet all of the following conditions:

* You moved the conversation to the Workbin.
* The conversation is still open.
* A supervisor hasn't transferred the conversation to another agent.

The columns display Customer (customer name or identifier), Subject (email subject or conversation topic), Date (the date the conversation moved to the Workbin), and Workbin Duration (elapsed time since the last Workbin placement, which resets when the conversation is retrieved and re-parked). Workbin Duration stops counting when the conversation exits the Workbin.

**Searching Within the Personal Workbin**
Use the search bar within the Personal Workbin to find conversations.

**Workbin Duration**
The Workbin Duration column displays the elapsed time since the conversation was last parked in the Workbin. The timer updates every minute.

Select the Workbin Duration column header to sort conversations by how long they have been in the Workbin. Sorting in descending order shows the oldest conversations first, helping to prioritize follow-ups and avoid Turn Around Time (TAT) breaches. 

**Duration format**
Elapsed time is displayed as `DDd:HHh:MMm`.

| Scenario                                            | Timer Behavior                                                  |
| :-------------------------------------------------- | :-------------------------------------------------------------- |
| Conversation moved to the Workbin.                 | The timer starts from the moment the conversation is parked.    |
| Conversation retrieved from the Workbin.           | Timer stops.                                                    |
| Conversation moved back to the Workbin after retrieval. | The timer resets and restarts from the new placement time. |

The duration always reflects the most recent Workbin placement, not the cumulative total time across multiple placements.

**Viewing Conversation Details**
Select any conversation in the list to open its detailed view. From there, you can review the full conversation context before deciding to resume handling.

**Assigning a Workbin Conversation**

To resume a Workbin conversation:

1. Select the conversation in the Personal Workbin to open the detailed view.
2. Select **Assign to Me**.

   ![Assign to me](/ai-for-service/contact-center/console/images/workbin-detailed.png)

> <Note>You can assign a conversation to yourself even if you have reached your email slot limit.</Note>

**Personal Workbin Behavior**

| Behavior                  | Detail                                                                              |
| :------------------------ | :---------------------------------------------------------------------------------- |
| Slot consumption          | Conversations in the Personal Workbin don't consume an interaction slot.           |
| Handling time             | Handling time doesn't accumulate while a conversation is in the Workbin.          |
| Interaction status        | Status remains unchanged until the conversation is reassigned or resumed.           |
| Supervisor reassignment   | Conversations reassigned by a supervisor are removed from your Personal Workbin.   |
| Search                    | Available within the Personal Workbin; consistent with the main tray.              |
| Slot limit on self-assign | The slot limit doesn't block self-assignment from the Personal Workbin.           |
...
- Doesn't apply to messages in the user's console.
...
<Note>If the customer ends the chat before the transfer completes, the transfer is dropped and the conversation isn't assigned to any queue or agent.</Note>
...
![ACW Timer](/ai-for-service/contact-center/console/images/acw-timer.png)
...
<Note>If you don't complete the disposition and notes during the ACW period, the system automatically closes the conversation with a system-generated disposition.</Note>
...
The **Copy** option lets you copy the full disposition as plain text.

![Copy Disposition](/ai-for-service/contact-center/console/images/copy-disposition.png)
...
![Capture Disposition During Transfer](/ai-for-service/contact-center/console/images/capture-disposition-transfer.png)
...
2. When the transfer panel opens, select one or more dispositions. The list is populated based on your account's configured dispositions.
   ...
   ![Disposition During Transfer](/ai-for-service/contact-center/console/images/select-disposition.png)

3. Complete the transfer.

   * If a disposition is selected, it's tagged to the interaction at the point of transfer and stored separately from any closure disposition captured later.
   * If the dropdown is skipped, the transfer proceeds without a disposition.

   <Note> Disposition selection is optional and doesn't affect the transfer. Disposition data captured at transfer is available in Analytics and reporting.</Note>
...
<Note>CSAT doesn't apply when the customer interacts with external agents or when an external consult occurs.</Note>
...
The History widget displays the customer's previous interactions across all supported channels — Chat, Email, and Voice — in a unified timeline. Use it to review prior context, previous resolutions, and the agents or bots involved before continuing the conversation.
Conversations are sorted by Acceptance Time in ascending order, showing the earliest interaction first and the most recent last.

**Conversation cards**

Each historical interaction appears as a conversation card. Select a card to open the full transcript.

| Area     | What It Displays                                                                                                                                            |
| :------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Header   | Acceptance time and total conversation duration.                                                                                                            |
| Body     | Conversation subject or transfer-triggering intent. For voice or email interactions without a detected intent, the conversation subject or title displays.   |
| Metadata | Channel icon (Chat, Email, or Voice), bot name if automation handled part of the interaction, and avatars for all agents who participated.                   |
| Footer   | Conversation status or disposition codes.                                                                                                                   |

**Multiple dispositions**

When a conversation has more than one disposition code, the card shows the first two followed by a +N indicator for the remaining count. Select +N to expand and view all disposition codes.

Example:

![History Widget](/ai-for-service/contact-center/console/images/history-widget.png)

Selecting +2 displays the two additional disposition codes.

Select any conversation card to view the full transcript.

![Transcript details](/ai-for-service/contact-center/console/images/transcript-details.png)
...
Agents can manage how the Agent Console alerts you to incoming interactions and activity. Notification settings cover sound alerts, browser push notifications, and repeat behavior.

1. Select the **Bell** icon in the upper right corner of the console.

   ![Notifications](/ai-for-service/contact-center/console/images/notifications.png)

2. Select **Settings** icon to open the notification preferences.

   ![Notification Preferences](/ai-for-service/contact-center/console/images/notifications-panel.png)
...
2. Set the repeat interval (default: 5 seconds) and click **Save**.
...
![Enabled Notification](/ai-for-service/contact-center/console/images/sound-through-headphones.png)

Disabled (default):

![Disabled Notification](/ai-for-service/contact-center/console/images/sound-through-speaker.png)

### Queue-specific Sound Profiles

Queue-Specific Sound Profiles let you assign a distinct sound notification to one or more queues. When an interaction arrives from a configured queue, it plays the assigned sound instead of the default alert, letting you instantly distinguish high-priority or VIP queues from standard queues by audio cue. This setting applies to voice, chat, and email interactions.

**How It Works**

* Queues with a sound profile assigned play that sound when an interaction arrives or is transferred to them.
* Queues without a profile assigned continue to use the default notification sound.
* If an interaction is transferred and the target queue has a configured sound profile, that sound plays — regardless of whether the transfer occurs before or after you accept the interaction.

**Configuring a Queue Sound Profile**

To assign a sound to a queue:

1. Select **+ Add Queue-Specific Incoming Notification**.
2. Search for and select one or more queues. You can select a single queue or multiple queues in the same profile.

   ![Queue Selection](/ai-for-service/contact-center/console/images/notification-queue-selection.png)

3. Select the notification sound to assign.
4. Select the **Play** icon to preview the selected sound before saving.
5. Select **Save**.

The profile is active immediately. Incoming interactions from the configured queues play the assigned sound.

**To assign a different sound to additional queues**:

Repeat the steps above. You can create as many profiles as needed, with no limit on the number of queue mappings. Queues that no longer have a profile return to the default notification sound.

**Sound Profile Behavior Reference**

| Scenario                                                   | Sound Played                              |
| :--------------------------------------------------------- | :---------------------------------------- |
| Interaction arrives from a queue with a profile assigned   | Assigned queue sound                      |
| Interaction arrives from a queue with no profile assigned  | Default notification sound                |
| Interaction transferred to a queue with a profile assigned | Assigned queue sound (before or after acceptance) |
| Interaction transferred to a queue with no profile assigned | Default notification sound               |
| Profile deleted from the queue                             | Default notification sound resumes        |

<Note>Web and repeat notification settings apply to queue-specific sounds in the same way they apply to the default sound. No separate configuration is required.</Note>
...
![Agent Profile](/ai-for-service/contact-center/console/images/agent-profile-queue-skill.png)
...
The Reset to Default action restores your original skill and queue assignments as configured by your supervisor. It's available in the Queues and Skills section and resets all de-selections.
...
<Note>Language selection doesn't apply to the Platform.</Note>
