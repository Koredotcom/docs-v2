...
* Pauses when the call is on hold and resumes when the call is active again.
...
When an agent's network is interrupted: a configured message plays → if end call is configured, the call ends; otherwise → an AI context summary is generated from the transcript and attached to the interaction → the call is re-injected into the queue at highest priority → a new agent receives and accepts it. 

<Note>Each conversation can be rerouted a maximum of two times. The maximum queue wait time configuration also applies to rerouted calls.</Note>

Session Continuity

* The session continues with the new agent as a fresh connection (similar to a call transfer).
* The session closes only when the customer disconnects or the call ends naturally.
* The new agent's accept or decline behavior follows the same rules as normal inbound calls; if declining is enabled, declined rerouted calls re-enter the queue at the highest priority and are routed to a different agent. 
...
### Personal Work Bin View

The Personal Work Bin View provides agents with a dedicated space to track and manage conversations that are moved to the Work Bin.

The Personal Work Bin View icon is visible in the Console only after an administrator enables the feature. [Learn more](/ai-for-service/contact-center/configurations/settings/configuration-settings#configure-email-work-bin).

**Accessing the Personal Work Bin**

Select the **Personal Work Bin** icon in the upper right corner.

![Work Bin Icon](/ai-for-service/contact-center/console/images/console-workbin.png)

The view lists all open conversations that are moved to the Work Bin and aren't reassigned by a supervisor.

![Personal Work Bin Emails](/ai-for-service/contact-center/console/images/work-bin-details.png)

<Note>If a supervisor reassigns a conversation to another agent, it's removed from your Personal Work Bin.</Note>

**What Appears in the Personal Work Bin**

The view displays conversations that meet all of the following conditions:

* You moved the conversation to the Work Bin.
* The conversation is still open.
* A supervisor hasn't transferred the conversation to another agent.

**Searching Within the Personal Work Bin**
Use the search bar within the Personal Work Bin to find conversations.
...
**Assigning a Work Bin Conversation**

To resume a Work Bin conversation:

1. Select the conversation in the Personal Work Bin to open the detailed view.
2. Select **Assign to Me**.

   ![Assign to me](/ai-for-service/contact-center/console/images/work-bin-email-details.png)

> <Note>You can assign a conversation to yourself even if you have reached your email slot limit.</Note>

**Personal Work Bin Behavior**

| Behavior                  | Detail                                                                              |
| :------------------------ | :---------------------------------------------------------------------------------- |
| Slot consumption          | Conversations in the Personal Work Bin don't consume an interaction slot.           |
| Handling time             | Handling time doesn't accumulate while a conversation is in the Work Bin.          |
| Interaction status        | Status remains unchanged until the conversation is reassigned or resumed.           |
| Supervisor reassignment   | Conversations reassigned by a supervisor are removed from your Personal Work Bin.   |
| Search                    | Available within the Personal Work Bin; consistent with the main tray.              |
| Slot limit on self-assign | The slot limit doesn't block self-assignment from the Personal Work Bin.           |
