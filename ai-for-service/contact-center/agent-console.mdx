...
2. Turn on the **Dark Mode** toggle.
   ...
   The console switches to the dark color scheme.

<Note>Dark colors configured for Skills or Disposition codes - for example, black - may not be visible in dark mode. Use lighter shades to keep codes readable in both light and dark modes.</Note>
...
* Recordings are stored in WebM (.webm) format.
...
Call notification for the new agent:

The following call notification displays below the Queue:

> Transferred from `Agent Name` due to a network issue.

Original agent experience: When the interruption flow triggers, the active call is removed from the agent's screen and the Console returns to its default state.
...
Columns are displayed in the following order.

| Column           | Description                                                                                                                                              |
| :--------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Customer         | Customer name or identifier.                                                                                                                             |
| Subject          | Email subject or conversation topic.                                                                                                                     |
| Date             | The date when the conversation was moved to the Workbin.                                                                                                 |
| Workbin Duration | Displays the elapsed time since the last Workbin placement. Resets when the conversation is retrieved and re-parked. Stops when the conversation exits the Workbin. |
