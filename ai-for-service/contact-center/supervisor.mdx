...
3. Select an agent name to view their conversation details (Chat, Email, and Call).
   
   ![Queue Details](/ai-for-service/contact-center/console/images/queue-performance-metrics.png)

For Chat, Messaging, and Voice: the assigned agent's name appears immediately upon assignment, even before they respond. For Work Bin conversations, the Queues view displays the name of the last agent who handled the interaction before it was moved to Work Bin.
...
<Note>Supervisors can switch between conversations when the transcript slider is open. Selecting a different conversation automatically refreshes the slider content, without requiring the slider to be closed and reopened.</Note>

<Warning>Metrics in this view count events as they occur. A single conversation may be counted more than once if it triggers the same event multiple times.</Warning>

#### Filters

**Create a filter:**

1. Select **Filter** and then select **New Filter**.

2. Select **Queues**, **Channels**, **Type**, **State**, and **Languages**. You can also filter by agent groups or search for specific agents.
   ![Filter Options](/ai-for-service/contact-center/console/images/filter-options-queues.png)

3. Select **Save & Apply**, enter a name (up to 20 characters), and confirm.
...
- Select an agent to view their ongoing conversation and engagement metrics.
  ![Engagement Metrics](/ai-for-service/contact-center/console/images/agent-performance-metrics.png)
...
- Supervisors can switch between conversations when the transcript slider is open. Selecting a different conversation automatically refreshes the slider content, without requiring the slider to be closed and reopened.
</Note>
...
<Note>In the Queues and Agents tabs, historical metrics filtered by a specific duration don't auto-refresh. Data updates only when the page is hard reloaded or the duration filter is changed.</Note>
...
<Note>
- Agents aren't listed in Agent Transfers when Chat and Email channel slots are set to zero.
- For Work Bin conversations, the Interactions view displays the name of the last agent who handled the interaction before it was moved to Work Bin. This displays under the Last Handling Agent in the Participant column and is visible for the conversations in the workbin.
</Note>
...
**Participant filter behavior**

| Filter Includes                              | Participant Filter Matches Against                                                                 |
| :------------------------------------------- | :------------------------------------------------------------------------------------------------- |
| Active interaction states (for example, Waiting, Engaged). | Current active participant.                                                          |
| Workbin                                     | Last handling agent — the agent who most recently handled the interaction before it was moved to Workbin. |
| Both active states and Work Bin.             | Current active participant for active states; last handling agent for Workbin.                     |

Selecting one or more participants displays interactions for each selected state.

Without bot-led interactions:

The following filter options are displayed when bot-led interactions are turned off or not available in the account.

![Filter Options](/ai-for-service/contact-center/console/images/interaction-filter.png)

With bot-led interactions enabled:

The following filter options are displayed when bot-led interactions are turned on in the account.

![Participant Filter](/ai-for-service/contact-center/console/images/participant-filter.png)

<Note>Bot-led interactions don't appear by default. To view them, select **Automation** or **Automation/Agents** from the Participant filter. The default is **Agent** to prevent information overload.</Note>

Default quick filters:

Following quick filters are available by default:
...
![Channel Selection](/ai-for-service/contact-center/console/images/channel-selection.png)
...
## Sort (Queues, Agents, and Interactions) and Transcript Slider

Sorting Conversations by Engaged Time

Selecting the Engaged column header sorts conversations by how long each conversation was engaged with an agent, in ascending or descending order.

When sorted in descending order, the sort order remains stable as new conversations arrive. Incoming conversations are placed in the correct position based on their engaged time and don't move to the top of the list. This prevents list reordering or visual jitter during active queue monitoring.

Sort behavior:

| Sort Applied          | New Conversation Arrives | Behavior                                          |
| :-------------------- | :----------------------- | :------------------------------------------------ |
| Engaged (descending)  | Yes                      | Placed based on engaged time.                     |
| Engaged (ascending)   | Yes                      | Placed based on engaged time.                     |
| No sort applied       | Yes                      | Displays at the top of the list (default behavior). |

The sort order is re-evaluated consistently for all list updates, including state changes to existing conversations.

**Switching Conversations When the Transcript Slider Is Open**

When the transcript slider is open for a conversation, supervisors can select a different conversation without first closing the slider. The slider refreshes automatically and displays the transcript of the newly selected conversation.
This lets you move between conversations during live monitoring without interruption.

Transcript slider switching behavior:

| Action                                      | Result                                                          |
| :------------------------------------------ | :-------------------------------------------------------------- |
| Select a conversation (slider closed)       | The slider opens and displays the conversation's transcript.    |
| Select a different conversation (slider open) | The slider refreshes with the new conversation's transcript.  |
| Close the slider, then select a conversation | The slider opens with the selected conversation's transcript.  |

<Note>The transcript slider refresh is seamless. No reload or manual close is required when switching between conversations.</Note>

---

## Skill and Queue Subscription Visibility

The Monitor tab displays agent-level skill and queue de-selections. Supervisors can identify agents who have modified their skills or queues and take corrective action without leaving the Monitor view.

Agents View

The Agents view displays each agent's current routing eligibility, including any skill or queue de-selections.
...
* See which skills and queues the agent has deselected.
   ...
   ![Agents Queue Override](/ai-for-service/contact-center/routing/images/agents-queue-override.png)
...
* Identify all agents in Off-Queue status.
   ...
   ![Queue Status](/ai-for-service/contact-center/routing/images/queue-status.png)

* Select Restore for any Off-Queue agent to reinstate their routing eligibility for that queue.

   ![Queue Status detailed](/ai-for-service/contact-center/routing/images/queue-status-detailed.png)
---
...
![Blending Indicators](/ai-for-service/contact-center/agent-and-supervisors/agent-management/images/blending-queues.png)
...
<Note>Transcripts aren't real-time. Select the **Refresh** icon to update them.</Note>
...
### Queue Transfer Restriction Override

By default, queue-level transfer restrictions apply when you reassign a conversation from the Monitor tab. The destination queue list shows only the queues permitted by the source queue's transfer rules.
Users with the [Override Queue Transfer Restrictions in Monitor](/ai-for-service/user-management/users-and-role#agent-and-supervisor-experience-monitor-console) permission can select any eligible destination queue when reassigning from the Monitor tab, regardless of the source queue's transfer rules.

Permission behavior:

| Context                               | Queue Transfer Restrictions Apply      |
| :------------------------------------ | :------------------------------------- |
| Agent Console - all users.            | Yes                                    |
| Monitor tab - user without permission. | Yes                                   |
| Monitor tab - user with permission.   | No — all eligible queues available.    |

<Note>This permission doesn't affect Agent Console transfers. Queue transfer restrictions are enforced in the Agent Console for all users.</Note>
...
<Note>Conversations that honor the agent wait time aren't available for bulk transfer.</Note>
...
- Force-closing a conversation (individually or in bulk) automatically attaches a system-generated disposition. If an agent had already selected a disposition, it's overwritten by the system-generated one.
</Note>
...
<Note>
- Conversations can't be transferred when the agent is on a live call.
- Conversations can't be transferred to offline agents.
...
- Conversations in TERMINATED, TRANSIENT_DROPOFF, and TRANSIENT_CLOSED states can't be transferred. they're excluded when you use **Select All**.
</Note>
...
1. Select **Add Group**.
2. In the New Group window, enter the Group Name, Description, and Agent Status (optional). Statuses selected here appear to agents in this group alongside system-defined ones.
3. Select **Save**.
...
<Note>You can't rename the pre-configured groups: Unassigned and Default Group.</Note>
...
Agent statuses show what agents are doing-Available, Offline, Busy, Away, and any custom statuses you create.
...
1. Select **+ New Status**.
2. Enter the Status Name, Status Type, Description, and Assign to Agent Groups (Required for custom statuses; makes the status available to agents in those groups).
...
3. Customize the Status Name, Description, and Message to Agent.
4. Select **Update**.
...
| Conversation Timer Metric | Controls which timing metric the conversation timer displays for all agents in the Agent Console. Defaults to Total Chat Handling Time. |
| Capture Disposition During Transfer | Captures the disposition of conversations during agent transfer. |
...
| `On Agent Network Interruption` | When an Agent network interruption event is received | Voice | User | Agent names |
...
By default, all Conversation Status Control messages for the Email channel are enabled. Administrators can individually enable or disable specific messages (due reminder for agent inactivity, customer idle, due reminder for auto expiry, and on close).
...
![Blended Agents-Queue Level](/ai-for-service/contact-center/agent-and-supervisors/agent-management/images/blended-agents-queues.png)
...
![Additional Routing Configuration](/ai-for-service/contact-center/agent-and-supervisors/agent-management/images/additional-routing-configuration.png)
...
<Note>Set the total conversation limit per user from the User section only. This setting isn't available in the user profile under the Monitor tab.</Note>
...
See [User Management](/ai-for-service/administration/user-management#managing-users) for details.

---

### Conversation Timer Metric

The conversation timer displays in the Agent Console for each active interaction. Administrators can configure which timing metric the timer shows. The selected metric applies to all agents.

Only one metric can be active at a time.

To configure the timer metric:

1. Go to **Contact Center AI** > **Agent & Supervisors** > **Agent Management** > **Agent Settings** > **Conversation Timer Metric**.
2. Select a metric from the available options.
3. Select **Save**.
   
   ![Chat Conversation Timer Display](/ai-for-service/contact-center/agent-and-supervisors/agent-management/images/timer-display-logic.png)

| Metric                              | What It Measures                                                                                                          | Starts When                              | Resets When                                                              |
| :---------------------------------- | :------------------------------------------------------------------------------------------------------------------------ | :--------------------------------------- | :----------------------------------------------------------------------- |
| Total Chat Handling Time (default)  | Total duration from when the interaction entered the contact center, including queue wait time, agent handling, and transfers. | Interaction enters the contact center.   | Interaction closes.                                                      |
| Agent Handling Time                 | Time the assigned agent has spent handling the conversation.                                                             | The agent accepts the conversation.      | Conversation transfers to another agent.                                 |
| Agent Last Message Time             | Elapsed time since the agent last sent a message.                                                                 | The agent sends a message.               | The agent sends another message.                                         |
| Interaction Handling Time           | Cumulative time agents have actively handled the interaction across all transfers.                                         | Each agent accepts.                      | Pauses when the interaction is unassigned or waiting in queue.           |

---

#### Capture Disposition During Transfer

If enabled, agents can add dispositions while transferring conversations across voice and digital channels. [Learn more](/ai-for-service/contact-center/supervisor#capture-disposition-during-transfer).
...
- Deleted sets can't be restored.
</Note>
...
<Note>Deleted disposition codes can't be restored and no longer appear to agents in the Agent Console.</Note>
