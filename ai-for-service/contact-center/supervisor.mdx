...
<Note>Agents aren't listed in Agent Transfers when Chat and Email channel slots are set to zero.</Note>
...
| `On Customer Interruption after Agent Transfer` <br /> Note: The Network Disconnection Notification Delay setting defines how long the system waits before notifying agents of a customer disconnect. If the customer reconnects within the configured delay, the notification is suppressed. Supported range is 3 seconds to 1 minute (default: 3 seconds).| Customer disconnects; conversation waits with agent | Live Chat | Agent, User | Agent names |
