---
title: Migrate SearchAssist to Search AI
noindex: true
hidden: true
mode: center
---
...
Learn more about [Search AI](/ai-for-service/searchai).

<Note>After September 30, 2026, SearchAssist is not available.</Note>
...
- The role-based access control feature offered for content ingested via connectors is enhanced to automatically fetch and store the users associated with a group or permission entity. This removes the need to associate users with permission entities via APIs manually. [Learn More](/ai-for-service/searchai/content-sources#role-based-access-control-racl). 

![Search AI architecture](/ai-for-service/getting-started/images/searchai/migration/architecture.png "Search AI architecture and components")
...
- Scheduled recrawls are migrated and run automatically. However, Search AI doesn't support the **custom schedule** and **every weekday** options, these configurations aren't migrated.
...
- If filters are configured in SearchAssist, they're automatically applied in Search AI.
- Any existing permissions data (`sys_racl`) in the content is automatically applied upon training. Note that the RACL feature in Search AI is more advanced than in SearchAssist. [Learn more about RACL in Search AI](/ai-for-service/searchai/content-sources#racl-in-search-ai).
...
- Structured data can be manually imported into Search AI using the [JSON connector](/ai-for-service/searchai/connectors/json). This connector enables ingestion and indexing of structured data in JSON format. Use the sample file to prepare the structured data in the expected format and then upload the file.
- Alternatively, structured data can also be imported via APIs. Refer to [this for details](/ai-for-service/apis/searchai/ingest-data).
...
- You can manage FAQs using the [Knowledge Module](/ai-for-service/automation/knowledge-ai/manage-faqs) under Automation AI. 
...
- You can also manually add FAQs or [extract them from unstructured web pages or files ](/ai-for-service/automation/knowledge-ai/knowledge-extraction)into the application. 
- Alternatively, use the [JSON connector](/ai-for-service/searchai/connectors/json) to add FAQs by saving the question as chunkTitle and the answer as chunkText.

### Indexing Configurations

Index fields are fully replicated, but traits aren't. There is no longer a need for index settings, as multilingual capabilities are now integrated. Some workbench stages are be replicated if they're relevant to Search AI, while others are modified for compatibility.

#### Index Fields

Index fields
Index fields are fully replicated and mapped to corresponding fields in Search AI. The custom fields in SearchAssist are automatically mapped to the corresponding fields in Search AI. [Learn more](/ai-for-service/searchai/connectors/unified-schema).
...
Index Configurations must be reconfigured manually after migration. For more details, refer to [this](/ai-for-service/searchai/index-configuration). 

<Note>In Search AI, the BGE-M3 vector model is selected by default for generating embeddings. You can also set up a custom embedding model.</Note>
...
| Document Workbench Stages | Migration scope |
|:--------------------------|:------------------|
...
Additionally, you can also configure the Chunk Workbench manually in Search AI to apply additional processing on extracted chunks. [Learn More](/ai-for-service/searchai/index-configuration#content-enrichment-using-workbench).

#### Index Settings

Search AI supports 100+ languages and can handle all the languages supported by the underlying LLM and embedding models. Refer to [this ](/ai-for-service/searchai/tips-and-faqs#multilingual-support)for language-specific configuration and recommendations. No specific configuration is required for the supported languages.
...
- The `context` object used in the condition block now refers to the context object and session variables provided by the AI for Service platform. [Learn More](/ai-for-service/automation/intelligence/context-management).
...
Some custom configurations are replicated as Advanced Configurations in Search AI, some configurations have been productized, and some custom configurations aren't replicated as they're no longer relevant.

| Config | Auto Migrated | Notes |
|--------|---------------|-------|
| Chunk Extraction Method | Yes | If the extraction type is “layout,” then a new extraction strategy with layout is created. |
| Chunk Token Size | Yes | Automatically replicated. Moved to Extraction Strategy Configuration. Refer to [this](/ai-for-service/searchai/index-configuration#adding-a-strategy) for more details. |
| Chunk Vector Fields | Yes | Automatically replicated. This is available as part of the [Vector configuration](/ai-for-service/searchai/index-configuration#vector-configuration). Refer to [this ](/ai-for-service/searchai/index-configuration)for details. |
| Number Of Chunks | No | Instead of setting the number of chunks, you can set the token budget. This allows the application to dynamically calculate the number of chunks based on the selected LLM and extraction strategies. This is available as part of [Answer Configuration](/ai-for-service/searchai/retrieval-answer-configuration#answer-generation). |
| Rewrite Query | No | This feature has been enhanced. Search AI offers two ways to implement this.  1. [Query Rephrase for Advanced Search API](/ai-for-service/searchai/agentic-rag-and-business-rules#query-rephrase-agent) - This is implemented via Agentic RAG. Queries can be dynamically enhanced with contextual information through APIs. 2.[ Rephrase User Query](/ai-for-service/generative-ai-tools/genai-features-searchai#rephrase-user-query) - This is implemented by the platform to enhance or reconstruct incomplete or ambiguous user inputs using the conversation context. |
| Chunk Retrieval Strategy | Partial | Some of the older retrieval methods have been deprecated. With Search AI, you can configure both Vector Retrieval and Hybrid Retrieval through the Retrieval page. By default, the hybrid retrieval strategy is used. However, if the “Enable Vector Search” configuration is enabled in SearchAssist, or if the Chunk Retrieval strategy is set to ‘vector’, vector search is automatically enabled in Search AI. [Learn more.](/ai-for-service/searchai/retrieval-answer-configuration) |
...
| Rerank Chunks | Yes | This is available as an [advanced configuration](https://platform.kore.ai/builder/app/advanceConfiguration). |
| Rerank Chunk Fields | Yes | This is available as an [advanced configuration](https://platform.kore.ai/builder/app/advanceConfiguration). |
| Maximum re-rank chunks | Yes | This is available as an [advanced configuration](https://platform.kore.ai/builder/app/advanceConfiguration). |
| Chunk Order | Yes | This is available as a config field in the [Answer Configuration](https://platform.kore.ai/builder/app/answergeneration) page. |
...
**Small Talk** is also deprecated from Search AI, but can be managed through conversational flows in Automation AI. If required, this must be manually set up after migration. Go to the[ SmallTalk page in Automation AI](/ai-for-service/automation/intelligence/conversation-management#small-talk) and provide the details of the small talk to be managed.[ Learn More](/ai-for-service/automation/intelligence/conversation-management#small-talk). 
...
In Search AI, [Business rules](/ai-for-service/searchai/agentic-rag-and-business-rules) and [Agentic Rag](/ai-for-service/searchai/agentic-rag-and-business-rules) capabilities provide more flexible, context-aware control over retrieval, eliminating the need to configure ranking rules for each unique user query.

Hence, this functionality has been deprecated and doesn't need migration.

#### Search and Result Interface

Custom interfaces can be built using [Search AI’s public APIs](/ai-for-service/apis/searchai/api-list). By default, the AI for Service Platform provides agent experiences across 40+ channels that can be easily integrated with minimal configuration. Refer to the [respective documentation](/ai-for-service/channels/adding-channels-to-your-bot) for more details.

This must be manually set up after migration completes.

### APIs and API Scope

APIs and API Scope configurations aren't migrated and must be reconfigured manually in Search AI. The APIs in Search AI are more advanced and offer enhanced functionality compared to SearchAssist. Refer to the Search AI API documentation for the full list of available APIs and to take advantage of the advanced capabilities. [Learn More](/ai-for-service/apis/searchai/api-list).

### SDK

If your SearchAssist implementation uses the SearchAssist SDK to render the search interface, you must switch to the Search AI SDK after migration. Refer to the Search AI SDK documentation to set up and configure the SDK for your application. [Learn More](/ai-for-service/sdk/bot-sdk-introduction).

## Initiating the Migration Process

1. Click on the *Set up Search AI* link in the banner at the top of the page in SearchAssist. ![SearchAssist Home](/ai-for-service/getting-started/images/searchai/migration/SA-home.png "SearchAssist Home") 
2. Select the app to be migrated. ![Select App](/ai-for-service/getting-started/images/searchai/migration/select-app.png "Select App")
...
![Migration Success](/ai-for-service/getting-started/images/searchai/migration/success.png "Migration Success")

If you aren't on an Enterprise plan, the Search AI app is created during migration but access to certain features may be restricted. Contact Support to enable full access.
...
![Search AI app review](/ai-for-service/getting-started/images/searchai/migration/searchai-app.png "Search AI app review")
...
![alt_text](/ai-for-service/getting-started/images/searchai/migration/test-answers.png "image_tooltip")

### Testing Answers End-to-End in the AI for Service Platform

Use this method to test the complete application flow.

<Note>When you create a new app in the AI for Service platform, DialogGPT orchestration is enabled by default. In this mode, Search AI-based answer generation is disabled.</Note>

When you click *Test* in the upper-right corner to test answers:
...
* Enable Knowledge from Search AI as a conversation type.
* Disable DialogGPT orchestration.

**Enable Knowledge from Search AI**

Click on Conversation Types and enable Knowledge from Search AI.

![Add Search AI as Knowledge](/ai-for-service/getting-started/images/searchai/migration/add-knowledgeai.png "Add Search AI as knowledge")

![Add Search AI as Knowledge](/ai-for-service/getting-started/images/searchai/migration/enable-knowledge.png "Add Search AI as knowledge")

**Disable DialogGPT** 

Go to the options and click Disable DialogGPT.

![Disable DialogGPT](/ai-for-service/getting-started/images/searchai/migration/disable-dialogGpt.png "Disable DialogGPT")
