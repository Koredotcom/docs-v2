---
title: Configuring the Freshdesk Action
sidebarTitle: Configure Freshdesk
---
<Badge icon="arrow-left" color="gray">[Back to Actions Integrations](/ai-for-service/integrations/overview#actions)</Badge>

Connect the Platform to Freshdesk to create, view, update, search, and delete tickets. See [Freshdesk documentation](https://www.freshworks.com/freshdesk/resources/) for details.

---

## Supported Authorization Types

The platform supports Basic Auth for Freshdesk integration. See [App Authorization Overview](/ai-for-service/app-settings#authorization-profiles) for details.

| Authorization Type | Supported |
|---|---|
| Pre-Authorize the Integration | Yes |
| Allow Users to Authorize the Integration | Yes |

---

## Prerequisites

Before enabling the Freshdesk action:

- Create a [Freshdesk](https://www.freshworks.com/freshdesk/resources/) developer account.
- Copy the **API Key** and **Domain name** of your Freshdesk account.

---

## Step 1: Enable the Freshdesk Action

Go to **App Settings > Integrations > Actions** and select **Freshdesk**.

### Pre-authorize the Integration (Basic Auth)

1. Select **Freshdesk** in the **Available Actions** region.
2. In the **Configurations** dialog, select the **Authorization** tab.
3. Set **Authorization Type** to **Pre-authorize the Integration** > **Basic Auth**.

   ![Auth type](/ai-for-service/integrations/actions/images/freshdesk-action-img2.png)

4. Enter the following fields:

   | Field | Description |
   |---|---|
   | User Sub Domain | Domain name of the Freshdesk account |
   | API Key | Secret API key of your Freshdesk account |

5. Click **Enable**. The Integration Successful pop-up appears on first configuration.

   ![Enable integration](/ai-for-service/integrations/actions/images/freshdesk-action-img3.png)

<Note>The Freshdesk action moves from Available to Configured.</Note>

### Allow End Users to Authorize (Basic Auth)

1. In the **Configurations** dialog, select the **Authorization** tab.
2. Set **Authorization Type** to **Allow Users to Authorize the Integration** > **Basic Auth**.
3. Click **Select Authorization** > **Create New**.

   ![Create new auth profile](/ai-for-service/integrations/actions/images/freshdesk-action-img4.png)

4. Select the authorization mechanism (for example, **Basic Auth**). See [App Authorization Overview](/ai-for-service/app-settings#authorization-profiles).
5. Enter the following credentials:

   | Field | Description |
   |---|---|
   | Name | Name for the Basic Auth profile |
   | Tenancy URLs | Select No |
   | Base URL | Base tenant URL for the Freshdesk instance |
   | Authorization Check URL | Auth check URL |
   | Description | Description of the auth profile |

   ![Auth credentials](/ai-for-service/integrations/actions/images/freshdesk-action-img5.png)

6. Click **Save Auth**.

   ![Save auth](/ai-for-service/integrations/actions/images/freshdesk-action-img6.png)

7. Select the new profile.
8. Click **Enable**. The Integration Successful pop-up appears.

   ![Enable integration](/ai-for-service/integrations/actions/images/freshdesk-action-img7.png)

---

## Step 2: Install Freshdesk Action Templates

1. On the Integration Successful dialog, click **Explore Templates**.

   ![Explore templates](/ai-for-service/integrations/actions/images/freshdesk-action-img8.png)

2. Click **Install** for the desired template.

   ![Begin installation](/ai-for-service/integrations/actions/images/freshdesk-action-img9.png)

3. Click **Go to Dialog** to view the dialog task.
4. A dialog task is auto-created for each installed template.
5. Select the desired dialog task and click **Proceed** (for example, **Create a Ticket**).

   ![Select dialog task](/ai-for-service/integrations/actions/images/freshdesk-action-img10-tem-img2.png)

6. The canvas opens with all required entity nodes, service nodes, and message scripts.

   ![Dialog task canvas](/ai-for-service/integrations/actions/images/freshdesk-action-img11.png)
