...
1. Log in to your Genesys Cloud account. Note the login URL from the browser address bar (for example, `https://login.mypurecloud.com`).
...
2. Copy the WebSocket URL (for example, `wss://streaming.mypurecloud.com/v2/websocket`).
...
4. On the **Configurations** tab, enter:
   ...
   ![Genesys agent credentials](/ai-for-service/integrations/agents/images/genesys-at.png)
...
10. Publish the app. See [Publishing App](/ai-for-service/deployment/publishing-app).
...
2. Create an Agent Transfer Node task (for example, a `PlatformAgent` task with an `AgentTransfer` node configured to use Genesys). See [Agent Transfer Node](/ai-for-service/automation/dialogs/node-types/agent-transfer-node#agent-transfer-node).

   ![Agent Transfer Node task](/ai-for-service/integrations/agents/images/configuring-the-genesys-agent-img4.png)
...
5. The connection is established and the conversation continues. When an agent begins interacting with a user, the system generates a message ID and exposes it as a context variable `{{context.session.BotUserSession.agentHandoffMeta.messageId}}`. You can pass this message ID as a parameter in the service node to retrieve details of the agent who handled the conversation.
   ...
   ![Connection established](/ai-for-service/integrations/agents/images/configuring-the-genesys-agent-img5.png)
...
![Genesys metadata script](/ai-for-service/integrations/agents/images/genesys2.png)
...
![Participant Data](/ai-for-service/integrations/agents/images/genesys5.png)
...
![GetParticipantData action](/ai-for-service/integrations/agents/images/genesys4.png)

![Flow variable mapping](/ai-for-service/integrations/agents/images/genesys3.png)

![Transfer to ACD config](/ai-for-service/integrations/agents/images/genesys6.png)
...
![Dynamic queue step 1](/ai-for-service/integrations/agents/images/genesys1.png)

![Dynamic queue step 2](/ai-for-service/integrations/agents/images/genesys8.png)

![Dynamic queue step 3](/ai-for-service/integrations/agents/images/genesys7.png)
