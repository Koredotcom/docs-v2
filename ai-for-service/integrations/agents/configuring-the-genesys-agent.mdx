...
2. Create an Agent Transfer Node task (for example, a `PlatformAgent` task with an `AgentTransfer` node configured to use Genesys). See [Agent Transfer Node](/ai-for-service/automation/dialogs/node-types/agent-transfer-node).
