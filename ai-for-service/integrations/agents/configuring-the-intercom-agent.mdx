...
2. Create an Agent Transfer Node task (for example, a `PlatformAgent` task with a `UserChats` node configured to use Intercom). See [Agent Transfer Node](/ai-for-service/automation/dialogs/node-types/agent-transfer-node).
