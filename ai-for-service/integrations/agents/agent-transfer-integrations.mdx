...
**Fallback options:**
- Redirect to a specific node (for example, Apology or Retry node)
...
5. In the **Error Prompt** section, configure:
   ...
   - **Go To Node**: Node to route to after failure (for example, Retry or Escalation).
...
- Use descriptive node names (for example, `Agent_Transfer_Internal`, `Agent_Transfer_External`) to simplify debugging.
...
<Note>Agent responses are routed through BotKit for the following out-of-the-box agent integrations: Genesys, Zendesk, Salesforce, and Kore Agent.</Note>
...
- The platform doesn't provide a built-in option for end-users to end a live agent chat, except when using Salesforce integration.
...
When the platform transfers a user to a live agent, it can include a link to the user's prior conversation with the AI agent to provide context for the handoff. Enable or disable the conversation link in the [Agent Transfer node](/ai-for-service/flows/node-types/agent-transfer#general-settings). The link can be accessed up to 10 times. 
...
The platform supports WhatsApp Cloud API read receipts for end users, providing visibility into message delivery and read status across bot and agent conversations. This capability is automatically available for all existing and new WhatsApp Cloud API integrations. End users can also see a typing indicator while an agent is composing a message.

Agents handling WhatsApp Cloud API conversations can now view real-time message delivery and read status for messages exchanged with end users.
