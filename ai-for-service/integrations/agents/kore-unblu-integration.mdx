...
Unblu is a conversational platform enabling agents and advisors to collaborate with customers via live chat, secure text messaging, and video chat. This guide covers the BotKit-based integration between the AI Agent and Unblu for agent transfer scenarios.
...
5. Start the integration:
   ...
   <Note>The webhook URL must be the same in API Extensions, `config.json`, and the Unblu external messenger Endpoint URL (that's, the BotKit URL in API Extensions, `app.url`, and `unblu.custom.external_messenger`).</Note>
...
- Multiple named areas can be mapped to a single AI Agent, allowing it to accept Unblu dialog offer requests from different origins (SDKs or named areas).
