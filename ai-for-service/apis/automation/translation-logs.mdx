...
| **Method** | **GET** |
| :---- | :---- |
| **Endpoint** | `https://{{host}}/api/{{version}}/{{streamId}}/translationlogs?sessionId={{sessionId}}&fromDate={{fromDate}}&toDate={{toDate}}&limit=50&offset=0` |
...
| `streamId` | App ID or Stream ID. You can access it from the App Settings --> App Profile page of the app. |
...
| Parameter | Description |
| --- | --- |
| `sessionId` | Conversation session ID used to fetch translation logs for a specific conversation session. |
| `fromDate` | Start time for fetching logs. Use UTC timestamp format to avoid time zone related mismatches: `YYYY-MM-DDTHH:MM:SS.000Z`. Example: `2026-04-27T18:14:06.498Z`. Use of `YYYY-MM-DD`may not return accurate results across timezone boundaries. |
| `toDate` | End time for fetching logs. Use UTC timestamp format: `YYYY-MM-DDTHH:MM:SS.000Z`. Example: `2026-04-27T23:59:59.999Z`. |
| `limit` | Number of records to fetch. The maximum allowed value is `50`. If not specified, the API returns up to `50` records. |
| `offset` | Number of records to skip before fetching results. If not specified, the API starts from `0`. |

<Note>At least one of the query parameters — `sessionId`, `fromDate`, or `toDate` — must be provided to fetch translation logs.</Note>
...
[
    ...
    },
    {
        ...
        "inputPayload": {
            "endPoint": {
                "host": "koreai-translate.laratranslate.com",
                "port": "",
                "path": "/translate",
                "protocol": "https",
                "method": "post",
