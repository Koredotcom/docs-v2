...
<Note>When `dateFrom` or `dateTo` is specified as a date only (`YYYY-MM-DD`), the platform interprets it as `YYYY-MM-DDT00:00:00` (the start of that day). To query a full day, specify the complete timestamp—for example, `"dateFrom": "2026-07-20T00:00:00Z"` and `"dateTo": "2026-07-20T23:59:59Z"`—or set `dateTo` to the following day (for example, `"dateTo": "2026-07-21"` to retrieve all data for July 20).</Note>
