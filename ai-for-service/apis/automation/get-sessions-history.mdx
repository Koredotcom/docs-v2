...
| `sessionId` | Optional | | Array of up to 50 session IDs. If used, all other filters are ignored. **Note:** Querying using `sessionId` returns `sessionTags` only for sessions from the last 14 days. To retrieve `sessionTags` for older sessions, use `dateFrom` and `dateTo` with a date range of less than 7 days.  |
