...
| `botId` | Bot identifier or Stream identifier. Access it from the [General Settings](/ai-for-service/apis/automation/general-settings) page of the bot.
...
| `dateFrom` | Optional | String | Start date to fetch records in in ISO 8601 format. Defaults to 7 days before `dateTo`. When `dateFrom` or `dateTo` uses the `YYYY-MM-DD` format, the platform interprets the value as the start of that day (`YYYY-MM-DDT00:00:00`). To retrieve data for a full day, specify the complete timestamp or set `dateTo` to the following day. For example, use `"dateFrom": "2026-07-20T00:00:00Z"` with `"dateTo": "2026-07-20T23:59:59Z"` or `"dateTo": "2026-07-21"`. |
| `dateTo` | Optional | String | End date to fetch records in in ISO 8601 format. Defaults to today if neither date is provided. |
