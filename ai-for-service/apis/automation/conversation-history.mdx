...
<Note>Timeline events, including the new system-triggered dialog message, are available only via Get Messages v2 — not via the Conversation History Lightweight API or Get Analytics API.</Note>
...
| `includeTimelineEvents` | Optional | Boolean | Includes system-generated timeline events (Agent Joined, User Dropped Off, Session Ended, Agent Transfer, System-triggered dialog) in the response. Default: false. Applies to getMessagesV2 only. |
...
| `includeTimelineEvents` | Optional | Boolean | Includes system-generated timeline events (Agent Joined, User Dropped Off, Session Ended, Agent Transfer, System-triggered dialog) in the response. Default: false. |
