...
To fetch conversation details and summary between the bot and user, or the user and agent. Supports pagination via `skip/offset` and `limit` parameters.
...
| `botId` | Required | Bot identifier or Stream identifier. Access it from the [General Settings](/ai-for-service/app-settings) page of the bot. |
...
| `conversationId` | Optional | Unique identifier for the conversation. This field is required when `sessionId` or `call ID` is not entered. Use either `sessionId` or `conversationId`; if both `sessionId` and `conversationId` are provided, `sessionId` takes precedence. |
...
<Note>
If the session ID is available, the call ID isn't mandatory and vice versa. If both are present, the session ID is used.
</Note>
...
```json expandable=true
{
   "conversationInfo": {
       ...
       "callId": "9eb6c49e-f647*******c-cc207e3a6080",
