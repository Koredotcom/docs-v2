...
Sends various types of messages to users via `IVR`, `Google Chat`, `Webhook`,`msteam`, and `WhatsApp`channels.
      ...
      "value": {
        ...
        "attachments": [{
          ...
          "content": {
            ...
            "version": "1.0",
            "body": [
              { "type": "TextBlock", "text": "Hello from Kore API", "wrap": true, "size": "medium", "weight": "bolder" },
              { "type": "Image", "url": "https://raw.githubusercontent.com/OfficeDev/Microsoft-Teams-Adaptive-Card-Samples/refs/heads/main/samples/recipe/assets/recipe_image.png", "size": "stretch", "altText": "recipe" }
            ]
          }
        }]
      }
    },
    "identities": [{ "email": "john.do@example.com"}]
  }
```
...
|---|---|---|---|
| `channel` | `string` | Required | Name of the channel for notifications (for example, `ivr`, `hangouts`, `whatsapp`,`msteams`)
...
| Field | Type| Required/Optional | Description  |
|---|---|---|---|
| identity field | `string` | Required | Identity key depending on the channel: `ivr` uses `email`, `hangouts` uses `spaceId`, `whatsapp` uses `phoneNumber`. **Note:** Other channels (for example, `msteams`, `slack`) aren't yet supported in public API v2. |
