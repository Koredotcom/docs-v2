...
| `vectorWeights` | Array of objects. Object Fields include `vectorName`(string, unique name of the vector as configured in the app) and `weight`(integer, weight to be assigned to the vector). Overrides the weight assigned to one or more named vectors for this request. Weights must sum to 100. Any active vector not listed will be assigned 0 weight for this request. | No. When not provided in the request, app level values apply.  |
