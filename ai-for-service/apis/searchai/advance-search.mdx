...
| `userId` |Specifies the unique identifier of the end user for analytics and usage tracking. This field is used only for analytics and does not affect access control. User permissions are managed separately through RACL. |No. | 
| `sessionId` | Specifies the unique identifier of the user session or conversation for analytics and session tracking. This field is used only for analytics and does not affect access control. |No. | 
