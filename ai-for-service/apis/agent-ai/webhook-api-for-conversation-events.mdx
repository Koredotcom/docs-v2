---
title: Webhook API for Conversation Events
noindex: true
hidden: true
---
...
[← Back to API List](/ai-for-service/apis/agent-ai/api-list)
...
| **Authorization Header** | `auth: {{JWT}}` <br/>See [How to generate the JWT Token](/ai-for-service/apis/automation/api-introduction#generating-the-jwt-token) |
...
<Info>Invoke this event **only once**, at the final termination of the conversation—whether normal or unexpected—to prevent duplicate processing.</Info>
...
**First Agent Accepts (INTERNAL_TRANSFER_EVENT)**
...
| 5 | `END_OF_CONVERSATION` | — | — | Signals the formal closure of the conversation. Ensures capture of all post-call analytics and Quality Management data. |

<Note>In a redial scenario, keep the **conversationId** the same as the original dropped call. This ensures the system treats the session as a continuation, not a new conversation, and preserves the full interaction history for analytics.</Note>
...
{
 "code": 400,
 "message": "Linked bot details couldn't be found. Please verify that the botId provided in the request body is correct."
}
