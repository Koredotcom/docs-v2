...
curl --location --request POST
     'https://{{host}}/agentassist/api/v1/public/{{IID}}/contacts/import-contacts' \
      ...
      --header 'auth: <token>' \
      --form 'result_file=@"/home/johndoe/Documents/john/sample.csv"'
```

<Note> Saving contacts through the public API allows duplicate names with different numbers but prevents the same number from being associated with different or identical names. Backward compatibility to delete existing duplicate numbers will not be provided.</Note>
