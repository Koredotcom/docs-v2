---
title: Create a Queue API
sidebarTitle: Create a Queue
---

Use this API to create a queue with the given `streamId`.
  ...
  "agents": [
    {
      ...
      "skills": [
        { "id": "s-e1e22e03-fe32-5748-b830-02d161a3xxxx", "name": "SILVER" },
        { "id": "s-e1e22e03-fe32-5748-b830-02d161a3xxxx", "name": "GOLD" }
      ]
    }
  ],
  "skills": [
    {
      ...
      "name": "SILVER",
      "isExpireEnabled": true,
      "expireTime": { "min": 10, "sec": 20 },
      "skillGroupName": "Test",
      "skillGroupColor": "#070707"
    }
  ],
  "agentGroups": [
    { "id": "ag-ec83505-5152-494d-8071-3b68513cxxxx", "name": "Default Queue" }
  ],
  "advancedSettings": {
    "preferredAgentTimeout": { "min": 10, "sec": 20 },
    "isSkillMatchEnabled": true,
    "isAffinityMatchEnabled": true
  }
}
...
| `isAdvancedRoutingEnabled`                | Whether advanced routing is enabled.                                   | boolean, optional |
| `hoursOfOperation.id`                     | Hours of operation ID.                                                 | string, required  |
| `hoursOfOperation.name`                   | Hours of operation name.                                               | string, optional  |
| `agents.userId`                           | User ID of the agent. Get this from GET /agents.                       | string, required  |
| `agents.aId`                              | Agent ID. Get this from GET /agents.                                   | string, required  |
| `agents.name`                             | Name of the agent.                                                     | string, optional  |
| `agents.isPreferredAgent`                 | Indicates if the agent is a preferred agent.                           | boolean, required |
| `skills.id`                               | Skill ID. Get this from GET /skills.                                   | string, required  |
| `skills.name`                             | Name of the skill.                                                     | string, optional  |
| `skills.isExpireEnabled`                  | Indicates if the skill should expire.                                  | boolean, required |
