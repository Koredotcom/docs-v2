---
title: Outbound Calling VG v2
sidebarTitle: Outbound Calling VG v2
---
...
When the dialed call is answered, the answering machine detection (AMD) feature starts listening to the outbound call, and after a short period, sends an indication of whether a person or a machine has answered the call.
...
| Method      | POST                                                                                                     |
|:-------------|:----------------------------------------------------------------------------------------------------------|
| **Endpoint**    | `https:/{{host}}/api/1.1/public/bot/{{botId}}/V2/smartassist/dialout` |
...
| `timers`              | An object containing various timeout properties. [Learn more](/ai-for-service/apis/contact-center/outbound-calling-vg-v2#timers-configuration) | object, optional |
...
| `ringDurationInMs`           | **Ringing to Connection Delay** - It's the time between when the call starts ringing and when the other person answers and the call connects.                                                                                               | number, required |
...
* When the machineDetection parameter is set to disconnect, and the system detects a machine using either `amd_machine_detected` or `amd_tone_detected`, the call will be marked as `machineDetected` in the Interactions page under the Status column.  
    ![Status](/ai-for-service/apis/contact-center/images/status.png) 
