...
| `Auditor Response` | String | Response provided by the auditor for the metric. Returns `null` if no response is available. |
...
| `AuditDate` | String |Date and time when the audit was completed. Returns `null` if the audit has not been completed. |
...
| `AuditorName` | String |Name of the auditor assigned to the evaluation. Returns `null` if no auditor is assigned. |
