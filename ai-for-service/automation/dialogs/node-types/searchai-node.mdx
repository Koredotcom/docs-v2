---
title: Search AI Node
sidebarTitle: Search AI Node
noindex: true
hidden: true
mode: center
---
...
A **Search AI node** can be placed in the automation workflows when there is a need to retrieve contextually relevant information from large content sources like documents, FAQs, or third-party applications to execute the next step in the automation. It can also be used as a fallback mechanism when the regular automation isn't able to find any matching intent.
...
2. Click **GenAI** and then click **Search AI Node**. Alternatively, you can drag and drop the **Search AI Node** to the required location on the canvas. For more information on adding nodes, see [add a node](/ai-for-service/flows/create-and-manage-flows#add-a-node).
...
<Note>Changes made in this section apply to all dialog tasks using this node.</Note>
...
The settings in the Instance Properties tab are specific to the current dialog task and don't reflect in other dialog tasks that use this node.
...
**Custom Tags**: Add tags to build custom profiles of your AI Agent's conversations. [Click here for more](/ai-for-service/analytics/automation/custom-dashboard/custom-meta-tags).

---

### Connections Properties

To set up node connection conditions, please follow the steps outlined in [Adding IF-Else Conditions to Node Connections](/ai-for-service/automation/dialogs/node-types/node-connections). These settings apply only to this instance and won't affect other uses of the node. These settings are available only if this is the last node in a sequence.
...
* [Use Agent Node and Search AI to Generate Answers](/ai-for-service/automation/how-tos/use-agentnode-and-searchai-to-generate-answers)
