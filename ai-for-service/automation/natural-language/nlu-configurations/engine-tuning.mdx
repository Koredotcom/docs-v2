...
| **ML_languageSupportViaML**                     | Enables ML-based intent, entity, and interruption detection.  | Enable / **Disable**  (default)          | French and German Languages only.              |
...
| **Suppress_Fallback_On_TaskFulfilment** | When enabled, this prevents the platform from triggering the Fallback or Intent Not Found event if a dialog ends with a 'fulfilled' reason and the "End of Task" event is turned off. | Enable, Disable (default) |
