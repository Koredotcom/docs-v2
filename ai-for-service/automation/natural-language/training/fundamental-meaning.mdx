...
| Step | Description |
|---|---|
| **Tokenization** | Splits text into tokens. Retains hyphenated words; splits digit-hyphen combos (for example, `2-3` → `2 - 3`). |
| **Substitution** | Expands abbreviations, contractions, and misspellings using system concepts. Bot substitutions apply custom replacements (for example, `Chevy` → `Chevrolet`). |
| **Merging** | Combines sequences of single words, numbers, or dates into single tokens (for example, `credit card`, `twenty-five`). |
...
| **Concepts** | Predefined or custom groups of related terms. Used in patterns (for example, `~world_country`, `~fruit`). |
...
See [NLP Guidelines — Patterns](/ai-for-service/automation/natural-language/nlp-guidelines#patterns) for full pattern operator reference.
...
| Setting | Description |
|---|---|
| **Intent Detection using Task Name Words** | Disabled: only match if input starts/ends with exact task name. Useful for tasks sharing words (for example, multiple "order" intents). Enabled by default. |
