...
- **Indirect entity extraction** — infer entity values from context (for example, "seat in front with extra legroom" implies First Class).
...
A Trait Type is a collection of related traits (for example, `Flight Fare` containing `Economy`, `Business`, `First Class`).
...
- **Few-shot Model (default)** — embedding-based. Options: BGE M3 (default) and Pretrained MPNet.
...
- When a trait name is modified, manually update all rules that reference it — the platform doesn't do this automatically.
</Note>
