...
| **Stemming** | Reduce to stem (for example, "Running" → "run"). Output may not be a real word. |
| **Lemmatization** | Reduce to base dictionary form (for example, "housing" → "house"). |
| **N-grams** | Combine co-occurring words for context (for example, "New York City" as a tri-gram). |
