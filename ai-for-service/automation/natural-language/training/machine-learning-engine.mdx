...
|---|---|
| **Standard**  | Bag-of-words classification. Suitable for most use cases. |
| **Zero-shot Model** | No training required; uses semantic similarity. Requires NLP V3. |
| **Few-shot Model (Kore.ai Hosted Embeddings) (default)** | LLM-based semantic similarity. Reduces training data  equirements. Requires NLP V3. |
| **MLP-BOW** (Deprecated) | Multi-layer perceptron with bag-of-words features. |
| **MLP-WordEmbeddings** (Deprecated) | MLP with pre-trained word embeddings. |
| **LSTM** (Deprecated) | Recurrent neural network. Captures sequential context. |
| **CNN** (Deprecated) | Convolutional neural network. Captures local n-gram patterns. |
| **Transformer** (Deprecated) | Transformer-based model. High accuracy for English. |
| **KAEN** (Deprecated) | Kore.ai custom model. Improved accuracy for English. |
...
| Setting | Description |
|---|---|
| **Intent Detection using Task Name Words** | When disabled, intent is detected only if input starts/ends with the exact task name. Useful for tasks with shared words (for example, multiple "order" tasks). Enabled by default. |
