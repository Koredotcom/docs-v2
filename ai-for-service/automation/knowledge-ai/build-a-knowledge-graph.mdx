...
The Knowledge Graph (KG) turns FAQ content into a structured conversational experience. You can use an ontology-based hierarchy or the LLM-based Few-Shot model (no ontology required). See [Knowledge Graph Terminology](/ai-for-service/automation/knowledge-ai/knowledge-graph-terminology) for concept definitions.
...
3. Type the node name and press **Enter**. (Duplicate child node names under the same parent aren't allowed.)
...
<Note>
After any change, click **Train** (top-right) to send updates to the KG engine. Untrained changes don't affect responses.
</Note>
