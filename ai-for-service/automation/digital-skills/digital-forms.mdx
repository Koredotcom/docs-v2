...
5. You can set the **Properties** for each of the nodes added.
    ...
    * The [Form Node](/ai-for-service/automation/dialogs/node-types/working-with-the-form-node). Following Component Properties are of special interest:
        * **Submit Message** – Message displayed to the end-user on successful submission of the form
        * **Web/Mobile SDK Form Behavior** – Using this option you can either have the form displayed *inline* the chat window or *open on a full page*. Also, you can either go ahead with the default submit prompt or configure the setting to display a custom and more specific message to be shown in chat. [Learn more](/ai-for-service/automation/dialogs/node-types/working-with-the-form-node#component-properties).
    
        * **Bot Action Node,** in case of the conversation experience flow, can also be configured in the [Logic Node](/ai-for-service/automation/dialogs/node-types/working-with-the-logic-node) as follows:
...
<Note>We urge you not to make changes to the connection settings as this affects the AI Agent's performance.</Note>
    ...
    * **Sub-dialog Node** is configured as a normal [Dialog Node](/ai-for-service/automation/dialogs/node-types/working-with-the-dialog-node) as follows:
