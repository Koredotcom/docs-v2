...
| [Digital Skills](/ai-for-service/automation/digital-skills/digital-skills-overview) | Create interactive user interfaces, digital forms and digital views, that retrieve information from users or present it to them. |
...
| **Errors & Warnings** | Messages displayed during errors — for example, *End date can't be before the start date.* |
...
<Note>Use the toggle to enable or disable Statements. By default, all are enabled. Disabling a Statement excludes it from runtime processing, while other Standard Response categories continue to function normally. Use the filter to toggle all options at once. Disable the standard response for each language separately.</Note>
...
| When task isn't accessible to user | Developer tries to access an app task they don't have permission for. |
...
| Location provided by user is invalid | AI Agent can't validate input for a Location-type entity. |
...
| Ask at least one optional field | User didn't provide any optional field value when at least one is required (<Tooltip headline="Action task" tip="Action tasks collect, modify, and post information in systems of record, such as scheduling an appointment, searching for a product, or updating critical information.">Action Task</Tooltip> only). |
| Topic retry limit exceeded | Developer mode only. Dialog looped through the same nodes beyond the allowed limit. |
| Universal fallback response | AI Agent can't generate a response for any reason, or maximum node execution limit exceeded. |
...
| App choosing the default account | Shown during alert amend when AI Agent can't parse the user's response. |
...
| Not understanding the input at confirmation | User's response during alert frequency confirmation doesn't match the expected input. |
...
| Ask user to change dependent field | A dropdown depends on another field, but the Platform can't fetch data from that field. |
...
| Confirmation Dialog for task with skipped fields | Confirmation before executing an Action/Info task when some optional fields weren't provided. |
...
| Listing all alerts after tasks | Shown when app only has alert tasks and intent isn't identified. |
...
| User prompt when a task is discarded due to server call failure | API call didn't complete successfully. |
| Specific app isn't available | App isn't yet published. |
...
| User says I am done | Shown when user indicates they're finished (done, that's all, that's it). |
...
| Notify the user that we've canceled the paused task | User chose not to resume the on-hold task during confirmation. |
| Notify the user that we've canceled the current task to switch to another | Hold option is *Discard the current task and switch to new*. |
| When an interruption is identified but hold isn't allowed | New intent detected during task execution; new intent added to follow-up list (current task continues). |
...
| Ask the user to enter alert interval | Frequency is an interval, for example, every 4 hours (Alert only). |
| Ask the user to enter alert time | Frequency is a specific time, for example, daily at 9:00 am. |
...
| Dialog task associated to FAQ isn't available | The dialog linked to a FAQ was suspended, deleted, or not assigned to the user. |
...
| User provided value isn't unique | User's input causes ambiguity when selecting from a partially matched intent list. |
| App didn't find any options based on user's choice | Option set is empty, or no account credentials are available. |
...
| Ask user to enter time | User provided only a date for a DateTime entity (for example, scheduled appointment). |
| Invalid time — ask to try again for a field | User's time input for a DateTime entity is missing or invalid. |
| Invalid time — ask to try again | User entered an invalid time value (for example, Aug 29 at 25). |
...
| Can't understand your request | AI Agent fails to identify intent; lists available tasks and FAQs. |
...
| Ambiguous date | User entered an ambiguous date (for example, 6/9); both interpretations shown. |
| Ambiguous airport | User entered an airport name that matches multiple airports (for example, London). |
...
<Note>When a user provides input after 15+ minutes of inactivity and the input is within the previous session context, it's treated as an interruption. If the user starts a new utterance, the old context is discarded.</Note>
...
| **Don't present ambiguous intents and continue on the current task** (default) | AI Agent ignores ambiguous intents and continues the current task. |
...
<Note>This behavior applies only to NLP-based apps.</Note>

| Option | Behavior |
|---|---|
| **Respond to Search AI and resume the on-hold task** (default) | AI Agent responds to the Search AI request by putting the current task on hold, then resumes it. |
| **Execute the Search AI using Hold & Resume settings** | Applies the configured Hold & Resume settings to Search AI. |
...
| **Intent-aware** | Interjections like *hello* at the start of an utterance aren't treated as small talk if an intent follows — for example, *Hello, I need to book a flight* triggers the book-a-flight intent, not Small Talk. |
...
<Note>When you open Small Talk in an existing app, you are prompted to migrate Standard Response greetings to Small Talk. This migration is permanent — greetings are removed from Standard Responses and can't be retrieved if you proceed.</Note>
...
<Warning>Importing replaces all existing Small Talk data.</Warning>
