...
A sub-intent is a child intent that's only active when a specific parent intent is matched. The Platform adds an output context to the parent and a matching input context to the child, linking them together.

### Ways to Define Sub-Intents

| Method | Description |
|---|---|
| **Intent Node within a Dialog Task** | The sub-intent exists only within that dialog and isn't available elsewhere. [Learn more](/ai-for-service/automation/dialogs/node-types/user-intent-node) |
| **Existing Dialog Task (Sub-Intent-only)** | Reusable across multiple dialogs. The dialog can't be invoked by user utterance and doesn't appear in the Help task list. [Learn more](/ai-for-service/automation/dialogs/node-types/dialog-node) |
...
Follow-up intents are user intents that came up during a dialog but weren't handled at the time. The AI Agent captures them in the `FollowupIntents` array.

<Note>The AI Agent doesn't capture follow-up intents if **Don't Hold and Resume** is selected at the node, task, or app level.</Note>
...
| **No** — Handle the follow-up task list via node connections (default). | The AI Agent doesn't surface follow-up intents automatically. Developers can still access them via the `FollowupIntents` array. |
