...
| Action | Description |
|---|---|
| **Initiate a Task** | Launches a selected Dialog task (Standard or Hidden). If the dialog isn't published, the AI Agent shows an error. Preview errors using Debug mode. |
...
|---|---|---|
| **Intent not Identified** | Intent | AI Agent can't identify user intent. |
...
Fires when the AI Agent can't understand the user's intent. Configure one of:
...
You can override default behavior with a custom Dialog Task that calls `koreUtil.getAmbiguousIntents()` to retrieve intents and confidence scores. [Learn more](/ai-for-service/apis/automation/koreutil-libraries#koreutil-getambiguousintents).
...
**Interruption behavior:** When this event is enabled, interruption follows node-, dialog-, or app-level settings — except that if **Continue the current task and add a new task to the follow-up task list** is selected, the interrupting task isn't added to the follow-up list. [Learn more](/ai-for-service/automation/intelligence/conversation-management#manage-interruptions)

---

### Variable Namespaces

Go to **More Options > Manage Variable Namespaces** at the top of the Events screen to associate variable namespaces with events. This option appears only when Variable Namespace is enabled for the AI Agent. [Learn more](/ai-for-service/app-settings/managing-namespace).
...
**Defaults:** Always enabled with the **Show Message** action. Can't be disabled.
...
6. Set the **Event Configuration**:
   ...
   - **Auto-generate Response** — Uses LLM and Generative AI to generate the repeated response. Requires the Advanced NLU model. Click **Enable Now** to activate.
     ...
     <Note>When Auto-generate is enabled, filler messages aren't used.</Note>
...
<Note>Introduced in v7.0. Not supported in all languages. [Learn more](/ai-for-service/app-settings/language-management/multi-lingual-bot-behavior).</Note>
