...
**Intent Preconditions Filtering**

After Chunk Shortlisting, DialogGPT by default evaluates context-tag-based preconditions configured for intents and sub-intents in the shortlisted dialogs. Only dialogs that satisfy the configured preconditions are forwarded for final intent detection. Developers can trace this filtering through the Intent Preconditions node in DialogGPT NLP Insights and the Intent Preconditions Processing step in the Debug Logs. 
...
| Model |
|-------|
| Azure OpenAI - GPT-4o, GPT-4o-mini, GPT-5-mini, GPT-5.1, GPT-5.2, GPT-5.4, GPT-5.4 Mini, GPT-5.4 Nano|
| OpenAI - GPT-4o, GPT-4o-mini, GPT-5.2, GPT-5.3 Instant, GPT-5.4, GPT-5.4 Mini, GPT-5.4 Nano |
| Anthropic - Claude Haiku 4.5, Claude Sonnet 4.5, Claude Sonnet 4.6, and Claude Opus 4.6 |
| Amazon Bedrock |
...
**Timeout (seconds)**: Select the maximum time the platform waits for a Conversation Management model response before timing out. Range: 1-60 seconds. Default: 60 seconds. In voice interactions, a lower timeout reduces the period of silence when the LLM provider is unavailable or slow to respond, enabling the platform to recover or execute fallback logic sooner. 
...
8. (Optional) Adjust Temperature, Max Tokens, and Conversation History Length (The maximum number of recent messages that can be sent as context is 50).
9. Select the model timeout duration.
...
| Precondition Filtering | Discarded and retained chunks|
