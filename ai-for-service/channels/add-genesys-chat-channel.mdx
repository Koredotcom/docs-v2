...
## Pre-requisites for Digital Bot Connector v2

Before you configure the integration, ensure that the following prerequisites are met:

* Ensure that the **Digital Bot Connector** is available in your Genesys Cloud organization. If the integration isnt visible, it may still be in Limited Availability (LA). Contact your Genesys Cloud Account Executive to enable it.
* Ensure that the user performing the configuration has the `Integration > BotConnector > View` and `Integration > BotConnector > Send` permissions, or is assigned the **Integration Admin** role.
...
5. Select the **Grant Types** as **Client Credentials**.
