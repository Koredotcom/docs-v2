...
The **Platform** now helps a **Google Business Messaging Partner** to create rich and asynchronous messaging experiences within apps and during calls through the GBM channel enablement. This feature helps seamlessly integrate real-time business agents to AI Agents.
...
When Google Business Messaging is configured as a Channel on the Platform, the conversations between the customers and brands is enabled by creating agents. You can manage agents with either your own account or a GBM Partner Account. The process to create an agent differs in each scenario.

*  If you have a **GBM Partner Account**, the Platform registers each Business as a partner. You can enable conversations for brands you manage by creating Business Messages agents directly on GBM Developer Console.
...
<Tip>Please note the project number to register as a partner.</Tip>

## GBM Channel Enablement on the Platform

To set up the GBM channel on the Platform and launch the agent, follow the steps below:
...
![Google business ](/ai-for-service/channels/images/GBM-1.png)
...
3. Enter the values for the following fields in the **Create a Business Messages partner account** window:
    ...
    * **Region**: The region you want to host the Business Messages service.
    ...
    <Note>    To launch a production agent with Business Messages, please register with a corporate email, not a personal email.</Note>
4. Review and Accept the **Business Messages Terms of Service**.

5. Click **Create**.  ![create business partner](/ai-for-service/channels/images/GBM-2.png)
...
<Note>To manage multiple brands, you must create an agent for each brand.	</Note>
...
<Note>If you manage multiple brands, repeat the steps to create an agent for each brand.</Note>
...
2. Click **Create Agent**.  ![create agent](/ai-for-service/channels/images/GBM-3.png)

3. For **Agent type**, select **Business Messages**.
![brand name](/ai-for-service/channels/images/GBM-4.png)

4. Enter the values for the **Brand name** and **Agent name** fields.
...
![configure webhook](/ai-for-service/channels/images/GBM-5.png)

5. Copy the **Webhook URL** from the Platform under **Channels & Flows** > **Channels** > **Digital** > **All** > **Google Business Messaging** > **Configurations**.
...
![endpoint url](/ai-for-service/channels/images/GBM-7.png)

7. Click the **Refresh** icon for the **Client token** field to generate the **Client token**.

8. Click **Verify**.  ![client token](/ai-for-service/channels/images/GBM-8.png)

The **GBM platform** sends a **POST** request to the Platform’s webhook with the **ClientToken** and **Secret Key** parameters for verification before sending/receiving messages.
...
![service account](/ai-for-service/channels/images/GBM-9.png)

2. Click the **Create key** button.
![create key](/ai-for-service/channels/images/GBM-10.png)

3. Click **Create** on the confirmation window to download the JSON file with the **service account key**.  ![service account key](/ai-for-service/channels/images/GBM-11.png)
![service key](/ai-for-service/channels/images/GBM-12.png)
...
To configure the AI Agent for the service account on the Platform, follow the steps below:

<Tip>For the private key, please copy only the key value within the **BEGIN PRIVATE KEY** and **END PRIVATE KEY** header tags. The Platform adds these tags by default.</Tip>

1. Copy the **client_email**, **client_id**, **client_token (alphanumeric value)**, and **private_key** values from the downloaded JSON file.

2. Paste the values in the respective fields under **Channels & Flows** > **Channels** > **Digital** > **All** > **Google Business Messaging** > **Configurations**.  ![client details](/ai-for-service/channels/images/GBM-14.png)
