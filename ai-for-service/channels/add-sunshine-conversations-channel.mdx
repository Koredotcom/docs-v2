---
title: Adding the Sunshine Conversations Channel
sidebarTitle: Adding the Sunshine Conversations Channel
---
...
AI Agents built on the Platform can be deployed on the **Sunshine Conversations** platform. This channel integration supports conversations with AI Agents and provides a seamless hand-off to agents on **Zendesk**.
...
3. Click the **Authorize** button under **Configurations** on the **Sunshine Conversations** panel.
    ![Sunshine Channel](/ai-for-service/channels/images/Sunshine-1.png)
...
7. Once the integration is created successfully, you’ll be redirected to the Platform to complete the channel enablement process.

## Switchboard Integration for Agent Handoff

The Platform lets you hand off the conversation to your Zendesk agents via **Zendesk Switchboard** using the **Agent Transfer** node.

The **Switchboard Integration API** is used to route the user’s conversations from Zendesk to the Platform. This API has a pass control system to decide whether the conversation is to be routed to an AI Agent or a live agent.
...
Developers can define custom metadata to be sent with the existing metadata from the Platform to Zendesk. The additional metadata can also be used to create a Zendesk ticket.
...
For example: When you provide `SessionKey` as **key** and `apikeyvalue` as **value**, Kore.ai generates the field variable `SessionKey.apikeyvalue` to fetch the **session API key** and pass it to Zendesk as metadata.

**Important Information on Session Closure Metadata**

* The Platform doesn't trigger the session closure message when the Agent transfer is initiated, and the conversation isn't handed over back to the platform again.
...
* For all the new Apps or existing apps that don't have at least one **SessionClosure** metadata tag, the **SessionClosure** switch is turned off by default.
* For all the existing apps that have at least one **SessionClosure** metadata tag, the switch is turned on by default.
*  The Platform introduces the [KoreUtil.getSunshineConversationsMetadata](/ai-for-service/apis/automation/koreutil-libraries#koreutil-closeconversationsession) function to let a App developer fetch the stored metadata (Get API operation), and use it for the following:
...
* Any additional changes or updates to the channel configurations should go through the [Publishing your App](/ai-for-service/deployment/publishing-app).

**Selecting the Default and Next Integrations for Transferring Control**

When you have a pre-existing switchboard integration, the Platform allows you to configure it as the default for the initial pass control during conversation initiation. Alternatively, you can select the Kore switchboard integration to designate the pass control.
...
3. Paste these values in their respective fields on the Platform under **Sunshine Conversations** > **Configurations** > **OAuth** > **Enable Agent Handoff** section.
    ![alt_text](/ai-for-service/channels/images/sunshine-(1).png)
...
4. Click **Configure** for Switchboard Integration to set up the integrations available in your Sunshine Conversations account app.
    ![alt_text](/ai-for-service/channels/images/sunshine-(5).png)
...
5. Select the required option for the following integrations in the **Switchboard Configuration** window:
    ...
    3. **Next Switchboard Integration to Agent**: Select the next switchboard integration to which control is transferred after an agent conversation with a user.
        ...
        ![alt_text](/ai-for-service/channels/images/sunshine-(3).png)
...
7. On the **Zendesk Sunshine Configurations** window, follow either step below to set **Session Closure Metadata**:
    * Disable the toggle switch if you don’t want the Platform to pass the session metadata to Zendesk.
        ![alt_text](/ai-for-service/channels/images/sunshine-(4).png)
    * Enable **SessionClosure Metadata**, and do one of the following:
        * Provide at least one **Key** and **Value** input (optional) to allow Kore.ai to capture the session metadata using the switchboard integration API and share it with Zendesk. Adding these parameters and saving the configuration will create a Zendesk ticket with the conversation details and close it.
        ![alt_text](/ai-for-service/channels/images/sunshine-(2).png)
...
8. Select **Yes** for **Enable Channel**.
    ![alt_text](/ai-for-service/channels/images/sunshine-(6).png)
9. Click **Save**.

## Templates Support

Sunshine Conversations templates are pre-defined structures you can use to send rich messages to users for consistent and engaging interactions on the Platform. Templates help create a set of “canned messages” or “agent saved replies,” which is especially useful for messages that are difficult to craft on the fly because they require external resources.
...
* **Compound Message**: Combines multiple elements like text, images, buttons, and quick replies to create an engaging and informative message. [See example](https://docs.smooch.io/guide/template-messages/#compound-message).
...
* **Carousel Message**: Allows you to send a series of cards that users can swipe through. Each card can contain an image, title, subtitle, and buttons for further actions. [See example](https://docs.smooch.io/guide/template-messages/#carousel-message) .
...
* **File Message**: Allows you to send a file to the user. This can be useful for sharing documents, PDFs, images, or other types of files. [See example](https://docs.smooch.io/guide/template-messages/#file-message) .
...
* **Form Message**: Allows you to collect information from users through a structured form. This can include text fields, dropdown menus, radio buttons, and more. [See example](https://docs.smooch.io/guide/template-messages/#form-message) .
...
* **Quick Reply**: Provides users with predefined response options that they can select with a single tap. This can streamline interactions and guide users towards specific actions. [See example](https://docs.smooch.io/guide/template-messages/#quick-reply) .
...
* **Location Request**: Allows you to request the user’s location. This can be useful for services that require location information, such as finding the nearest store or providing location-based services. [See example](https://docs.smooch.io/guide/template-messages/#location-request) .
