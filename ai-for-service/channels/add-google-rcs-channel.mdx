...
sidebarTitle: RCS Business Messaging
noindex: true
hidden: true
---
...
2. publish the app for the business to initiate the testing process.
...
1. From the Platform, open the assistant that you want to enable Google RCS Business Messaging.
2. Navigate to **Channels & Flows** > **Channels** > **Digital** > **All** > **RCS Business Messaging**.
3. Select the **Configurations** tab.
4. Set **Request Access** to **Yes**.
5. Click **Save** button.
...
**Create an App and publish the app for the business to initiate the testing process** A Bot Agent (on Google RCS) will be created by the Kore.ai support team based on inputs received from you. You will be notified of its creation. Follow the below steps to publish the app:

1. **Create App, Enable Event and publish the app** - This is required for the platform to receive a notification when a new user opts into talking to the bot via Google RCS.
    * **Create an App** on the Platform **App Settings** > **Dev Tools** > **API Scopes** > **New**  platform from **API Extensions** > **API Scopes** and enable the following scope: “_RCS Message Events_”.
    ![create rcs app](/ai-for-service/channels/images/rcs.png)
    ...
    * From **Natural Language** > **Default Conversation** > **Event Handlers**, configure the following events:
        ...
        * RCS Opt-Out: action to be taken when the user Opts-out of the RCS Messaging.
        ![rcs messaging](/ai-for-service/channels/images/rcs1.png)

    * **Publish** the channel.
2. Trigger opt-in and/or broadcast requests:
    * **Trigger opt-in**: Every time a new user opts in to use Google RCS, platform needs to be informed about this new user. To inform the platform, call the **msgevents API** with the below details: Method: `POST` Request Headers: `auth :<JWT>` Request Payload:
    ...
    * **Broadcast Request**: To send alert or broadcast messages for an already opted-in user, you can call the **msgevents API** with the below details: Method: `POST` Request Headers: `auth : <JWT>` Request Payload: \
        ```
        `{ \
        "event" : "broadcast", \
            "eventDetails":{ \
...
5. The **RCS Business Messaging OptOut** event will be invoked when the user sends the message as configured in the RCS Opt-Out event. The platform will notify the business that the user has Opted out of receiving messages through the webhook URL (The URL is captured in Step 1). Any request to send messages to the users that have opted-out will be rejected. After having opted out, if the user continues conversing with the bot agent (without explicitly opting in again) standard message will be triggered. The message can be managed from the Events section in the Platform.
...
* When user opts out of receiving notifications:
    ...
    ```
* When a device that doesn't support RCS (OR) the bot agent isn't launched receives an `opt-in` or `broadcast` request:
    ```
    `{"event":"RCS_NOT_SUPPORTED_IN_TARGET_DEVICE",
    "userIdentity":"<Mobile number with country code>",

    "message":"This operation is blocked because the RBM agent hasn't launched and the recipient hasn't been invited and accepted the invitation to become a tester."
    }`
    ```
...
* After having opted out, if the user continues conversing with the bot agent (without explicitly opting in again) standard message will be triggered. The message can be managed from the Events section in the Platform.

## Launch the Bot Agent

After having verified your bot agent, you can launch for end users by following the steps below:

1. On the Platform, from the **Channels & Flows** > **Channels** > **Digital** > **All** > open the **RCS Business Messaging** page.
