...
7. Next, to add Infobip’s **Base URL**, follow these steps:
    ...
    * On the **WhatsApp Business Messaging** panel, select the **Configurations** tab and provide inputs for the following fields:
        ...
        * **Infobip API**: Select the type of Infobip API used for communication purposes. Available options include **WhatsApp Outbound API** and **Omni Failover API**.
        * **Channel-based Sessions**: Enable this toggle to treat each inbound number as an independent channel. Conversations from the same number continue in the existing session, while those from a different number start a new session. Available only for Infobip. Enabling this for existing apps may disrupt ongoing sessions if inbound numbers are already linked to experience flows.
        ...
        * Within the Accounts section, enable and configure Infobip account details (the platform supports the addition of up to 10 Infobip accounts) as follows:
          * **Name**: The name of the Infobip account.
          * **Phone Number**: Copy and paste the number displayed on the **Infobip** portal for your account under the **Connect** section (please refer to step 3). The Platform now supports **adding multiple phone numbers on the WhatsApp Messaging Channel** for the same AI Agent. The AI Agent responds to only the source number from which it receives the message rather than a common phone number. [Learn more](/ai-for-service/channels/whatsapp-support-for-multiple-phone-numbers).
...
2. [Configure the Channel Enablement](#step-2-configure-the-channel-enablement).
    ...
    * [Configure the Callback URL/Webhook URL](#step-2c-configure-the-callback-or-webhook-url).
...
7. Once the platform verfies the account with your **Facebook Business ID**, it displays **WABA ID**, **Mobile number**, and **Password**.
...
### Step 2c: Configure the Callback or Webhook URL
...
Once you enable **Associate an App**, you must manually select an app and configure it for the GupShup app (by clicking **Copy** and pasting the **Client ID** and **Client Secret** (shown above) from the Platform).

<Note>This channel configuration isn't a part of the Import/Export feature. The user should manually enable the channel for the target App. The integration flow remains the same even with the App Association feature.</Note>
...
To integrate **Karix** to your WhatsApp Business Messaging channel and enable it for an app, you’ll need to perform the following setup under the **Configuration** tab.

<Note>To complete this procedure, you must already have a Karix account. This is one of the messaging provider for enabling WhatsApp Business Messaging.</Note>
### Prerequisites
...
1. Gather the following details from the **Karix support team**.
    ...
    * Account Key
    * From
2. Copy the Webhook URL from the **Platform** and share it with the **Karix support team**. The Karix support team will need to configure this URL on their platform for a successful handshake between Platform and Karix.
...
1. Log in to the [Meta Developer Portal](https://developers.facebook.com/)  and select **Apps**.
![App-button](/ai-for-service/channels/images/App-button.png)

2. Now, select **Create App** to create a new app or select an existing app.
![create-app](/ai-for-service/channels/images/create-app.png)

3. Enter the details in the following tabs as per your business needs and select **Next** to create a WhatsApp Business Account (WABA):
    - App details
    - Use cases
    - Business
    - Requirements
...
4. On the app in the app dashboard, select your created app.
...
4. Select Add.
5. Name the system user, choose Admin as the user role, and select Create System User.
...
4. Find and Copy the **App ID** and **App Secret ID** and paste it as required in the **Step 4**
    ...
    <Note>    Ensure that you enter details in all required columns. Without all the required fields, your app won't be approved by Meta.</Note>
    ![App-settings-basic](/ai-for-service/channels/images/App-settings-basic.png)

#### Step 3: Configure the Callback URL

1. Select **Add product** under the Products section on the left navigation bar.

    <Note>The **Facebook Login for Business** isn't available as default. Therefore, we recommend that you search for it to add under the My products section. Ignore, if it's already available.</Note>
    ![Products-add-products](/ai-for-service/channels/images/products-add-products.png)

2. Now, select the **Settings** tab of Facebook Login for Business.
![Facebook-settings](/ai-for-service/channels/images/facebook-settings2.png)
...
3. On the Settings page, enter the Valid OAuth Redirect URLs from the **Platform Configuration Page** as follows:
    ...
    <Note>      We recommend you verify the Callback URL using the Redirect URL Validator to ensure the accuracy of the path URL.   ![Validauth-redirecturl](/ai-for-service/channels/images/Validauth-redirecturl.png)</Note>

#### Step 4: Select Authorization Type

Enter the Phone Number IDs and select the connection Authorization Type.

Channel-based Sessions: Channel-based Sessions scope conversation sessions by the inbound Phone Number ID.

| State       | Session Behavior                                                                                                                                                                                                                              |
| :----------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Enabled            | Sessions are scoped by inbound Phone Number ID. A new session starts whenever an end-user contacts a different inbound number, even if a session with another number is already active. Messaging the same inbound number continues the existing session. |
| Disabled (default) | Sessions follow the standard platform behavior based on the user's identity. The inbound Phone Number ID doesn't influence session scoping.                                                                                                   |

<Note>Enabling Channel-based Sessions for an existing app that already has active sessions may disrupt those sessions if inbound numbers are already linked to experience flows.</Note>
...
![Temporary Authorization](/ai-for-service/channels/images/temporary-token.png)
...
![Permanent Authorization](/ai-for-service/channels/images/permanent-tokens.png)

#### Step 5: Verify the Webhook URL and Token

1. From the **Platform App builder Configuration Page**, copy the **URL** and **Token**.

    <Note>    We recommend you verify the Token and Callback URL by publishing your application. This ensures your token and URL is valid.  </Note>
...
<Note>We recommend you to select the latest version of the messaging app while subscribing</Note>
