---
title: Generating DKIM keys using OpenSSL
sidebarTitle: Generating DKIM keys using OpenSSL
noindex: true
---
...
<Note>Keep your `dkim_private.pem` file secure. This key should only be accessible by your mail server or the service responsible for signing your outgoing emails. Never share it publicly.</Note>
...
*Alternatively, you can manually open `dkim_public.pem` in a text editor and copy the long string between `-----BEGIN PUBLIC KEY-----` and `-----END PUBLIC KEY-----`.*
...
**Hostname/Name**: `your_selector._domainkey.example.com` (Replace `your_selector` with a unique identifier, for example, `default`, `mail`, `2025q3`. Replace `example.com` with your actual domain.)
