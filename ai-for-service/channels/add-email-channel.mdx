...
Generate and Configure DKIM for this domain by providing your own private key. The Private key must use 1024-bit to 2048-bit RSA encryption, and be PEM-encoded. Refer[ Generating DKIM keys using OpenSSL](generate-dkim).
...
Whenever the customer responds to the email thread, it's directed back to the customer contact center domain. From there, it is seamlessly forwarded to the Kore domain, ensuring a continuous and efficient communication loop.
...
2. You can add email addresses manually or using bulk upload.
    ...
    2. **Bulk Upload**:
        ...
        4. On completion, map the CSV file fields with system fields.
            ...
            <Note>            If the CSV file contains only one column, field mapping is not required and will not be displayed.</Note>
        5. Click **Validate**.
