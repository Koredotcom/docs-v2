...
* Standard Responses
* Attachments
* Drafts [Learn more](/ai-for-service/contact-center/agent-console#email-interactions)

You can use the Kore.ai Email domain or create your custom Email Domain to configure your email addresses.

To configure the email domains, navigate to **Flows & Channels** > **Channels** > **Digital** > **All** > **Email**.
![Email Icon](/ai-for-service/channels/images/email-digital-page.png)

Configure DKIM and session settings under More Options. 
![Email Settings](/ai-for-service/channels/images/email-settings.png)

## Session Identity and Grouping

### User Identify Source

**Reply-To for User Identity**: When enabled, the platform uses the Reply-To address for user and session identification rather than the From address. This is useful when multiple emails share the sender address but have different Reply-To values. This toggle is off by default.

### Session Grouping

**Channel-Based Sessions**: When enabled, the platform creates a new session for each unique **To** email address. Emails sent to the same address continue the existing session, while emails sent to different addresses automatically start new sessions. This configuration helps in keeping issues organized and separate.

**Advanced Grouping**: Advanced Grouping determines how the Email channel associates incoming emails with existing sessions after Channel-Based Sessions are applied. Use Advanced Grouping to control whether email conversations are grouped based on email threading information or email subjects.

Select one of the following options:

* **None**: The system uses only Channel-Based Sessions to determine session continuity. It groups emails sent to the same configured email address according to your channel-based session settings.

* **Thread-Based Sessions**: The system uses email threading headers to identify and continue existing sessions. Use this option if your email clients and mail servers consistently preserve threading information.

    When enabled:

    - The platform creates sessions using email thread reference headers.
    - Replies within the same thread continue the existing session.
    - New threads start fresh sessions.
    - The platform preserves conversation context when users or agents add recipients mid-thread.
    - You must enable Channel-Based Sessions to use this option.

* **Subject-Based Sessions**: The system uses normalized email subjects to identify and continue existing sessions. Use this option when email threading headers are unavailable, modified, or unreliable.

    When enabled: 

    - Emails with the same normalized subject are added to the same session.
    - Replies with common reply or forward prefixes continue the existing session.
    - Changing the subject creates a new session.
    - Emails without a subject always create a new session.


    ![Email Settings](/ai-for-service/channels/images/session-identity-grouping.png)
...
When enabled, the platform checks whether the provided domain has a verified DKIM record. If the domain is verified, the platform applies it and allows the channel setup to continue. If the domain isn't yet verified, the user must complete DKIM verification before proceeding.

### Step 1: Generate DKIM

Generate and Configure DKIM for this domain by providing your own private key. The Private key must use 1024-bit to 2048-bit RSA encryption, and be PEM-encoded. Refer [Generating DKIM keys using OpenSSL](/ai-for-service/channels/generate-dkim).
...
{/* AG: domain example in step 3 below seems incorrect w.r.t brackets and period. */}
...
3. Enter the following details and then click  **Verify Domain**.
    ...
    * **Selector Name**: A DKIM selector is a string that identifies which DKIM public key to use for signature verification. It's part of the DKIM DNS record structure.
        ...
        **DNS Record Format**: `[selector]._domainkey.[[domain.com](http://example.com)]`
        
        **Example**: If your selector is "ses2024" and the domain is "example.com," the DNS record would be: `ses2024._domainkey[.example.com](http://.example.com)`
        
        ![Email Settings](/ai-for-service/channels/images/new-dkim.png)

4. The Verification in Progress message is displayed. 
 ![Email Settings](/ai-for-service/channels/images/msexchange2.png)
...
5. Click **Confirm**. You can navigate to the DKIM Configuration page to check the latest status. This might take up to 72 hours. The status can be one of the following:
    ...
    * **Temporary Failure**: DNS issues, will retry automatically.  
    ![Email Settings](/ai-for-service/channels/images/msexchange3.png)
...
Whenever the customer responds to the email thread, it's directed back to the customer contact center domain. From there, it's seamlessly forwarded to the Kore domain, ensuring a continuous and efficient communication loop.
![Email Flow](/ai-for-service/channels/images/email-flow.png)

### Create a Kore Domain Email Address

Steps to create an email address in the Kore domain:

1. The Kore.ai domain is created by default. Click the **+ New Email ID** to add the email ID.
...
2. On the Email Configuration tab, enter the following details.
    ...
    5. Under the Enable Channel section, select **Yes** to enable the channel.
        ![Email Configuration](/ai-for-service/channels/images/kore-email-configuration.png)
    ...
    6. Turn on the **Email Template Design (Optional)** toggle to configure the header and footer of the emails that are sent via the configured email address, and do the following:
        1. Click **Edit Header** and **Edit Footer** to configure the header and footer details.
            ![Email Template Design](/ai-for-service/channels/images/email-template-design.png)
    ...
    7. Click **Preview** to see a preview of the Email template.
        ![Email Template Preview](/ai-for-service/channels/images/email-template-preview.png)

3. Click **Save.**
4. A success message is displayed and the email address is created.
    ![Email Address Created](/ai-for-service/channels/images/kore-email-created.png)

    <Note>The option to link a flow directly from the Email Channels list is excluded. Users would need to return to the Experience Flows section, locate the linked flow, and publish it again for any changes to take effect. This option is omitted to maintain a consistent user experience and avoid unnecessary steps.</Note>
...
1. Go to **Deploy** > **Deploy Management** > **Publish**.
    ![Publish Page](/ai-for-service/channels/images/publish-page.png)

2. Select the email address under channels and click **Proceed**.
    ![Select Email](/ai-for-service/channels/images/select-email.png)

3. Enter the Comments and click **Confirm**.
    ![Comments](/ai-for-service/channels/images/comments-publish.png)

4. A success message is displayed and the email address is published.
    ![Email Address Published](/ai-for-service/channels/images/email-published.png)

## Custom Email Domain

Custom email domains add a professional look to your email communications, reinforcing your brand identity with every email you send or receive. Customers perceive custom emails as established, reliable, and legitimate compared to generic email addresses. For example, You can create [help@yourbusiness.com](mailto:help@yourbusiness.com), a branded email account that uses your organization's domain name instead of a domain provided platform. You can add up to three email accounts such as sales@, support@ and info@ in each custom domain for better organization and delegation. By granting access to your email accounts within the domain, you can track the activity of the configured accounts, respond to incoming emails, and send outbound emails. Email filters are less likely to flag emails from custom domains as spam, ensuring better deliverability rates.
...
2. On the email configuration tab, enter the following details.
    ...
    - Under the Enable Channel section, select **Yes** to enable the channel.
        ![Email Configuration](/ai-for-service/channels/images/custom-email-config.png)

    - Turn on the **Email Template Design (Optional)** toggle to configure the header and footer of the emails that are sent via the configured email address, and do the following:
        1. Click **Edit Header** and **Edit Footer** to configure the header and footer details.
            ![Email Template Design](/ai-for-service/channels/images/email-template-design.png)

        2. In the **Edit** window, change the header and footer details, and click **Save**.
            ![Edit Header-Footer](/ai-for-service/channels/images/edit-window-change-header-footer.png)

            ![Save Header-Footer](/ai-for-service/channels/images/edit-window-click save.png)

    - Click **Preview** to see a preview of the Email template.
        ![Template Preview](/ai-for-service/channels/images/custom-template-preview.png)

3. Click the **Test Connection** to verify the domain ownership. A success message is displayed.
    ![Test Connection](/ai-for-service/channels/images/test-connection.png)

4. Click **Save**.

#### Step 3: Email Blocklist

Email blocklisting lets admins to specify a list of blocklisted email addresses during email address creation preventing blocking interactions originating from blocklisted email addresses, automations, and subsequent agent transfers are prevented, reducing unnecessary processing, and ensuring that resources are allocated to valid interactions.
...
* If a match is found between the incoming email address and the blocklist, any pre-configured automations linked to the receiving email address will be disabled for that specific interaction. Automated agent transfers triggered by the interaction will be prevented, ensuring that blocklisted emails don't initiate further actions within the system.
...
1. Click the **Email Blocklist** tab and turn on the **Enable Email Blocklist** toggle.
    ![Email Blocklist Tab](/ai-for-service/channels/images/email-blocklist-tab.png)
...
2. You can add email addresses manually or using bulk upload.
    1. **Manual entry**:
        1. Enter the email address in the search dialog box and click **Add**.
            ![Manual Entry](/ai-for-service/channels/images/manual-blocklist-upload.png)

            <Note>The search box can be used to find existing email addresses or to add a new one.</Note>
        2. The email address is updated successfully.
            ![Manual Entry Updated](/ai-for-service/channels/images/manual-blocklist-updated.png)

    2. **Bulk Upload**:
        1. Click **Bulk Upload**.
            ![Bulk Upload Button](/ai-for-service/channels/images/bulk-upload-button.png)
        ...
        2. A pop-up window is displayed. Click **Select a CSV file to Upload** and upload the CSV file. You can download a sample CSV file by clicking **Download sample CSV file**.
            ![Select CSV File](/ai-for-service/channels/images/select-csv.png)

        3. The upload progress is displayed.
            ![Upload Progress](/ai-for-service/channels/images/upload-progress.png)

        4. On completion, map the CSV file fields with system fields.
            ![Map Fields](/ai-for-service/channels/images/map-fields.png)

            <Note>If the CSV file contains only one column, field mapping isn't required and won't be displayed.</Note>
        5. Click **Validate**.
            ![Validate](/ai-for-service/channels/images/validate.png)

        6. The details of the mapped contacts, duplicates, and errors (if any) are displayed. Click **Save**.
            ![Save Button](/ai-for-service/channels/images/save-mapped-fields.png)

        7. A success message is displayed and the email blocklist is updated.
            ![Bulk Blocklist Updated](/ai-for-service/channels/images/bulk-upload-button.png)
...
3. Enter a name for the app (for example, `KoreAI Email Integration`).
...
4. Add the following permissions:
    ...
    * `User.Read.All`
    ...
    ![Microsoft Exchange](/ai-for-service/channels/images/msexchange4.png)
...
5. Click **Add**.
6. **Important**: Copy the **Value** of the client secret immediately and store it securely. This value won't be displayed again once you navigate away.
...
3. Review the instructions and click **Next** to proceed to the **Configuration** tab.  
![Microsoft Exchange](/ai-for-service/channels/images/ms-configuration.png)
...
* Microsoft Graph Explorer:[Microsoft Graph Explorer](https://developer.microsoft.com/en-us/graph/graph-explorer)
