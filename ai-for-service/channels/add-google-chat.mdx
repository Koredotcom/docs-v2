---
title: Adding the Google Chat Channel
sidebarTitle: Adding the Google Chat Channel
noindex: true
---
...
A JSON file will be generated and you need to copy the **client_email**, and **private_key** that needs to be input into the Platform. .
...
3. (Optional) Enable **Dynamic Search (Typeahead)** to provide relevant, real-time suggestions from external systems as users type. Specify the custom URL and configure authentication. The platform forwards the search term to the configured endpoint and displays the returned suggestions. 
4. Select **Yes** and select **Save** to enable Google Chat channel.
![save channel](/ai-for-service/channels/images/google-chat.png)
