---
title: Voice Gateway Deployment and Operations
sidebarTitle: Deployment and Operations
---
...
| - | `52.220.160.153` | `10.155.5.10` | - | `https://sg-prod-korevg-sbc1.kore.ai` |
| `sg-prd-korevg-siprtp-2` | `13.215.50.35` | `10.155.6.230` | ap-southeast-1b | - |
| - | `18.136.127.58` | `10.155.6.210` | - | `https://sg-prod-korevg-sbc2.kore.ai` |
| `sg-prd-korevg-web-server-blue-1` | - | `10.155.77.112` | ap-southeast-1a | `https://sg-prod-korevg-webserver.kore.ai` |
| `sg-prd-korevg-web-server-blue-2` | - | - | ap-southeast-1b | `https://sg-prod-korevg-grafana.kore.ai` |
| `sg-prd-korevg-monitoring-server-blue` | - | `10.155.77.194` | ap-southeast-1a | `https://sg-prod-korevg-homer.kore.ai` |
| - | - | - | - | `https://sg-prod-korevg-jaeger.kore.ai` |
| `sg-prd-korevg-recorder-blue-1` | - | `10.155.82.177` | ap-southeast-1b | `https://sg-prod-korevg-rec1.kore.ai` |
| `sg-prd-korevg-recorder-blue-2` | - | `10.155.81.222` | ap-southeast-1a | `https://sg-prod-korevg-rec2.kore.ai` |
...
<Note>To ensure successful usage of our Cloud, make sure to configure the necessary ports and services on your company firewall. Complete details can be found in the "Firewall Requirements" section within the above-provided link.</Note>
...
* What's the inbound protocol the SIP provider is using (that's, UDP / TCP / TLS)?
...
* Will you let and bypass traffic for `*.kore.ai` and `*.twilio.com` domains (HTTPS) on 443, 8443, 3478, and 5349 ports?
    - [ ] Yes
    - [ ] No

* Per the [firewall requirements](/ai-for-service/channels/voice-gateway/minimum-system-requirements-and-supported-browsers#firewall-requirements), will you be able to allowlist and let the traffic to all IP addresses and domains (SBCs, STUN/TURN, etc.)?
    - [ ] Yes
    - [ ] No

* As per the [firewall requirements](/ai-for-service/channels/voice-gateway/minimum-system-requirements-and-supported-browsers#firewall-requirements), will you be able to let all mentioned UDP ports in a bi-directional way?
    - [ ] Yes
    - [ ] No
...
![Call Flow Diagram](/ai-for-service/channels/voice-gateway/images/call-flow-diagram.png)

### Agent Desktop Architecture (Client Inside Firewall-Asymmetric NAT)

![Agent Desktop Architecture-Asymmetric](/ai-for-service/channels/voice-gateway/images/agent-desktop-architecture.png)

### Agent Desktop Architecture (Client Inside Firewall-Symmetric NAT)

![Agent Desktop Architecture-Symmetric](/ai-for-service/channels/voice-gateway/images/agent-desktop-architecture-symmetric.png)

### Firewall Requirements

* [AU Region](/ai-for-service/channels/voice-gateway/au-firewall-rules)
* [DE Region](/ai-for-service/channels/voice-gateway/germany-firewall-rules)
* [EU Region](/ai-for-service/channels/voice-gateway/eu-firewall-rules)
* [IN Region](/ai-for-service/channels/voice-gateway/in-firewall-rules)
* [JP Region](/ai-for-service/channels/voice-gateway/japan-firewall-rules)
* [UAE Region](/ai-for-service/channels/voice-gateway/uae-firewall-rules)
* [US Region](/ai-for-service/channels/voice-gateway/us-firewall-rules)
* [UK Region](/ai-for-service/channels/voice-gateway/uk-firewall-rules)
...
* Ensure the network strength meets our defined parameters. See [Minimum System Requirements and Supported Browsers](/ai-for-service/channels/voice-gateway/minimum-system-requirements-and-supported-browsers) for reference.
* After completing these steps, run [User Diagnostics](/ai-for-service/contact-center/agent-console#user-diagnostics).
...
| If your firewall supports the pinhole feature, does it support the following modules? <br/> **NAT hole punching** <br/> **NAT traversal** <br/> **TCP hole punching** <br/> **UDP hole punching** | **NAT hole punching:** Without support, the system can't establish a direct connection between peers when one or both are behind firewalls or NAT-enabled routers. <br/> **NAT traversal:** Without support, devices behind a NAT can't communicate with external devices. NAT traversal enables direct communication without port forwarding or manual configuration. <br/> **TCP hole punching:** Without support, the system can't establish TCP connections between peers using NAT traversal. <br/> **UDP hole punching:** Without support, the system can't establish bidirectional UDP connections between peers using NAT traversal. |
...
| Can you allow and let traffic to all required IP addresses and domains (SBCs, STUN/TURN, and others) as specified in the firewall requirements? | Without allowlisting, the system can't establish connections, perform NAT traversal, or transmit IP packets between peers. |
