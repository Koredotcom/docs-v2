---
title: Voice Gateway Basic Configuration
sidebarTitle: Voice Gateway Basic Configuration
---
<Badge icon="arrow-left" color="gray">[Back to Voice Channels](/ai-for-service/channels/#voice-channels)</Badge>

Voice Gateway is a comprehensive voice automation solution that manages inbound call automation for Contact Center AI. It integrates with existing voice systems or uses the native voice processing capabilities, enabling seamless transitions between automated and human interactions within the Platform.
...
2. Configure an available number with the following steps:
    ...
    3. Select an App for outbound calls from the dropdown. AgentAI triggers this app when the agent makes an outbound call with this phone number.
        ![Forward To Phone Number](/ai-for-service/channels/voice-gateway/images/phone-number.png)
...
<Note>This feature is only available when using our Twilio account. It's not supported for SIP trunk setups.</Note>
...
1. Contact support and specify the required country or region.
...
1. The number appears in the SIP DID numbers list-[Create Start Flows](/ai-for-service/flows/create-and-manage-flows#create-a-start-flow) and associate them with the toll-free number.
...
<Note>Lead times vary by country and depend on local regulatory approval timelines.</Note>
...
2. Select a **Start Flow** you want to add to this number and select **Done**. You can add a start flow by clicking **+ New Start Flow**. [Learn more](/ai-for-service/flows/create-and-manage-flows#create-a-start-flow).
...
3. Alternatively, select **Update**, go to the **Forward to Phone Number** window, and select **Remove**. A success confirmation message appears.
    ...
    <Note>The phone number cannot be deleted if there are active voice campaigns or flows attached to the channel.</Note>
...
2. On the **Configure SIP Trunk** page, configure the following:
    1. **Product Selection**: Select the product for SIP Trunk configuration. You can select from the following options:
        ...
        - **Network**: To configure the Network, you can select one of the following:
            ...
            ![SIP Trunk Setup](/ai-for-service/channels/voice-gateway/images/sip-uri.png)

            - Under **Domain Name**, provide the following:
                - **Fully Qualified Domain Name (FQDN)**: The domain name specifies all domain levels, including the top-level domain and the root zone. [Learn more](/ai-for-service/channels/voice-gateway/deployment-and-operations#ips-ports-and-protocols).
                ...
                ![DNS Resolve Method](/ai-for-service/channels/voice-gateway/images/dns-resolve-method.png)
            ...
            ![MS Teams](/ai-for-service/channels/voice-gateway/images/ms-teams.png)
        ...
        - Enter the **SIP Headers**. You can include all available agent data in the SIP headers, enabling customers to use only the parameters relevant to their needs. The system lists the parameters as key-value pairs:
            ...
            <Note> The caller number specified in the [Script Task](/ai-for-service/flows/node-types/script-task) is passed through the SIP headers when a third-party desktop application transfers the call to an agent.</Note>
        
        If you select Agent AI:

        **Select the SIP Trunk connection method**: Select the method based on your third-party vendor's requirements ([SIPREC](#siprec) or [WebSocket](#websocket)).
        ...
        - If you select SIPREC:
            ...
            - **Record the calls**: Enable or disable call recordings for third-party Agent Desktop integrations. Access these recorded calls using a [public API](/ai-for-service/apis/contact-center/api-list#call-recording-apis).
            - (Optional) By default, the system selects **E.164 Syntax**. Selecting this option prepends a + before the DID number.
            ...
            - **Call control parameters**: Define the parameters to control the call behavior. Select **Add Parameter**, enter the **Parameter Name** and **Value**, and select **Save**. [Learn more](/ai-for-service/channels/voice-gateway/speech-customization#introduction-to-call-control-parameters).
            - Enter the **SIP Headers**. You can include all available agent data in the SIP headers, enabling customers to use only the parameters relevant to their needs. The system lists the parameters as key-value pairs:
                - X-AgentName: `{{agentName}}`
                - X-AgentPhoneNumber: `{{agentPhoneNumber}}`
                - X-AgentEmailID: `{{agentEmailId}}`
                ...
                - X-AgentFirstName: `{{agentFirstName}}`
                - X-AgentLastName: `{{agentLastName}}`
                ![agentai-siprec-selection](/ai-for-service/channels/voice-gateway/images/agentai-siprec-selection.png)
        ...
        - If you select **WebSocket**:
            ...
            - **Record the calls**: Enable or disable call recordings for third-party Agent Desktop integrations. Access these recorded calls can using a [public API](/ai-for-service/apis/contact-center/api-list#call-recording-apis).
            - **Call control parameters**: Define the parameters to control the call behavior. Select **Add Parameter**, enter the **Parameter Name** and **Value**, and select **Save**. [Learn more](/ai-for-service/channels/voice-gateway/speech-customization#introduction-to-call-control-parameters).
...
2. Select a **Start Flow** to add to individual numbers and select **Done**. You can add a start flow by selecting **+ New Start Flow**. [Learn more](/ai-for-service/flows/create-and-manage-flows#create-a-start-flow).

3. The attached flows appear. A pie icon appears under the attached flows. Hovering over the pie icon displays **Configured**.

<Note>You cannot attach channels to the Default Welcome Voice flow or Default Welcome Chat flow. Channels already attached to these flows continue to function as configured. However, if a channel is reattached to a different flow, it cannot be reattached to a Default Welcome Voice flow or Default Welcome Chat flow.</Note>
...
![Bot no Input](/ai-for-service/channels/voice-gateway/images/bot-no-input.png)
...
This section outlines the steps to configure Automatic Speech Recognition (ASR) and Text-to-Speech (TTS) for your Voice Gateway. You can configure the voice preferences to personalize the ASR Engine and the voice that plays for your TTS conversions by going to the Voice Preferences tab and selecting **Manage**.
    ![Voice Preferences](/ai-for-service/channels/voice-gateway/images/voice-preference.png)
...
2. Select **Done** once you have completed configuring your voice preferences. The set voice, language, and dialect apply to automated customer responses that use text-to-speech.
    ...
    ![Voice Preferences Configuration](/ai-for-service/channels/voice-gateway/images/config-voice-preferences.png)

<Note>If a “TTS ERROR” appears when you dial a Voice Call and execute the Start Flow attached to an automation, the issue might occur because the voice name is not properly selected in the configuration. Even if you select a voice and save the changes, the system might not register the selection. Reselect the voice and make a small edit in the flow description (for example, add or remove a character). This action prompts the system to recognize the change and ensures that the TTS functions correctly and the audio plays as expected.</Note>
...
<Note>This feature is available in SmartAssist only.</Note> 

### Supported ASR, TTS, and Voice Biometrics Providers

Voice Gateway supports integration with third party ASR, TTS, and Voice Biometrics providers. [Learn more](/ai-for-service/channels/voice-gateway/third-party-asr-tts-support)
...
1. Contact support and request a Custom Provider Configuration.
...
1. Create a new Flow.
2. After the Start node, drag an [IVR Menu](/ai-for-service/flows/node-types/ivr-digit-input) node.
3. In the IVR Menu node, create prompts for each language option (for example, "Press 1 for English," "Press 2 for Hindi," "Press 3 for Telugu").
    ![IVR Menu](/ai-for-service/channels/voice-gateway/images/ivr-menu.png)

4. For each language option, connect the number key (for example, "1") to a new [Script node](/ai-for-service/flows/node-types/script-task). This is the key step where the language is set.
    ![IVR Digit Input](/ai-for-service/channels/voice-gateway/images/ivr-digit.png)
...
5. For each Script node:
    ...
    ![Script Node](/ai-for-service/channels/voice-gateway/images/script-node.png)

<Note>Use two-letter language codes in **lowercase**. For example, use "en", not "EN",  "hi", not "HI", </Note>
...
7. Inside the Run Automation node:
    ...
    - Choose the Dialog Flow you've built for your AI Agent.
        ![Run Specific Dialog](/ai-for-service/channels/voice-gateway/images/run-specific-dialog.png)

    - Configure 'Agent Transfer' under 'Connection Rules'.
    - Reference configuration of the entire flow.
        ![Reference Config](/ai-for-service/channels/voice-gateway/images/reference-config.png)
...
2. The platform enables you to configure different languages within the same flow. Look for a **language selector** on the app header.
    ![Language Selector](/ai-for-service/channels/voice-gateway/images/language-selector.png)
...
4. For each node (like a **Message** node or **Entity** node), enter the text in the selected language.
    ...
    **Example:** For a Message node, if you've selected Hindi, you'll enter the Hindi text in the **Bot Response** box.
    ![Message Node](/ai-for-service/channels/voice-gateway/images/message-node.png)
...
2. You can set specific call control parameters that override the default settings. This is useful for:
    ...
    - Using a different TTS provider or voice in a particular language.
        ![TTS Provider](/ai-for-service/channels/voice-gateway/images/tts-provider.png)

    - Choosing a different ASR provider that's better at understanding a particular accent or language.
        ![ASR Provider](/ai-for-service/channels/voice-gateway/images/asr-provider.png)

For more details on these advanced settings, refer to the [Call Control Parameters](/ai-for-service/channels/voice-gateway/speech-customization#supported-call-control-parameters).
...
<Note>Double-check that the language codes are in lowercase in the script node and that you've configured both the IVR and the Run Automation nodes correctly.</Note>
...
* Channel level-Applies the v3 model globally to all calls on the channel. See [Channel Level Call Control](/ai-for-service/channels/voice-gateway/speech-customization#channel-level-call-control).
* Node level-Applies the v3 model at a specific point in the conversation flow (for example, in a Script Node or Script Task). See [Node Level Call Control](/ai-for-service/channels/voice-gateway/speech-customization#node-level-call-control).
...
<Note> Audio tags (for example, `[excited]`, `[whispers]`) are only supported with the `eleven_v3` model. Ensure the model is set before using audio tags in your prompts. For more details on `ttsOptions`, see [Structure of ttsOption` > ElevenLabs](/ai-for-service/channels/voice-gateway/speech-customization#elevenlabs). </Note>
...
- **Use punctuation intentionally**: The V3 model interprets punctuation as delivery cues. Ellipses (`/ai-for-service/channels/voice-gateway.`) introduce pauses, and capitalization increases emphasis.
...
1. Open the [Dialog Task](/ai-for-service/automation/dialogs/manage-dialogs) and select a [Message Node](/ai-for-service/automation/dialogs/node-types/message-nodes), [Entity Node](/ai-for-service/automation/dialogs/node-types/entity-node), or [Confirmation Node](/ai-for-service/automation/dialogs/node-types/confirmation-nodes).
...
You can configure the voice call properties by going to the Voice Preferences tab and selecting **Configure** on the **Voice Call Properties** section.
    ![Configure Voice Call Properties](/ai-for-service/channels/voice-gateway/images/voice-call-properties.png)
...
- Terminate Call
    ![End of Task Behavior](/ai-for-service/channels/voice-gateway/images/end-of-task-behavior.png)
...
- **Initiate Task**: Select a task from the dropdown menu that initiates on detecting the event.
    ![Initiate Task](/ai-for-service/channels/voice-gateway/images/initiate-task.png)

- **Run Script**: Enter the script that runs on detecting the event.
    ![Run Script](/ai-for-service/channels/voice-gateway/images/run-script.png)

- **Show Message**: Select **+ Add Response**, enter the message that displays on detecting the event, and select **Done**.
    ![Add Response](/ai-for-service/channels/voice-gateway/images/add-response.png)

### Call Termination Handler

Specify the intent (dialog) to handle the call termination event from the dropdown.
    ![Call Termination Handler](/ai-for-service/channels/voice-gateway/images/call-termination-handler.png)
...
Define the maximum number of retries allowed. Select **Save**. A success message appears, and saves the voice call properties.

## System Error Settings

Configure how the system handles a call when a system error occurs (no response from the bot), such as ending the call with a message or transferring it to an agent via SIP with optional headers.

**End the Call**

1. Select **End the Call** and enter the message to play before the call ends.
2. Select **Save**

**Transfer the Call**

1. Select **Transfer the Call** and enter the message to play before the call transfer.
2. The SIP Transfer method is pre-filled.
3. Enter the Redirect URL. For example, `sip:123.456.789:5060`.
4. (Optional) Enter the SIP Headers (Key and Value). For example, `X-Reason: SystemError`.
5. Select **Save**.
    ![Call Transfer Settings](/ai-for-service/channels/voice-gateway/images/system-error-settings-call-transfer.png)

### Configure Different “Refer-To” Numbers for Different Experience Flows

In the current bot configuration, System Error Settings apply globally across all experience flows in the bot.

However, in scenarios where different Lines of Business (LOBs) use different experience flows, you can configure system errors to transfer calls to different phone numbers depending on the LOB.

This can be achieved by:

* Passing the Refer-To phone number through a SIP Header in the SIP INVITE request.
* Dynamically reading that header in System Error Settings using the format:

    `{{SIPHEADER.<header_name>}}`

    Example:

    Redirect URL: `{{SIPHEADER.contact}}`

    This dynamically determines the transfer destination based on the header value received in the SIP INVITE.

#### Configuration

**Step 1. Configure System Error Settings**

Navigate to the [System Error Settings](#system-error-settings) configuration in your bot and enter in the following details.
...
The Redirect URL reads the value of the contact header from the SIP INVITE request.

**Step 2. Send Refer-To Number in SIP Invite**

When sending the SIP INVITE request, include a SIP header containing the fallback phone number for that LOB.

Example header:

Contact: `sip:+14085550100@example.com`
...
```
INVITE sip:bot@kore.ai SIP/2.0
Via: SIP/2.0/UDP client.example.com;branch=z9hG4bK776asdhds
Max-Forwards: 70
From: "Customer" <sip:+14085550001@example.com>;tag=1928301774
To: <sip:bot@kore.ai>
Call-ID: a84b4c76e66710@example.com
CSeq: 314159 INVITE
Contact: sip:+14085550100@example.com
User-Agent: Customer-SIP-Client
Content-Type: application/sdp
Content-Length: 142

v=0
o=- 3747 3747 IN IP4 client.example.com
s=Voice Call
...
When a system error occurs, the system:
...
Example LOB Configuration

Different LOBs can send different header values.
...
If a system error occurs in the Insurance experience flow, the call is transferred to:
...
* The Redirect URL must use the format `{{SIPHEADER.<header_name>}}` to read header values dynamically.
* Ensure the correct header name used in the Redirect URL matches the header sent in the SIP INVITE.
</Note>

### Published Mode Restrictions

When the app is in Published mode, configuration-modifying actions across the Voice Gateway module are restricted to prevent unintended changes to a live environment. Restricted actions are disabled or hidden in the UI.

Hovering over a disabled action displays the following tooltip:

**This action is unavailable while the app is in Published mode.**

The table below explains which actions are restricted and what remains accessible.

| Section | Restricted Actions | Available Actions |
|:---|:---|:---|
| Phone Numbers | Buy New Phone Number, Attach Flow | View |
| Phone Numbers — individual number | Update, Delete (hidden) | View |
| SIP Trunk | Configure SIP Trunk, Attach Flow | View |
| SIP Trunk — individual SIP number | Update, Delete (hidden) | View |
| Voice Preferences | Edit/Update (hidden) | View |
| Voice Call Properties | Edit/Update (hidden) | View |
| System Error Settings | Edit/Update (hidden) | View |

To make configuration changes, the app must first be taken out of Published mode.
