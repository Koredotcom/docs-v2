...
Configuring `backchannelWords` signals Voice Gateway to keep the audio playing when a caller speaks one of these words during playback, instead of treating it as a barge-in.

**How it works**

1. Audio playback starts.
1. The caller speaks during playback.
1. Voice Gateway normalizes the detected phrase - converting to lowercase, trimming whitespace, and stripping punctuation - and checks it against the configured list.
1. If the phrase matches a configured backchannel word, audio continues playing and Voice Gateway sends a `speechDetected` event with backchannel metadata. Barge-in doesn't trigger.
1. If the phrase doesn't match, standard barge-in behavior applies.
...
```json
{
  "backchannelWords": ["uh-huh", "okay", "got it", "sure", "yes"]
}
```
