...
Lets the system to begin generating a response before the user fully stops speaking, improving responsiveness and real-time interaction.
...
**Parameter:** `sttProvider`, `deepgramModel`

Use Deepgram Flux to support callers who may switch languages mid-call — for example, an English-speaking agent and a Hindi-speaking customer on the same call, or a bilingual user alternating between Spanish and English. The flux-general-multi model handles recognition for all supported languages within a single uninterrupted ASR session.

**Enable Automatic Mid-Call Language Switching**

**Parameter:** `sttProvider`, `deepgramModel`

Use Deepgram's flux-general-multi model to automatically detect and switch between supported languages during a call without restarting the ASR stream. Supports 10 languages including English, Hindi, Spanish, French, German, Russian, Portuguese, Japanese, Italian, and Dutch.
