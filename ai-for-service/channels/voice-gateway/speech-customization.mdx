...
To configure ASR (Automatic Speech Recognition) and TTS (Text-to-Speech) in a ClickToCall flow, add the call control parameters in the [Script Task](/ai-for-service/flows/node-types/script-task#script-task-node) node at the start of the flow. Set the parameters must in the following format:
