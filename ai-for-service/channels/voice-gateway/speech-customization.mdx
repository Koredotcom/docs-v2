---
title: Speech Customization (Call Control Parameters)
sidebarTitle: Speech Customization (Call Control Parameters)
noindex: true
---
...
<Note>Automatic Speech Recognition (ASR) and Speech-to-Text (STT) are two terms that refer to the same technology. Both involve converting spoken language into written text by analyzing and interpreting audio input. The terms are used interchangeably, describing the same function—transforming speech into readable, actionable text.</Note>
...
The call control section is Available In [Entity Node](/ai-for-service/automation/dialogs/node-types/entity-node)/[Message Node](/ai-for-service/automation/dialogs/node-types/message-nodes#ivr-properties)/[Confirmation Node](/ai-for-service/automation/dialogs/node-types/confirmation-nodes#ivr-properties) > IVR Properties > Advanced Controls. [Learn more](/ai-for-service/automation/agent-flows#configuring-grammar).
![Node Level Call Control](/ai-for-service/channels/voice-gateway/images/node-level-call-control-parameters.png)
...
Voice Gateway supports several third-party service providers for ASR/TTS. [Learn more](/ai-for-service/channels/voice-gateway/third-party-asr-tts-support)
...
<Note>Apply the following parameters only when STT is set as Recognizer. Otherwise, the system uses default bot-level or Voice Gateway settings. These properties are applied at the session level.</Note>
...
| `enableTTSChunking`         | Boolean | Splits sentences based on the following punctuations `full stop (.)` and `question mark (?)`   | `true`                               |
...
| Use case                                      | Parameter                                           | Benefit                                                                                                                                                                           |
|-----------------------------------------------|-----------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Reduce voice bot response latency             | `enableTTSChunking`                                 | Enables chunking of long bot responses so the system starts audio playback before processing the full text, improving time-to-first-audio and conversational flow.                |
| Improve turn detection in voice conversations | `eotThreshold`, `eagerEotThreshold`, `eotTimeoutMs` | Controls how the system detects when a user finishes speaking, reducing interruptions, delays, and overlapping speech.                                                            |
| Enable faster AI response generation          | `eagerEotThreshold`                                 | Lets the system begin generating a response before the user fully stops speaking, improving responsiveness.                                                                       |
| Improve speech recognition for domain terms   | `entityPrompt`                                      | Provides hints for domain-specific terms, such as order IDs or product names, to improve recognition accuracy.                                                                    |
| Enable multi-language voice bots              | `sttProvider`, `deepgramModel`                      | Uses Deepgram Flux to support callers who switch languages mid-call, handling recognition for all supported languages within a single uninterrupted ASR session.                  |
| Enable automatic mid-call language switching  | `sttProvider`, `deepgramModel`                      | Uses Deepgram's flux-general-multi model to automatically detect and switch between supported languages during a call without restarting the ASR stream, supporting 10 languages. |
| Support Indian language voice automation        | `sttProvider`, `ttsProvider`, `ttsLanguage`, `sttLanguage`, `voiceName` | Uses Sarvam ASR and TTS to build voice bots for Indian language speakers. Configure speech rate and variation through `ttsOptions`. |
| Privacy and compliance control                | `mipOptOut`                                         | Prevents the system from using customer audio data for model training.                                                                                                            |
| Debugging and quality monitoring              | `azureAudioLogging`                                 | Enables logging to help analyze recognition errors, audio quality, and language detection issues during testing and troubleshooting.                                              |
| Handle caller silence in Realtime LLM sessions.                                             |`userNoInputTimeoutMS`, `botNoInputTimeoutMS`, `botNoInputSpeech`, `botNoInputRetries`, `botNoInputGiveUpTimeoutMS` |  Plays a re-engagement nudge when the caller is silent after the LLM finishes its turn, and handles LLM response delays - preventing abandoned calls from staying connected indefinitely.  |

## Cloned Voices

### Configure a Custom Cloned Voice (ElevenLabs Integration)

Voice Gateway supports custom voices cloned in ElevenLabs. To activate a cloned voice for an agent, share the ElevenLabs API credentials with the support team. Once integration is complete, enable the voice through call control parameters-no redeployment required.

Get the ElevenLabs API key for authentication from the ElevenLabs account under **Profile > API Key**.

Setup Steps:

1. Raise a support ticket with your credentials: Submit a [Support ticket](https://www.support.kore.ai) with your API key, the team completes the integration on your account and shares the new label name.
2. Activate using call control parameters: On completion of the integration, enable the cloned voice in your Voice AI Agent by passing the appropriate call control parameters in your flow configuration. No code changes or redeployment needed.

Example call control parameters to use the new cloned voice:
...
<Note>Treat the ElevenLabs API key as a password. Share it only through secure support tickets and don't include it in screenshots, logs, or public channels.</Note>
...
<Note>This feature is only available for Microsoft.</Note>
...
<Note>Barge-In applies at the node level.</Note>
...
| `backchannelWords`       | Array of strings | All   | Defines words or phrases that don't interrupt the bot when spoken during audio playback. The audio continues playing and the system sends a `speechDetected` event instead of triggering barge-in. This setting applies only during playback - after playback completes, the system treats these words as normal speech.                     | uh-huh, okay, got it  |

### Turn Detection Parameters

Turn detection determines when a caller has finished speaking by combining voice activity detection, acoustic turn probability, streaming ASR transcripts, text-based end-of-utterance scoring, and expected-answer hints. This model is ASR-provider agnostic.

When `turnDetectionEnable` is set to `true`, the platform automatically derives expected-answer configuration from the entity selected in the Entity node. The following parameters are fetched and sent automatically:

* `turnDetectionExpectedType`
* `turnDetectionExpectedLength`
* `turnDetectionExpectedMin`
* `turnDetectionExpectedMax`
* `turnDetectionExpectedOptions`
* `turnDetectionExpectedPattern`
* `turnDetectionOpenEnded`

| Parameter                       | Type              | Description                                                                                                                                                                                                                                                                          | Example                                      |
| :------------------------------ | :---------------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :------------------------------------------- |
| `turnDetectionEnable`           | Boolean           | Turns on intelligent turn detection for the speech gather. When turned on, the platform determines when the caller has finished speaking without requiring manual speech-segmentation or delay configuration.                                                                          | `true`                                       |
| `turnDetectionPrefetch`         | Boolean           | Turns on speculative transcript hints so the application can begin preparing a response before the turn is committed. The application must not play the response until it receives the final committed event. Default: `false`.                                                        | `true`                                       |
| `turnDetectionModel`            | String (enum)     | Selects the acoustic turn-detection model. Supported value: `smart_turn` (default).                                                                                                                                                                                                  | `"smart_turn"`                               |
| `turnDetectionEouThreshold`     | Number            | Controls the balance between response speed and patience. Lower values respond faster; higher values wait longer and reduce the chance of interrupting the caller. Values map to the nearest supported profile: 0.3, 0.4, 0.5, 0.6, or 0.7. Supported range: 0.3 - 0.7. Default: 0.5. | `0.5`                                        |
| `turnDetectionOpenEnded`        | Boolean           | Indicates that the question expects a narrative or multi-part answer. Applies longer holds and prevents aggressive fast commits at sentence boundaries. Use for questions such as "Please describe the issue." Default: `false`.                                                        | `true`                                       |
| `turnDetectionExpectedType`     | String (enum)     | Identifies the expected shape of the caller's answer. Supported values: `digits`, `phone`, `date`, `time`, `email`, `spelling`, `choice`, `yesno`, `number`, `regex`. Auto-derived from the configured entity type when `turnDetectionEnable` is `true`.                              | `"digits"`                                   |
| `turnDetectionExpectedLength`   | Integer           | Specifies the exact expected number of digits or spelled characters. Applicable to `digits` and `spelling`. The answer is held while below this length and considered complete when the required length is reached.                                                                    | `6`                                          |
| `turnDetectionExpectedMin`      | Number or Integer | Specifies the minimum acceptable value or length. For `digits`, the minimum number of digits. For `number`, the minimum permitted numeric value.                                                                                                                                      | `4`                                          |
| `turnDetectionExpectedMax`      | Number or Integer | Specifies the maximum acceptable value or length. For `digits`, the maximum number of digits. For `number`, the maximum permitted numeric value. Reaching the maximum lets the answer be considered complete.                                                                          | `6`                                          |
| `turnDetectionExpectedOptions`  | Array of strings  | Specifies permitted answers for the `choice` expected type. Matching is case and punctuation-insensitive, and the option may display within a longer short response.                                                                                                                  | `["billing", "technical support", "sales"]`  |
| `turnDetectionExpectedPattern`  | String            | Defines a regular expression for structured answers that can't be represented by the standard expected types. Applicable only when `turnDetectionExpectedType` is `regex`. The complete normalized answer must match the pattern.                                                      | `"[A-Z]{3}[- ]?\\d{6}"`                     |

Example Configuration:

```json
{
  "turnDetectionEnable": true,
  "turnDetectionModel": "smart_turn",
  "turnDetectionEouThreshold": 0.5,
  "turnDetectionOpenEnded": false,
  "turnDetectionExpectedType": "digits",
  "turnDetectionExpectedLength": 6
}
```
...
<Note>All timeout-related parameters apply at the node level.</Note>

| Parameter                        | Type                  | Scope | Description                                                                                                                                       | Example                        |
| :-------------------------------- | :--------------------- | :----- | :------------------------------------------------------------------------------------------------------------------------------------------------- | :------------------------------ |
| `userNoInputTimeoutMS`           | Number (milliseconds) | All   |Maximum wait time to receive user input after the bot finishes its turn. If set to 0, Voice Gateway waits indefinitely. Applies to both dialog-based bots and Realtime Speech-to-Speech (S2S) LLM sessions.                                                          | `"userNoInputTimeoutMS": 20000 → waits 20 seconds before timeout.` |
...
| Parameter               | Type             | Supporting STT or TTS          | Description                                                                                                                                              | Example |
|:------------------------------|:----------------------|:----------------------------------|:--------------------------------------------------------------------------------------------------------------------------------------------------------------|:-------------|
...
| `sttMinConfidence`           | Number (0.1-0.9)     | All                              | If the transcript's confidence score is less than this threshold, the input is ignored and the timeout prompt is triggered. Ensures only high-confidence transcriptions are accepted. | `"sttMinConfidence": 0.5` |
...
| Parameter | Type | Supporting STT or TTS | Description | Example or More Info |
|:---------------|:----------|:-------------------------|:------------------|:--------------------------|
...
| Parameter | Type | Supporting STT or TTS  | Description  | Example or Notes  |
|:---------------|:----------|:-------------------------|:------------------|:----------------------|
...
| `diarization` | Boolean |—| Enables speaker diarization (that's, identifying speakers). | `"diarization": true`|
...
| Parameter               | Type      | Supporting STT or TTS | Description                                                                                                                                                                          | Example/Notes                                          |
| :--------------------------- | :------------- | :---------------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :------------------------------------------------------------ |
| `awsAccessKey`              | String        | —                      | AWS access key used to authenticate requests.                                                                                                                                            | `"awsAccessKey": "AKIAIOSFODNN7EXAMPLE"`                     |
...
| `awsSecurityToken`          | String        | —                      | Temporary security token for requests using AWS STS. Optional.                                                                                                                           | `"awsSecurityToken": "FQoGZXIvYXdzEDQaDJ..."`                |
| `awsRegion`                 | String        | —                      | AWS region for service requests (for example, `us-west-2`, `eu-central-1`).                                                                                                              | `"awsRegion": "us-west-2"`                                   |
| `awsVocabularyFilterName`   | String        | —                      | Vocabulary filter name used to exclude specific words or phrases from transcription.                                                                                                     | `"awsVocabularyFilterName": "BlockedWordsFilter"`            |
| `awsVocabularyFilterMethod` | String (enum) | —                      | Method used to handle filtered words:<br />• `"remove"`-Removes the word from the transcript<br />• `"mask"`-Replaces the word with asterisks<br />• `"tag"`-Tags the word for reference | `"awsVocabularyFilterMethod": "mask"`                        |
| `awsLanguageModelName`      | String        | —                      | Custom language model name used for domain-specific transcription.                                                                                                                       | `"awsLanguageModelName": "FinanceDomainModel"`               |
| `awsPiiEntityTypes`         | Array         | —                      | List of PII entity types to detect.                                                                                                                                                      | `"awsPiiEntityTypes": ["NAME", "EMAIL", "SSN"]`              |
| `awsPiiIdentifyEntities`    | Boolean       | —                      | Indicates whether the system detects and highlights PII in the transcript.                                                                                                               | `"awsPiiIdentifyEntities": true`                             |
...
| `deepgramkeyterms`         | String               | —                       | Boosts recall accuracy for key terms or phrases. Improves Keyword Recall Rate (KRR) by up to 90%.                                                                                                                                                                                                   | `["PIN number", "account ID"] → Special terms to highlight in transcription`                                       |
...
2. Configure Call Control Parameters
  ...
  **Parameter Details**
  ...
  | `deepgramModel` | - | `flux-general-multi` | The Deepgram model to use for speech recognition. Set `flux-general-multi` to enable multilingual recognition with automatic mid-call language switching across supported languages.|
...
Sarvam ASR is a speech-to-text provider optimized for Indian languages. Configure it using call control parameters. For additional information on the supported languages, refer to the [list of supported languages](https://docs.sarvam.ai/api/api-guides-tutorials/speech-to-text/how-to/specify-language-codes).

| Parameter    | Type   | Supporting STT or TTS | Description                                                                 | Example   |
| :----------- | :----- | :----------------- | :-------------------------------------------------------------------------- | :-------- |
| `sttProvider`  | String | STT                | Specifies Sarvam as the speech recognition provider.                        | "sarvam"  |
| `sttLanguage`  | String | STT                | Sets the language for speech recognition. Must be one of the supported language codes. | "en-IN"   |
...
| Parameter        | Type                 | Supporting STT or TTS                   | Description                                                                                                                                                                                                                       | Examples                                       |
|:------------------|:----------------------|:---------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|:------------------------------------------------|
...
<Note>Each TTS provider has its own set of customizable parameters. For more detailed information on the parameters they support, refer to their official websites.</Note>
...
* `speed`: Controls the speed of the generated speech. The default value is 1, and the allowable values are `>=0.7` and `<=1.2`. Values less than 1 slow down the speech, while values > 1 speed it up. [Learn more](https://elevenlabs.io/docs/conversational-ai/customization/voice/speed-control).

**Setting the ElevenLabs v3 Model via ttsOptions**

In addition to the voice customization parameters listed above, you can use `ttsOptions` to select a specific ElevenLabs model. To use the ElevenLabs v3 (`eleven_v3`) model-which is required for features such as [audio tags](/ai-for-service/channels/voice-gateway/configure-voice-gateway#audio-tags-elevenlabs-v3)-pass the `model_id` parameter inside `ttsOptions`.
...
**Aura-2 voice controls**

When the model is set to an Aura-2 voice (for example, `aura-2-thalia-en`), two additional `ttsOptions` parameters are available: `speed` and `pronunciations`.

| Parameter       | Range      | Default | Description                                                                                                          |
| :-------------- | :--------- | :------ | :------------------------------------------------------------------------------------------------------------------- |
| `speed`         | 0.7 - 1.5  | 1.0     | Controls the speech rate. Values below 1.0 slow speech down; values above 1.0 speed it up.                          |
| `pronunciations` | —         | —       | Defines pronunciation overrides for specific words using IPA (International Phonetic Alphabet) notation.             |

These settings appear only when an Aura-2 model is selected. Values outside the supported range, or malformed pronunciation entries, are rejected with a validation error before the request is sent. [Learn more](https://developers.deepgram.com/docs/tts-voice-controls).
...
Sarvam TTS supports Indian language voice synthesis. Use `ttsOptions` to configure speech rate and variation.
...
| Parameter   | Range      | Default | Description                                                                                             |
| :---------- | :--------- | :------ | :------------------------------------------------------------------------------------------------------ |
| pace        | 0.5 - 2.0  | 1       | Controls speech rate. Values below 1 slow speech down; values above 1 speed it up.                     |
| temperature | 0.01 - 1.0 | 0.6     | Controls variation in speech output. Lower values produce more consistent output; higher values introduce more variation. |
...
| Parameter              | Type                                | Supporting STT or TTS                      | Description                                                                                                                                                                                                                                                                     | Example/Notes |
| :-------------------------- | :--------------------------------------- | :------------------------------------------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :------------------- |
| `continuousASRTimeoutInMS` | Number (milliseconds)                   | STT: Google, Microsoft<br />TTS: Not required | Duration of silence (in milliseconds) after receiving a transcript before returning the result. If another transcript arrives within the timeout, the system combines transcripts and continues recognition. The system returns the final result after silence exceeds the timeout. | `5000` (5 seconds)  |
| `continuousASRDigits`      | Digit (for example, `*`, `%`, `&`, `#`) | STT: Google, Microsoft<br />TTS: Not required | DTMF key that terminates the gather operation and returns the collected results immediately.                                                                                                                                                                                        | Example: `#`        |
...
![Handle Bot Delay](/ai-for-service/channels/voice-gateway/images/handle-bot-delay.png)
...
| :--------------------------- | :-------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------- |
| `botNoInputGiveUpTimeoutMS` | Number         | Timeout (in milliseconds) to wait for a bot response before disconnecting the call.<br />If no response is received within this duration, Voice Gateway disconnects the call.<br />**Default:** 30 seconds | Yes (Default: 30 sec) |
| `botNoInputTimeoutMS`       | Number         | Timeout (in milliseconds) before playing a prompt to the user.<br />If no input is received from the bot, Voice Gateway plays a textual prompt (`botNoInputSpeech`) or an audio file (`botNoInputUrl`).          | Yes                   |
| `botNoInputRetries`         | Number         | Number of retry attempts after a no-input timeout.<br />For example, if set to `2` and timeout is `1000` ms, the system plays the prompt two additional times if no bot response is received.                    | Yes                   |
| `botNoInputSpeech`          | String/Array | Prompt played when no input is received from the bot.<br />Supports:<br />- Plain text<br />- SSML<br />- Audio URL<br />Example: `["https://audiourl", "This is second message"]`                     | Yes                   |
| `botNoInputUrl`             | String         | URL of an audio file played when the bot doesn't respond within the defined timeout.                                                                                                                            | Yes                   |

Example:
![Example Configuration](/ai-for-service/channels/voice-gateway/images/message-node-parameters.png)

<Note>  `botNoInputSpeech` can contain multiple messages, including audio URLs. Example: `botNoInputSpeech` = `[“this is first delay Msg”, https://thisdummy.wav, “ this is third textual Message”]`.</Note>
...
<Note>  If you receive a response from the API and don’t want to play the full music, immediately abort the music and play the Message node prompt using the channel override utility function:</Note>
   `print(voiceUtils.abortPrompt(“Dummy message“))` → (The message parameter is optional).
      ![Optional Message Parameter](/ai-for-service/channels/voice-gateway/images/optional-message-parameter.png)
...
| :-------------------------------- | :------------------------------------------------------------------------------------ | :------------------------------------------------ | :----------------------------------------------------------------------------------------------------------------------------- |
| `listenDuringPrompt`             | Boolean (true or false) Similar to Barge-In                                       | STT-Google and Microsoft TTS-Not Required | If `false`, the bot doesn't listen for user speech until the response finishes playing. Default: `true`.              |
| `bargeInMinWordCount`            | Number                                                                               | STT-Google and Microsoft TTS-Not Required | If Barge-In is sensitive, the bot only interrupts playback after speaking the specified number of words. Default: 1. |
| `bargeInOnDTMF`                  | Boolean                                                                              | STT-Google and Microsoft TTS-Not Required | Lets users to press a key to interrupt the audio playback. After pressing a key, the user can speak their input.            |
| `dtmfCollectInterDigitTimeoutMS` | Number (milliseconds)                                                                | STT-Google and Microsoft TTS-Not Required | Time allowed between DTMF key presses before sending all digits to the bot.                                                   |
| `dtmfCollectSubmitDigit`         | Number                                                                               | STT-Google and Microsoft TTS-Not Required | Special digit that submits all collected DTMF input immediately, bypassing the timeout or max digit limit.                    |
| `dtmfCollectMaxDigits`           | Number                                                                               | STT-Google and Microsoft TTS-Not Required | Maximum number of DTMF digits to collect. Example: If set to `5` and input is `1234567`, only processes `12345`.        |
| `dtmfCollectminDigits`           | Number                                                                               | STT-Google and Microsoft TTS-Not Required | Minimum number of DTMF digits to collect. Default: 1.                                                               |
| `dtmfCollectnumDigits`           | Number                                                                               | STT-Google and Microsoft TTS-Not Required | Exact number of DTMF digits to collect.                                                                                       |
| `input`                          | Array of strings Valid values: `['digits']`, `['speech']`, `['digits', 'speech']` | STT-Google and Microsoft TTS-Not Required | Specifies allowed input types.Default: `['digits']`.                                                                 |

### Backchannel Words

Backchannel words are short acknowledgments - like uh-huh, okay, or got it - that callers say while listening without intending to interrupt. By default, any speech during playback triggers barge-in and stops the audio.

Configuring `backchannelWords` signals Voice Gateway to keep the audio playing when a caller speaks one of these words during playback, instead of treating it as a barge-in. When Voice Gateway matches an utterance to a configured word, it:

- Skips intent detection and dialog execution for that input.
- Leaves session context — including the last captured user input — unchanged.
- Excludes the utterance from NLP Insights and intent-training data.
- Doesn't count the utterance against no-input timeouts or retry limits.
- Still records the utterance in the conversation transcript, marked as a backchannel phrase.

**Configuration**

To set `backchannel` Words at the node-level:

- Each entry must be non-empty after trimming and have a maximum length of 100 characters.
- Lists support a maximum of 100 entries.
- Voice Gateway removes duplicate entries automatically after normalization.
- An empty array or an omitted parameter turns off the feature for that scope with no runtime impact.

**How it works**

1. Audio playback starts.
2. The caller speaks during playback.
3. Voice Gateway normalizes the detected phrase - converting to lowercase, trimming whitespace, and stripping punctuation - and checks it against the configured list.
4. If the phrase matches a configured backchannel word, audio continues playing and Voice Gateway sends a `speechDetected` event with backchannel metadata. Barge-in doesn't trigger.
5. If the phrase doesn't match, standard barge-in behavior applies.


After playback completes, the system treats these words as normal speech and doesn’t classify them as backchannels.

**Array semantics**

| Value              | Behavior                                        |
| :----------------- | :---------------------------------------------- |
| Non-empty array    | Replaces the active backchannel word list.       |
| Empty array (`[]`) | Clears the list and turns off backchannel classification. |
| Parameter omitted  | Keeps the current active list unchanged.         |

**Example Configuration**

```json
{
  "backchannelWords": ["uh-huh", "okay", "got it", "sure", "yes", "correct"]
}
```

<Note>Backchannel detection uses exact normalized phrase matching. Voice Gateway converts detected speech to lowercase, trims whitespace, and strips punctuation before comparing it against the list.</Note>
...
For example, if a user speaks in English, the conversation continues in English. If the user switches to Spanish, the language switches to Spanish. [Learn more](/ai-for-service/app-settings/language-management/managing-languages-for-multilingual-vas#adding-a-language-to-an-ai-agent).

**Configuration Steps**:

1. In Bot Builder (on the child bot), navigate to **Languages,** add a new language (for example, Spanish), and enable it.
2. Select English as the default language from the language dropdown menu. [Learn more](/ai-for-service/app-settings/language-management/managing-languages-for-multilingual-vas#adding-a-language-to-an-ai-agent).
...
To configure ASR (Automatic Speech Recognition) and TTS (Text-to-Speech) in a ClickToCall flow, add the call control parameters in the [Script Task](/ai-for-service/flows/node-types/script-task) node at the start of the flow. Set the parameters must in the following format:
...
## Best Practices for Updating ASR or TTS

### When to update ASR/TTS

Every ASR/TTS update can trigger a connection or reconnection. During reconnection, messages may remain in a buffer until the system establishes the connection, which can delay or affect the call experience.

The system generally has sufficient time to configure ASR/TTS while establishing the call connection. Avoid unnecessary updates during the conversation.

Don't update ASR/TTS unless the change is required.

### Best practices

<Steps>
  <Step title="Configure ASR or TTS in the Experience Flow">
    Select the required ASR, TTS, and labels in the Experience Flow itself.
  
    If additional parameters are required, configure them in the first node of the bot whenever possible. This practice avoids repeated configuration changes and unnecessary connection reinitialization.
  </Step>

  <Step title="Pass required parameters when updating ASR or TTS">
    When using custom labels, pass the required parameters whenever an ASR or TTS is updated:
    ...
    * `ttsLabel`
    ...
    If any of these parameters are missing, the system may create the connection using the default cloud vendor instead of the intended custom configuration. This gap can lead to unexpected behavior.

    Example: When updating the `azureEndPointId` for ASR with a custom label, pass the corresponding `sttLabel` along with the other required parameters. Otherwise, the system may route the request to the default cloud vendor.
  </Step>
  
  <Step title="Avoid unnecessary TTS updates for neural voices">

    If the Neural TTS voice in use supports multiple languages, check whether the current voice supports the new conversation language. If the voice supports the language, the call control parameters, TTS language, and voice name don't require an update when the conversation language changes.

    Example:

    ```text
    ttsLanguage = en-IN
    voiceName = en-IN-Diya:DragonHDLatestNeural
    ```

    If the current voice supports the new conversation language, don't update the call control parameters only to change the language. The Experience Flow uses the existing neural voice for the supported language without triggering another TTS update.

    This voice supports both English and Hindi. Therefore, when the flow switches to Hindi and the current voice supports Hindi, the call control parameters, TTS language, and voice name don't require an update.

    Example:

    ```text
    ttsLanguage = hi-IN
    voiceName = hi-IN-Kalpana:DragonHDLatestNeural
    ``` 

    If the current voice doesn't support the new conversation language, update the call control parameters with both the appropriate `ttsLanguage` and a `voiceName` that supports that language. Otherwise, the system may not apply the requested language correctly, and the update can trigger a reconnection.
  </Step>

  <Step title="When changing TTS, keep the language configuration consistent">

    When a TTS configuration change is necessary, verify that the language code in the TTS language parameter matches the language the voice name represents (except Neural voices from Azure).

    Example:

    ```text
    ttsLanguage = en-AU
    voiceName = en-AU-Natasha
    ```

    The TTS language and voice language code must match whenever the selected voice requires language-specific configuration.
  </Step>

  <Step title="Apply the same principle to ASR">

    The same best practice applies to ASR. Avoid updating ASR parameters unless the change is required.

    For example, when using a custom ASR label and updating the azureEndPointId, pass the custom label along with the required ASR parameters.

    If the configuration omits the label, the system may create the connection using the default cloud vendor, which can lead to unexpected behavior.

    **Recommendation**: Configure the required ASR or TTS settings at the beginning of the call and avoid updates during the conversation. When an update is required, pass the complete configuration to ensure the configuration uses the intended provider and settings.
  </Step>
</Steps>
