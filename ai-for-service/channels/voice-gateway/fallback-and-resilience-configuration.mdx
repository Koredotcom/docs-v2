...
AI for Service provides three layers of failure protection. Each layer targets a different stage of a call:
...
If the platform is unreachable when a call arrives, enable **Answer Call on Bot Response**. The call isn't answered, and the upstream SIP system receives an error code to route the caller to an alternative destination such as a live agent queue or backup IVR.
...
### Voice Gateway Component Failure Handling

Voice Gateway components such as FreeSWITCH, ASR, and TTS can crash or restart under high traffic or memory exhaustion. These failures interrupt active calls and leave callers on dead air if the existing session cannot continue. The system routes new calls to the restarted component or to another available redundant component.

This section explains how to configure fallback options — playing a message, transferring the call, or ending it gracefully — so that callers receive feedback instead of dead air.

To configure error handling and define how Voice Gateway responds when a component failure affects an active call, follow these steps:

1. Open **Error Handling** in the **Automation Node**.
2. To end a call gracefully, configure a message that the system plays to the caller before it disconnects the call.
    
    ![Component Failure Handling](/ai-for-service/channels/voice-gateway/images/component-failure-handling.png)
