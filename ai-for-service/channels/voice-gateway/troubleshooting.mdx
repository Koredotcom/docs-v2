...
Before the app publishing fails, open the Inspect tab in your browser. In the Network tab, search for the dockstatus API. You can see multiple APIs with the name dockstatus. When the app publishing failure message appears, open the most recent dockstatus API.
![Inspect](/ai-for-service/channels/voice-gateway/images/inspect-element-page.png)

In the response, search for SIPTRUNK as shown in the figure. If the SIPTRUNK status displays as `"status": "FAILURE"`, you can report the issue to the Support team.
![Dockstatus](/ai-for-service/channels/voice-gateway/images/dockstatus-api.jpg)

## Agent Transfer isn't Working

If a request isn't assigned to an available agent and the app is responding with "No agents are available right now," even though agents are online, follow these steps:
