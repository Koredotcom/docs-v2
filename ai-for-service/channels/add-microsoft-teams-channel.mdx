...
4. (Optional) toggle the **Extended Interaction Handling** and select any one.
    ...
    * Select **Dynamic Search (Typeahead)** and enter the Custom URL. The platform forwards the search term to this endpoint and displays the returned suggestions, providing users with relevant, real-time suggestions from external systems as they type.
