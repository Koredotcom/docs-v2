...
* **Discourse Analyzer**: Discourse analyzer helps enterprises generate conversation flows using historical chat or call transcripts. Chat and call transcripts are analyzed using neural network-based machine learning models to identify intents and discourse patterns to fulfill a specific intent. This is an out-of-the-box feature that's in Beta state. Contact Support to try this feature.
...
4. On the **Configurations** tab, perform the following steps:
    1. Use **Kore.ai IVR Sandbox** for testing your AI Agent. See [Sandbox Configuration ](#sandbox-configuration)for more details.
        ...
        <Note>If you save the configuration after associating  an app without enabling IVR Settings, the platform pre-populates the required settings for Sandbox. 2. Associate an App with the IVR channel, either by creating a new one or selecting an existing one. If you don't have any apps, a message is displayed as shown in the following screenshot.   ![associate app](/ai-for-service/channels/images/IVR-4.png)</Note>
...
5.  From the **Voice Call Properties** tab, set the configurations.
    ...
    <Note>If you have enabled IVR Sandbox, the following settings are required to be configured on the platform. These are pre-populated if you don't enable the IVR settings. In case you have enabled the settings, ensure the values are the same as follows:       * Enable Transcription set to Yes.       * Transcription engine source set to builtin:speech/transcribe.       * IVR Data Extraction Key set to user input.       * ASR Confidence Threshold Key set to userinput confidence.       * ASR Threshold Confidence set to 50.</Note>
...
{/* AG: A step above has 2 large images showing together - consider clubbing these or removing or add another step to separate.
In the note above, don't add list items as those don't render in the output as a list.
Once, check the numbering of above steps with the old content.
Article has too many images - reduce the usage.
*/}
...
<Note>It isn't available for on-prem installation.</Note>
...
{/* AG: Step 4 above seems weird. It's just FYI and doesn't have any action for the user. */}
...
<Note>* For a quick overview of the JWT token, refer to Introduction to JWT tokens. * The host name, `{{host}}` refers to an environment URL, for example, `https://platform.kore.ai`. It changes based on the domain name.</Note>
**Call The AI Agent From IVR Call flow**

{/* AG: List inside note doesn't render. Don't use. */}
