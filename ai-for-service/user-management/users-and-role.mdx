...
Users whose accounts are disabled appear with a **Disabled** tag. If a user with disabled Contact Center permissions sends an invite, the new user's Contact Center permissions are also disabled.
...
App Owners and Administrators with [Manage Custom Profile Fields](#user-management) permission can create custom fields to capture agent metadata beyond the default profile attributes. Once created, a custom field appears in the profile of every agent in the account.

To add a custom field:

1. Select **Manage Profile Fields** in the upper-right corner.
2. Select **Add Custom Field**.
3. Enter the required details and select **Save**.

The field appears in the profile of all agents. Field values can be updated in two ways:

* From the UI - Edit the value directly in the agent profile and select Save.
* Using API - Use the [Agent APIs](/ai-for-service/apis/contact-center/api-list#agent-creation-apis) to read or write custom field values. 
...
3. **Widget Features**-Attachments and emojis in live chat, Agent Forms and Links to let agents send configured forms and hyperlinks during interactions.
...
![Chat and Voice Settings](/ai-for-service/user-management/images/user-attribute-chat-voice.png)
...
3. To remove a queue, select the **Delete** icon.
...
3. To remove a skill, select the **Delete** icon.
...
Users whose accounts are disabled can only edit the Name, Phone Number, and Role fields of other users.
...
- You can't delete the CCAI App Owner.
...
2. In the **Invite User** window, configure:
   - **Name**: Group name. See [Create a Group](/ai-for-service/administration/groups-management#create-a-group).
...
| **User Management** | Manage Users of App, App Owner, and co-developers of an App. | Full | Full | View | Full | No | Full / View / No / View & Invite / Edit / Edit & Delete |
| **User Management > Manage Custom Profile Fields** | Manage custom profile fields for agents. | Yes | Yes | No | No | No | Yes / No |
| **User Management > Edit User Information** | Allow users to modify user-related information. | Yes | Yes | No | Yes | No | Yes / No |
| **Agent Settings** | Modify global agent settings. | Full | Full | Full | Full | None | View / None / Full |
| **Agent Settings > Agent Console Chat Timer Display Logic** | Allow users to modify agent console chat timer display logic. | Yes | Yes | Yes | Yes | No | Yes / No |
...
### User Management Permission Options

The User Management permission supports six access levels. Each level defines which user lifecycle actions the role can perform.

| Permission     | Create Users | Edit Users | Delete Users | View Users | Invite Users |
| :------------- | :----------- | :--------- | :----------- | :--------- | :----------- |
| Full Access    | ✓            | ✓          | ✓            | ✓          | ✓            |
| Edit & Delete  | —            | ✓          | ✓            | ✓          | —            |
| Edit           | —            | ✓          | —            | ✓          | —            |
...
<Note>Setting the User Management permission to View, View & Invite, or None turns off all child permissions. If you set the parent permission to Edit, Edit & Delete, or Full Access, the system restores the previous child permission configurations.</Note>

### Agent and Supervisor Experience Agent Desktop Console
...
| **Wallboards in Analytics** | Access to the Wallboards menu in Analytics. | Yes | Yes | Yes | Yes | Yes| Yes / No |
| **Contact Center Wallboards in Configuration** | Access to the Wallboards menu in Configuration.  | Yes | Yes | No | No | No | Yes / No |

### Agent and Supervisor Experience Monitor Console
...
| **Override Queue Transfer Restrictions in Monitor** | Allows users to bypass queue-level transfer restrictions when changing queue assignments from the Monitor tab. Queue transfer restrictions continue to apply in Agent Console. | No | No | No | No | No | Yes / No |
...
The following permissions are implicitly granted to App Owners only and aren't available to any other role:
...
<Note>You can't delete default roles (App Owner, App Developer, App Tester, Supervisor, Agent). Deleted roles can't be restored.</Note>
