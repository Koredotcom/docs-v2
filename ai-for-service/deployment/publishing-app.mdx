...
End users can interact with an AI Agent only after it's published. The platform provides a publishing workflow for administrators to review and approve changes before they reach end users.
...
| **Published** | Contains components approved for end-user access. Developers can't edit the Published copy directly. |
...
| **Suspended** | Admin suspended the agent. End users can't access it; developers can still work on the in-development copy. |

<Note>A Published task can't be deleted. Admins can Suspend it if it's no longer needed. A Suspended task can be Upgraded to create a Configured copy for further changes.</Note>

---

## Publishing an Agent

Only the agent owner can publish. Developers with edit access can't.
...
<Note>If Dialog Tasks are selected under **Publish > Tasks**, their training data is published automatically and NL Model components can't be customized separately. Deselect the tasks first to customize ML Utterances, Patterns, and Rules individually. Selecting NL Model components and then adding Dialog Tasks triggers a warning to reset publishable components.</Note>
