...
4. You can add or remove the Environment and Content variables to these namespaces. A variable can belong to one or more namespaces. You can't remove the Environment and Content variables from the default namespace.
...
2. If the variable that's not mapped is referenced by the component, it will flag a "_variable not found_" error.
...
Use this option to **view, edit,** or **delete** namespaces. Deleting a namespace that is mapped to any component is restricted. You need to unmap the namespace from all associated components before deletion. As mentioned earlier, you can't delete the default namespace.
