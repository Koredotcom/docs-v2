...
Post the release of v8.1 of the platform, for on-prem installations, multiple value sets can be stored for these environment variables using Collections, [refer here for more](collections.md).

> **Note:** Ensure that the Environment or Content variable name does not contain any hyphen.
...
- **Languages**: Under **App Settings > Language Management > Translation Configurations**, while configuring the translation engine, you can use the regular/encrypted environment variable value (type `{{env.environment variable name}}`) to set up the encrypted variable name as the access key as follows:
    ...
    - **Authorization Profiles** – You can use encrypted/regular environment variables on the following fields while defining a new authorization mechanism for all types of authorizations:
        ...
        - **Authorization Check URL**, **Field Key** for form field, and **Field Key** and **Field Value Authorization Fields** for Authorization Type _Basic Auth_. [Learn more](../dev-tools/bot-authorization/setting-up-authorization-using-basic-auth.md).
        - **Client Id**, **Client Secret Key**, and **Authorization URL** for Authorization Type _Oauth V2_. [Learn more](../dev-tools/bot-authorization/setting-up-authorization-using-oauth-v2.md).
        ...
        - **Consumer Key**, **Consumer Secret**, **Request Token Link**, **Access Token Link**, and **User Authorization Link** for Authorization Type _Oauth V1_. [Learn more](../dev-tools/bot-authorization/setting-up-authorization-using-oauth-v1.md).
        - **Authorization Check URL** and **Field Key** (for Form Field) for Authorization Type _API Key_. [Learn more](../dev-tools/bot-authorization/setting-up-authorization-using-an-api-key.md).
        - **Field Key** for Form Field, **Token URL**, **Field Key**, and **Field Value** for Authorization Field, and **Refresh Token URL** for Authorization Type _Oauth V2 Password Grant Type_. [Learn more](../dev-tools/bot-authorization/setting-up-authorization-using-oauth-v2-password-grant.md).
...
3. In the **Add Variable** panel, provide the following details:
    ...
    - Assign a **Namespace** if enabled ([refer here for more](../managing-namespace.md)).
