...
In the next section, define the Web/Mobile Client channel for the Travel Planning Sample App by creating a new client app and defining channel settings. [See here](../../channels/add-web-mobile-client.md) for a detailed explanation.
...
11. Click **Save**.
    ...
    > **Note:** By default, the Travel Sample Planning App is configured with **Target Audience** set to **Enterprise Users** on the **Build** tab under **Configurations -> General Settings.** You can optionally define this App for **General Public** use. Once published, the **Target Audience** cannot be changed.
...
21. For ver7.2 of the platform, the Web SDK repo structure underwent a major change. Follow the appropriate steps based on which repo you are using:
    ...
    **b. For older Web SDK downloaded before ver7.2 (before Feb 2020):**
    ...
    2. Update the following parameters to run as local host:
        ...
        > **Note:** The URL in the above snippet refers to the location where your app is hosted. For this tutorial, the JWT server from step 15 is used.
...
For example, a user may start a conversation with an anonymous or randomly generated identity. After exchanging messages, the user becomes authenticated by logging into your website. At that point, the known identity can be passed to the app from the SDK as part of the [JWT Grant API](../../sdk/sdk-security.md#about-jwt) call using the parameter `identityToMerge`. The Platform merges the user identities and allows the user to resume an ongoing conversation using the new known identity.
...
- When both the new and merged identities are not present in the system, the new identity is created and a new conversation starts using the new identity.
- When the new identity is present but the merged identity is not, the conversation starts or continues (if active session) using the new identity.
...
From ver8.0 of the platform, you can directly add Custom Meta Tags from all supported internal channels (Web SDK/IVR/IVRVoice/Webhook channels). Define Session, User, and Message level meta-tags. These tags are added to the Conversation Session as soon as it is created.
