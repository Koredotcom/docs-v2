...
If the Auth Token expires, the Refresh Token is used to generate a new Auth Token. If the Refresh Token itself expires, the platform automatically generates a new Refresh Token (when the checkbox is enabled), without requiring manual intervention.
