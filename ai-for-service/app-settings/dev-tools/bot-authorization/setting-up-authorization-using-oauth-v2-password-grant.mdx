...
In the **Token URL** field, optionally define a URL to test the authorization settings from XO Platform before deploying the AI Agent. You can use dynamic fields, path parameter fields, query fields, and more. For example, `http://{tenant}.example.com/test/{{tokenId}}`.
