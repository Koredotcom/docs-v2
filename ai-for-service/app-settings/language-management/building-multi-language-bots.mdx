---
title: Get Started with Multilingual AI Agent
...
1. There is a set of basic building blocks to a multilingual AI Agent: the language in which it talks to users, the language in which you train it, and the process through which it detects and selects which languages to use.ontinue reading this article to learn more.
2. You can create a new AI Agent or add new languages to an existing one. Read more about managing languages [here](/ai-for-service/app-settings/language-management/managing-languages-for-multilingual-vas).
3. Multilingual AI Agents have language-specific components and some features exhibit different behaviors compared to single language assistants. Read more about this [here](/ai-for-service/app-settings/language-management/multilingual-vas-components-feature-support).
4. Translation can be automated using pre-built translation services from providers such as Microsoft or Google; or custom services, including any that you may build in-house. Read [here](/ai-for-service/app-settings/language-management/managing-translation-services) to learn more.
...
| Catalan - ca        | Kurdish - km                  | Swahili - sw           |
| Cantonese - yue     | Kyrgyz - ky                   | Tamil - ta             |
| Cebuano - ceb       | Kinyarwanda - rw              | Tagalog - tl           |
| Chinese - izh       | Latin - la                    | Tibetan - bo           |
| Corsican - co       | Luxembourgish - lb            | Telugu - te            |
| Croatian - hr       | Laothian/Laos/lao - lo        | Tajik - tg             |
| Czech - cs          | Lithuanian - lt               | Thai - th              |
| Danish - da         | Latvian - lv                  | Turkmen - tk           |
| Dutch - nl          | Marathi - mr                  | Tagalog/Filipino - fil |
| English - en        | Malay - ms                    | Turkish - tr           |
| Esperanto - eo      | Malagasy - mg                 | Tatar - tt             |
| Estonian - et       | Maori - mi                    | Uighur/Uyghur - ug     |
|  Finnish - fi       | Macedonian - mk               | Urdu - ur              |
| French - fr         | Maltese - mt                  | Ukrainian - uk         |
| Frisian - fy        | Malayalam - ml                | Uzbek - uz             |
| German - de         | Mongolian - mn                | Vietnamese - vi        |
| Greek - el          | Nepali - ne                   | Wolof - wo             |
| Galician - gl       | Norwegian - nb                | Welsh - cy             |
| Georgian - ka       | Nyanja - ny                   | Xhosa - xh             |
| Gujarati - gu       | Oriya/Odia - or               | Yiddish - yi           |
| Hausa - ha          | Punjabi - pa                  | Yoruba - yo            |
| Hawaiian - haw      | Polish - pl                   | Zulu - zo              |
| Hebrew - he         | Portuguese (Brazilian) - pt   |                        |
| Hindi - hi          | Portuguese (European) - pt_pt |                        |
...
The following are the NLU Languages supported by the platform. While most of the NLU features are supported in all languages, there are some exceptions, [see here for more details](/ai-for-service/app-settings/language-management/multi-lingual-bot-behavior).


|                                  |                                    |
|----------------------------------|------------------------------------|
| Arabic - ar                      | Kazakh (post v7.2 release) - kk    |
| Cantonese - yue                  | Marathi (post v9.0 release) - mr   |
| Chinese Simplified - zh_cn       | Norwegian (post v8.1 release) - nb |
| Chinese Traditional - zh_tw      | Polish (post v7.0 release) - pl    |
| Catalan (post v9.0 release) - ca | Portuguese (Brazilian) - pt        |
| Dutch - nl                       | Portuguese (European) - pt_pt      |
| English - en                     | Russian (post v7.0 release) - ru   |
| French - fr                      | Swedish (post v7.1 release) - sv   |
| Finnish (post v6.4 release) - fi | Slovenian - sl                     |
| Hindi (post v8.1 release) - hi   | Spanish - es                       |
| German - de                      | Tagalog - tl                       |
| Indonesian - id                  | Telugu (post v9.0 release) - te    |
| Italian - it                     | Tamil (post v9.0 release) - ta     |
| Japanese - ja                    | Ukrainian (post v7.0 release) - uk |
| Korean - ko                      |                                    |

### Language-specific NLU Models

The platform supports language-specific NLU models for 29 languages. These models are pre-trained to understand system entities, concepts, sentiment, etc. in specific languages.

- In most cases, the NLU Language can be the same as the Conversation Language for the 29 languages listed above.
...
- Translation of user input isn't required as the model is pre-trained to understand over 100 languages.
...
- Input Translation and Response Translation aren't required for this flow.
...
- You can also use this flow if the language you want to enable isn't supported as an NLU Language.
...
- You can also use this flow if the language you want to enable isn't supported as an NLU Language.
- Input Translation isn't required for this flow as the multi-lingual model understands over 100 languages.
- You will need to enable the Response Translation option if the responses are defined in a language other than the conversation language.
...
1. The AI Agent identifies user language from every utterance. In case a change is detected, it will get a confirmation from the user regarding the switch and will proceed as per the user response. These standard responses can be customized using the [getCurrentOptions utility](/ai-for-service/apis/automation/koreutil-libraries#koreutil-getcurrentoptions).

    <Note>The current conversation will be discarded in case the user wants to switch languages.</Note>
...
2. Language selection settings - In addition, you may want to configure the language selection options. From the **Settings**, click **Configurations -> Language Management**. Under **Language Selection Logic** (scroll down for the option), for **language selection time frame**, set to one of the following:
    ...
    3. **Every User Message**: Identifies the user's language from every utterance. In case a change is detected, the AI Agent will get a confirmation from the user regarding the switch and proceed as per the user response.
      ...
      <Note>The current conversation will be discarded in case the user wants to switch languages. This is the default setting for multilingual AI Agent.</Note>
