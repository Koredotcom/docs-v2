...
- If a PII value is used in the Service Node definition, the platform uses the redacted value by default to make service calls. However, you can send the original data for scenarios such as passing it to a backend system. Select the appropriate option while configuring the [Service Node](/ai-for-service/automation/dialogs/node-types/service-node).
- You may use the Redaction of PII Data configuration of the Entity Nodes to present the original values of a redacted entity to users for confirmation. Select the appropriate option while configuring the [Entity Node](/ai-for-service/automation/dialogs/node-types/entity-node).
...
The Platform allows you to secure sensitive data input during a conversation. Enabling sensitive data redaction and using custom regex patterns to identify and secure sensitive information is an effective approach.
...
The platform enables you to define sensitive information and its handling before sending it to large language models (LLMs). To learn more, see [Data Anonymization](/ai-for-service/generative-ai-tools/data-anonymization).
...
2. Store the entity's associated PII token in `botUserSession.piiEntities`. 
![Reuse Sensitive Entities Across Dialogs](/ai-for-service/app-settings/advanced-settings/images/entities-across-dialogs.png)
