...
Each guardrail runs on a separate fine-tuned model hosted and periodically updated to detect emerging threats and prompt injection patterns.
...
Custom Guardrails let you define organization-specific validation rules for LLM interactions. Define the validation logic in a custom prompt, then associate the prompt with an LLM to create the custom guardrail. Each custom guardrail evaluates either LLM inputs or LLM outputs, but not both simultaneously. The platform supports up to five custom guardrails. 
...
Configure whether guardrails and feature calls run in parallel or in series to help reduce processing latency.

**Evaluate System and Custom Guardrails in Parallel**
When enabled, system and custom guardrails run simultaneously. When disabled, system guardrails run before custom guardrails. Default: enabled.
