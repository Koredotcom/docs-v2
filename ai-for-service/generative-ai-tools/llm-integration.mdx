...
Azure OpenAI is an out-of-the-box (OOB) integration. You can authorize all models using variables and add models to the OOB integration. For models, you must use custom prompts—the Platform doesn't provide system prompts or templates.
...
For excluded entity types:

* `Entity Type`: `Redacted`

If no entities are present, output:
...
1. (Optional) To tailor generated summaries, for the **Summarizing Conversation** model, select one of the following: 
    * **Default**: Use the default system prompt.
    * **Custom**: Click **Configure**, add one or more instructions, and then save your changes. Click **Validate** to generate a sample summary using your instructions. Review the output, update the instructions if needed, and validate again until the summary meets your requirements. 
