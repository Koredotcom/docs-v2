...
### Tailor Conversation Summaries with Custom Instructions

Optionally, configure free-text instructions in the XO GPT Conversation Summarization system prompt to control summary generation. Use these instructions to include or exclude specific entities or details, specify the language or format, organize the output into sections, or limit the summary length. You can add up to five instructions with a combined limit of 10,000 characters. 

<Note> Custom instructions guide how summaries are generated and formatted, but results may vary. Refine and test your instructions to improve adherence.</Note>
...
1. (Optional) For the **Summarizing Conversation** model, select one of the following: 
    * **Default**: Use the default system prompt.
    * **Custom**: To tailor generated custom summaries, click **Configure**, add one or more instructions, and then save your changes. Click **Validate** to generate a sample summary using your instructions. Review the output, update the instructions if needed, and validate again until the summary meets your requirements. 
