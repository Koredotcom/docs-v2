...
|----------|----------------|-------|
| **Azure OpenAI** | Integration Name, API Key, Sub-Domain, Deployment ID | Integration Name identifies the integration; API Key authenticates your account; Sub-Domain identifies your Azure resource endpoint; Deployment ID specifies the deployed model. | 
| **OpenAI** | Integration Name, API Key, Custom Endpoint (optional) | Integration Name identifies the integration; API Key authenticates your OpenAI account; Custom Endpoint connects to a custom OpenAI endpoint when required. | 
...
### Multiple Integrations for OpenAI and Azure OpenAI

The platform allows up to five configurations each for OpenAI and Azure OpenAI, with each integration maintaining its own authorization details and model set, particularly useful when setting up fallback models, which require the primary and fallback to use different integrations.

Each integration requires a unique combination: API Key + Custom Endpoint for OpenAI, or API Key + User Sub Domain for Azure OpenAI. The platform prevents duplicate combinations.

Each integration has an editable Integration Name. Existing OpenAI and Azure OpenAI configurations are automatically migrated as each provider's first integration, named after the provider, and can be edited at any time. 
...
Azure OpenAI is an out-of-the-box (OOB) integration. You can authorize all models using variables and add models to the OOB integration. For new models, you must use custom prompts—the Platform doesn't provide system prompts or templates.
...
OpenAI is an out-of-the-box (OOB) integration. You can authorize all models using variables and add models to the OOB integration. For new models, you must use custom prompts; the Platform doesn’t provide system prompts or templates. 
...
Anthropic is an out-of-the-box (OOB) integration. You can authorize models using variables. The Platform doesn't provide system prompts or templates—you must use custom prompts.

<Note>The current implementation doesn't support streaming responses for Claude models.</Note>

**Steps:**

1. Go to **Generative AI Tools** > **Models Library** > **Configure Now** for Anthropic, then click **Next**.
2. Complete [Authorization](#authorization).
2. In the Models section, toggle the required model on. 
3. (Optional) click **+Add** to add a [new model](#add-a-new-model-to-a-pre-built-integration).
4. Read the **Policy Guidelines**, select the checkbox, and click **Save**.
5. In the Connection Status pop-up:
   - **Successful**: click **Next**.
...
Amazon Bedrock is an out-of-the-box (OOB) integration. The Platform doesn't provide system prompts or templates—you must use custom prompts.
...
<Note>Without this step, the Platform can't assume your IAM role. Both your AWS account and Kore.ai's environment must explicitly trust each other for cross-account access.</Note>
...
9. Click **Confirm & Save**. The configured model is listed in the Models Library. The next step is to enable [GenAI Features](/ai-for-service/generative-ai-tools/genai-features).
...
3. On the **Auth** tab, select an existing authorization profile or create a new one. See [App Authorization Overview](/ai-for-service/app-settings#authorization-profiles).
...
### Custom Instructions for XO GPT Models

Optionally, configure free-text instructions for the XO GPT Conversation Summarization and DialogGPT models. These instructions work with the system prompt to guide model-generated responses. You can add up to five instructions, with a limit of 10,000 characters for each model. Existing PII Data Masking, Data Anonymization, and Display/TTL settings continue to operate independently. 

Use the instructions to:

- DialogGPT - Conversation Orchestration: Provide guidance on how DialogGPT should orchestrate conversations, including dialog prioritization, routing, fallback behavior, and other conversation-level instructions.

- Conversation Summarization: Specify the entities or details to include or exclude, the language or format to use, how to organize the summary, or the summary length.


<Note>Responses are LLM-generated and non-deterministic. Although the model follows the provided instructions, adherence is not guaranteed in every case. Validate the output before using it in production.</Note>




#### Sample Instruction for Conversation Summarization

**Language**

* Determine the customer's primary language using only the customer's messages.
* Ignore all bot and system messages when determining the customer's language.
* If the customer uses multiple languages, choose the language they use most often or the language that contains most of the conversation.
* Generate the entire summary and all entity labels in the customer's primary language.

**Summary**

* Generate a concise, accurate, and factual summary of the conversation.
* Include:
    * The customer's primary request or issue.
    * Important actions taken by the bot.
    * Information explicitly provided by the customer.
    * Any outcomes or pending next steps.
* Don't omit important information.
* Don't add, infer, or hallucinate information that's not explicitly stated in the conversation.

**Entity Extraction**



* Extract every entity explicitly provided by the customer.
* Preserve entity values exactly as provided unless the entity type is listed under Excluded Entity Types.
* Don't infer, normalize, translate, or modify entity values.
* Don't generate entities that the customer didn't explicitly provide.

**Excluded Entity Types**

Never include the following entity types anywhere in the generated output:


|  |  |  |
| --------------------- | --------------------- | --------------------- |
| Credit Card Number    | Debit Card Number     | CVV                   |
| PIN                   | Password              | OTP                   |
| Security Answer       |                       |                       |
...
| Entities             | `Entity Type`: `Exact Value` |
| Excluded entity types | `Entity Type`: `Redacted`    |
| If no entities are present           | `Entities: None`             |
...
3. On the **Models** tab, toggle on the required models.
   ...
   ![Kore.ai XO GPT models](/ai-for-service/generative-ai-tools/images/xo-gpt-custom-instruction.png)

1. (Optional) For the **DialogGPT** and **Summarizing Conversation** model, select one of the following: 
    * **Default**: Use the default system prompt.
    * **Custom**: To customize the model behavior, click **Configure**, add one or more instructions, and then save your changes. Click **Validate**, provide sample input to the model, and click **Test**. Review the response, update the instructions if needed, and validate again until the response meets your requirements.
    ![Kore.ai XO GPT models](/ai-for-service/generative-ai-tools/images/xo-gpt-summarization-model-preview.png)
...
Administrators can configure a fallback model for each primary model to maintain GenAI availability during outages or transient failures.

To set this up, select a fallback model from the Model Library. It must use a different integration and can't be the same model, and specify a timeout period, which defines how long the fallback model stays active before the platform resumes sending requests to the primary model. You can also use a custom model as the fallback, ensuring it uses the same payload structure as the primary model, since the fallback reuses the primary model's prompt. 

The platform logs fallback activity and makes it available in analytics for monitoring and troubleshooting.


To configure a Fallback Model, follow these steps:

1. Go to **Generative AI** > **Models Library**.
2. In the Fallback Model section, click **+ Add Model**.
3. Select the **Primary Model** and **Fallback Model**. If a custom model is selected as fallback, enter the model ID. Make sure the payload structures for both models are identical to avoid failures. 
4. Select the **Timeout** duration in minutes after which the platform resumes requests to the primary model. The **Timeout** duration supports 1 (default), 2, 5, 10, 30, or 60 minutes, and 3 or 6 hours.
![Kore.ai XO GPT models](/ai-for-service/generative-ai-tools/images/fallback-model.png)
...
| Field                   | Description                                                                                                  |
|-------------------------|--------------------------------------------------------------------------------------------------------------|
| **Maximum Tokens**      | Token usage limit for notification purposes. Exceeding this limit triggers an alert but doesn't block usage. |
