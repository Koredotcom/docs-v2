...
LLM-layer anonymization works alongside [globally declared PII](/ai-for-service/app-settings/advanced-settings/pii-data-masking). Global settings take precedence.

---

## Limit Anonymization Scope

Enable **Limit Anonymization Scope** to anonymize only user input and conversation history instead of the entire outbound LLM request payload. This helps prevent non-PII content that matches a configured regex from being unintentionally anonymized while protecting sensitive user data. Default: Disabled.

<Note>Publish the app to apply the changes.</Note>
