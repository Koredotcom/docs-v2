...
| **Negation** | Model incorrectly processes negation terms (for example, "not," "never"). |
...
|-----------|----------------|-------------------|---------------|----------|----------|
| "Update my address to 1234 Elm St" | "Your address hasn't changed" | "Your address has been updated to 1234 Elm St" | Negation | Address Update | Model failed to process negation correctly. |
...
<Note>
When sharing data, mask all sensitive information. Don't include real customer data, it's used for training purposes. Kore.ai strictly prohibits the use of actual customer data for model training.
</Note>
