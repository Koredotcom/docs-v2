...
**Show Advanced Settings** — Configure Primary and Fallback TTS independently. [Learn more](/ai-for-service/channels/voice-gateway/speech-customization#labels-and-fallback-provider-related-parameters).
...
<Note>The Channel field of default Start Flows can't be edited.</Note>
...
- Deleted flows can't be restored.
- Default start flows can't be deleted.
...
| [IVR Digit Input](/ai-for-service/flows/node-types/ivr-digit-input) | Voice | Collect numeric input (for example, phone number, last 4 card digits). |
...
<Note>
- Deleted nodes can't be restored unless a previously saved version contains them. Restoring a version may overwrite other changes — proceed with caution. See [Restore Versions](/ai-for-service/deployment/app-versioning#restoring-a-version).
- The **Start** node can't be deleted.
</Note>
