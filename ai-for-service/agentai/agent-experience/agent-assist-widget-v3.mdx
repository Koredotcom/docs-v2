...
| **Landing Summary** | Displays a summary of the bot-customer conversation and the call transfer reason when a call transfers from the bot to a human agent. |
...
The **Assist Tab** is the primary agent workspace. It automatically monitors conversations and displays Dialog Tasks, FAQs, Knowledge AI results, Playbooks, Agentic Copilot messages, and coaching prompts.
...
- Displays the **Generate NBA** button so you can manually trigger an NBA suggestion when **Proactive Mode** is off. The button appears after a new customer message arrives and the previous NBA generation finishes.
- Retrieves knowledge base content based on customer intent.
- Shows historical automation runs and conversation history.
- Displays the **Landing Summary** with the bot-to-human conversation summary and, for call transfers, the **Call Transfer Reason**. 
- Supports file and image attachments up to 25 MB.
- Allows on-demand summary generation (up to 2 times per conversation).

<Note>The **Generate NBA** button sends only the latest customer query to Agentic Copilot. For more accurate NBAs, include additional metadata, such as the full chat history or a conversation summary, to give Agentic Copilot sufficient context for relevant suggestions. To include this metadata, go to **Linked Services** > **Pass Metadata**, and select the desired information, such as **Agent-Customer Chat History**. You must also add the same variables to the prompt in the Agentic Platform.</Note>
...
| **Generate NBA** | Manually trigger an NBA suggestion. |
...
<Note>
- Auto-send turns off automatically if Agentic Copilot fails to respond.
...
The **Landing Summary** appears when a conversation transfers from the bot to a human agent. It gives the agent a quick overview of the bot-customer interaction before the transfer.

The **Landing Summary** includes the following:

* A summary of the bot-customer conversation before the transfer.
* **Call Transfer Reason** — For bot-to-human call transfers, the Landing Summary includes a **Call Transfer Reason** section that explains why the bot transferred the call to the agent.
...
* **Submit option**-Sticky at the bottom of the form.

  ![Submit Option](/ai-for-service/agentai/agent-experience/widget-content-update/inline-form-submit-button_8.png)
...
1. Select **More** > **Feedback**.
   ...
   ![Feedback Option](/ai-for-service/agentai/agent-experience/widget-images/overall-widget-feedback-button.png)
...
| Setting | Description |
|---------|-------------|
| **Proactive Mode** | Controls the widget's listening mode. When on, the platform generates NBA suggestions automatically. When off, agents can select the **Generate NBA** option to manually trigger an NBA suggestion. Agents can turn it off at runtime. If an admin turns it off, agents can't turn it back on. |
...
| **Proactive option** | Top-right corner of the widget | Settings tab |
