...
|-------------------------------------------------------------------|------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| NICE CX Integration Hubs > Add Agent Assist App                   | Agent AI URL                                   | The domain of Agent AI.                                                                                                                 | - If it's legacy Agent AI, the URL is `https://agentassist.kore.ai`.<br />- If it's Platform, the URL is `https://platform.kore.ai`.<br />- If it's on-prem, the URL is the origin where your Agent AI is hosted. |
...
| NICE CX Integration Hubs > Add Agent Assist Hub                   | Voice Gateway (VG)                       | For SaaS in the US region, the value is `wss://savg-webserver.kore.ai`.<br />For on-prem, refer to the corresponding host.                |                                                                                                                                                                                                                                                                    |
...
8. Add the following settings with config parameters:
    ...
    4. **Audio Relay Endpoint**: `{Voice Gateway(VG)}/audiosocket/nice?sipuri=<Bot SIP URI><DNS>&token=<Token>&botId=<Bot ID>&accountId=<Account ID>&agentassist=true`
...
8. In the PopURL, update the URL with the following values: 
   ...
   - Pop Destination: Contact Panel (it will be preset for imported script)
...
9. Click **Accept**. 
10. The Kore Agent AI widget loads inside the contact panel.
