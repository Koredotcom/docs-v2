...
* Agent AI
    ...
    * An existing Agent configured with:
        * Channels → Voice Gateway (VG) set up
...
| Nice CX Integration Hubs > Add Agent Assist App | Voice Gateway (VG) | For SaaS in the US region, the value is `wss://savg-webserver.kore.ai`. For on-prem, refer to the corresponding host. | |
...
8. Add the following settings with config parameters:
    ...
    * **Audio Relay Endpoint**: `{[Voice Gateway(VG)]}/audiosocket/nice?sipuri=<Bot SIPURI><DNS>&token=<Token>&botId=<BotID>&accountId=<AccountID>&agentassist=true`
