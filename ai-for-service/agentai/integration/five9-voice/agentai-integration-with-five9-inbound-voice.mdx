...
|-----|------------|-----------|
| `Identifier` | A unique identifier in the Data Table to fetch bot details at run time. It can be a skill name or a campaign name. | |
...
8. The system forwards the transcriptions to Agent AI backend.
9. Agent AI backend shares the transcripts with Agent Desktop Widget (Agent AI Transcript Tab).
10. Agent AI backend shares the transcripts with Agent AI bot, and the bot recognizes the intents and sends responses back to the Agent AI Widget. This process continues until the end of the call.
...
Map each skill or campaign to only one Agent AI bot. This setup renders the correct Agent AI widget for conversations routed through the defined skill or campaign.
...
You have a skill named Chat_Skill, that handles incoming chat requests, and you want the Agent AI widget to render for conversations routed to this skill. You must create the following details of the third-party configuration record in your Agent AI account:
    ...
    Ensure you have selected the Third-Party Configuration setting in **Agent AI** > **Configurations** > **Widget Settings** > **Agent AI Settings** > **Agent AI General Settings**.
...
10. Click the **Edit** icon next to the parameters and change per the following:
    * call_id -> callid
    * domain_id -> domainid
    * id -> agentid
    * session_id -> conversationid
    * skill_name/campaign_name -> identifier
11. In the **Parameters** section, click the **Add** dropdown list, and select **Constant**.
...
14. By default, the system selects **Call** for **Trigger Event** and **On Call Accepted** for **On Call Event**.
15. Click **Done**.
...
5. In the **Integrations** section, add the connector you created earlier. 
...
7. Configure the following Destinations and enter the **Trust Token** value (provided by Kore).
...
Follow [this doc](/ai-for-service/agentai/agent-experience#access-custom-data) for the detailed steps on how to access Custom Data.

You can access the Secure Custom Data as `{{context.session.UserContext.secureCustomData.<key>}}`

Follow the same steps of [this doc](/ai-for-service/agentai/agent-experience#access-custom-data) for detailed information on how to access Secure Custom Data, except for the path being `{{context.session.UserContext.secureCustomData.<key>}}`
