...
6. In **Query Module**, use the [Conversation Details and Summary API](/ai-for-service/apis/automation/conversation-details-and-summary#conversation-details-and-summary-api) to retrieve data from Kore platform when users hang up during interaction with Kore bot. 

### SIP Trunk Provisioning

The Five9 team sets up the SIP trunk on their side so that Kore can identify the DID number during a SIP INVITE from Five9. Since this is a backend change, the Five9 team handles this configuration. As Kore is an ISV partner, Five9 covers these steps in their onboarding documents. 
