...
| Nice CX Integration Hubs > Add Agent Assist App | Voice Gateway(VG) | For the US region, value is `wss://savg-webserver.kore.ai`. For on-prem, refer to corresponding host. | |
...
8. Add the following settings with config parameters:
    ...
    * Audio Relay Endpoint: `{Voice Gateway(VG)}/audiosocket/nice?sipuri=<Bot SIP URI><DNS>&token=<Token>&botId=<Bot ID>&accountId=<Account ID>&agentassist=true`
