---
title: Agent AI Integration with NICE MAX Desktop for Outbound Calls
sidebarTitle: Agent AI Integration with NICE MAX Desktop for Outbound Calls
noindex: true
hidden: true
---
...
* **Provide Studio Script for Outbound Voice**: The support team assists you in provisioning the Studio script to ensure seamless integration with the CXOne platform.
...
| Nice CX Integration Hubs > Add Agent Assist App | Voice Gateway | For the US region, value is `wss://savg-webserver.kore.ai`. For on-prem, refer to corresponding host. | |
...
8. Add the following settings with config parameters:
    ...
    * Audio Relay Endpoint: `{Voice Gateway}/audiosocket/nice?sipuri=<Bot SIP URI><DNS>&token=<Token>&botId=<Bot ID>&accountId=<Account ID>&agentassist=true`
...
6. Update the values for the corresponding parameters:
    ...
    2. Custom Data: Use the **customdata** object in the script to pass information to the Kore AI Agent. During runtime, this customdata is accessible in the UserContext of the AI Agent. Refer to [Access Custom Data in Agent AI Bot](/ai-for-service/agentai/agent-experience#access-custom-data) for instructions on how to access customdata in the AI Agent.
