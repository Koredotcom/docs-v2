...
| Install Audiohook for Voice Streaming | Voice Gateway (VG) | For SaaS in US region, value is `savg-webserver.kore.ai`.<br />For on-prem, refer to corresponding host. |  |
...
5. Open the **Audiohook**, and go to the **Configuration** tab.
   ![audiohook-configuration](/ai-for-service/agentai/integration/genesys/kore-data-table/audiohook-configuration-41.png)
    ...
    2. The **Connection URI** format should be similar to `wss://{Voice Gateway(VG)}/audiosocket/genesys/?sipuri=xxx&token=xxx&botId=xxx&accountId=xxx&agentassist=true`.
...
* Voice Gateway(VG) host name to be referred from the corresponding environment being used at Saas or on-prem.
</Note>
