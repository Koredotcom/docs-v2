...
| Create an Interaction Widget in Genesys for Agent AI | Interaction Widget URL params | - `multibot=true` represents usage of new version of Agent AI integration that supports agent specific AI Agent<br />- `x_metadata` - URL-encoded JSON containing data required by Kore Middleware service<br />- `x_passthru_metadata` - Optional, can be JWE/JWT/base64/URL-encoded JSON stored as custom data | `x_metadata` JSON object structure:<br />&#123;"datatable":&#123;"name":"&lt;datatable name&gt;","token":"&lt;jwt token&gt;","qDelimiter":"&lt;Paste the special character used in the queue name&gt;"&#125;&#125; |
...
3. Create a new Data Table by clicking the **+ Add** button, as highlighted on the following screenshot:
   ![add-data-table](/ai-for-service/agentai/integration/genesys/images/add-data-table.png)
...
4. Enter the following details in the **New Table** page:
    ...
    4. **Indexes** for the table.
        ...
        * **Is Unique** flag to define if the index contains unique values. (Enable **is Unique**.)
        * **Column & Sort Order** - List of columns to include in the index; you can select multiple columns and specify the sort order (ascending or descending) for each of the selected columns. (Select QueueIdentifier with ascending order.)
...
![create-new-data-table](/ai-for-service/agentai/integration/genesys/kore-data-table/create-new-data-table-9.png)
...
<Note>
* x_passthru_metadata is an optional query parameter. Required when you wish to pass custom data to Agent AI.
...
* sendEventsToVG: Determines whether events should be sent to VG to pause or resume transcriptions when a call is placed on hold or resumed. This helps reduce Automatic Speech Recognition (ASR) costs by avoiding transcription of hold music from Genesys.
...
2. Build `x_metadata`:
    ...
    2. Click **Data** > **Data Tables**, and copy the **Data Table** name:
       ![copy-data-table-name](/ai-for-service/agentai/integration/genesys/kore-data-table/copy-data-table-name-19.png)
    ...
    4. Copy the **Client ID** and **Client Secret** values.
       ![client-id-client-secret-values](/ai-for-service/agentai/integration/genesys/kore-data-table/client-id-client-secret-values-21.png)
...
6. Once finished, the **Assigned Permissions** view looks like the following image:
...
![audio-monitoring-toggle](/ai-for-service/agentai/integration/genesys/kore-data-table/audio-monitoring-toggle-27.png)
    ...
    **Audio Monitoring**: Voice settings on a Queue now have an additional **Audio Monitoring** option, separate from **Voice Transcription**. This gives you control over which features you turn on. Audio Monitoring activates in the queue only if your organization activates Audiohook Monitor.

## Chat Setup

### Create/Update Architect Inbound Message Flow in Genesys for the Agent Queue

This step manages incoming messages to the Genesys platform. When you initiate a chat from the [web chat messenger](https://developer.genesys.cloud/devapps/web-chat-messenger), it first gets directed to this inbound message flow.
...
4. Click the **+Add** button to create an **Inbound Message Flow**.
   ![create-inbound-message-flow](/ai-for-service/agentai/integration/genesys/kore-data-table/create-inbound-message-flow-29.png)
...
2. Click **New Configuration**.
   ![messenger-new-configuration](/ai-for-service/agentai/integration/genesys/kore-data-table/messenger-new-configuration-31.png)
...
3. Click **New Deployment**.
   ![new-messenger-deployment](/ai-for-service/agentai/integration/genesys/kore-data-table/new-messenger-deployment-36.png)
4. Enter a name and description.
   ![messenger-deployment-details](/ai-for-service/agentai/integration/genesys/kore-data-table/messenger-deployment-details-37.png)
...
9. Copy the Messenger snippet and deploy the Messenger snippet to your website.
    * If you are building a custom Messenger, copy the deployment key.
        ...
        ![messenger-window](/ai-for-service/agentai/integration/genesys/kore-data-table/messenger-window-38.png)
...
3. Enter `audiohook` in the search bar.
   ![audiohook](/ai-for-service/agentai/integration/genesys/kore-data-table/audiohook-39.png)
...
4. Install a new audiohook app by clicking the **Integrations** button on the top-right corner.
   ![audiohook-monitor](/ai-for-service/agentai/integration/genesys/kore-data-table/audiohook-monitor-40.png)
5. Open the **Audiohook**, and go to the **Configuration** tab.
   ![audiohook-configuration](/ai-for-service/agentai/integration/genesys/kore-data-table/audiohook-configuration-41.png)
...
* Voice Gateway host name to be referred from the corresponding environment being used at SaaS or on-prem.
</Note>
...
6. Follow these steps to get the **Connection URL**:
    ...
    4. Click the **Copy** button next to the autogenerated **Connection URL** field.
...
10. Use the **ClientID** and **ClientSecret** of the AI Agent you have used while configuring the Audiohook. The system uses these credentials to validate the audio stream signature by Kore.
![audiohook-credentials](/ai-for-service/agentai/integration/genesys/kore-data-table/audiohook-credentials-44.png)

11. Click **Save**.

## Create/Update Architect Inbound Call Flow in Genesys for the Agent Queue

When a call comes to a Genesys number, it passes through an architect flow. If you don't have an ACD architect flow for the queue, create one by following this section.

#### Steps to create a call architect flow

1. In [Genesys Cloud](https://apps.mypurecloud.com/), go to **Admin** > **Architect** > **Architect**.
![genesys-architect](/ai-for-service/agentai/integration/genesys/kore-data-table/genesys-architect-45.png)
3. Click the **+Add** button to create an **Inbound Call Flow**.
![create-inbound-call-flow](/ai-for-service/agentai/integration/genesys/kore-data-table/create-inbound-call-flow-46.png)
...
You can access Custom Data and Secure Custom Data in Welcome Events in the Agent AI configuration page and in Dialog Tasks in the Agent AI automation page.

You can access Custom Data as `{{context.session.UserContext.customData.internalMetaData.<key>}}`.
...
You can access Secure Custom Data as `{{context.session.UserContext.secureCustomData.<key>}}`.

Follow the same steps of [this doc](/ai-for-service/agentai/agent-experience/access-custom-data-in-agent-ai) for detailed information on how to access Secure Custom Data,exception for the path being `{{context.session.UserContext.secureCustomData.<key>}}`.
...
1. In **Call Routing**, assign the **Architect flow** created in **[Step-4](#steps-to-create-a-call-architect-flow)** to a Genesys Phone Number.
   ![call-routing](/ai-for-service/agentai/integration/genesys/kore-data-table/create-call-route-53.png)

   ![create-call-route](/ai-for-service/agentai/integration/genesys/kore-data-table/create-call-route-53.png)
