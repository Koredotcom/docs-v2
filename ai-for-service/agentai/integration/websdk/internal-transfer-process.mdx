---
title: Internal Transfers - SDK Integration Guide
sidebarTitle: Internal Transfers - SDK Integration Guide
noindex: true
hidden: true
mode: center
---
...
* For all types of **entry** events, such as cold, warm, or no-transfer scenarios, you must invoke the `internalTransfer` method when the SDK emits the **ON_SOCKET_CONNECTION** event.
...
<Note>The **conversation ID remains constant** throughout all these events to ensure continuity in Agent AI's context.</Note>
...
|---|---|---|---|
| `transfertype` | String | Type of transfer: `"NA"` - for first entry and final exit, `"cold"` - for cold transfers, `"warm"` - for warm transfers, `"redial"` - for reconnecting to the same conversation after an abrupt end | `"NA"` / `"cold"` / `"warm"` / `"redial"` |
| `transition` | String | Action: `"entry"` - when an agent joins, `"exit"` - when an agent leaves | `"entry"` / `"exit"` |
| `participant` | Object | Information about the agent involved in the event | `{...}` |
| `participant.identity` | String | Unique ID of the agent | `"agentId1"` |
| `participant.name` | String | Agent's display name | `"AgentName1"` |
| `participant.type` | String | Type of participant (always `"agent"` in these scenarios) | `"agent"` |
...
When the customer connects with the first agent, it's unknown whether a transfer occurs later.
To establish the conversation in the SDK, send an **initial entry event**.
...
In this case, the desktop needs to inform the SDK that it's **rejoining the same conversation** to avoid creating new segments.
...
<Note>
* In Internal Warm Transfer, Agent AI Welcome Event (any Dialog) doesn't trigger in any of the channels (Chat, Voice, Email).
* In Warm Transfer, only the first agent performs the dialog tasks one at a time. In the conversation, the other agent behaves passively.
</Note>
