---
title: View Customized Events
sidebarTitle: View Customized Events
---

## BEFORE_VIEW_INIT

You can customize the chat window by listening to various events. For example, you can modify the view before it's initialized.

`BEFORE_VIEW_INIT` is the generic event for all the elements that are going to render in the DOM.

The `BEFORE_VIEW_INIT` event delivers an HTML response and a payload before they're rendered in the Agent AI widget. This event allows us to customize the data and behavior for each individual event within the widget, enabling tailored content and functionality based on specific needs.
...
This event allows you to customize (icons, text font, style, and color) the footer elements that's, Search, Transcript, and Assist.
