...
sidebarTitle: SDK Customization
noindex: true
hidden: true
---
...
| `default`  | Boolean          | Determines if this tab is the default active tab when the widget loads. Only one tab must be true. | `true`, `false` |
| `isEnabled`| Boolean          | Controls tab visibility in the widget. `true`: visible, `false`: hidden.  | `true`, `false` |


**Behavior Summary**

- `isEnabled`: true appears in the widget. False is hidden.
...
- If Desktop Settings updated, then the new changes reflect only after changing conversationId (new Conversation).
...
| `message`    | String   | Message content (processed and sent through the WebSocket connection)                 | `"Hello!"`   |
...
| `lastName`  | String   | Agent's last name                  | `"Doe"`                                        |
| `author`    | `Author` | Agent information                  | `{ id: 'agent123', type: 'AGENT', firstName: 'Jane', lastName: 'Doe' }` |
...
aaWindowInstance.sendAgentMessage({
  author: { id: "agent123", type: 'AGENT', firstName: "Jane", lastName: "Doe" },
...
This method triggers a WebSocket event and the response is handled by the socket message listeners.
...
- **Disabled**—The dialog task entities aren't detected or extracted automatically and other proactive functionalities remain unaffected.
- **Enabled**—The dialog task entities are automatically detected and extracted.

## User interface (UI) override option

This feature allows user interface to follow the exact user experience (UX) implementation as in My AI Agent (Bot). This feature doesn't require any changes to the existing design.
...
This method disables the widget. When you disable the widget, you can still see the widget but can't interact with it.
...
This banner notifies users that the widget is currently non-functional due to configuration settings and how to contact the Agent AI admin for assistance.
