...
* **API Method**: Select the HTTP method (GET or POST) that Agent AI uses to invoke the external API.
...
* **API Endpoint**: Provide the complete API endpoint URL from which Agent AI fetches the data. For example, Salesforce Apex REST endpoint for retrieving case information.
    ...
    <Note>You can add static query parameters that aren't associated with the Salesforce Case object directly to the API endpoint. For example, `https://your-domain.my.salesforce.com/services/apexrest/case?isSorted=true`</Note>
