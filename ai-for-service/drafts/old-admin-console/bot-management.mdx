...
After a developer publishes a bot from Bot Builder, it appears on the **Deployment Requests** page. Bots aren't available to users until the admin approves and assigns them.
...
| Setting | Description |
|---------|-------------|
| **Auto Approve** | Don't approve, auto-approve all, or auto-approve specific components (New Channels, New Tasks, Channel Upgrades, Task Upgrades) |
...
Consumer bots are approved bots with the deployment model set to **Consumer**. they're available to customers immediately after approval — no user assignment is needed.
...
<Note>Bot assignments can't be automated unless deployment requests are already set to auto-approve.</Note>
