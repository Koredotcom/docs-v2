...
Use this section to synchronize and import user information, mailing lists, and groups from your organization's directory. The sync can be automated to ensure seamless and up-to-date integration.

**Prerequisites**

## Prerequisites

Before you configure Active Directory synchronization, ensure that you:

1. Have administrative access to read and execute actions on the LDAP server.
2. Understand LDAP queries and LDAP server administration.
3. Have access to a Microsoft Active Directory LDAP server.

## Configure a Cloud-Based Connector

Use a cloud-based connector to securely connect to your Active Directory server over the internet.

1. In Admin Console, navigate to **User Management** > **Directory Sync** and select **Configure**.

    ![Run manual sync](/ai-for-service/administration/images/sync-with-company-directory.png)

2. The Cloud-Based Connector dialog is displayed. Follow the steps given here to proceed. 
![Run manual sync](/ai-for-service/administration/images/cloud-based-connectors.png)

3. On the **Connection** tab, define the following fields:

    - **Host Name**: The name of the domain host, for example, `www.example.org` or `10.12.6.151`.
    ...
    - **Test Connection**: Select to test the authentication and host configuration settings.
...
7. On the **Schedule** tab, enable and define the synchronization schedule with the Active Directory.
...
To view the last run report for an Active Directory sync using the cloud-based connector, in the Admin Console User Management module, go to the Directory Sync page and click here.
...
To view historical reports for Active Directory sync using the cloud-based connector, in the Admin Console User Management module, go to the Directory Sync page and, in the Current Settings section, click Sync History.


**Viewing a Report**

In the **History of Active Directory Synchronization Runs** dialog, to view the results of an import, in the **Report** column, click **Sync Results**.
