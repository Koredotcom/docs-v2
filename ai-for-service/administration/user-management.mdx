...
Clicking on a specific User row from the Users page opens the **User Details** page. On the page, modify [User Profile](#profile), [Sessions/Devices](#sessions-and-devices), and [Manage Product Access](#manage-product-access).
...
* **Groups** to which the user is assigned. From the dropdown, select the groups for the given User. [Refer here for more on Managing Groups](/ai-for-service/administration/groups-management).
...
<Warning> No confirmation message is displayed, and the session is immediately and abruptly ended for the user. </Warning>
...
* **Privacy**: Users should be aware that their sessions in one workspace might be visible to administrators of other workspaces they're part of.
...
You can also edit user details, reset user's password, or delete a user.

![Other Options](/ai-for-service/administration/images/more-user-options-new-admin-console.png)

{/* AG: Better quality screenshot required. Blur dummy or internal names in it. */}
...
<Note> When SSO is enabled, password policies aren't in effect for Admins or Kore.ai users; the SSO service provider handles the password policies. </Note>

<Tip> If SSO is enabled, Admins can still log on to the Admin Console directly using their password credentials and bypass SSO. For more information, see [Logging On Using Your Admin Password Instead of SSO](/ai-for-service/administration/single-sign-on). </Tip>

{/* AG: Removed anchor #logging-on-using-your-admin-password-instead-of-sso for now; add the correct one. */}
