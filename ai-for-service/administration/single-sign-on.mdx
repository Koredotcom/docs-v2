...
![SSO](/ai-for-service/administration/images/sso-new-admin-console.png)
...
![SSO SAML](/ai-for-service/administration/images/sso-saml-new-admin-console.png)
...
4. In the administrative console for your Single Sign-On provider, you will also need to define the URLs that are used to exchange data between the application and your SSO provider. While the URL names may vary by SSO provider, you will need to define these URLs: 
   ...
   3. **Assertion Consumer Service (ACS) URL** or **Callback URL** as `https://idp.kore.com/authorize/callback`.
...
3. In the **Configure SSO for SAML** section, select an identity provider, and then define the settings for one of:  
   ...
   7. **SAML Attribute Mapping**: Enable this option to map SAML Attributes with AI for ServiceGroup Names or Admin Roles Names. Once enabled, you can:  
      ...
      * define one or more mapping pairs. Each pair includes  
        ...
        3. Attribute Type: Refers to the type of mapping to be associated that's Group Name or Role Name  
...
{/* AG: Check the idproxy-dev links - should we be linking to -dev URL? These links may not work outside Kore. */}
...
7. In **Configure SAML**, provide the *Single Sign-On URL.* To obtain this:  
    ...
    5. For on-prem accounts, the ACS URL for SP Initiated SAML Flow is displayed as `https://idproxy-dev.kore.com/authorize/callback`. This needs to be updated as "single sign on URL" in Okta platform and `https://idproxy-dev.kore.com` needs to be updated as the "Audience URL"
...
8. To configure the **ACS URL for IDP-Initiated SAML Flow**, copy and paste the complete URL that you get from your admin. In case you are using internal redirection URLs, please reach out to your internal admin/network teams. To get the **ACS URL for IDP Initiated SAML Flow**, check "Use this Recipient URL and Destination URL". Enter the **Audience URI** field as ACS URL for SP Initiated SAML Flow `https://idp.kore.com/authorize/callback`.
    ...
    <Note> This URL is a temporary URL. Once you add the AI for Service app in Okta, this URL changes as per your organisation's domain requirements. You must copy the final ACS URL again and paste it in the field. For more information, read [How to configure Okta SSO in AI for Service Platform](https://community.kore.ai/t/sso-how-to-configure-okta-sso-in-kore-ai-xo-platform/2618) community post.</Note>

9. Select **Finish**.

10. On the **Sign On** tab, in the **Settings** section, select **View Setup Instructions**.
...
12. In the Okta **X.509 Certificate** field, copy the certificate data, and then paste the data into the AI for Service **Certificate** text box. 
    ...
    <Note> Only data after the **BEGIN CERTIFICATE** header and before the **END CERTIFICATE** footer should be copied and pasted into AI for Service. </Note>
...
* **SAML Attribute Mapping**: Enable this option to map SAML Attributes with AI for Service Group Names or Admin Roles Names. Once enabled, you can:  
  ...
  * define one or more mapping pairs. Each pair includes  
    ...
    * Attribute Type: Refers to the type of mapping to be associated, that's Group Name or Role Name  
...
8. In the **X.509 Certificate** section, copy the certificate data, and then paste the data into the AI for Service **X.509 Certificate** text box. 
    ...
    <Note> Copy and paste only the data after the `BEGIN CERTIFICATE` header and before the `END CERTIFICATE` footer in AI for Service. </Note>
...
![SSO OneLogin](/ai-for-service/administration/images/using-single-sign-on-img4.png)

### Bitium Configuration

{/* AG: Bitium is gone. If the business doesn't remain then I strongly suspect that the below steps will change to use GCP OR the config is no longer supported. */}

* **Single Sign-On URL**: The HTTP SSO endpoint for *Bitium* to enable Service Provider-initiated SAML flow, for example, `https://www.bitium.com/7655`.  
...
7. In the Bitium **X.509 Certificate** field, copy the certificate data, and then paste the data into the Admin Console **Certificate** text box.  
    ...
    <Note> Only data after the **BEGIN CERTIFICATE header and before the **ENCERTIFICATE** footer should be copied and pasted into AI for Service. </Note>
...
![Bitium portal](/ai-for-service/administration/images/using-single-sign-on-img5.png)
  ...
  * **SAML Attribute Mapping**: Enable this option to map SAML Attributes with AI for Service Group Names or Admin Roles Names. Once enabled, you can:  
    ...
    * In the administrative console for your Single Sign-On provider, you will also need to define the URLs that are used to exchange data between AI for Service and your SSO provider. While the URL names may vary by SSO provider, you will need to define these URLs:  
      * **Assertion Consumer Service (ACS) URL** or **Callback URL** as `https://idp.kore.com/authorize/callback`.
...
{/* AG: Check the above links - these show 400 bad request in the browser. */}
...
When SSO is enabled for an account, administrators can exclude specific users from mandatory SSO authentication. Excluded users sign in with platform credentials, while all other users continue to authenticate through the configured identity provider (IdP). This is useful when organizations enforce SSO for employees while enabling external collaborators, such as partners and contractors, to sign in using platform credentials without being provisioned in the organization's IdP.
To exclude users, search for their email address and add them to the SSO exclusion list. Removing a user from the exclusion list restores mandatory SSO authentication. Changes take effect on the user's next sign-in.
