...
sequenceDiagram
    ...
    User->>QP: "What's the return policy for electronics?"
    Note over QP: Query understanding → Intent detection → Query expansion
    QP->>R: Processed query
    Note over R: Vector search → Keyword search → Hybrid ranking
    R-->>AG: Chunk 1
    R-->>AG: Chunk 2
    R-->>AG: Chunk 3
    R-->>AG: Chunk 4
    Note over AG: LLM synthesizes answer from retrieved chunks with citations
    AG-->>User: "Electronics can be returned within 30 days of purchase..."<br/>[Source: Return Policy Document, Section 2.3]
```
