...
|:-----|:----------------------------|:----------------------------------------------------------------------------|
| 1    | Content Ingestion           | Importing source documents that's used to generate answers                  |
...
| Layout-aware | Layout-based segmentation; content between headers treated as chunks | Structured documents with clear headings |
| Advanced HTML| Extracts data from tables and images in HTML files along with textual content|Rich HTML pages|
| Markdown| Converts source documents into structured markdown format before chunking | PDF files|
| Image based | Converts pdf files into image format, preserving visual structure and contextual relationships |Complex PDF files with non textual layouts|
| Custom | Allows chunking using external third party service |Custom docs with special layout|
...
| Enterprise Application Integration | Real-time access to business data, say from Salesforce, ServiceNow, and Jira  |
