---
title: Agent AI Release Notes 2024 Archive
noindex: true
hidden: true
---

<Badge icon="arrow-left" color="gray">[Back to the latest release notes](/ai-for-service/release-notes/agent-ai)</Badge>
...
The Chat or Conversation Details and Summary API now includes Agent messages to help generate the complete conversation summary for those who use the Chat History API to access conversations.
...
* Internal and External Answers can't be combined in one app.
...
When a single error message is triggered during dialog task execution, the **Assist** tab displays the error message and the number of occurrences. If the same error message is triggered again, the message doesn't repeat, but the number of occurrences increases.  
**Multiple Error MessageS**

When multiple (different) error messages are triggered, they're all displayed in the **Assist** tab, with a count indicating the total number of error messages.  
...
* New URI: [Genesys](https://platform.kore.ai/koreaiaa-genesys/)
...
If the file or image isn't automatically detected, you can override and manually attach the file. To maintain system efficiency, attachments are limited to one file at a time and must not exceed 25 MB in size.
...
**When Enabled**: When a dialog task is enabled for a specific channel, it's triggered automatically or manually by the agent during interaction with customers on that specific channel.
...
* A warning is displayed when a user enters incorrect values in the configuration fields -  “couldn't establish connection with SearchAssist. Check all parameters and try again. Hint: Ensure that the domain URL doesn't contain "https://" and doesn't have a forward slash "/" at the end.”
* As soon as a user lands on the configuration page, the following hint is displayed: “Ensure that the domain URL doesn't contain "https://" and doesn't have a forward slash "/" at the end.” 
...
* Handling Incomplete Summaries: The new model handles incomplete summaries more effectively, providing more comprehensive and useful insights even when the conversation isn't fully concluded.
