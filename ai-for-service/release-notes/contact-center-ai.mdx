...
A new `Include In-Progress Conversations` option displays active conversations with their current status and wait time. Agent Custom ID, a new Support field, and a configurable date format have also been added. [Learn more→](/ai-for-service/analytics/contact-center/reports/interaction-details-report.mdx)
...
The new public API retrieves translation logs for debugging. For each message, it returns the input sent to the translation engine, the output received, the session ID, and the timestamp.
