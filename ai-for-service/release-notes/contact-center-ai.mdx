...
Notes in the Console and Conversations Dashboard now support translation based on the user's selected language. It supports translated-only and dual-view modes for conversation translation, with no additional configuration required. [Learn more→](/ai-for-service/contact-center/agent-console#notes-translation)

<p className="rn-title">Configuration</p>

<p className="rn-subtitle">Restrict Translation Languages to User Assignment</p>

Added a setting “Limit Translation Languages to User's Assigned Languages.” When selected, the translation dropdown shows only the logged-in user's assigned languages across Agent Console, Monitor, and Interactions. [Learn more→](/ai-for-service/contact-center/configurations/settings/configuration-settings#translation-configurations)

<p className="rn-subtitle">Disposition Capture During Transfers</p>

Added a setting “Capture Disposition During Transfer.” When active, a dropdown appears during agent or supervisor transfers, allowing optional selection of one or more dispositions.

Selected dispositions are stored with the interaction and remain available for reporting, while transfers continue without interruption if skipped. [Learn more→](/ai-for-service/contact-center/configurations/settings/configuration-settings#translation-configurations)

<p className="rn-title">Analytics</p>

<p className="rn-subtitle">Interaction Details Report Updates</p>

Added “Include in-progress conversations” option to show active conversations with current status and wait time. Included Agent Custom ID and a new Support field, along with a configurable date format. [Learn more→](/ai-for-service/analytics/contact-center/reports/interaction-details-report.mdx)

<p className="rn-subtitle">Open Wallboards in Separate Window </p>

Added a “Launch Wallboard in New Window” option in both Configuration and Wallboards views. Selecting this action opens the chosen wallboard in a separate browser window for independent viewing. [Learn more→](/ai-for-service/analytics/contact-center/wallboards#launch-wallboard)

<p className="rn-title">API</p>

<p className="rn-subtitle">Translation Log API</p>

Introduced a public API to retrieve translation logs for debugging. For each message, the API returns the input sent to the translation engine and the output received, along with the session ID and timestamp.
