...
The `Deleted` label has been replaced with `Inactive` wherever Agent Name appears across reports, dashboards (Queues, Agents, Interactions), wallboards, filters, and configuration screens now return `Inactive` instead of `Deleted,` ensuring consistency across UI and data outputs.
