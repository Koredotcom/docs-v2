...
* On the **Linked Services** > **Agentic Configuration** page, an Agentic app that's currently in use shows an "in use" status and can’t be deleted.  
...
The Salesforce package now includes a language parameter to support AgentAssist's multi-language capability. During installation, users select a language that sets the *interactive_language* parameter (for example, interactive_language=Spanish), which is sent to AgentAssist to conduct all interactions (for example, welcome messages, task running, sentiment analysis) in the selected language.
