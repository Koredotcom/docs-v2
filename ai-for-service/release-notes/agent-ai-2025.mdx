...
The Overall Widget Feedback feature allows admins to configure feedback options that agents can select for the Agent AI widget during conversations. It's available in ‘Feedback Settings’ and is disabled by default. Once enabled, admins can: 
...
* Platform: Identify and surface intents, such as dialogs and FAQs, through the Platform.
...
* Welcome Events: When you turn on Agentic Copilot, this section displays the message *Agentic Copilot is enabled, and it will run after the execution of the enabled Welcome Events*.  
...
* On the **Linked Services** > **Agentic Configuration** page, an Agentic app that's in use shows an "in use" status and you can’t delete it. 
* On the Agentic Configurations > App Information page:  
    ...
    * The X-Api Key is renamed to API Key.   
...
* For customer messages outside the Agentic app's defined scope, the widget displays a message indicating it's not trained to handle them.  
* When you generate multiple Agentic responses with the ‘Allow auto send Message’ option turned on:  
    * The timer for each message starts only after the previous message is sent.  
    * If the view doesn't auto-scroll to the latest message, an arrow icon appears in the bottom-right corner, indicating the number of Agentic messages stacked below.  
...
The following Agent AI v3 features are added to the latest SDK:  
...
This update has introduced a service node and a new tag-based approach to generate the landing summary (a concise summary) of the conversation. This summary is used internally to improve intent identification accuracy. It's generated after the Welcome Dialog task ends and isn't visible to agents. However, the detailed summary from the message node continues to be visible to the agents.
...
The Salesforce package now includes a language parameter to support AgentAssist's multi-language capability. During installation, users select a language that sets the 'interactive_language' parameter — for example, 'interactive_language=Spanish'. AgentAssist uses this parameter to conduct all interactions in the selected language, including welcome messages, task running, sentiment analysis.
