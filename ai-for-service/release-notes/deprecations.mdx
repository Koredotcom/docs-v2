...
## ML Network Deprecation

The MLP-BOW, MLP-WordEmbeddings, LSTM, CNN, Transformer, and KAEN network types will be deprecated and removed on September 26, 2026. [Supported network types](/ai-for-service/automation/natural-language/training/machine-learning-engine#ml-network-types) after this date include Standard, Multi-lingual, Zero-shot Model, and Few-shot Model (Kore.ai Hosted Embeddings).

Action required: 


* Before September 26, 2026, if your model uses a deprecated network, migrate it to a supported network and retrain it; otherwise, your app will stop functioning.
* After September 26, 2026, deprecated networks will be automatically migrated to the recommended Few-shot Model paired with the BGE-M3 embedding model. Retrain your app to restore functionality. For more information, see [Embeddings Model Accuracy](/ai-for-service/automation/knowledge-ai/knowledge-graph-overview#embeddings-model-accuracy).


## Embedding Model Deprecation

The Qwen2-7B-Instruct, LaBSE, and MPNet embedding models will be deprecated and removed on September 26, 2026.[ Supported embedding models](/ai-for-service/automation/knowledge-ai/knowledge-graph-overview#few-shot-knowledge-graph) after this date include Pre-trained MPNet and BGE-M3. Migrate to a supported embedding model before the deprecation date. 
...
Microsoft retired older versions of `gpt-4o` (`v2024-05-13` and `v2024-08-06`) and `gpt-4o-mini` on Azure Standard deployments (pay-as-you-go) on March 31, 2026. Use the table below to determine if you need to act.
...
Update all custom prompts that use the `max_tokens` parameter to replace `max_tokens` with `max_completion_tokens`. The `gpt-5` series of models don't support `max_tokens`. If you haven't updated, you see this error: `"Unsupported parameter: 'max_tokens' isn't supported with this model. Use 'max_completion_tokens' instead."`
...
Microsoft updates your deployment after their model retirement. Your model runs on `gpt-5.1` or `gpt-5-mini` after the update. Follow the steps in [Scenario A](#scenario-a—your-deployment-was-auto-updated).
...
![Vector Configuration](/ai-for-service/release-notes/images/deprecations/vector-model.png "Vector Configuration")
...
![Crawler](/ai-for-service/release-notes/images/deprecations/webpage.png "crawler")

Click on the banner to start the upgrade process.

![Crawler Update](/ai-for-service/release-notes/images/deprecations/webpage1.png "crawler update")
...
![update re-ranker](/ai-for-service/release-notes/images/deprecations/re-ranker.png "update re-ranker model")
...
The Voice Authentication (Voice Biometrics) feature is removed from the UI and is currently unavailable. Other Language & Speech configurations aren't affected.
