...
The MLP-BOW, MLP-WordEmbeddings, LSTM, CNN, Transformer, and KAEN network types will be deprecated and removed on August 29, 2026. [Supported network types](/ai-for-service/automation/natural-language/training/machine-learning-engine#ml-network-types) after this date include Standard, Multi-lingual, Zero-shot Model, and Few-shot Model (Kore.ai Hosted Embeddings).

Action required: 


* Before August 29, 2026, if your model uses a deprecated network, migrate it to a supported network and retrain it; otherwise, your app will stop functioning.
* After August 29, 2026, deprecated networks will be automatically migrated to the recommended Few-shot Model paired with the BGE-M3 embedding model. Retrain your app to restore functionality. For more information, see [Embeddings Model Accuracy](/ai-for-service/automation/knowledge-ai/knowledge-graph-overview#embeddings-model-accuracy).


## Embedding Model Deprecation

The Qwen2-7B-Instruct, LaBSE, and MPNet embedding models will be deprecated and removed on August 29, 2026.[ Supported embedding models](/ai-for-service/automation/knowledge-ai/knowledge-graph-overview#few-shot-knowledge-graph) after this date include Pre-trained MPNet and BGE-M3. Migrate to a supported embedding model before the deprecation date. 
