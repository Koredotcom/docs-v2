...
Interrupted calls are returned to the queue at the highest priority and display a `Transferred from <Agent Name> due to a network issue` notification. Interruption data is captured per conversation — including error type, recovery action, and queue wait time — and surfaced via conversation APIs and a new Recovered Errors filter on the Conversations page. [Learn more→](/ai-for-service/contact-center/agent-console#re-routing-voice-calls-due-to-network-interruption)
