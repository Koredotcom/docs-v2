...
Call recording statuses have been updated for clearer visibility into failures. Calls under two seconds now surface a dedicated unavailability status, and a new media_generation_failed status covers failures during recording generation and audio conversion. The deprecated partial_failed status has been replaced by the more precise partial_download_failed and partial_upload_failed statuses. Both the Fetch Call Recording and Fetch Call Recording V2 APIs reflect these updated statuses and messages. [Learn more→](/ai-for-service/analytics/overview/conversations#call-recording)
