...
The `getMessages`, `getMessagesV2`, and `messages-lite` APIs now support an `includeLatency=true` parameter that returns a per-turn latency breakdown for bot response messages on the Voice Gateway, covering ASR, bot processing, and TTS segments. [Learn more→](/ai-for-service/api/automation/conversation-history)
