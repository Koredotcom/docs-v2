...
AI Agent metrics now send custom metadata to the Agentic App via Quality AI. Users can configure additional fields, in addition to the default conversation ID, to include in the execute API payload. A new setup option lets users map custom field registry values to API headers, enabling the structured transfer of conversation-specific data for downstream use. 
