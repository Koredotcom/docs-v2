...
![enable sso](../images/enable-sso.png)
...
![sso updated successfully message](../images/sso-configuration-updated-successfully.png)

Additionally, the timestamp of when you enabled SSO is displayed as shown below:
![sso timestamp](../images/sso-timestamp.png)
...
![saml-okta-selection](../images/saml-okta-selection.png)
...
![okta create app integration](../images/okta-create-app-integration.png)

7. In the **Create a new app integration** window, select **SAML 2.0** and click **Next**.
   ![create a new app integration](../images/create-a-new-app-integration.png)
8. On the **Create SAML Integration** page, provide the **App Name** under **General Settings**, and click **Next**.
   ![create saml integration](../images/create-saml-integration.png)
...
13. On the **How to Configure SAML 2.0** for `<app-name>` application page, do the following from Okta into AI for Process:
    ...
    * Copy the **Identity Provider Issuer** value into the **Identity provider issuer**.
    ![copy identity provider issuer](../images/copy-Identity-Provider-Issuer.png)
...
15. Click **Download certificate** under **Actions** for the required certificate.
    ![download certificate](../images/download-certificate-saml.png)
16. Once the certificate is downloaded, open it in Notepad and copy the data between the **BEGIN CERTIFICATE** header and **END CERTIFICATE** footer.
    ![okta certificate](../images/okta-certificate-notepad.png)
17. Paste the value into the **Certificate** field on AI for Process' SSO setup page.
    ![paste okta certificate](../images/paste-okta-certificate.png)
...
18. Click **Save**.
    ...
    ![okta sign in page](../images/okta-sign-in-page.png)
...
8. Copy the following values from **SSO** > **Enable SAML2.0** on OneLogin and paste them into the relevant fields on AI for Process' SSO setup page:
   ...
   * **OneLogin Issuer URL**: Paste into **Issuer URL**.
   ![paste one login issuer url](../images/paste-onelogin-issuer-url.png)
9. In the **OneLogin X.509 Certificate** field, click **View Details**. The **Standard Strength Certificate (2048-bit)** page is displayed.
   ![one login view details](../images/onelogin-view-details.png)
...
To add a new certificate, click **+Add new**.
![add new x 509 certificate](../images/add-new-x-509-certificate.png)
...
11. Copy the following field values from AI for Process' SSO setup page into the relevant fields in OneLogin:
    ...
    * ACS URL for IDP Initiated SAML Flow.
    ![copy acs urls](../images/copy-acs-urls.png)
...
![one login sign in page](../images/onelogin-sign-in-page.png)
...
![other sso provider](../images/other-sso-provider.png)
...
To add a new certificate, click **+Add new**.
![paste parameters for other provider](../images/paste-parameters-other-configuration.png)
...
![copy idp url](../images/copy-idp-url.png)
...
![paste azure parameters](../images/paste-azure-parameters.png)
...
![paste ws federation data](../images/paste-ws-federation-data.png)
...
![click disable sso](../images/click-disable-sso.png)

3. In the **Disable SSO** confirmation window, click **Yes**.
![disable sso screen](../images/disable-sso-screen.png)

The following screen is displayed after SSO is disabled.
![enable sso screen](../images/enable-sso-screen.png)
...
![manage sso users](../images/manage-users-sso.png)

You can add multiple users to the list, as shown below:
![exclude multiple users](../images/add-multiple-users-to-exclude.png)
...
For roles and permissions for API-scoped apps, see [Module-wise Permissions and Access Levels](../user-management/role-management.md#module-wise-permissions-and-access-levels).
...
|---------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Deploy workflow     | Deploy a specific workflow into an environment. It allows the user to control the deployment process either synchronously or asynchronously. [Learn more](../../workflows/deploy-a-workflow.md). |
...
1. Click **Create an API App** or **Create an App**.
   ![create api app](../images/create-an-api-app.png)

2. Click **Untitled app** and provide the app name.
    ![provide app name](../images/provide-app-name.png)

3. Select the required scopes from the list.
    ![select scopes](../images/scopes-selection.png)
...
4. Click **Next**.
    ...
    A success message is displayed, and the following window is displayed. Follow the steps in [Create an API Key](#create-an-api-key) to complete the process.
     ...
     ![success app creation](../images/api-app-creation-success.png)
...
2. In the **Create new API key** dialog, provide a name and click **Generate Key**.
    ![generate new api key](../images/generate-your-new-api-key.png)

3. Once the key is successfully generated, click **Copy and Close** to copy the API key.
   ![create new api key](../images/create-new-api-key-window.png)
...
![api keys list](../images/api-keys-list-for-apps.png)
...
![apps list](../images/api-apps-list.png)
...
1. Hover over and click the **Edit** icon for the required app.
    ![access edit icon](../images/hover-edit-api-scopes.png)
...
2. In the App's configuration window, do the following:
    ...
    * To change the scopes, click the **API Scopes** tab and select/unselect the listed scopes.
        ![change api config](../images/edit-api-app-name.png)

    * To delete an API key, follow the steps below:
         * Click the **API Keys** tab.
         * Hover over and click the **Delete** icon for the required key.
           ![access delete icon](../images/hover-delete-api-key.png)
         * Click **Delete** in the confirmation window.

            ![delete api keys](../images/del-api-key.png)
...
![edit success message](../images/edit-success-message-api-app.png)
...
1. Hover over and click the **Delete** icon for the required app.
   ![access delete](../images/delete-api-app-icon.png)

2. Click **Delete** in the confirmation window.

      ![delete api app](../images/delete-api-app.png)
...
![create auth profile](../images/click-create-auth-profile.png)

   Otherwise, click **Add new auth**.
![add new auth profile](../images/add-new-auth-profile.png)

2. In the **New Authorization Mechanism** dialog, select the required option for **Authorization Type**.
   ![select oauth v2](../images/select-oauthv2.png)
...
4. Provide the values for the mandatory authorization fields. For details, refer to [Define Authorization Fields](#define-authorization-fields).
   ...
   ![validation errors](../images/form-validation-errors.png)
...
7. Click **Save new Auth**.
   ![save new auth](../images/save-new-auth.png)
...
1. Click **+ Add Additional Field** in the **New Authorization Mechanism** window and enter one or more key-value pairs.
    ...
    ![add additional field](../images/add-additional-field-click.png)
...
3. Click **Done**.
    ![add additional field form](../images/add-additional-field-form.png)

The new field is added to the additional fields list. You can edit or delete this custom field.
![new additional field](../images/new-additional-field.png)
...
![add new auth field](../images/add-auth-field-new.png)
...
![auth profile summary](../images/auth-profile-summary.png)
...
![successful connection](../images/successful-oauth-connection.png)
...
![delete auth profile](../images/delete-auth-profile.png)

2. Click **Delete** in the following window.

    ![confirm delete auth profile](../images/confirm-deletion.png)
