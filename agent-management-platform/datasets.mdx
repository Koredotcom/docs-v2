...
Datasets are curated collections of agent interactions (sessions and traces) used for evaluation, regression testing, and quality assurance. They can also act as golden sets for validating agent behavior for known scenarios or edge cases.
...
|------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------|
| **Policy Evaluation**  | Test policies for a controlled dataset before enabling them on live traffic.                                                                |
...
| **Session Management** | Manually add or remove sessions at any time.                                                                      | You can't manually add sessions. You can edit the filter criteria at any time.                                                                       |
| **Auto-Refresh**       | Doesn't update automatically.                                                                                     | Sessions refresh automatically as new telemetry arrives.                                                                                             |
...
![Dataset Detail](/agent-management-platform/images/dataset-detail.png)
...
Evaluate dataset sessions with one or more policies.

1. Select **Run Policies on Dataset**.
2. Choose one or more policies.
3. Run evaluations across all sessions.

<Note>Multiple policies can run in parallel, allowing you to test different rules or metrics at the same time.</Note>
...
- Deletion removes only the dataset reference. The underlying telemetry data isn't affected.
- You can't delete datasets linked to active policies or evaluations.

### Export a Dataset

To export a dataset for offline workflows, model training, or external analysis, follow these steps:

1. Select the **more options** menu (⋮) in the upper-right corner of the dataset.
