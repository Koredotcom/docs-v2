...
| **Link Skill** | You run packaged agent logic on a configured LLM connection. |
...
**Streaming for skills**

A **Streaming** toggle appears below the version logs. Turn it on to allow streaming responses for the configured skill, or turn it off to disable them. The toggle state persists with the agent configuration.
...
The platform disables publishing when async connection verification fails. The platform displays one of the following error statuses:

- "Agent configuration pending (No agent configured)" if you have not linked an agent.
- "Agent configuration pending (Agent Async connection failed)" if the async connection verification failed.
