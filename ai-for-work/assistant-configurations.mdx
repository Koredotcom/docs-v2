...
MCP servers can also run these guardrails before any tool executes. For details, see [Tool guardrails](#tools).
...
Configure the tools that the server exposes and whether guardrails run before tool execution.

**Tool guardrails**

The **Enable guardrail** toggle at the top of the Tools section controls whether the account guardrails run before any tool on the server executes.

| Toggle state | Behavior |
|--------------|----------|
| **On** | The guardrails that administrators configure in [Guardrails](#guardrails) run before every tool execution, across all tools on the server. |
| **Off** | Tools run without the guardrail step. |

<Note>The toggle stays unavailable until you enable at least one guardrail for the account. Hover over the unavailable toggle to view the message "Guardrails disabled for the account."</Note>

**Add tools**

Select **Add tool** to open the tool picker. The picker has two tabs, **Prebuilt** and **Custom**, and opens on the **Prebuilt** tab. You can add tools from multiple systems and agents in a single flow.

To add prebuilt tools:

1. On the **Prebuilt** tab, select a system, such as `M365 Email`, `Jira`, `Zendesk`, or `Slack`, to view its tools.
2. Select one or more tools.
3. Select **Done**.
4. To add tools from another system, repeat these steps.

To add custom tools:

1. Select the **Custom** tab. The tab lists every agent in the account. The first five entries are the Enterprise Knowledge agents, followed by all other agents in alphabetical order.
2. Use the search box to find an agent.
3. Select one or more agents.
4. Select **Done**.

The Tools section lists every tool you add and shows the tool count split into **Prebuilt** and **Custom**. The platform pre-populates each custom tool with the agent's purpose as its tool description.

**Manage custom tools**

Hover over a custom tool to view the available actions:

| Action | Result |
|--------|--------|
| **Edit description** | Edit the tool description. The platform preserves edited descriptions within this server's configuration. |
| **Remove** | Remove the tool from the server. To add the tool again, select **Add tool** and select it from the **Custom** tab. |

**Manage prebuilt tools**

The Tools section groups prebuilt tools by system.

- To remove a system and all its tools in one action, hover over the system and select **Remove**. The button shows the number of tools added from that system.
- To remove individual tools, select the system to open a side panel that lists its selected tools. Hover over a tool and select **Remove**.
- To add more tools from the same system, select **Add more tools from \<system name\>** in the side panel. The tool picker opens on that system and lists all its available tools.

#### Auth settings

Configure how MCP clients register and authenticate with the server through OAuth. The **Auth settings** step offers two registration methods, **Dynamic OAuth** and **Static OAuth**. The method you configure also controls whether end users see the server in their MCP server listing.

**Dynamic OAuth**

Dynamic OAuth lets supported AI clients register with the server without a pre-created client.

To configure Dynamic OAuth:

1. Turn on the **Dynamic OAuth** toggle. The **Allowed Domains** button appears.
2. Select **Allowed Domains** to view the supported frontier domains, such as ChatGPT, Anthropic, and Cursor. All frontier domains are enabled by default.
3. Enable or disable each domain.
4. Optional: add a custom domain to allow a client that the list does not include.

You can delete custom domains only. To block a frontier domain, disable it.

**Static OAuth**

Static OAuth lets you register specific clients and issue each client its own credentials.

To add a static client:

1. Under **Static OAuth**, select **Add Client**.
2. Enter a **Client Name**.
3. Enter one or more **Redirect URL(s)**. To remove a redirect URL, delete it before you save the client.
4. Select **Generate Client ID and Secret**. The button stays unavailable until you enter the client name and a redirect URL. The platform generates a **Client ID** and a **Client Secret**.
5. Select **Done**. The client appears under **Static OAuth**.

To manage an existing static client:

| Action | Result |
|--------|--------|
| **Edit** | Update the client name or redirect URLs. |
| **Regenerate** | Generate a new **Client Secret** for the client. |
| **Delete** | Remove the client from the server. |

**End-user visibility**

| Auth configuration | End-user MCP server listing |
|--------------------|-----------------------------|
| **Dynamic OAuth** on | Shows the server with its MCP server URL. |
| **Dynamic OAuth** off | Hides the server. |
| **Static OAuth** | Hides the server until the end user grants access through the AI assistant that connects to the server. After that, the listing shows the server without an MCP server URL, and the end user can revoke the connected app access. |
