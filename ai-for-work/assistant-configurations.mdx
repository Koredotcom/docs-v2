...
The page displays a model selector that determines which LLM connection powers memory and personalization:

* The selector starts disabled and becomes available only after you turn on at least one memory setting.
* The selector lists all LLM connections configured in the account.
* The platform uses the selected model to generate memory chunks and to power personalization features.

If all memory settings are off, the selector remains disabled regardless of available LLM connections. If the account has no configured LLM connections, the selector displays an empty placeholder state.
