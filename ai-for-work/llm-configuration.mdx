...
| **Amazon Bedrock** | In the AWS Console, navigate to the Bedrock service and confirm that your target models are enabled in the correct region. Create an IAM role with the required Bedrock access policy, then copy the IAM Role ARN. You also need the STS API endpoint for your region (for example, `https://sts.us-east-1.amazonaws.com`) and either an IAM Role ARN or an Access Key ID and Secret Access Key pair. Verify model availability per region using the [AWS region endpoints documentation](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_temp_region-endpoints.html). |
