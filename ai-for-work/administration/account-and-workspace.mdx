...
#### Manage Workspace Models

The **Models** tab in workspace settings controls which LLM models a workspace can use. Instead of exposing every model integrated in the account to all workspaces, administrators enable only the models each workspace needs and set the order in which they appear.

The tab lists every model from [LLM Connections](/ai-for-work/llm-configuration), with the model name, provider, and connection name for each entry.

<Note>Only administrators can access the **Models** tab. Account administrators can update the settings, while workspace administrators have view-only access.</Note>

To manage workspace models:

1. Go to **Workspace Management** in the **Account Hub**.
2. Select the workspace to open its Settings page.
3. Select the **Models** tab.
4. Enable or disable each model.
5. Drag enabled models into the order you want.
6. Select or clear the option to auto-enable new LLM connections.

| Setting | Behavior |
|---------|----------|
| **Enabled models** | Appear in every model selector in the workspace, such as skills and prompt agents, in the order you set. The first enabled model acts as the workspace default. Enabling a model adds it to the bottom of the enabled list. |
| **Disabled models** | Do not appear in any model selector in the workspace. Disabled models display below enabled models in LLM Connections order, and you cannot reorder them. |
| **Model order** | Only enabled models show a drag handle. The order persists across sessions and applies to all workspace members. |
| **Auto-enable new connections** | When selected, new LLM connections become enabled and join the bottom of the enabled list. When cleared, new connections join the disabled group. Changing this option does not affect existing models. |

Model settings apply per workspace, so changes in one workspace never affect another. Workspaces created before v1.21.0 have every LLM connection enabled, in LLM Connections order. New workspaces start with every LLM connection disabled.
...
Personal workspace settings also include a **Models** tab that works the same way as in account workspaces. The model configuration applies to all personal workspaces and stays independent of account workspace settings. For details, see [Manage Workspace Models](#manage-workspace-models).
