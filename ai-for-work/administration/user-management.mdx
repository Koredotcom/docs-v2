...
The Employee Directory includes an **Organization Chart** tab that displays your organization's reporting hierarchy visually. Select the **Organization Chart** tab on the Employee Directory page to load the hierarchy view.

<Note>
The Organization Chart tab is only available after the Employee Directory is configured and published.
</Note>
...
Filters applied on the Organization Chart highlight only the cards matching the filter criteria. Non-matching cards remain visible in a disabled state to preserve hierarchy context but are not interactive.
...
| **Full View** | Opens the chart in full-screen mode; all interactions remain available |
