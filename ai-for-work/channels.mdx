...
The Slack app processes uploaded attachments as context, matching the file support of the web application. Attachment handling depends on the agent configured for the channel or the direct message.

**Supported Files**

| Rule | Value |
|------|-------|
| Formats | PDF, XLS, DOC, CSV, TXT |
| Maximum file size | 10 MB per file |
| Maximum files per message | 5 |

**Where Attachments Apply**

- **Direct messages**: The app takes attachments into context in a new conversation and within an existing thread.
- **Channels**: The app processes attachments when it listens actively and when a user tags it directly. The public or private response follows the mode configured for that channel.

Files that users upload in Slack sync to the corresponding board thread in the AI for Work web and mobile applications.

**Error Messages**

| Condition | Message |
|-----------|---------|
| File exceeds 10 MB | "The uploaded file exceeds 10 MB. Please upload a file smaller than that." |
| More than 5 files attached | "Please upload a maximum of 5 files." |
| Unsupported format | "Please upload a file in one of these formats: PDF, XLS, DOC, CSV, TXT." |
| The configured agent does not process attachments | "The agent configured by your admin does not process attachments. Please reach out to \<name of the person who added that agent configuration\>" |

Error messages appear in the public or private mode that the channel uses.

<Note>Attachment processing requires an Autonomous Agent that supports attachments. A channel or direct message configured with a Search Agent does not process attachments. Slack always requires an agent binding, so the web application behavior of uploading attachments without selecting an agent does not apply.</Note>

**Active Listening Edge Case**

In a channel, if a user uploads a file in an unsupported format or above the size limit and does not tag the app, active listening ignores the message silently and shows no error message.

### Artifacts and Thoughts

The Slack app renders generated artifacts directly in the channel, so users can view them without opening the web application.

The app displays thoughts in a plan view inside the same message as the response. It does not post a separate loading message, so the thoughts and the streamed response appear together in a single message.
