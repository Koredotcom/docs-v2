...
## v1.21.0 September 23, 2026

This update includes new features and feature enhancements summarized below.

<p className="rn-title">Guardrails for MCP Server Tools</p>

The Tools section of an MCP server now includes an Enable guardrail toggle. When you turn it on, the guardrails that administrators configure in the Admin Console run before any tool on the server executes. The toggle stays unavailable until you enable at least one guardrail for the account. A redesigned Add tool flow also organizes tools into Prebuilt and Custom tabs, so you can add tools from multiple systems and agents in one step. [Learn more→](/ai-for-work/assistant-configurations#tools)

<p className="rn-title">Model Management in Workspace Settings</p>

Administrators can now scope the LLM models each workspace can use, instead of exposing every integrated model to all workspaces. A new Models tab in Workspace Settings lets you enable or disable models and set their display order, with the first enabled model acting as the workspace default. A per-workspace option controls whether new LLM connections become enabled automatically, and personal workspaces include the same tab with independent settings. [Learn more→](/ai-for-work/administration/account-and-workspace#manage-workspace-models)

<p className="rn-title">Desktop Application</p>

AI for Work is now available as a desktop application, giving you access to the assistant, agents, and search without opening a browser. Download the application from [work.kore.ai/download](https://work.kore.ai/download).

<p className="rn-title">Live Response Flow</p>

The assistant now streams its thoughts live in a new response flow while it processes your query.

<p className="rn-title">Attachments Grouped with Parent Pages in Enterprise Search</p>

Enterprise Search results now show attachments within their parent page result instead of listing them as separate results.

<p className="rn-title">Search Platform Integration</p>

AI for Work now integrates with the ABL-based Search Platform, so Enterprise Search and search agents use the updated search infrastructure.

<p className="rn-title">Follow-up Queries for Generated Images</p>

Image generation now supports follow-up queries, so you can refine or modify a generated image in the same conversation without starting over.

<p className="rn-title">File Type Support in the Execution Container</p>

The execution container now supports all file types, so the assistant can process any file you provide during code execution. You can also request changes to these files in follow-up queries.

<p className="rn-title">Trace Log Performance Improvements</p>

This release improves trace log performance, so trace logs load faster when you review agent executions.

---

## v1.20.0 August 20, 2026

This update includes new features and feature enhancements summarized below.

<p className="rn-title">Share Chats</p>

You can now share a conversation as a link once the first query receives a response. Select share via link to generate the link, then copy or regenerate it, and use the update action to push later messages to the same link. Recipients open a read-only copy with sources and downloadable files, and they can't continue the conversation. [Learn more→](/ai-for-work/getting-started/how-to-use#share-chats)

<p className="rn-title">Employee Profile Link Sharing</p>

The Enterprise Directory now includes a copy profile link option on contact detail pages, on contact cards in the listing view, and on contact cards in search results. Copying a link displays a confirmation message, so you can share a colleague profile without navigating the directory again. [Learn more→](/ai-for-work/getting-started/how-to-use#share-a-profile-link)

<p className="rn-title">URL-Based Query Execution</p>

Enterprise Search now runs a query directly from a URL parameter and lands you on the Search tab with results already displayed. Searches you run in the interface update the browser URL with the query, the active tab, and any filters, so back and forward navigation, refresh, and shared links reproduce the same result state. Shared links respect each recipient's own permissions. [Learn more→](/ai-for-work/getting-started/how-to-use#search-from-a-url)

<p className="rn-title">Project Collaborators</p>

Projects now support collaborators with View only and Can edit roles. Editors can rename the project, add or remove files, and update instructions, while only the owner can add or remove collaborators. The owner can also share a project in view only mode with user groups that administrators have whitelisted for project access. [Learn more→](/ai-for-work/getting-started/how-to-use#sharing-a-project)

<p className="rn-title">Attachments, Artifacts, and Thoughts in Slack</p>

AI for Work now processes attachments in Slack direct messages and channels, matching the web application limits of PDF, XLS, DOC, CSV, and TXT files up to 10 MB each and five files per message. Files uploaded in Slack sync to the corresponding thread in the web and mobile applications. Slack also renders generated artifacts inline and shows reasoning thoughts and the streamed response in a single message. [Learn more→](/ai-for-work/channels#attachments)

<p className="rn-title">Archive and Manage Chats</p>

You can now archive a chat thread and restore it later. A new Manage Chats section under Settings groups archive all chats, manage archived chats, and delete all chats. Archived threads support search, unarchive, and delete, along with unarchive all and delete all actions. [Learn more→](/ai-for-work/getting-started/how-to-use#manage-chats)

<p className="rn-title">Skills on OpenAI and Azure OpenAI Connections</p>

Autonomous agents now run skills on `OpenAI`, `Azure OpenAI`, and `Anthropic` connections. Select an LLM connection in the skill selection popup, then choose an existing skill or upload a new folder or .zip file. If the linked key expires or becomes invalid, the configuration marks the skill with an error state and end users see a message to contact the agent creator. [Learn more→](/ai-for-work/custom-agents#step-2-configure-agentic-app)

<p className="rn-title">File Generation with Gemini Models</p>

Gemini models accessed through Vertex AI now support file generation, including PDF and image output. Previously, file generation required an OpenAI connection.

<p className="rn-title">New Model Support</p>

AI for Work now supports `QWEN`, expanding the model options available for your assistant and agents.
