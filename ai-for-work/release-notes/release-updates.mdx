...
AI for Work now brings **Skills** to autonomous agents. Add an agent, then upload a skill as a folder or .zip file to extend what the agent can do. Manage skill versions through version logs, and control streaming responses with a dedicated toggle. [Learn more→](/ai-for-work/custom-agents#autonomous-agent)

<p className="rn-title">Agents as MCP Tools</p>

Administrators can now expose AI for Work agents as tools on MCP servers, making them available to Claude, ChatGPT, and other MCP-compatible clients. A new multi-step creation wizard configures the server identity, tools, authentication (API token or OAuth), and access limits. Add prebuilt agents, or search across workspaces to add custom agents with editable tool descriptions. End users can view every connected app on OAuth servers, revoke individual app access, and regenerate auth token URLs when needed. [Learn more→](/ai-for-work/assistant-configurations#mcp-server)

<p className="rn-title">Organization Chart Enhancements</p>

The org chart now surfaces employees without a reporting manager. Select the **Show unassigned employees** checkbox to jump to a dedicated section listing them, and expand any employee with reportees to view direct reports inline. Administrators can also control the chart root and administrators can override this selection at any time. [Learn more→](/ai-for-work/administration/user-management#organization-chart)
...
Each LLM connection now exposes a parameter configuration section. Adjust **Temperature**, **Top P**, **Max Tokens**, **Reasoning Effort**, **Stop Sequences**, and more using sliders, dropdowns, and toggles that match each parameter type. The interface shows only the parameters your selected provider and model support, tooltips explain each parameter, and a **Reset to Default** option restores recommended values. Existing connections continue to work with provider defaults. [Learn more→](/ai-for-work/llm-configuration#model-parameters)

<p className="rn-title">Sync employee data with Google Service Accounts</p>

The Enterprise Directory now supports **Google Service Accounts** as a data source, alongside Microsoft, LDAP, and the Push API method. Connect a pre-configured Google service account to sync employee data automatically from your identity provider, keeping the directory and org chart current without manual uploads. [Learn more→](/ai-for-work/administration/user-management#enterprise-directory)

<p className="rn-title">Announcements to User Groups</p>

Administrators can now send announcements to selected user groups instead of the entire account. Choose **Selected User Groups** alongside the existing **Everyone in Account** and **Agent Specific** options, then multi-select groups from the list. The interface displays the number of selected groups, and the announcement reaches every member of those groups. [Learn more→](/ai-for-work/assistant-configurations#announcements)

<p className="rn-title">Model Selection for Memory and Personalization</p>

Administrators can now select the LLM connection that generates memory chunks and drives personalization features. The model selector activates when you enable at least one memory feature and lists all configured LLM connections. [Learn more→](/ai-for-work/getting-started/how-to-use#personalisation-and-memory)

<p className="rn-title">Reasoning Thoughts and Tool Visibility</p>

When a reasoning model processes your request, the assistant now displays each tool call as a live label while it works. Click any label to open a side panel showing the complete execution flow, including code inputs and outputs and web search results. [Learn more→](/ai-for-work/getting-started/how-to-use#reasoning-thoughts)

<p className="rn-title">New Model Support</p>

AI for Work now supports `Sonnet 5`, `Opus 4.8`, and `Fable 5`, expanding the model options available for your assistant and agents. [Learn more→](/ai-for-work/llm-configuration)
