...
The **Tools** section of an MCP server now includes an **Enable guardrail** toggle that runs account guardrails before any tool executes. [Learn more→](/ai-for-work/assistant-configurations#tools)
...
AI for Work now integrates with the Artemis-based Search Platform, so Enterprise Search and search agents use the updated search infrastructure.
