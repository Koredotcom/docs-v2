...
The **Tools** section of an MCP server now includes an **Enable guardrail** toggle that runs account guardrails before any tool executes. A redesigned **Add tool** flow groups tools into **Prebuilt** and **Custom** tabs. [Learn more→](/ai-for-work/assistant-configurations#tools)

<p className="rn-title">Model Management in Workspace Settings</p>

Administrators can now scope the LLM models available to each workspace. Use the new **Models** tab in **Workspace Settings** to enable, disable, and reorder models. [Learn more→](/ai-for-work/administration/account-and-workspace#manage-workspace-models)
