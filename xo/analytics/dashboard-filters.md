...
The dashboard filters are used to filter and view the analytics data for AI Agents on the following dashboards in the **ANALYTICS** section based on the user’s selection(s):
...
!!! note
    ...
    By default, the AI Agents dashboard displays data for the past 24 hours.

## Filter Criteria Descriptions

The following table describes all the filter criteria available on the Kore.ai platform:
...
<li><strong>Custom</strong> – You can choose a custom date range to filter the records. Choose the start date and end date in the calendar and click <strong>Select</strong> to filter the records. The Platform allows filtering a date range of up to 90 days.
...
<li><strong>Active Conversations</strong>: These are the ongoing conversations where the users interact with the AI Agents.

<li><strong>Closed Conversations</strong>: These are the conversations where an active session between the Agents and the customer has ended. Learn more.
</li>
</ul>
   </td>
  </tr>
...
For ‘Enterprise AI Agents’, the email address (kore registered email address) is available under Channel ID.
...
<p>
Channel-specific ids are shown only for the users who have interacted with the Agents during the selected period.
</li>
</ul>
   </td>
  </tr>
  <tr>
   ...
   <td>If it is a multi-lingual AI Agents, you can select specific languages to filter the conversations that occurred in those languages. The page shows the conversations that occurred in all enabled languages by default.
<p>
This criterion is <strong>not available for the Performance tab </strong>of NLP Insights.
  ...
  <tr>
   ...
   <td>When the AI Agents is published in multiple channels, select this filter to view the data for various channels enabled for the AI Agents.
<p>
...
This criterion is<strong> not available for the Intent Not Found tab</strong> of NLP Insights.
  ...
  <tr>
   ...
   <td>Select the <strong>Trained </strong>option to filter the conversations that only contain trained utterances to the AI Agents.
<p>
...
<p>
Developers include both the AI Agents owners and shared developers.
   </td>
  </tr>
  <tr>
